...
- The [`next/image`](/docs/api-reference/next/image.md) component is designed to be used with a server side image optimization service (either a next.js server, or one of the other [loaders](/docs/basic-features/image-optimization.md#loader) available). For static image optimization at build time (similar to Gatsby.js) please refer to a community package such as [next-optimized-images](https://github.com/cyrilwanner/next-optimized-images).
