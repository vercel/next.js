...
`next export` allows you to export your Next.js application to static HTML, which can be run standalone without the need of a Node.js server. We only recommend using `next export` if you don't need any of the [unsupported features](#unsupported-features) requiring a server.
...
It's possible to use [`getInitialProps`](/docs/api-reference/data-fetching/getInitialProps.md) API instead of `getStaticProps`, but it comes with a few caveats:
...
We recommended to migrate towards `getStaticProps` over `getInitialProps` whenever possible.
