...
Note: `next.config.js` is serialized when creating the minimal `server.js` file so if the legacy [`serverRuntimeConfig` or `publicRuntimeConfig` options](/docs/api-reference/next.config.js/runtime-configuration.md) are being used the values will be specific to the build time provided values.
