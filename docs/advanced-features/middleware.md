...
export function middleware(request: NextRequest) {
  if (request.nextUrl.pathname.startsWith('/api/about')) {
    return NextResponse.rewrite(new URL('/api/about-2', request.url))
  }

  if (request.nextUrl.pathname.startsWith('/api/dashboard')) {
    return NextResponse.rewrite(new URL('/api/dashboard/user', request.url))
  }
}
```
