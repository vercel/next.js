...
Search engines often use the `src` attribute as the canonical url for an image.
...
You can use the `overrideSrc` property to set the `src` attribute instead of using the fallback that was automatically generated from the `src` prop input.
