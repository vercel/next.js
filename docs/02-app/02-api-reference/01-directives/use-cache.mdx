...
By default when using the `use cache` directive Next.js sets a **[revalidation period](/docs/canary/app/building-your-application/data-fetching/fetching#revalidating-cached-data) of fifteen minutes** with a near infinite expiration duration, meaning it's suitable for content that doesn't need frequent updates.
...
```tsx filename="app/actions.ts" highlight={7,8,9}
import {
...
```tsx filename="app/submit.ts" highlight={7}
'use server'
