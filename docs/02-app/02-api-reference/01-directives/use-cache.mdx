...
- By default, `use cache` uses an **in-process memory cache** which can be configured to persist with your own architecture. However, as the feature is under development, we don't yet recommend it for production yet.
