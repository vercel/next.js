...
- React will attempt to [serialize](https://react.dev/reference/rsc/use-server#serializable-parameters-and-return-values) the arguments (or props) passed to a cached function. This means they will be converted to a format like JSON and automatically used as the cache keys.
- A cached function can take non-serializable arguments (like a function or a piece of JSX), but their values will be completely opaque inside of the cached function. For example, if a cached function receives a `children` prop with some JSX, the cached function can return it like `<div>{children}</div>` but it won't be able to introspect it like reading `children.props`. The actual `children` value will be filled in during the request and won't be a part of the cache.
...
- Unlike [`unstable_cache`](/docs/app/api-reference/legacy-apis/unstable_cache) which only supports JSON data, `use cache` can cache any serializable data React can render, including the render output of components.
