...
Enable `use cache` with the `dynamicIO` flag in your `next.config.js`:
...
Next.js treats all asynchronous operations as potentially dynamic, unless you specify otherwise. You need to indicate to Next.js how you want it to handle rendering. If you define a fallback using a `Suspense` boundary, everything within that boundary is potentially dynamic. By contrast if you don't use a `Suspense` boundary, Next.js will prerender the whole component tree.

To explicitly cache certain asynchronous operations and achieve static behavior, you can use the  `use cache` directive. This allows you to optimize rendering performance by caching results from async data requests, while still enabling dynamic rendering where needed.
...
```tsx filename="app/components/my-component.tsx" highlight={1}
"use cache"
...
### Whats the difference between `unstable_cache` and `use cache`?

[`unstable_cache`](/docs/canary/app/api-reference/functions/unstable_cache) is limited to caching JSON data and requires you to define revalidation periods and tags manually. Because it involves more setup there's a higher chance of making mistakes. For example, if you use the wrong cache key to retrieve your cached data, you might not get the results you expected.

The `use cache` directive allows you to cache a wider range of data, including anything that React Server Components (RSC) can serialize, as well as data fetching outputs and component outputs. It automatically manages some of the complexities by tracking both inputs and outputs.

In addition to this, `use cache` serializes both inputs and outputs making it harder to accidentally poison your cache.
