...
- Any [serializable](https://react.dev/reference/rsc/use-server#serializable-parameters-and-return-values) arguments (or props) to the cached function, as well as any serializable values from the parent scope that it closes over, will be converted to a format like JSON and automatically become a part of the cache key.
