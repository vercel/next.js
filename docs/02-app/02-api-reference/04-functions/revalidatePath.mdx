...
> - Currently, `revalidatePath` invalidates all the routes in the client-side [Router Cache](/docs/app/building-your-application/caching#router-cache). This behavior is temporary and does not apply to the server-side [Route Cache](/docs/app/building-your-application/caching#full-route-cache) where **only the specific path** is invalidated.
