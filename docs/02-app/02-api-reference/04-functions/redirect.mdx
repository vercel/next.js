...
The `redirect` function accepts the following arguments:

```js
redirect(path, type, permanent)

// or you can also use named parameters to omit optional parameters
redirect({
  path: '/redirect-to',
  permanent: true,
})
...
| `permanent` | `false` (default) or `true`                                   | `true` will use the `308` status code, and `false` will use the `307` status code. |
