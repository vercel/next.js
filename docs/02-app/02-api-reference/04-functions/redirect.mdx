...
`redirect` does not return any value. Instead, it throws a NEXT_REDIRECT error internally. When using `redirect` within a `try/catch` block, you must invoke it in the `finally` block to ensure proper execution.
