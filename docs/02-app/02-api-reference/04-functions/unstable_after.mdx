...
`unstable_after()` allows you to schedule work to be executed after a response is finished. This is useful for background tasks and other side effects that should not block the response, such as logging and analytics.
...
To use `unstable_after()`, you need to enable it using `experimental.after` config in the `next.config.js` file:
