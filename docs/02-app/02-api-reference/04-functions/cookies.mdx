...
`cookies` is an **async** function that allows you to read the HTTP incoming request cookies in components, and read/write outgoing request cookies in a [Server Action](/docs/app/building-your-application/data-fetching/server-actions-and-mutations) or [Route Handler](/docs/app/building-your-application/routing/route-handlers).
...
### Cookies in synchronous components

To use `cookies` in a synchronous component, such as a Client Component, you can use React's [`use`](https://react.dev/reference/react/use) function to read the promise:

```tsx filename="app/ui/component.tsx" switcher
'use client'
...
export default async function Component() {
  const theme = use(cookies()).get('theme')
}
```

```js filename="app/ui/component.js" switcher
'use client'
