...
- `options`: This is an object that controls how the cache behaves. It can contain the following properties:
  - `tags`: An array of tags that can be used to control cache invalidation. NextJS will not use this to uniquely identify the function.
...
export default async function Page({ params }: { params: { userID: string } }) {
 const getCachedUser = unstable_cache(
  ...
  return { id: params.userID };
    ...
    [params.userID], // here we add the userID in the keyParts so NextJS can make separate caches for the different userID's.
    {
