...
> **Tags vs KeyParts**:
>
> - tags: They are used for cache invalidation, NextJS does not use it to identify the function signature. 
> - keyParts: They are used by NextJS to uniquely identify the key of the data cached.
...
export default async function Page({ params }: { params: { userID: string } }) {
 const getCachedUser = unstable_cache(
  ...
  [params.userID], // here we add the userID in the keyparts so NextJS can make seperate caches for the different userID's.
  {
