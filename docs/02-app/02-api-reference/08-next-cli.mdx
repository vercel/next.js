---
title: Next.js CLI
description: The Next.js CLI allows you to develop, build, and start your application, and more. Learn more about it here.
---
...
[Turbopack](https://turbo.build/pack) (beta), our new bundler we're testing and stabilizing through Next.js, helps speed up local iterations while working on your Next.js application.

To use Turbopack in development mode, add the `--turbo` option like so:
...
Arguments:
  [directory]    A directory on which to build the application. If no directory is
                 provided, the current directory will be used.

Options:
  -d, --debug    Eanbles a more verbose build output.
