...
In case of diagnosing installation issue in more detail, you can run `next info --verbose` to get the additional information about system and the installation of next-related packages.
This will generate lengthy output, if it's hard to copy paste running `next info --verbose --out-file filename` will generate text report file instead of emitting to stdout.
