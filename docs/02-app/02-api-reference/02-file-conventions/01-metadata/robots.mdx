...
Customize user-agent rules (example):

You can pass an array of user-agents to the `rules` property to customize.
And make sure that `robots.js` or `robots.ts` file returns a [`Robots` object](#robots-object).
