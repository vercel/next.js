...
#### Using Node runtime with local assets

This example uses the Node runtime to fetch a local image on the file system and pass it as an `ArrayBuffer` to an `img` element `src`:
...
export async function GET() {
  ...
  const logoSrc = Uint8Array.from(logoSource).buffer
