...
The `instrumentation.js|ts` file is used to integrate observability tools into your application. This allows you to track the performance and behavior of your application, and to debug issues in production.
...
- `context`: The context in which the error occurred. This can be the type of router (`'App Router'` or `'Pages Router'`), and/or (Server Components (`'render'`), Route Handlers (`'route'`), Server Actions (`'action'`), or Middleware (`'middleware'`)).
