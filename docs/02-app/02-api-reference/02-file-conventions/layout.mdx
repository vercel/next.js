...
Layouts cannot access `pathname`. This is because layouts [do not rerender during client-side navigation](/docs/app/building-your-application/routing/linking-and-navigating#4-partial-rendering) and are Server Components by default (render on the server), which could lead to stale pathnames.

To access pathname in your layout, you can extract the logic that depends on pathname into an imported Client Component and use Next.js APIs such as [`usePathname`](https://nextjs.org/docs/app/api-reference/functions/use-pathname) to access the current pathname. This way, the logic will be re-evaluated on the client during client-side navigation.
...
See the [examples](/docs/app/building-your-application/routing/layouts-and-templates#examples) section for common UI patterns using `pathname`.
