...
In the matcher configuration, several key options are available:

- `source`: The path or pattern used to match the request paths. It can be a simple string for direct path matching or a pattern for more complex matching.
...
- `locale` (optional): A boolean key that, when set to false, ignores locale-based routing in path matching.
