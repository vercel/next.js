...
- Using `unstable_noStore` inside [`unstable_cache`](/docs/canary/app/api-reference/legacy-apis/unstable_cache) will not opt out of static generation. Instead, it will defer to the cache configuration to determine whether to cache the result or not.
