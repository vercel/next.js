...
export default function pixelBinLoader({ src, width, quality }) {
  ...
  const optimizeTransformation = `/t.resize(w:${width})~t.compress(q:${
    quality || 75
  })/`
  const baseURL = `https://cdn.pixelbin.io/v2/${cloudName}${optimizeTransformation}`
  const adjustedSrc = src.startsWith('/') ? src.slice(1) : src
  const fullPath = new URL(adjustedSrc, baseURL)
  if (!fullPath.searchParams.has('f_auto')) {
    fullPath.searchParams.set('f_auto', 'true')
  }
