...
To enable the `dynamicIO` flag, add it to the `experimental` section of your `next.config.js` file as follows:
...
- The [`cacheLife` configuration](/docs/app/api-reference/next-config-js/cacheLife) with `use cache`
- The [`cacheTag` function](/docs/app/api-reference/directives/use-cache#revalidate-on-demand-with-cacheTag)
