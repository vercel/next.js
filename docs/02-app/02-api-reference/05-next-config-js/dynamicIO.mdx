...
It is expected to be used in conjunction with [`use cache`](/docs/canary/app/api-reference/directives/use-cache) so that your data fetching happens at runtime by default unless you define specific parts of your application to be cached with `use cache` at the page, function, or component level.
...
- The [`use cache` directive](/docs/canary/app/api-reference/directives/use-cache)
- The [`cacheLife` function](/docs/canary/app/api-reference/next-config-js/cacheLife) with `use cache`
- The [`cacheTag` function](/docs/canary/app/api-reference/functions/cacheTag)
