...
To enable the `dynamicIO` flag, add it to the `experimental` section of your `next.config.ts` file:
...
- The [`cacheLife` function](/docs/app/api-reference/next-config-js/cacheLife) with `use cache`
