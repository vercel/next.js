...
To define a profile, enable the [`dynamicIO` flag](/docs/app/api-reference/next-config-js/dynamicIO) and add the cache profile in the `cacheLife` object in the `next.config.js` file. For example, a `blog` profile:
