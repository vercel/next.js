...
Starting in Next.js 15, the `next/server/testing` package contains utilities to help unit test `next.config.js` files.

The `getResponseFromNextConfig` function runs the [`headers`], [`redirects`], and [`rewrites`] functions from `next.config.js` with the provided request information and returns `NextResponse` with the results of the routing.

> The response from `getResponseFromNextConfig` only considers `next.config.js` fields and does not consider middleware or filesystem routes, so the result in production may be different than the unit test.

```js
import { getRedirectUrl, getResponseFromNextConfig } from 'next/server/testing'
