...
Set the build output type. Supported values:

- `'export'`: Export your app as static HTML (static site)
- `'standalone'`: Build a standalone server app (Node.js server)
...
When using `output: 'export'`, your app will:

- Generate static HTML files during `next build`
- Disable server-side features (e.g., API routes, `getServerSideProps`)
- Output to `/out` directory by default

Learn more about [static exports](https://nextjs.org/docs/app/building-your-application/deploying/static-exports).
