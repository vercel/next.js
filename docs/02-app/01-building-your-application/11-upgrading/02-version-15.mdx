...
To update to the Next.js version 15 release candidate, run the following command:

```bash filename="Terminal"
npx @next/codemod@canary upgrade
```

If the upgrade utility doesn't work for you, or you prefer to manually upgrade, read on for more information about the breaking changes.
...
> **Good to know:**
>
> - If you see a peer dependencies warning, you may need to update `react` and `react-dom` to the suggested versions, or you use the `--force` or `--legacy-peer-deps` flag to ignore the warning. This won't be necessary once React 19 is stable.
...
APIs that would opt into [dynamic rendering](/docs/app/building-your-application/rendering/server-components#dynamic-rendering) and that previously could be accessed synchronously have now been made asynchronous in preparation for greater rendering flexibility with features like [Partial Prerendering](/docs/app/api-reference/next-config-js/ppr). These APIs have been enumerated below.
...
## `params` & `searchParams` are now asynchronous

Accessing properties on the `params` prop that get provided to a Layout/Page/Route Handler, or accessing the `searchParams` prop that gets provided to a Page component have also been made asynchronous.

- `props.searchParams` now returns a Promise containing the relevant search parameters
- `props.params` now returns a Promise containing the relevant segment params.

> **Good to know:**
>
> These changes also apply to the `searchParams` and `params` props that are provided to `generateMetadata` and `generateViewport`.
...
To help with the migration to these new asynchronous APIs and props, a [codemod](/docs/app/building-your-application/upgrading/codemods#migrate-to-async-dynamic-apis) is available that will attempt to automatically transform all of these APIs to their async form. Additionally, these APIs & properties can still be accessed synchronously, however you will see a warning during development.
