...
- The minimum `react` and `react-dom` version has been bumped to 19.

## `fetch` requests

[`fetch` requests](/docs/app/api-reference/functions/fetch) are no longer cached by default. To opt `fetch` requests into caching, you can pass the `cache: 'force-cache'` option.
...
When navigating between pages via `<Link>` or `useRouter`, [page](/docs/app/api-reference/file-conventions/page) segments are no longer reused from the client-side router cache. However, they are still reused during browser backward and forward navigation.

To opt into caching page segments, you can use the [`staleTimes`](/docs/app/api-reference/next-config-js/staleTimes) config option:
