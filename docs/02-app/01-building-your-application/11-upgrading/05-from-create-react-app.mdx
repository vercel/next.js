...
> **Good to know**: Next.js ignores the CRA's `public/manifest.json` file, additional iconography other than the favicon, and [testing configuration](/docs/app/building-your-application/testing), but if these are requirements, Next.js also supports these options. See the [Metadata API](/docs/app/building-your-application/optimizing/metadata) for more information.
