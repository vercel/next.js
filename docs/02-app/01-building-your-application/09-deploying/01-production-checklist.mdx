...
- **[Prefetching](/docs/app/building-your-application/routing/linking-and-navigating#2-prefetching):** When a link to a new route enters the user's viewport, Next.js prefetches the route in background. This makes navigation to new routes almost istant. You can opt out of prefetching, where appropriate.
...
- **[Dynamic Functions](/docs/app/building-your-application/rendering/server-components#dynamic-functions):** Be aware that dynamic functions like [cookies](/docs/app/api-reference/functions/cookies) and [useSearchParams](/docs/app/api-reference/functions/use-search-params) will opt the whole route into [Dynamic Rendering](/docs/app/building-your-application/rendering/server-components#dynamic-rendering) (or your whole application if used in the [Root Layout](/docs/app/building-your-application/routing/pages-and-layouts#root-layout-required)). Ensure dynamic function usage is intentional and wrap them in `<Suspense>` boundaries where appropriate.

> **Good to know**: [Partial Prerendering](/blog/next-14#partial-prerendering-preview) will allow parts of a route to be dynamic without opting the whole route into dynamic rendering.
...
- **[Static Assets](/docs/app/building-your-application/optimizing/static-assets):** Use the `public` directory to automatically cache your application's static assets, e.g. images.
...
- **[Opengraph (OG) images](/docs/app/api-reference/file-conventions/metadata/opengraph-image):** Create OG images to prepare your application for social sharing.
...
- **TypeScript and [TS Plugin](/docs/app/building-your-application/configuring/typescript):** Use the TypeScript and the TypeScript plugin for better type-safety, and to help you catch errors early.
...
- **[Lighthouse](https://developers.google.com/web/tools/lighthouse):** Run lighthouse in incognito to gain a better understanding of how your users will experience your site, and to identify areas for improvement.
...
- **[Regions](https://vercel.com/docs/edge-network/regions):** Ensure your application and database are deployed in the same region to reduce [latency](https://developer.mozilla.org/en-US/docs/Web/Performance/Understanding_latency).
- **[Edge Runtime](/docs/app/building-your-application/rendering/edge-and-nodejs-runtimes):** If using the Edge Runtime, consider that multiple requests from the Edge to your database might take longer than rendering on the Node.js runtime which can be closer to your database. Consider pinning your Edge location to a [preferred region](/docs/app/api-reference/file-conventions/route-segment-config#preferredregion) to reduce latency.
...
- **[Logging](https://vercel.com/integrations#logging), [Monitoring](https://vercel.com/integrations#monitoring), and [Observability](https://vercel.com/integrations#observability):** See Vercel's integrations page for a list of available tools and services to help you monitor your application's performance.
- **[Analytics Dashboard](https://vercel.com/analytics):** Use the Vercel dashboard for real-world performance insights based on actual visitor data, offering a practical view of how your website is performing in the field.

> **Good to know:**
>
> To get a comprehensive understanding of the best practices for production deployments on Vercel, including detailed strategies for improving website performance, refer to the [Vercel Production Checklist](https://vercel.com/docs/production-checklist).

Following these recommendations will help you build a fast, reliable, and more secure application for your users.
