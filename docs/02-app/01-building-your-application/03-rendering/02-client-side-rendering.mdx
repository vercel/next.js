...
React Server Components allow us to interactive UI that can be rendered on the client. There are a couple of benefits to doing the rendering work on the client, including:

- **Interactivity**: Client Components can use state, effects, and event listeners, meaning they can provide immediate feedback to the user and update the UI without making a network request.
...
You can use the React `"use client"` directive to tell React which components should be rendered on the client.
