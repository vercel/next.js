...
`"use client` is used to declare a [boundary](/docs/app/building-your-application/rendering#network-boundary) between a Server and Client Component modules. This means
...
> **Defining multiple `use client` boundaries**:
>
...
In Next.js, Client Components are rendered differently depending on whether the user is visiting your application for the first time or if it's a subsequent navigation.
...
Sometimes, after you've declared the ``"use client"` boundary, you may want to switch back to the server environment. For example, you may want to reduce the client bundle size, fetch data on the server, or use an API that is only available on the server.
