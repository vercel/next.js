...
> To understand how Client and Server Components are rendered, see the [Rendering Client and Server Components](/docs/app/building-your-application/rendering/client-and-server-components) page.

## Dynamic Rendering

With Dynamic Rendering, both Server _and_ Client Components for a route are rendered on the server at **request time**.

During rendering, if a [dynamic function](#dynamic-functions) or uncached data is discovered, Next.js will switch to dynamically rendering the whole route. This table summarizes how dynamic functions and data caching affect whether a route is statically or dynamically rendered:
