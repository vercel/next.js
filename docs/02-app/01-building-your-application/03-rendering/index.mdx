...
- The [Environments](#environments) your application code can be executed in: the server and the client.
...
Understanding these differences is key to effectively using React and Next.js. We'll cover the differences and use cases in more detail on the [Server](/docs/app/building-your-application/rendering/server-environment) and [Client](/docs/app/building-your-application/rendering/server-environment) rendering pages, for now, let's continue building on our foundation.
...
<Image
  alt="Module Graph"
  srcLight="/docs/light/module-graph-example.png"
  srcDark="/docs/dark/module-graph-examples.png"
  width=""
  height=""
/>
...
When building hybrid applications, it's helpful to think of the flow of the code in your application as **unidirectional**. In other words, your application code flows in one direction: from the server to the client.
...
If you need to access the server from the client, you send a **new** request to the server rather than re-use the same request. This makes it easier to architect hybrid applications, and understand where you should place the Network Boundary.
