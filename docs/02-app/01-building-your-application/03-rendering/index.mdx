...
Behind the scenes, the work is split into two parts: **client module graph** and the **server module graph**. The client module graph contains all components that are executed on the client, and the server module graph contains all the components that are executed on the server.
