...
It's not possible to opt out of the Router Cache. However, you can opt out of prefetching by setting the `prefetch` prop of the `<Link>` component to `false`. The routes will still be cached when visited.
...
By default, the `<Link>` component automatically prefetches [static routes]() and adds the React Payload to the Router Cache.
