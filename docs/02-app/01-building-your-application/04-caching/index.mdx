...
Next.js provides persistent caching for data fetched using `fetch` or `unstable_cache`, allowing you to:

- Revalidate on demand with `cacheTag`.
- Set a time-to-live (TTL) with `cacheLife`.
...
#### When to Use `cacheTag`

- Group related data (e.g., all product listings).
- Revalidate dynamically when data changes (e.g., after a CMS update).

#### When to Use `cacheLife`

- Set a fixed revalidation interval (e.g., refresh every hour).
- Avoid stale data for time-sensitive content (e.g., stock prices).

### Combining `cacheTag` and `cacheLife`

- If both are set, the cache revalidates when either:
  1. The `cacheLife` (TTL) expires.
  2. `revalidateTag` is called manually.
