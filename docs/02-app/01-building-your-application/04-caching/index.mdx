...
| ------------------------------------------- | ------------------------------ | ------------------- | -------------------------------------------------------- | ------------------------------- |
| [Request Memoization](#request-memoization) | Return values of functions     | Server              | Share data between React components in a tree            | Per-request lifecycle           |
| [Data Cache](#data-cache)                   | Return values of data requests | Server              | Share data across user requests and deployments          | Persistent (can be revalidated) |
...
This behavior changes depending on whether the route is statically or dynamically rendered, data is cached or uncached, inital visit and subsequent navigations. Depending on your use case, you can configure the caching behavior for individual routes and data requests.

## Request Memoization

In React Server Components, the [`fetch` API](#fetch) automatically **memoizes** data requests that have the same URL and options. This means you can call the same data in multiple places in a React component tree while only executing it once.
...
By using `fetch`, you can fetch data in the components that need it without props drilling and worrying about the performance impact of making multiple requests for the same data. Memoization applies to `fetch` requests in `generateMetadata`, `generateStaticParams`, Layouts, Pages, and other Server Components
...
You can use the [`cache`](#fetch-optionscache) and [`next.revalidate`](#fetch-optionsnextrevalidate) options of `fetch` to configure the caching behavior. In the browser, `cache` indicates how a fetch request will interact with the browser's HTTP cache, and with Next.js, the `cache` option also indicates how a server-side fetch request will interact with the Data Cache.
