...
- `GET` functions in Route Handlers are not cached by default. To opt `GET` methods into caching, use a route config option such as `export const dynamic = 'force-static'` in your Route Handler file.
...
- When navigating between pages via `<Link>` or `useRouter`, [page](/docs/app/api-reference/file-conventions/page) segments are not reused from the client-side router cache. However, they are reused during browser backward and forward navigation. To opt into caching page segments, use the [`staleTimes`](/docs/app/api-reference/next-config-js/staleTimes) config option.
  ...
  [Layouts](/docs/app/api-reference/file-conventions/layout) and [loading states](/docs/app/api-reference/file-conventions/loading) _are_ cached and reused on navigation.
