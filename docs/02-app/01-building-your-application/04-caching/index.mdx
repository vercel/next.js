...
| [Full Route Cache](#full-route-cache) | Rendered HTML and RSC payload  | Server              | Reduce rendering cost and improve performance                 | Persistent (can be revalidated) |
...
Next.js has a built-in Data Cache that persists the result of data fetches across incoming requests and deployments.
...
>
> You may see the terms **Static Data Fetching** and **Dynamic Data Fetching** being used. Static Data Fetching refers to cached data requests (default), whereas Dynamic Data Fetching refers to data that is fetched on every request, at request time.
...
3. Next.js uses the React Server Component Payload and JavaScript instructions to pre-render **HTML** on the server.

> **What is React Server Component Payload?**
>
> The React Server Component Payload is a compact string representation of the React Component tree. It's used by React on the client to update the DOM. The React Server Component Payload contains:
>
...
On the server, at build time, Next.js caches the React Server Component Payload, Client Component JavaScript, and rendered HTML.
...
You can opt out of the Full Route Cache, or in other words, dynamically render components at request time, by:
...
- **Opting out of the [Data Cache](#data-cache)**: This will fetch data on every request, which in turn will skip the Full Route Cache.
- **Using the route segment config options `dynamic = force-dynamic` or `revalidate = 0`**: This will skip the Full Route Cache and the Data Cache. Meaning components will be rendered and data fetched on every request.

> **Static vs. Dynamic Rendering**:
>
> You may see the terms **Static Rendering** and **Dynamic Rendering** being used. Static Rendering refers to caching the render output on the server, at build time, whereas Dynamic Rendering refers to rendering components on every request, at request time.
...
- **Session**: The cache persists across [soft navigations](/docs/app/building-your-application/routing/linking-and-navigating#soft-navigation). However, it's cleared on page refresh.
- **Automatic invalidation period**: The cache of an individual segment is automatically invalidated after a specific time. The duration depends on whether the route is [statically](/docs/app/building-your-application/rendering/static-and-dynamic-rendering#static-rendering-default) or [dynamically](/docs/app/building-your-application/rendering/static-and-dynamic-rendering#dynamic-rendering) rendered:
...
- Revalidating data on-demand by path with ([`revalidatePath`](/docs/app/api-reference/functions/revalidatePath)) or by cache tag with ([`revalidateTag`](/docs/app/api-reference/functions/revalidateTag)) in Server Actions, which in turn will invalidate the Router Cache.
- Using `cookies.set` or `cookies.delete` in a Server Actions invalidates the Router Cache to prevent routes that use cookies from becoming stale (e.g. authentication).
...
- Revalidating the Data Cache in a [Route Handler](/docs/app/building-your-application/routing/router-handlers) **will not** immediately invalidate the Router Cache as the Router Handler isn't tied to a specific route. This means the [automatic invalidation period](#duration-and-invalidation) will be applied, and the Router Cache will continue to serve the previous payload until a hard refresh, or the automatic invalidation period has elapsed.
- Revalidating the Data Cache in a [Server Action](/docs/app/building-your-application/data-fetching/server-actions) you can use [`revalidatePath`](#revalidatepath) or [`revalidateTag`](#fetchoptionsnexttag-and-revalidatetag) in a [Server Action](/docs/app/building-your-application/data-fetching/server-actions).
...
The `refresh` option of the `useRouter` hook can be used to manually refresh a route. This completely clears the Router Cache, and makes a new request to the server. `refresh` does not revalidate the Data or Full Route Cache, it reuses the cached data and rendering result, if it exists.
...
Since the render output depends on data, using `cache: 'no-store'` will also skip the Full Route Cache for the route where the `fetch` request is used. That is, the route will be dynamically rendered, but you can still have other cached data requests in the same route.
...
`revalidatePath` allows you manually revalidate data **and** re-render the route segments below a specific path in a single operation. Calling `revalidatePath` methods revalidate the Data Cache, which in turn invalidates the Full Route Cache. It does not immediately invalidate the Router Cache due to the [automatic invalidation period](#duration-and-invalidation).
...
> 2. `res.revalidate` applies specifically to the Full Route Cache, and not the Data Cache or Router Cache.
> 3. `res.revalidate` returns a promise, whereas `revalidateTag` and `revalidatePath` are non-blocking, _fire-and-forget_ APIs.

### Dynamic Functions

`cookies`, `headers`, `useSearchParams`, and `searchParams` are all dynamic functions that depend on runtime information. Using them will opt a route out of the Full Route Cache, in other words, the routes will be dynamically rendered.
...
The following Route Segment Config options will opt out of the Data and Full Route Cache:
