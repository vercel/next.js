...
React's `cache` function allows you to **memoize** the return value of a function, saving the result to memory. This means you can call the same function multiple times while only executing it once when rendering a React tree.

In React Server Components, the [`fetch` API](#fetch) uses an implementation similar to `cache` to **memoize** requests for the same url and options. This means you can call the same fetch multiple times while only executing it once when rendering a React tree when the url and options are the same.

{/* Diagram */}

Since `fetch` requests are automatically memoized, you can fetch data in the components that need it without props drilling and worrying about the performance impact of making multiple requests for the same data. Memoization applies to `fetch` requests in `generateMetadata`, `generateStaticParams`, Layouts, Pages, and other Server Components.
...
React's `cache` function is used specifically for memoizing the return values of functions when rendering the React tree. Since the memoization is not shared across requests and only applies during the rendering of the React tree, there is no revalidation. 

#### Opting out

To opt-out of memoization in `fetch` you can pass an `AbortController` `signal` to the request.
...
There is no opt-out for the `cache` function as that has to be intentionally added to functions.
...
At request time, the React Server Payload is stored in the client-side [Router Cache](#router-cache) - a separate in-memory cache, split by individual routes. This Router Cache is used to improve the navigation experience by prefetching.
