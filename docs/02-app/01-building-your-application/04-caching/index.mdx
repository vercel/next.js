...
{/* TODO: Update introduction in light of PPR, add video, should say "components" instead of "routes" */}

By default, Next.js will cache as much as possible to improve performance and reduce cost. This means routes are [**statically rendered**](/docs/app/building-your-application/rendering/server-components#static-rendering-default) and data is decoupled from rendering, allowing you to cache and revalidate individual data requests.

{/* Simplified diagram showing client, router cache, Server */}

There are two main caching layers in Next.js:

- **Data Cache**: Stores data requests across user requests and deployments. This **reduces the number of requests made to your data source** (e.g. database, CMS, API) during [rendering](/docs/app/building-your-application/rendering).
- **Router Cache**: Automatically stores the result of rendering on the server at build time (or during revalidation), and on the client at request time. This improves the [navigation](/docs/app/building-your-application/routing/linking-and-navigating#how-routing-and-navigation-works) experience (see [prefetching](/docs/app/building-your-application/routing/linking-and-navigating#2-prefetching) and [partial rendering](/docs/app/building-your-application/routing/linking-and-navigating#4-partial-rendering)), **reduces the number of server requests**, and computation cost of rendering.

Mutating data or revalidating **Data Cache** will in turn update the **Router Cache** by rerendering the components to show the latest data.

As a developer, you should:

1. Decide which data requests to cache or revalidate.
2. Decide whether a component should be statically or dynamically rendered (e.g. rely on runtime information such as cookies, headers).
3. Wrap components that are dynamic or rely on data in Suspense boundaries, to enable [streaming](/docs/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense).

Here's an overview of the APIs you can use to configure data requests in Next.js:

| API                                               | Purpose                                                                                        | Where                                             |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| [revalidatePath]()                                | Invalidate the Data and Router Cache after a mutation, based on path                           | Server Actions, Route Handlers                    |
| [revalidateTag]()                                 | Invalidate the Data and Router Cache after a mutation, based on tags                           | Server Actions, Route Handlers                    |
| [unstable_cache]()                                | Cache direct database queries or data fetches using third-party libraries (e.g. useSWR)        | Server Components, Server Actions, Route Handlers |
| [unstable_noStore]()                              | Opt data requests out of caching when using direct database queries and third-party libraries. |
| Server Components, Server Actions, Route Handlers |
| [Dynamic Functions]()                             | Access runtime information (opt a route out of static rendering)                               |                                                   |
| [React Suspense]()                                | Display a fallback while data is loading, or during dynamic rendering                          |                                                   |
| [React Cache](#react-cache)                       | Memoize data requests during a React render pass                                               |                                                   |
| [Native `fetch` API](#native-fetch-api)           | Memoize data requests during a React render pass.. Fetch, cache, and revalidate data           | Server Components, Server Actions, Route Handlers |
