---
title: Data Cache
description: Learn more about persistent data caching in Next.js
---
...
`fetch` requests are cached and [deduplicated]() automatically. But you can opt individual `fetch` requests out of caching by setting the `cache` option to `no-store`:
...
>
> Calling `router.refresh` will clear the Router cache, and re-render route segments on the server without invalidating cached data.
>
