...
For example, the file `public/avatars/me.png` can be viewed by visiting the `/avatars/me.png` path. The code might look like:
...
export function Avatar({ name }) {
  return (
    <Image src={`/avatars/${name}.png`} alt={name} width="64" height="64" />
  )
}
...
Next.js will not cache assets in the `public` folder because they may change. The default caching headers applied are:
