...
> **Warning:** The `worker` strategy is not yet stable. Use with caution.
...
To use the `worker` strategy with the App Router, you need to import the `Partytown` component in your root layout. This ensures that Partytown is properly set up in your application.
...
> **Note**: The `lib={"/_next/static/~partytown/"}` prop is required in the App Router to tell Partytown where Next.js stores its library files. Unless you choose to save these files in a different directory, you must include this path to ensure Partytown can properly load and run its scripts.
