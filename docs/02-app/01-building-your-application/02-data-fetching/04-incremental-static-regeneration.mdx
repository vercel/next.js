...
- [`cacheLife`](/docs/app/api-reference/directives/use-cache#time-based-revalidation-with-cachelife)
- [`cacheTag`](/docs/app/api-reference/directives/use-cache#revalidating-on-demand-with-cachetag)
...
### On-demand revalidation with `revalidateTag` and `cacheTag`

For most use cases, prefer revalidating entire paths. If you need more granular control, you can use the `revalidateTag` function in combination with the `cacheTag` function. For example, you can tag individual `fetch` calls:
...
If you are using an ORM or connecting to a database, you can use `use cache` and `cacheTag`:
