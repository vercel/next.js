...
To enable tainting in your application, set the Next.js Config [`experimental.taint`](/docs/app/api-reference/next-config-js/experimental#taint) option to `true`:
...
export async function Page({ searchParams }: { searchParams: { id: number } }) {
