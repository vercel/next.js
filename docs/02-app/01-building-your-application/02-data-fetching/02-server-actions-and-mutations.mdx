...
Server Components can use the inline function level or module level `"use server"` directive. To inline a Server Action, add the `"use server"` to the top of the function body:
...
- Server Actions are just functions. This means they can be reused anywhere in your application.
...
> - Refer to the [Zod documentation](https://zod.dev/) for more information.
...
If you would like to redirect the user to a different route after the completion of a Server Action, you can use [`redirect`](/docs/app/api-reference/functions/redirect) API. `redirect` should be called outside of the `try/catch` block:
