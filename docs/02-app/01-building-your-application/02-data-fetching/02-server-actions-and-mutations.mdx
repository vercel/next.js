...
- Since Server Actions execute on the server, they are an alternative to creating [Route Handlers](/docs/app/building-your-application/routing/route-handlers) (API endpoints) to mutate data.
  - Server Actions use the `POST` method, and only this HTTP method is allowed to invoke them.
...
#### Displaying a form loading state
...
Server Actions can also return [serializable objects](https://react.dev/reference/react/use-server#serializable-parameters-and-return-values). For example, your Server Action might handle errors from creating a new item by returning a message:
...
TODO: Recommend try/catch, errors are hidden with a digest
