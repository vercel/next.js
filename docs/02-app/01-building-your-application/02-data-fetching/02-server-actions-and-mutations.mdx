...
While it's common to use Server Actions with `<form>` elements, they can also be invoked from other parts of your code such as event handlers and `useEffect`.
...
> **Good to know:** To improve the user experience, we recommend using other React APIs like [`useOptimistic`](https://react.dev/reference/react/useOptimistic) and [`useTransition`](https://react.dev/reference/react/useTransition) to update the UI before the Server Action finishes or show a pending state.
