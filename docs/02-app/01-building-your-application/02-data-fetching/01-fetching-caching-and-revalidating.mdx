...
- The Next.js `unstable_cache` function is used to [memoize](/docs/app/building-your-application/caching#request-memoization) data requests.
...
export const getItem = unstable_cache(async (id: string) => {
...
export const getItem = unstable_cache(async (id) => {
  const item = await db.item.findUnique({ id })
  return item
}, ['item-id'], { revalidate })
```
