...
### Secured Server Action IDs

Next.js generates encrypted, non-deterministic IDs to allow the client to reference and call Server Actions. These IDs are periodically recalculated between builds for enhanced security.
...
Note that in Docker environments, IDs are not cached, so each build will generate new Server Action IDs.
