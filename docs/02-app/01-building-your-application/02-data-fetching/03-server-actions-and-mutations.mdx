...
Note that it is important to return an object instead of throwing an error here, because uncaught errors in Server Actions are considered to be internal server error and the error message [will not be sent to the client](/docs/app/building-your-application/routing/error-handling#securing-sensitive-error-information) in production to avoid leaking sensitive information. See more information on [error handling](#error-handling) in Server Actions.
...
Therefore, if you want to return a user-friendly error message to be displayed in the UI, you should return a serializable object with the error message, rather than throwing an error:
...
In the client, you can then use the returned object to display the error message. We recommend using the [`useActionState`](https://react.dev/reference/react/useActionState) hook to simplify this process. See [Server-side form validation](#server-side-form-validation) for an example.

#### Server Errors

When an **unexpected** error occurs, it'll be caught by the nearest [`error.js`](/docs/app/building-your-application/routing/error-handling) or `<Suspense>` boundary on the client. Therefore you can use `error.js` to let the user know that something has gone wrong.
