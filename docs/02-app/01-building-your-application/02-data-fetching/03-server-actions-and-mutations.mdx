...
> **Good to know:**
>
> - To display form errors in the user UI, you should return a **serializable object** instead of throwing an error (e.g. `throw new Error('...')`). Uncaught errors in Server Actions are considered to be an internal server error and the error message [will not be sent to the client](/docs/app/building-your-application/routing/error-handling#securing-sensitive-error-information) in production to avoid leaking sensitive information. See more information on [error handling](#error-handling) in Server Actions.
> - Throwing errors will trigger the nearest the nearest `error.js` boundary - see [Error Handling](/docs/app/building-your-application/data-fetching/server-actions-and-mutations#error-handling)
...
Uncaught errors in Server Actions are considered to be internal server errors. As a result, in production, the error message is omitted and instead [a generic error with a digest](/docs/app/building-your-application/routing/error-handling#securing-sensitive-error-information) is sent to avoid leaking potentially sensitive information.

To return a user-friendly error message to be displayed in the UI, you should return a **serializable object** with the error message, rather than throwing an error:

```ts filename="app/actions.ts" switcher
'use server'

export async function signIn(formData: FormData) {
  const passwordIsCorrect = await checkPassword(formData.get('password'))
...
We recommend using the [`useActionState`](https://react.dev/reference/react/useActionState) hook to display the error message. See [Server-side form validation](#server-side-form-validation) for an example.

#### Server Errors

When an **unexpected** server error occurs, you can use `try`/`catch` to know that something went wrong:

```tsx filename="app/components/login-button.tsx" switcher
'use client'
    ...
    <button
      onClick={async () => {
        try {
          ...
          if (e instanceof Error && 'digest' in e) {
            console.error(`Something went wrong. Digest: ${e.digest}`)
          } else {
...
```jsx filename="app/components/login-button.js" switcher
'use client'
...
If you use a [form action](#forms), uncaught errors will also be caught by the nearest [`error.js`](/docs/app/building-your-application/routing/error-handling).
...
To prevent potentially sensitive information from being exposed, Next.js automatically omits uncaught error messages in production, and attaches to the error instance a `digest` property that can help you identify and debug the error.

See more about [Handling Server Errors](/docs/app/building-your-application/routing/error-handling#handling-server-errors) in the error handling documentation.
