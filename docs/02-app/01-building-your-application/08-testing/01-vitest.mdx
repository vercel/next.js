...
> **Good to know:** Since `async` Server Components are new to the React ecosystem, Vitest currently does not support them. We recommend using an E2E tests for `async` components for now. You can still run **unit tests** for Server and Client Components with Vitest.
