...
There are several reasons why you would want to switch to Next.js:
...
1. **Slow initial page loading time**: If you have built your application with the [default Vite
   ...
   your application is a purely client-side application. Client-side applications — also known as
   single-page applications (SPAs) — often suffer from a slow initial page loading time. This
...
1. **No automatic code splitting**: The previous issue can be somewhat managed with code splitting.
...
1. **Choose the data fetching strategy**: depending on your need, Next.js allows you to choose your
...
1. **Middleware**: The [Next.js middleware](/docs/app/building-your-application/routing/middleware)
...
file, which is a [React Server Component](https://nextjs.org/docs/getting-started/react-essentials#server-components)
...
4. Next.js already includes by default the
   [meta charset](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta#charset) and
   [meta viewport](https://developer.mozilla.org/en-US/docs/Web/HTML/Viewport_meta_tag) tags, so you
...
[Client Component](/docs/getting-started/react-essentials#client-components) by the `'use client'`
directive. Without that directive the component would have been a
[Server Component](/getting-started/react-essentials#server-components).

On Next.js, Components are pre-rendered
([SSR](/docs/app/building-your-application/rendering#component-level-client-and-server-rendering))
by default before being sent to the client, but since we want to first have a purely client-side
application, you need to tell Next.js to disable the pre-rendering for the `<App>` component by
...
- Enable pre-rendering (SSR) of your application by first making sure to
  [safely access Web APIs](http://web.archive.org/web/20230318161007/https://nextjs.org/docs/migrating/from-create-react-app#safely-accessing-web-apis).
- Migrate from React Router to the [Next.js App Router](/docs/app/building-your-application/routing) to get:
  ...
  - [React Server Components](/docs/getting-started/react-essentials#server-components)
