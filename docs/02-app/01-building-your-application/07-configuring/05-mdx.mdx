...
The `@next/mdx` package, and related packages, are used to configure Next.js so it can process markdown and MDX. **It sources data from local files**, allowing you to create pages with a `.mdx` extension, directly in your `/pages` or `/app` directory.

Install these packages render MDX with Next.js:
...
This allows `.md` and `.mdx` files to act as pages or routes in your application.
...
> - Learn how to [render and style custom elements](#render-and-style-custom-elements).

## Rendering markdown

You can render markdown using Next.js's file based routing or by importing markdown files into other pages.
...
You can use markdown in these files and even import React components directly inside your MDX page:
...
Navigating to the `/my-mdx-page` route should display your rendered MDX page.

### Using imports

<AppOnly>

Create a new page within the `/app` directory and a markdown file wherever you'd like:
...
Create a new page within the `/pages` directory and a markdown file wherever you'd like:
...
Import the markdown file inside the page to display the content:
...
One of the benefits of using markdown is that it maps to native HTML elements, making writing fast without having to write code.
...
To style your markdown, you can provide your own components that map to the generated HTML elements. Styles and components can be done locally and globally.

### Global styles and components

Adding styles and components in `mdx-components.tsx` will effect _all_ markdown files in your application.
...
You can apply local styles and components to specific pages by passing them into imported markdown components. These will merge with and override [global styles and components](#global-styles-and-components).
