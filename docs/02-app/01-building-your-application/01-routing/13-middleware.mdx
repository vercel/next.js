...
- Authentication: Ensure secure access to sensitive content by authenticating users before allowing entry to certain pages. This process can also include checking session cookies to confirm user identity and maintain their session state.
- Server-Side Redirects: Direct users to the optimal page based on their role, location, or device type.
- Path Rewriting: Support A/B testing, feature rollouts, or legacy paths by dynamically adjusting request paths according to specific criteria.
- Bot Detection and Mitigation: Safeguard your resources and maintain service availability for real users by detecting and blocking unwanted bot traffic.
- Logging and Analytics: At the request entry point, capture data for analytics or logging to analyze traffic patterns, understand user behavior, or address issues swiftly.
- Feature Flagging: Without full deployments, dynamically manage feature availability for users, facilitating gradual rollouts or testing.
...
- Complex Data Fetching and Manipulation: Middleware is not designed for direct data fetching or manipulation. These complex operations should be handled within route handlers or server-side utilities.
- Heavy Computational Tasks: Middleware should be lightweight to ensure quick response times. Heavy computational tasks or long-running processes should be offloaded to background services or dedicated Route Handlers.
- Extensive Session Management: While Middleware can manage basic session tasks, it is not suited for extensive session management that requires persistent state, frequent updates, or complex logic. Such tasks should be managed by dedicated authentication services or within Route Handlers.
- Direct Database Operations: Performing direct database operations within Middleware is not recommended. Database interactions should be encapsulated within Route Handlers or server-side utilities to ensure data integrity and security.

## Convention

Use the file `middleware.ts` (or `.js`) in the root of your project to define Middleware. For example, at the same level as `pages` or `app`, or inside `src` if applicable.

> **Note**: While only one `middleware.ts` file is supported per project, you can still organize your middleware logic modularly. Break out middleware functionalities into separate `.ts` or `.js` files and import them into your main `middleware.ts`. This allows for cleaner management of route-specific middleware through [matchers](#matching-paths), aggregated in the `middleware.ts` for centralized control. By enforcing a single middleware file, it simplifies configuration, prevents potential conflicts, and optimizes performance by avoiding multiple middleware layers.

### Example

```ts filename="middleware.ts" switcher
import { NextResponse } from 'next/server'
import { authMatcher, authMiddleware } from '/auth'
import { rateLimitMatcher, rateLimitMiddleware } from '/middleware/rateLimit'

export const config = {
  matcher: [...authMatcher, ...rateLimitMatcher],
}
...
import { NextResponse } from 'next/server'
import { authMatcher, authMiddleware } from '/auth'
import { rateLimitMatcher, rateLimitMiddleware } from '/rateLimit'
