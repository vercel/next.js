...
Route Handlers are not cached by default. You can, however, opt into caching for `GET` methods. Other supported HTTP methods are **not** cached. To cache a `GET` method, use the [`use cache`](/docs/app/api-reference/directives/use-cache) directive.
...
You can [revalidate cached data](/docs/app/building-your-application/data-fetching/incremental-static-regeneration) using Incremental Static Regeneration (ISR), and the [`cacheLife`](/docs/app/api-reference/directives/use-cache#revalidating) function in combination with the [`use cache`](/docs/app/api-reference/directives/use-cache) directive.

In the example below, the `GET` methods output will be cached for 1 week:
