...
Starting in Next.js 15, the `next/server/testing` package contains utilities to help unit test Middleware files. Unit testing Middleware can help ensure that Middleware only runs on desired paths and the custom routing logic in Middleware works as intended before code reaches production.

The `doesMiddlewareMatch` function can be used to assert whether Middleware will run for the provided URL, headers, and cookies.

```js
import { doesMiddlewareMatch } from 'next/server/testing'
...
```js
import { isRewrite, getRewrittenUrl } from 'next/server/testing'
