...
In a Server Component, you would use [`redirect()`](/docs/app/api-reference/functions/redirect) instead.
...
> - `redirect` in the App Router also accepts absolute URLs and can be used to redirect to external links. For the Pages Router, you can use [window.location](https://developer.mozilla.org/en-US/docs/Web/API/Window/location).
