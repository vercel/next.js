...
- When using loading.js and Suspense, the page content is split into two phases:
  - **Preview Phase:** In this phase, the initial content (such as a skeleton or "loading" text) is rendered on the server-side by fallback or loading.js.
  - **Loading Phase:** In this phase, the main content is fetched from the server using JavaScript and replaces the initial content.
  
  **Important Note:**
  Search engines that do not support JavaScript will only see the content from the preview phase. Therefore, if your SEO-critical content is loaded in the loading phase, you must also include it in fallback or loading.js to be visible to search engines.
  
  **Example:**
  Suppose your main page content consists of a product list. If you want this list to appear in search results, you must also include the code for the list in fallback or loading.js.
