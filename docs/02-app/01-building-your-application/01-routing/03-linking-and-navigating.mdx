...
When incrementally migrating from `pages/` to `app/`, Next.js' router will automatically handle hard navigating between the two. To detect when going from `pages/` to `app/` there is a client router filter which uses probabilistic checking of `app` routes which can have occasional false positives. By default this should be very rare as we configure the false positive likelihood to be 0.01%. This can be customized via the `experimental.clientRouterFilterAllowedRate` `next.config.js` option.

Alternatively if you would like to disable this handling completely and manage the routing between `pages/` and `app/` manually you can set `experimental.clientRouterFilter` to `false` in `next.config.js`.
