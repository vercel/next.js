...
DISCLAIMER:. When you use the structure above, a `<Modal>` followed by `@auth` and finally accessed by `/login`, you need to comprehend due the nature of parallel, the whole context passed until now, if have some will node be referenced.
