...
Parallel routes are created using named **slots**. Slots are defined with the `@folder` convention. For example, the following file structure defines two explicit slots: `@analytics` and `@team`:
...
## Slot active state and navigation

By default, Next.js keeps track of the active _state_ (subpage) for each slot. However, the content rendered within a slot will depend on the type of navigation:
...
- **Hard Navigation**: Since Next.js cannot recover the active state of a slot after a full page load, Next.js will render the [`default.js`](#defaultjs) file for the other slots, or `404` if `default.js` doesn't exist.
...
You can define a `default.js` file to render as a fallback when Next.js cannot recover a slot's active state after a full page refresh.
...
On refresh, Next.js will first try to render the unmatched slot's `default.js` file for `@analytics`. If that's not available, a 404 gets rendered.
