...
{/* To do: Update main diagram to remove `props.team` and `props.analytics`, prefer destructuring */}
...
> **Good to know**:
>
> - The `404` for unmatched routes helps ensure that you don't accidentally render a route that is not a parallel.
...
You can use Parallel Routes to conditionally render a page based on certain conditions, such as user role. For example, to render a different dashboard page depending on the `/admin` or `/user` roles:
...
Within slots, you can create tabs to allow users to navigate each Parallel Routes independently. For example, the `@analytics` slot has two subpages: `/page-views` and `/visitors`.

{/* TODO: Add diagrams for tabs */}
...
- **Preserveing context** when the page is refreshed, instead of closing the modal.
- **Closing the modal on backwards navigation** rather than going to the previous route.
- **Reopening the modal on forwards navigation**.

Consider the following UI pattern, where a user can open a login modal from a layout, or by navigating to the `/login` route:

{/* TO DO: Update these diagrams */}
...
To implement this pattern, create a `/login` route that renders your main login page. This page will be used when navigating to `/login` directly.
...
Then, create a new `@modal` slot, and inside it, add a `default.js` file that returns `null`. This ensures that the modal is not rendered when it's not active.
...
Inside your `@modal` slot, intercept the `/login` route by creating a `/(.)login` folder with a `page.tsx` file.

```tsx filename="app/@modal/(.)login/page.tsx" switcher
...
```jsx filename="app/@modal/(.)login/page.js" switcher
import { Modal } from '@/app/ui/modal'
import { Login } from '@/app/ui/login'
...
> **Good to know:** The intercepting route convention, e.g. `(.)`, depends on your file-system structure. See the [convention](/docs/app/building-your-application/routing/intercepting-routes#convention) section for more information.

Although it's up to you to implement the design of your `<Modal>` component, you can leverage the Next.js router to open and close the modal. That is, ensuring the modal closes/opens when navigating backwards and forwards.
...
When the user clicks the link, the modal will open instead of navigating to the `/login` page because of the route interception. On refresh, the user will be taken to the `/login` page.

### Closing the modal

If a modal was initiated through client navigation, e.g. by using `<Link href="/login">`, you can dismiss the modal by calling `router.back()` or by using the `Link` component.
...
> **Good to know**: By separating the `<Modal>` functionality from the modal content (`<Login>`), you can not only reuse `<Modal>`, but make its children (or content) Server Components. See [interleaving Client and Server Components](/docs/app/building-your-application/rendering/composition-patterns#supported-pattern-passing-server-components-to-client-components-as-props) for more.
...
See the [Loading UI](/docs/app/building-your-application/routing/loading-ui-and-streaming) and [Error Handling](/docs/app/building-your-application/routing/error-handling) docs for more.
