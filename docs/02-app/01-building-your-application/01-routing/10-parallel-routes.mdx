...
You can use Parallel Routes to conditionally render a page based on certain conditions, such as user role. For example, to render a `/admin` or `/user` dashboard page depending on the user's role:
...
Within slots, you can create tabs to allow users to navigate each slot independently. For example, the `@analytics` slot has two subpages: `/page-views` and `/visitors`.

{/* TODO: Add diagrams for tabs */}

Within `@analytics`, create a `layout` to share the tabs between the two pages:
...
You can use Parallel Routes to create modals, for example:
...
```tsx filename="app/layout.tsx" switcher
export default async function Layout({ auth }: { auth: React.ReactNode }) {
  return (
    <>
      {/* ... */}
      {props.auth}
