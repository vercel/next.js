...
If an error is thrown inside `generateMetadata`, it will not be caught by the `error.js` boundary. Instead, it must be caught and handled inside of `generateMetadata` itself.
