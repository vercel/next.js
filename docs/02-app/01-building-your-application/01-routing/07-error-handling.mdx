...
During production, the `Error` instance forwarded to the client only includes a generic `message` and `digest` property.
...
During development, the `Error` instance will be serialized and forwarded from the Server Component to the `error.js` Client Component.
