...
> **Note**: Only assets that are in the `public` directory at [build time](/docs/api-reference/cli.md#build) will be served by Next.js. Files added to the directory after build time won't be discoverable until your code is rebuilt. We recommend using a third party service like [AWS S3](https://aws.amazon.com/s3/) for persistant file storage.
