...
When combined with [Incremental Static Regeneration](/docs/basic-features/data-fetching/incremental-static-regeneration.md), it will run in the background while the stale page is being revalidated, and the fresh page served to the browser.

Because `getStaticProps` runs at build time, it does **not** receive data that’s only available during request time, such as query parameters or `HTTP` headers, as it generates static `HTML`. If you need access to the request for your page, e.g. for authentication purposes, consider using [middlewares](/docs/middleware.md) additionally to `getStaticProps`.
