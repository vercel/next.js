...
For example, linked dependencies outside the project root (via `npm link`, `yarn link`, `pnpm link`, etc.) are not resolved by default. Configure [`experimental.turbopackAdditionalRoots`](/docs/turbopack/configuration#additional-roots-experimental) for symlinked dependencies.

Use [`turbopack.root`](/docs/turbopack/configuration#root-directory) when your application directly imports files outside the detected root rather than reaching them through symlinks.
