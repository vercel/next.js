...
The `experimental.turbopackChunking` option lets you configure Turbopack's production
JavaScript chunker. These options allow you to change the assumptions the chunker makes
about user behavior, tweak the raw size thresholds it uses, and enable the experimental
component chunks feature.
...
> **Good to know**: `minComponentChunkSize` and `generateComponentChunks` apply to the App Router only. The remaining options apply to both routers.
...
Component chunks apply to the App Router only.
