...
Pages are also validated during [Static HTML export](/docs/pages/guides/self-hosting#static-html-export) and any warnings / errors will be printed to the terminal. Any AMP errors will cause the export to exit with status code `1` because the export is not valid AMP.
...
When using [Static HTML export](/docs/pages/guides/self-hosting#static-html-export) statically prerender pages, Next.js will detect if the page supports AMP and change the exporting behavior based on that.
