...
If `cb` returns `false`, the Next.js router will not handle `popstate`, and you'll be responsible for handling it in that case. See [Disabling filesystem routing](/docs/pages/guides/custom-server#disabling-file-system-routing).
