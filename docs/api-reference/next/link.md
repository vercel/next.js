...
- `locale` - One of the locale identifier or `false` if the locale is already in the `href` path. Also, if no `locale` prop is provided, the currently active locale is used during client-transitions
