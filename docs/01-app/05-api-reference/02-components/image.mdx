...
- The image is [statically imported](/docs/app/api-reference/components/image#local-images)
...
`minimumCacheTTL` allows you to configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](/docs/app/api-reference/components/image#local-images) which will automatically hash the file contents and cache the image forever with a `Cache-Control` header of `immutable`.
