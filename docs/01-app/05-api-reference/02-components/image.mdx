...
- The image is [statically imported](/docs/app/building-your-application/optimizing/images#local-images)
...
Defines the size of the image at different breakpoints. Used by the browser to choose the most appropriate size from the generated `srcset`.
...
If the `Accept` header matches more than one of the configured formats, the first match in the array is used. Therefore, the array order matters. If there is no match (or the source image is [animated](#animated-images)), it will use the original image's format.
...
`minimumCacheTTL` allows you to configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](/docs/app/building-your-application/optimizing/images#local-images) which will automatically hash the file contents and cache the image forever with a `Cache-Control` header of `immutable`.
...
By default, the [Next.js Image Loader](/docs/app/guides/image-optimization#the-image-loader) does not optimize SVG images for a few reasons:
...
Use `className` or `style`, not `styled-jsx`. In most cases, we recommend using the `className` prop. This can be an imported [CSS Module](/docs/app/building-your-application/styling/css), a [global stylesheet](/docs/app/building-your-application/styling/css#global-styles), etc.
...
To create an image that automatically resizes to fit the full width of its container while maintaining its aspect ratio, set the `width` to `100%` and the `height` to `auto`.
...
To create an image that fills the full width of its parent container, add the [`fill` prop](#fill) with the `objectFit` prop set to `cover`.
