...
| [`replace`](#replace)       | `replace={false}`        | Boolean                 | -        |
| [`scroll`](#scroll)         | `scroll={false}`         | Boolean                 | -        |
| [`prefetch`](#prefetch)     | `prefetch={false}`       | String, Boolean or null | -        |
| [`onNavigate`](#onnavigate) | `onNavigate={(e) => {}}` | Function                | -        |
...
Next.js prefetches and loads the linked route (denoted by the `href`) and its data in the background to improve the performance of client-side navigations. If the prefetched data has expired by the time the user hovers over a `<Link />`, Next.js will attempt to prefetch it again. **Prefetching is only enabled in production**.

The following prefetch strategies can be passed to the `prefetch` prop:

- **`viewport` (default)**: Prefetching happens when a `<Link />` component enters the user's viewport (initially or through scroll).
- `predict`: Prefetching happens when user's mouse is moving towards the link.
- `intent`: Prefetching happens when user's mouse hovers the link.
- `true`: Same as `viewport`, but with `prefetchScope` set to `full`.
...
export default function Page() {
  return (
    <Link href="/dashboard/dynamic" prefetchScope="full">
      Dashboard
    </Link>
  )
}
