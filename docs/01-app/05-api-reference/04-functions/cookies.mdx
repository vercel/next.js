...
- `cookies` is a [Dynamic API](/docs/app/getting-started/server-and-client-components#dynamic-apis) whose returned values cannot be known ahead of time. Using it in a layout or page will opt a route into [dynamic rendering](/docs/app/getting-started/server-and-client-components#dynamic-rendering).
