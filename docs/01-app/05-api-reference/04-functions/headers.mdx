...
- `headers` is a [Dynamic API](/docs/app/getting-started/server-and-client-components#server-rendering-strategies#dynamic-apis) whose returned values cannot be known ahead of time. Using it in will opt a route into **[dynamic rendering](/docs/app/getting-started/server-and-client-components#dynamic-rendering)**.
