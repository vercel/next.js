...
The `generateStaticParams` function can be used in combination with [dynamic route segments](/docs/app/building-your-application/routing/dynamic-routes) to [**statically generate**](/docs/app/getting-started/caching-and-revalidating#static-rendering) routes at build time instead of on-demand at request time.
