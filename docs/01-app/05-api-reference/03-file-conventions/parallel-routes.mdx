...
Inside your `@auth` slot, intercept the `/login` route by importing the `<Modal>` component and its children into the `/login/page.tsx` file, and updating the folder name to `/(..)login/page.tsx`.
