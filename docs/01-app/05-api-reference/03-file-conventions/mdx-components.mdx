...
Use the file `mdx-components.tsx` (or `.js`) in the root or src folder of your project to define MDX Components. For example, at the same level as `pages` or `app`, or inside `src` if applicable.
