...
You can confirm if a route is [static](/docs/app/getting-started/server-and-client-components#static-rendering-default) or [dynamic](/docs/app/getting-started/server-and-client-components#dynamic-rendering) by building your application using `next build --debug`, and checking the output in your terminal. Static (or prerendered) routes will display a `○` symbol, whereas dynamic routes will display a `ƒ` symbol. For example:
...
- The presence of [Dynamic APIs](/docs/app/getting-started/server-and-client-components#dynamic-apis) which rely on runtime information.
