...
This means `use cache` cannot be used with [request-time APIs](/docs/app/getting-started/server-and-client-components#dynamic-apis) like `cookies` or `headers`.
...
> - If any of the nested children in the route use [Dynamic APIs](/docs/app/getting-started/server-and-client-components#dynamic-apis), then the route will opt out of prerendering.
