...
These are available building blocks. Platform adapters exist for several of these providers today, but not all adapters leverage the full CDN infrastructure (for example, edge-based PPR resuming or distributed KV for cache tags). See the [Deploying](/docs/app/getting-started/deploying#adapters) page for the current list. For CDN-specific caching considerations (including known limitations with `Vary` on custom headers), see [CDN Caching](/docs/app/guides/cdn-caching).
