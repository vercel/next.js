...
**Shared Cache Recommended** means multiple server instances benefit from a [shared cache backend](/docs/app/api-reference/config/next-config-js/cacheHandlers) to coordinate. Without shared cache, each instance maintains its own cache independently — features still work correctly on each instance, but revalidation events don't propagate across instances.

## CDN Infrastructure Compatibility

Every major CDN has the infrastructure primitives to build full Next.js support:
...
Implementation complexity varies, and the table above shows available primitives, not finished integrations. Platform adapters are available for several of these providers today, with more in development. For CDN-specific caching considerations (including known limitations with `Vary` on custom headers), see [CDN Caching](/docs/app/guides/cdn-caching).
...
Next.js's [rendering model](/docs/app/guides/rendering-philosophy) operates at the component level rather than the route level. Component-level rendering boundaries, per-function caching, and on-demand revalidation of any route are capabilities that transfer complexity from application code into infrastructure.

This is a trade-off. Frameworks with route-level rendering boundaries have simpler infrastructure requirements because the boundary is coarser. Next.js's finer-grained boundaries provide more flexibility for developers at the cost of more requirements for hosting platforms. When comparing framework hosting complexity, the capabilities each framework provides should be part of the comparison.
