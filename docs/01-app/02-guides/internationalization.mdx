...
function getLocale(request: NextRequest): string {
  // To persist locale preference, check a cookie before negotiating:
  // const savedLocale = request.cookies.get('locale')?.value
  // if (savedLocale && locales.some((l) => l === savedLocale)) return savedLocale
...
export function proxy(request: NextRequest) {
  ...
  // To persist locale preference, save to cookie:
...
To help search engines discover your language-specific URLs, you can declare alternate versions using the Metadata API:
...
export const metadata = {
  alternates: {
    languages: {
      ...
      'nl-NL': 'https://example.com/nl-NL',
      'x-default': 'https://example.com/en-US',
    },
  },
}
```

This generates `hreflang` tags that tell crawlers about available language variants. See [`alternates`](/docs/app/api-reference/functions/generate-metadata#alternates) for more options.
...
Store the user's locale preference in a cookie to persist their choice across sessions:
...
export function proxy(request: NextRequest) {
  ...
  // Try to use saved locale from cookie, otherwise detect it
  const savedLocale = request.cookies.get('NEXT_LOCALE')?.value
  const locale = savedLocale && locales.includes(savedLocale)
    ? savedLocale
    : getLocale(request)
  
  // Redirect with locale and save preference
  request.nextUrl.pathname = `/${locale}${pathname}`
  const response = NextResponse.redirect(request.nextUrl)
