...
For optimal SEO with internationalized content, add `hreflang` tags to indicate alternate language versions of your pages:
  ...
  return (
    <html lang={lang}>
      <head>
        ...
        <link rel="alternate" hrefLang="nl-NL" href="https://example.com/nl-NL" />
...
Consider storing the user's locale preference in a cookie to persist their choice across sessions:
...
export function proxy(request: NextRequest) {
  ...
  if (savedLocale && !pathname.startsWith(`/${savedLocale}`)) {
    request.nextUrl.pathname = `/${savedLocale}${pathname}`
    const response = NextResponse.redirect(request.nextUrl)
    return response
  }

  // Otherwise, detect locale and save preference
  const locale = getLocale(request)
  request.nextUrl.pathname = `/${locale}${pathname}`
  const response = NextResponse.redirect(request.nextUrl)
...
For better type safety with dictionaries, define types for your translations:
...
- **Always await params**: In Next.js 15+, route parameters are promises and must be awaited. Forgetting to `await params` will cause runtime errors.
- **Avoid client-side locale detection**: Detecting locale on the client can cause hydration mismatches. Always handle locale detection in middleware or on the server.
- **Consider default locale SEO**: For the default locale, you might want to serve content at both `/` and `/en-US` to avoid duplicate content issues.
- **Handle locale switching carefully**: When implementing locale switchers, ensure you preserve the current path structure to maintain user context.
