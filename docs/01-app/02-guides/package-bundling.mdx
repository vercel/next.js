...
Each analyzer run saves a snapshot that you can use on either side of a comparison. Use `--snapshot-name` to give the current run a recognizable name:

```bash filename="Terminal"
npx next experimental-analyze --snapshot-name before-refactor
```

Run the analyzer again after making changes, then use the **Compare from** and **to** controls to select either named snapshot.

### Step 5: Write output to disk for sharing or diffing
