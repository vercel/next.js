...
- [Next.js Bundle Analyzer for Turbopack](#nextjs-bundle-analyzer)
...
```bash filename="Terminal" package="yarn"
yarn next analyze --output
```

```bash filename="Terminal" package="pnpm"
pnpm next analyze --output
```

```bash filename="Terminal" package="bun"
bunx next analyze --output
```
...
### Server modules in client bundles

If an import trace reaches database code through a shared value, move that value to a separate module and import it directly from the Client Component. If a shared module already exists, update the import to use it.

Add `import 'server-only'` to the database module to catch future client imports at build time. The import change removes the database dependency from the bundle; `server-only` prevents it from returning. See [Preventing environment poisoning](/docs/app/getting-started/server-and-client-components#preventing-environment-poisoning) for an example.
