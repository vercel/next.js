...
To produce a production build **and** save bundle analysis in one command, use `next build --analyze` instead:
...
Build analysis is saved to `.next/diagnostics/analyze`. When browser source maps are enabled in your build configuration, `--analyze` still publishes them. Otherwise, it keeps mappings for analysis without publishing browser maps. The separate `next analyze` command opens the interactive analyzer without producing a production build.
