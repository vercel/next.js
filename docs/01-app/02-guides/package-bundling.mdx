...
If an import trace reaches a shared value through a server-owned module, such as a database schema, move the value to an environment-neutral module and import it directly from Client Components. Then [guard the server-owned module with `server-only`](/docs/app/getting-started/server-and-client-components#preventing-environment-poisoning) so later Client Component imports fail at build time. The import change removes the server module from the current client graph; the guard prevents new paths to it.
