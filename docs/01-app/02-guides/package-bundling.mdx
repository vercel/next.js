...
If a Client Component imports a shared value through a database or other server module, import the value from an environment-neutral module instead. Then [guard the server module with `server-only`](/docs/app/getting-started/server-and-client-components#preventing-environment-poisoning) to prevent future client imports. The import change removes the server dependency from the client graph; `server-only` makes an accidental client import a build error.
