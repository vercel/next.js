...
Build analysis is saved to `.next/diagnostics/analyze`. Browser source maps are only published if enabled in your build configuration. The separate `next analyze` command opens the interactive analyzer without producing a production build.
