...
A Client Component can pull database code into its bundle by importing a shared value from a module that also defines database tables. Use the import trace to find that dependency, then move the shared value to a separate module that can run in either environment. If that module already exists, import from it directly.

Add `import 'server-only'` to the module containing the database code to prevent future client imports. Changing the import removes the database dependency from the client bundle; `server-only` makes accidental imports a build error. See [Preventing environment poisoning](/docs/app/getting-started/server-and-client-components#preventing-environment-poisoning) for an example.
