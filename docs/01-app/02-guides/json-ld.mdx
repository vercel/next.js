...
Because this snippet uses `dangerouslySetInnerHTML`, and `JSON.stringify`, to prevent XSS injections, `HTML` tags are scrubbed from the `JSON-LD` payload, by replacing the character `<` with its unicode equivalent, `\u003c`.

Review your organization's recommended approach to sanitize potentially dangerous strings. Alternatively use community maintained alternatives for `JSON.stringify` such as, [serialize-javascript](https://www.npmjs.com/package/serialize-javascript).
