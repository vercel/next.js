...
Because this snippet uses `dangerouslySetInnerHTML`, and `JSON.stringify`, to scrub `HTML` tags from the `JSON-LD` payload, the character `<`, is replaced with its unicode equivalent, `\u003c`.
