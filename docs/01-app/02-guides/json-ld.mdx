...
Our current recommendation for JSON-LD is to render structured data using a native `<script>` tag instead of the `<Script>` component from `next/script` in your `layout.js` or `page.js`.

> **Good to know**: `next/script` is designed for executable JavaScript. JSON-LD isn’t executable code — it’s structured data with a non-JS MIME type — so `next/script` isn’t the right fit.
