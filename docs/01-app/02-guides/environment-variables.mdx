...
oPropertyAccessFromIndexSignature: true in your 	sconfig.json will cause TypeScript errors when accessing process.env properties with dot notation (e.g. process.env.NEXT_PUBLIC_ANALYTICS_ID). Since Next.js relies on dot-access to inline environment variables, you should keep this option disabled.
