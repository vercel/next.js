...
- [Update your Next.js application](#nextjs-version)
...
Use the Node.js version required by your target Next.js release. Next.js 16 requires **Node.js 20.9 or later**. See the [version upgrade guides](/docs/app/guides/upgrading) for other releases.

### Next.js Version

Follow the [version upgrade guide](/docs/app/guides/upgrading) for your target release before migrating routers. To install the latest release, use your preferred package manager:
...
- `isReady` has been removed. On a prerendered route, [`useSearchParams()`](/docs/app/api-reference/functions/use-search-params) makes the Client Component tree up to the nearest `<Suspense>` boundary render on the client. Wrap the component that uses it in `<Suspense>`; a static route can appear to work in development but fail its production build without this boundary.
...
In Next.js 15 and later, `cookies()`, `headers()`, `draftMode()`, and the page's `params` and `searchParams` props are asynchronous. Next.js 16 requires asynchronous access. Follow the [Async Request APIs guide](/docs/app/guides/upgrading/version-16#async-request-apis-breaking-change) for the target version when translating request-dependent data fetching.
...
In Next.js 15 and later, [`fetch()`](/docs/app/api-reference/functions/fetch) responses are not cached by default. Set `cache: 'force-cache'` explicitly to reuse fetched data across requests. Data caching and route prerendering are separate: verify the production build to preserve the static behavior of the original `getStaticProps` page. If Cache Components is already enabled, follow the [caching guide](/docs/app/getting-started/caching) instead.
