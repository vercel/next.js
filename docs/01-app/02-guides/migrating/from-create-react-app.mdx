...
> **Note**: Static export (`output: 'export'`) still does not support server runtime features that require a server. However, App Router dynamic routes can use `useParams` in this mode when `cacheComponents: true` is enabled. To use the full set of Next.js server features, remove `output: 'export'` from your `next.config.ts`.
