...
Create React App uses webpack for bundling. Next.js now defaults to [Turbopack](/docs/turbopack/configuration) for faster local development:
...
You can still provide a [custom webpack configuration](/docs/webpack/configuration) if you need to migrate advanced webpack settings from CRA.
