...
properties with no changes needed. `BASE_URL` is always `/` and does not reflect the Next.js
[`basePath`](/docs/app/api-reference/config/next-config-js/basePath) configuration.
...
If your Vite application configured a custom base URL, create a public environment variable for it:
...
3. **Update usages that need the configured base path to `process.env.NEXT_PUBLIC_BASE_PATH`**
