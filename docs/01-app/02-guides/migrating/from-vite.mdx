...
**`import.meta.glob`** is supported by Turbopack (the default Next.js bundler) with no changes needed. The `as` option (deprecated in Vite 5) is not supported. To load matched files as strings, register a loader for the extension and glob it as usual:

```js
// Before (Vite)
const modules = import.meta.glob('./dir/*.txt', { as: 'raw' })

// After (Next.js / Turbopack), with a loader registered for *.txt
const modules = import.meta.glob('./dir/*.txt', { eager: true })
```

See [Configuring webpack loaders](/docs/app/api-reference/config/next-config-js/turbopack#configuring-webpack-loaders) for the loader setup, and the [import.meta.glob docs](/docs/app/api-reference/turbopack#importmetaglob) for the full API.
