...
If your dev server is using too much memory, you can generate a memory report from a running Turbopack dev server. The report shows what the bundler is retaining, broken down by cell value type and ranked by reference count (a proxy for how much is being held onto). In Turbopack's model, **tasks** are cached computations (e.g. parsing a module, resolving imports) and **cells** are their stored output values.
...
- **Persistent** — cells that survived eviction, either because they are unevictable or because their task couldn't be evicted.

For the top transient types, the report also includes a sample dependency chain to help identify which computation is holding the data.
