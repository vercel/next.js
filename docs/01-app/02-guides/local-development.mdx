...
If your dev server is using too much memory, you can generate a memory report from a running Turbopack dev server. In Turbopack's model, **tasks** are cached computations (e.g. parsing a module, resolving imports) and **cells** are their stored output values.

The report is split into two sections:

- **Transient** — data eviction skips entirely. This is where per-request or per-route accumulation shows up.
- **Persistent** — cells and tasks that survived eviction, either because they are unevictable or because their task couldn't be evicted.

Each section reports two breakdowns: **cells grouped by value type** (ranked by reference count, a proxy for how much is being held onto) and **tasks grouped by task type** (which also surfaces tasks retained with no cells — only metadata). For the persistent section, each task type also reports _why_ eviction couldn't reclaim its tasks (e.g. `skipped_modified` when the data is dirty, `skipped_in_progress` when the task is executing), which points directly at what is being retained and why.
...
If you know the URL of your dev server, you can skip lock file discovery with `--url`:

```bash
npx next internal turbopack-memory --url http://localhost:3000
```
