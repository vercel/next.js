...
### Synchronous Request API warnings remain

Follow the [Async Request APIs](#async-request-apis-breaking-change) migration and
await each request API and update affected callers. In synchronous Client
Components, unwrap the promise with React `use`. Remove `UnsafeUnwrapped` casts
and codemod error comments only after the code uses the asynchronous API.

### An `experimental-edge` configuration error appears

Change the affected route segment runtime configuration to `edge` as described
in [Runtime Configuration](#runtime-configuration-breaking-change).

### An `@next/font` import cannot be resolved

Run the `next/font` codemod, inspect its dependency changes, and confirm imports
use the built-in `next/font` package.

### `NextRequest` no longer provides `geo` or `ip`

Replace `request.geo` and `request.ip` with the geolocation API provided by the
app's hosting provider, then test a request with known location data.
