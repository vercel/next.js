...
For `latest`, stable installs target npm's latest stable Next.js release.
Prerelease installs stay on their matching npm dist-tag (such as `canary`, `rc`,
or `beta`) and upgrade only across a major or minor version boundary. `future`
uses the same channel policy and can retain the installed version while adopting
available Future Defaults. Use the exact target supplied by the CLI; do not
switch release channels or resolve a different target.
