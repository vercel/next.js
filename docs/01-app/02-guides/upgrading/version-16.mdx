...
Recent Next.js releases added coding-agent support, including bundled docs and an `AGENTS.md` setup flow. If you're using an AI coding assistant, have it set up Next.js docs before running the upgrade. This keeps the agent grounded in the right APIs, conventions, and migration guidance while it plans and applies the changes.

The [AI Coding Agents guide](/docs/app/guides/ai-agents) explains how to point an assistant at the right docs before it edits code, which helps with this upgrade and leaves the project set up for future agent work.

#### Set up docs

To make version-matched docs available in the project before the upgrade starts, run:

```bash filename="Terminal"
npx @next/codemod@canary agents-md
```

After upgrading, verify the agent setup still points at the docs for the installed Next.js version. On Next.js 16.2 and later, that should be the bundled docs in `node_modules/next/dist/docs/`. If the pre-upgrade setup downloaded docs into `.next-docs/`, update `AGENTS.md` to point at the bundled docs, then remove `.next-docs/` once nothing references it.
...
Before editing code, follow the [Set up docs](/docs/app/guides/upgrading/version-16#set-up-docs) section so AGENTS.md points at version-matched Next.js docs. Use the [AI Coding Agents guide](/docs/app/guides/ai-agents) for the full setup.

Then read AGENTS.md and follow the [Next.js 16 upgrade guide](/docs/app/guides/upgrading/version-16) as the source of truth for the migration. Use its [codemod section](/docs/app/guides/upgrading/version-16#using-the-codemod) when you're ready to run the mechanical upgrade.

Talk through the plan in user-facing language before making broad changes. When the guide presents a choice, explain the trade-off and ask which path to take. Keep the migration scoped to the upgrade, inspect the diff, run the relevant checks, and fix remaining breaking changes. Use the runtime verification flow from the [AI Coding Agents guide](/docs/app/guides/ai-agents), such as `next-dev-loop` when available, to confirm the upgraded app still works. Summarize what changed and what was verified.

Before finishing, repeat the post-upgrade check in [Set up docs](/docs/app/guides/upgrading/version-16#set-up-docs) so the AI-agent setup is ready for the upgraded app.
```

### Using the Codemod

If you're upgrading without an agent, use the `upgrade` [codemod](/docs/app/guides/upgrading/codemods#160):
