...
### Async API errors remain after running codemods

Await the reported request API and update affected callers. In synchronous Client
Components, unwrap the promise with React `use`. Remove codemod markers and
`UnsafeUnwrapped` casts only after the asynchronous access is fixed.

### A webpack configuration error appears under Turbopack

Find the unsupported webpack plugin, loader, or alias. Migrate it to a Turbopack
option, or add `--webpack` to each affected development and build command when the
app still requires webpack. Run those commands again.

### The image optimizer returns `400 Bad Request`

Check whether `q`, `url`, or the source host violates the configured image
allowlists. Fix the caller or add the required value or pattern, then retry the
request. Do not disable the image restrictions.

### A parallel route fails on direct navigation or reload

Add an appropriate `default.js` file for the slot. Choose `null` or `notFound()`
based on the intended fallback behavior, then verify both client navigation and
reloads.

### Proxy requires the Edge runtime

Follow [Middleware to Proxy](#middleware-to-proxy). Keep middleware when Edge
execution is required instead of renaming it to proxy.

### Linting no longer runs during the build

Follow the [`next lint` removal](#next-lint-command) and inspect package scripts
and CI to ensure the ESLint CLI runs separately.
