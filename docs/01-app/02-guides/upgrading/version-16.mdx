...
After the codemod, review the applicable sections of this guide and any intervening major-version guides. Resolve `@next-codemod-error` findings and temporary `UnsafeUnwrapped*` casts, including [asynchronous helpers and their callers](/docs/app/guides/upgrading/version-15#migrating-helpers-and-their-callers). Next.js 16 removes the temporary synchronous Request API compatibility, so deleting a warning or casting a value does not repair the runtime behavior.

Preserve the app's existing bundler unless you intend to migrate it. Declining a Turbopack suggestion does not prevent Next.js 16 from using Turbopack by default. An app that must retain Webpack needs explicit `--webpack` flags in the relevant `next dev` and `next build` scripts; see [Turbopack by default](#turbopack-by-default). Check the app using the scripts it supports, and record any check that cannot run in your environment.
