...
The `experimental.dynamicIO` and `experimental.useCache` flags are replaced by top-level [`cacheComponents`](/docs/app/api-reference/config/next-config-js/cacheComponents):
