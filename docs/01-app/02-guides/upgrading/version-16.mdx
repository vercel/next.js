...
After the app is upgraded, use the runtime verification flow from the [AI Coding Agents guide](/docs/app/guides/ai-agents) to confirm it still works. If the upgraded app is on Next.js 16.3 or later with Turbopack, install or use the `next-dev-loop` skill when available; otherwise use the best available `next dev`, browser, and build checks and say what could not be verified. Open the key UI states and check the Next dev indicator plus browser and server logs, including responsive or mobile drawers, dialogs, menus, and overlays. Summarize what changed and what was verified.
