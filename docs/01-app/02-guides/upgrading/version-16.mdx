...
Starting with **Next.js 16**, the image generation functions (the default export in your `opengraph-image`, `twitter-image`, `icon`, and `apple-icon` files) are invoked with async props (they may be Promises and should be `await`ed inside the image function). However, `generateImageMetadata` itself continues to receive `params` synchronously (as it did previously) and should remain a synchronous function that returns metadata.

```js filename="app/shop/[slug]/opengraph-image.js"
// Next.js 16 - generateImageMetadata remains synchronous
export function generateImageMetadata({ params }) {
  const { slug } = params
  return [{ id: '1' }, { id: '2' }]
}

// Next.js 16 - image generation function receives async props and should await them
export default async function Image({ params, id }) {
  const { slug } = await params // params may be a Promise
  const imageId = await id // id may be a Promise when provided by generateImageMetadata
  // ...
}
```
