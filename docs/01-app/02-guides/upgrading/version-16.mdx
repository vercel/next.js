...
If you run `next build` without the `--turbopack` flag (the new default), and your Next config contains a `webpack` configuration, the **build will fail** to prevent misconfiguration issues. In this case, you must either:

- explicitly build with `--turbopack`
- migrate your Webpack configuration to Turbopack
- or [opt out of Turbopack](#opting-out-of-turbopack) using `--webpack`
...
We recommend using Turbopack for development and production. Submit a comment to this [thread](https://github.com/vercel/next.js/discussions/77721), if you are still unable to switch to Turbopack.
...
It is now moved to be top-level option `turbopack`:
...
In some projects client-side code, may import files containing Node.js native modules. This causes `Module not found: Can't resolve 'fs'` type of errors.
...
Once again, it is preferable to refactor your modules so that client code doesn't ever import from modules using Node.js native modules.
...
You don't have to manually write types for layouts, pages, or route handlers. Simply run `npx next typegen`, and the route aware [type helpers](/docs/app/api-reference/config/typescript#route-aware-type-helpers), [`PageProps`](/docs/app/api-reference/file-conventions/page#page-props-helper), [`LayoutProps`](/docs/app/api-reference/file-conventions/layout#layout-props-helper), [`RouteContext`](/docs/app/api-reference/file-conventions/route#route-context-helper) will become globally available in your app.
...
Learn more about when to use `updateTag` or `revalidateTag` [here](/docs/canary/app/api-reference/functions/updateTag#when-to-use-updatetag).
...
**Next.js 16** removes the experimental **Partial Pre-Rendering (PPR)** flag and configuration options. PPR is being integrated into [Cache Components](/docs/app/api-reference/config/next-config-js/cacheComponents).

Starting with **Next.js 16**, you can opt into PPR using the `cacheComponents` configuration. We have prepared a [PPR to Cache Components](#) migration guide.

Modify your Next config file from `ppr: true`:
...
To use [`cacheComponents`](/docs/app/api-reference/config/next-config-js/cacheComponents):
...
> **Good to know**: Using PPR in Next.js 16 works differently than in Next.js 15 canaries. If you are using PPR today, stay in the current version 15 canary you are using.
...
```js filename="next.config.js"
import { startDocsDevServer } from 'docs-lib/dev-server'
