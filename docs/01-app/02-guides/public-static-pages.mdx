...
Size the fallback to match the content that replaces it, or the page shifts when the promotion arrives. See [Designing loading UI](/docs/app/guides/loading-ui) for fallback geometry and reveal order.
