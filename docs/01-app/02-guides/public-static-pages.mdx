...
You can find the resources used this example here:
...
We can confirm this by running [`next build`](/docs/app/api-reference/cli/next-build).
...
Without guidance, the framework has to assume you want to fetch **fresh** data on every user request.
...
We can mark a function as cacheable using the [`'use cache'`](/docs/app/api-reference/functions/use-cache) directive.
...
This turns it into a [cache component](/docs/app/glossary#cache-component).
...
If we refresh again, we can see the page loads instantly because the cache component doesn't block the response.
...
When prerendering isn't an option, the way to unblock the response is streaming.

We can enable streaming by wrapping a component in a [Suspense boundary](/docs/app/glossary#suspense-boundary) and providing a fallback.
...
The fallback component renders immediately, unblocking the rest of the page. The inner component renders later, once its async work completes.
