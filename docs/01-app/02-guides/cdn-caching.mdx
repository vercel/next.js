...
> **Good to know:** For routes without PPR, the `next-router-state-tree` header is read during prefetch requests to determine which segments to include, which increases cache `vary` as it passes the current router state. For segment-level prefetches pathnames are used (e.g., `/page.segments/_tree.segment.rsc`). CDNs can cache these using standard pathname-based cache keys.
...
These headers can be omitted without causing errors. The server returns a valid response, just potentially larger or less targeted to the specific navigation:

**`next-router-state-tree`**, **`next-router-prefetch`**, **`next-router-segment-prefetch`**: if omitted, the server returns a full RSC payload instead of a targeted segment update or lightweight prefetch. For static and ISR pages, the response is identical regardless of these headers. For dynamic pages, the response is larger but still works for navigations.
