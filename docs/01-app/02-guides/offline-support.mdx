...
Start with the default: the pending loading state is enough for many apps. Add a banner in the root layout when you want connectivity visible on every route. Add a scoped fallback when a specific route deserves a tailored message. The three layer in that order, so you can adopt as much as you need.
...
If the route opts into [runtime prefetching](/docs/app/guides/runtime-prefetching) with [`prefetch = 'allow-runtime'`](/docs/app/api-reference/file-conventions/route-segment-config/prefetch#allow-runtime), Next.js resolves the per-link URL data at prefetch time and includes it in the prefetch. `/chats/42` then renders its messages from that prefetch immediately, even offline, instead of waiting for the connection to return.
...
A route-level [`loading.tsx`](/docs/app/api-reference/file-conventions/loading) does the same job. Next.js wraps the segment's page in a `<Suspense>` boundary using the `loading.tsx` export as its fallback, leaving the layout above rendered. While offline, the cached layout and fallback render from the prefetch, and the page resumes once the network returns. The `useOffline` hook, banner, and Server Action retry all behave the same way.
