...
{/* PLACEHOLDER: replace with the actual URLs before publishing */}
...
Turn on `experimental.useOffline`. This guide also enables [Cache Components](/docs/app/api-reference/config/next-config-js/cacheComponents), which lets you place the Suspense boundary as close as possible to the uncached data, with the [static shell](/docs/app/glossary#static-shell) rendered around it.
...
You also need a source page with a `<Link>` to the dashboard. Next.js prefetches the static shell of any `<Link>` that enters the viewport, and that prefetch is what makes the shell available offline.
...
Navigating to the dashboard while offline, the fallback now reads "Waiting for connection to load this section..." Restore connectivity and the metrics stream in; the fallback disappears.
...
Pick whichever fits your use case. One banner in a root layout covers every route. A scoped fallback is more targeted but only shows on routes where you add it. Both together is also a reasonable default.

The banner shows on every route while offline, and hides when connectivity returns.

The same pattern extends to parameterized routes. In this example we used a static route (`/dashboard`), but a route like `/chats/[id]` with a static shell renders that shell when navigating to `/chats/42` offline, and the dynamic messages behind its `<Suspense>` boundary load when the connection returns.
...
A route-level [`loading.tsx`](/docs/app/api-reference/file-conventions/loading) does the same job. Next.js wraps the segment's page content in a `<Suspense>` boundary with the `loading.tsx` export as its fallback; the layout above stays rendered. While offline, the cached layout and the `loading.tsx` fallback render from the prefetch, and the page resumes once the network returns. The `useOffline` hook, the banner, and the Server Action retry all work the same way.
