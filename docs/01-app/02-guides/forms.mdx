...
A user who loses connectivity mid-submit does not lose their work. With [`experimental.useOffline`](/docs/app/api-reference/config/next-config-js/useOffline) enabled, Server Actions stay pending until the network returns and complete on their own. Combine the pending state with the [`useOffline`](/docs/app/api-reference/functions/use-offline) hook to make the submit button offline-aware.
...
See [retry Server Actions after the network returns](/docs/app/guides/offline-support#retry-server-actions-after-the-network-returns) for the full pattern.
