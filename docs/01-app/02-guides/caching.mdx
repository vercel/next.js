...
## Request Memoization (React Features)

Next.js uses React's `cache` function to extend the `fetch` API and automatically **memoize** requests that have the same URL and options. This means you can call a fetch function for the same data in multiple places in a React component tree while only executing it once.
...
If you need to fetch fresh data on every request, you can use the `cache: 'no-store'` option in `fetch`. This will opt out of memoization for that specific request.

You can also use the [`signal`](https://developer.mozilla.org/en-US/docs/Web/API/AbortController/signal) property from [`AbortController`](https://developer.mozilla.org/en-US/docs/Web/API/AbortController) to abort a request. This can be useful for implementing features like request cancellation, but it does not prevent memoization. A new request with the same URL will still hit the cache.
...
{/* Gemini-TODO: I am aware of this TODO and will skip it as I cannot update diagrams. */}
...
| [`React.cache`](#react-cache-function)                                  |                            |                       |                       | Cache (any function) |
