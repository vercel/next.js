...
* **Static Rendering**: If a route is statically rendered (the default), `fetch` requests are cached in the **Data Cache**.
* **Dynamic Rendering**: If a route is dynamically rendered, `fetch` requests are not cached. This behavior is equivalent to setting `{ cache: 'no-store' }`.

You can override this default behavior using the `cache` option:
