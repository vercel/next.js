...
Next.js extends the native `fetch` API to provide a powerful caching mechanism. The default caching behavior of `fetch` depends on how your route is rendered:

Data returned from `fetch` is _not_ automatically cached in the Data Cache.

By default, when no `cache` or `next.revalidate` option is provided:

- With [static rendering](/docs/app/getting-started/partial-prerendering#static-rendering), the page is generated ahead of time (prerendered). The fetched data is stored in the [Data Cache](#data-cache) and the rendered output in the [Full Route Cache](#full-route-cache), and serves that result until the path is revalidated.

- With [dynamic rendering](/docs/app/getting-started/partial-prerendering#dynamic-rendering), the page is generated at request time. The fetch runs on every request and returns fresh data on each request.
