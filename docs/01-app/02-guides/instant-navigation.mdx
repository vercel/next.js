...
Validation surfaces the per-slug product fetch first. Extract the slug-dependent work into a sub-component and wrap it with `<Suspense>`:
...
`await props.params` and the product fetch now suspend together. The product-fetch error clears, and validation moves on to the next blocker.

### Step 2: Cache the featured fetch

Validation now fires on `getFeatured()`. Wrap the fetching function with `"use cache"`:
