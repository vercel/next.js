...
The [`next-instant-navigation-optimizer`](https://github.com/vercel/next.js/tree/canary/skills/next-instant-navigation-optimizer) skill packages this loop: it encodes the goal as a failing `@next/playwright` `instant()` test and works it to green one route at a time — for both initial load (hard navigation) and client-side navigation (soft navigation) — then ships the test as a regression guard.
