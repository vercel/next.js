...
Next.js can also generate an [**App Shell**](/docs/app/glossary#app-shell) per route: a fallback that renders instantly during client navigations when nothing else is ready. See the [Runtime prefetching](/docs/app/guides/runtime-prefetching) guide for how to enable them and how they pair with per-link prefetching.
