...
On a first visit to a destination, an absence assertion such as `toHaveCount(0)` can prove that deferred content has not committed. For repeat and browser-back navigations, Next.js may preserve a previously visited route in a [React Activity boundary](/docs/app/guides/preserving-ui-state#testing). Its DOM can stay hidden during the transition or reappear with its completed state. Use visibility-aware assertions for the UI the user currently sees instead of using the number of matching DOM nodes to infer what committed. To test that content is excluded from a prefetch or static shell, use a fresh destination. A repeat or browser-back test measures restoration behavior instead.
