...
See [Maximizing the static shell](/docs/app/getting-started/rendering-model#maximizing-the-static-shell) for patterns to reduce fallback coverage and pull more content into the shell.
...
- [Caching](/docs/app/getting-started/caching#cachelife) for how to set cache lifetimes with `cacheLife`, and [Revalidating the cache](/docs/app/getting-started/mutating-data#revalidating-the-cache) for expiring entries with `updateTag`
