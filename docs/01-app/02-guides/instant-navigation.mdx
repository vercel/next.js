...
See [Optimizing the static shell](/docs/app/guides/optimizing-the-static-shell) for patterns to reduce fallback coverage and make more completed UI available immediately.
...
### Test first visits and return navigations

A first-visit test and a return-navigation test cover different behavior. Use a fresh destination to test which content the App Shell or prefetch contains.

On a first visit, an assertion such as `toHaveCount(0)` can prove that deferred content has not committed. On repeat and browser-back navigations, Next.js may preserve a previously visited route in a [React Activity boundary](/docs/app/guides/preserving-ui-state#testing). Its DOM can stay hidden during the transition or reappear with its completed state.

For a return navigation, use visibility-aware locators to assert what the user sees. The number of matching DOM nodes does not show whether retained content is currently visible.
