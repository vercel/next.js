...
To adopt validation gradually, set the [`validationLevel`](/docs/app/api-reference/file-conventions/route-segment-config/instant#configuring-validation-defaults) to `'manual-warning'`:
...
At that level, only segments that explicitly export `unstable_instant = true` are validated. Opt segments in route by route as you adapt them.
