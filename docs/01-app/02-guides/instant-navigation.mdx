...
Test both ways users can reach a route:
...
A `<Suspense>` boundary can make one navigation type instant without covering the other.

```typescript filename="e2e/navigation.test.ts"
import { test, expect } from '@playwright/test'
import { instant } from '@next/playwright'

// Initial page load: the document loads from the URL and renders from the root.
test('product is instant on an initial page load', async ({
  page,
  baseURL,
}) => {
  await instant(
...
// Client navigation: the destination commits from its prefetched UI.
...
The `instant()` callback exposes static UI for an initial page load and prefetched UI for a client navigation. Other dynamic content remains blocked until the callback returns.
