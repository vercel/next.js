...
On a first visit to a destination, an absence assertion such as `toHaveCount(0)` can prove that deferred content has not committed. For repeat and browser-back navigations, Next.js may preserve a previously visited route in a hidden [React Activity boundary](/docs/app/guides/preserving-ui-state#testing). Its DOM remains in the document, but it is not part of the visible instant UI. In those tests, assert that the deferred content has no visible match while the intended instant UI is visible.
