...
description: Learn how to structure your app to prefetch and prerender more content so page loads and client navigations are instant.
...
This guide explains what makes a navigation instant, how to write a route that navigates instantly, how to visualize what's in the initial UI, and how to lock the behavior in with end-to-end tests.
...
{/* TODO: add a screenshot of the React DevTools Suspense panel listing boundaries with toggle controls */}
...
{/* TODO: add a before/after diagram showing a single high-up Suspense boundary being refined down to smaller boundaries closer to the data */}
...
The [`next-cache-components-optimizer`](https://github.com/vercel/next.js/tree/canary/skills/next-cache-components-optimizer) skill packages this loop: it encodes the goal as a failing `@next/playwright` `instant()` test and works it to green one route at a time, for both initial load (hard navigation) and client-side navigation (soft navigation), then ships the test as a regression guard.
