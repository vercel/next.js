...
Take the dashboard layout that was opted out earlier. Instead of `instant = false`, enable validation with samples and opt into runtime prefetching with the [`unstable_prefetch`](/docs/app/api-reference/file-conventions/route-segment-config/prefetch) export:
...
- [`unstable_instant` API reference](/docs/app/api-reference/file-conventions/route-segment-config/instant) for the full `samples` reference and disable flags
