...
{/* TODO: screenshot — Navigation Inspector panel showing "Loading shell" labeled "Page load" with the target URL */}

> **Good to know**: When `next dev` renders against an empty cache, the dev indicator turns orange ("Rendering (cold cache)") and leaves a dismissible "Cold cache" badge afterward. A "Cache disabled" variant appears when caches are bypassed. Reload once caches are warm to see a production-representative view.
...
{/* TODO: screenshot — Navigation Inspector panel showing "Loading shell" labeled "Client nav" with source and target URLs */}
...
Two things block at the top level: an uncached `fetch()` for the featured list, and `await props.params` for the slug. Both stall the prefetch and the in-flight navigation.

### Step 1: Cache the uncached fetch

Validation surfaces the uncached fetch first. Wrap the fetching function with `"use cache"`:
...
The result is cached at the fetch level. Featured products no longer block the prefetch and ship with the App Shell.
...
### Step 2: Move the param await into Suspense

The page still awaits `props.params` at the top level. Extract the slug-dependent work into a sub-component and wrap it with `<Suspense>`:

```tsx filename="app/products/[slug]/page.tsx" highlight={3-12,16-21}
import { Suspense } from 'react'
...
`await props.params` and the product fetch now suspend together. The featured section renders from cache as part of the App Shell. The product details stream in under their `<Suspense>` boundary.
