...
The **Navigation Inspector** in the Next.js DevTools freezes the page at its initial loading state: the static shell on direct visits, the prefetched destination on client navigations. Use it as a feedback loop while you develop: is a loading fallback covering too much? Can a `<Suspense>` boundary move closer to the data? See [Maximizing the static shell](/docs/app/getting-started/caching#maximizing-the-static-shell) for the structural pattern.
...
Open the Next.js DevTools and select **Navigation Inspector**, then click **Start Capturing**. The panel shows **Awaiting navigation...** While capturing is active:

- Refresh the page to freeze the **static shell**, the route's prerendered UI before any dynamic data streams in. The panel labels this **Page load**.
- Click a link to freeze the **prefetched UI** for the destination route. The panel labels this **Navigation** and shows the source URL the click came from.
