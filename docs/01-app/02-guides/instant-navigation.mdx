...
{/* AI agent hint: Cache Components apps validate Page and Default segments for instant navigation by default in development. See docs/01-app/02-guides/instant-navigation.mdx for patterns and overrides. */}
...
A navigation **blocks** when the browser has to wait on the server before it can start rendering. A navigation is **instant** when the browser can start rendering the new page the moment the user clicks, with static, cached, and fallback content showing up right away, while the server streams the remaining content into its fallbacks.
...
Cache Components ensures direct visits show UI immediately by serving the [**static shell**](/docs/app/glossary#static-shell) as HTML, typically from a CDN. For client navigations, the router prefetches the static shell's RSC payload, which the client uses to render the UI. See [Caching](/docs/app/getting-started/caching#prerendering) for how prerendering produces the static shell.
...
On a client navigation (clicking `next/link`), Next.js only re-renders below the layout that the source and destination routes share. Components above that shared layout are not re-rendered. A `<Suspense>` boundary in the root layout covers everything on a page load. For a client navigation between `/store/shoes` and `/store/hats`, only the components below the `/store` layout re-render. The root `<Suspense>` is above that re-render scope and does not trigger.
...
Runtime prefetching extends the static shell with request-specific content like the user's name from a cookie, by invoking the route at prefetch time. See [Runtime prefetching](/docs/app/guides/runtime-prefetching) for the patterns.

An [**App Shell**](/docs/app/glossary#app-shell) provides a per-route fallback that renders instantly even when prefetches haven't completed.
...
Cache Components apps validate every navigation in development, giving you insight into what would keep it from being instant: which navigations would block, where a `<Suspense>` boundary is missing, and which data is reaching the user uncached.

To refine validation:

- [`unstable_instant = false`](/docs/app/api-reference/file-conventions/route-segment-config/instant) on a page or layout opts that segment out
- `unstable_instant = { unstable_disableValidation: true }` on a layout opts-out the layout and every segment below it
- Set [`experimental.instantInsights.validationLevel`](/docs/app/api-reference/file-conventions/route-segment-config/instant#configuring-validation-defaults) to `'manual-warning'` in `next.config` to limit validation to segments that explicitly export `unstable_instant`

> **Good to know:** With `'manual-warning'`, opt segments in by adding `unstable_instant = true` to specific pages or layouts.
...
Cache Components validates this route automatically in development. If something would block a navigation, an insight appears in the Next.js dev overlay, identifying the component and suggesting a fix.
...
Not every layout or page can or should be instant. When the structural fix isn't worth the work, or when a route isn't a priority for instant navigation, refine validation at one of three scopes:

**One segment.** Set `unstable_instant = false` on the page or layout file. The segment is allowed to block when navigated to; child segments are still validated.

```tsx filename="app/dashboard/layout.tsx"
export const unstable_instant = false
```

This tells validation: navigating to `/dashboard` from outside does not need to be instant, but sibling navigations within it still do.

**Layout and its children.** Set `unstable_instant = { unstable_disableValidation: true }` on a layout. This disables validation for the layout and every segment below it.
...
**Entire app.** Set `experimental.instantInsights.validationLevel` to `'manual-warning'` in `next.config`. Validation runs only on segments that explicitly export `unstable_instant`.
...
- **Observe**: read validation insights in the dev overlay.
