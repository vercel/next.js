...
Then use it in a test. A route is reached two ways: a **direct visit** that loads the page from a URL, and a **client navigation** from another route. A `<Suspense>` boundary that makes one instant may not cover the other. Test both:

```typescript filename="e2e/navigation.test.ts"
import { test, expect } from '@playwright/test'
import { instant } from '@next/playwright'

// Direct visit: the page loads from the URL and the whole tree renders from the root.
test('product is instant on a direct visit', async ({ page, baseURL }) => {
  await instant(
    page,
    async () => {
      await page.goto('/store/hats')
      await expect(page.locator('h1')).toContainText('Baseball Cap') // in the shell
      await expect(page.getByText('In stock')).toHaveCount(0) // held back
    },
    { baseURL }
  )
  await expect(page.getByText('In stock')).toBeVisible() // streamed in after
})

// Client navigation: only the tree below the shared /store layout re-renders.
test('product is instant on a client navigation', async ({ page }) => {
...
Inside the `instant()` callback, only the App Shell is visible: the heading is there, and the streamed content is still held back. After the callback exits, the dynamic content streams in and you can assert on the full page.
