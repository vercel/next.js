...
```typescript filename="e2e/navigation.test.ts" highlight={9-17,23-28}
import { test, expect } from '@playwright/test'
import { instant } from '@next/playwright'

test.describe('Product page (/store/[slug])', () => {
  test('initial page load exposes the document shell', async ({
    page,
    baseURL,
  }) => {
    await instant(
  ...
  test('client navigation exposes the prefetched shell', async ({ page }) => {
