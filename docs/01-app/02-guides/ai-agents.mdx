...
related:
  title: Next Steps
  description: Configure Next.js features and workflows for AI coding agents.
  links:
    - app/guides/mcp
...
Next.js provides AI coding agents with version-matched documentation, managed project instructions, runtime visibility, structured errors, reusable workflows, and automatic feedback. These capabilities help agents use the APIs and patterns supported by your installed Next.js version, verify their work, and report framework friction for your review.

<span id="step-1-point-agents-at-the-bundled-docs" />

## Version-matched documentation

Every Next.js installation includes documentation that matches the installed framework version. The documentation is available at `node_modules/next/dist/docs/` and mirrors the structure of the [Next.js documentation site](https://nextjs.org/docs):
...
Next.js makes the bundled documentation discoverable through managed instructions in `AGENTS.md` or `CLAUDE.md` at the project root. Most AI coding agents, including Claude Code, Codex, Cursor, and GitHub Copilot, read these files when they start a session.
...
Next.js updates only the content between the agent rules markers and preserves the rest of the file. The [`agentRules` API reference](/docs/app/api-reference/config/next-config-js/agentRules) describes how to configure this behavior.

[Benchmark results on nextjs.org/evals](https://nextjs.org/evals) show that agents perform better when they read the bundled documentation.

### Version support

Next.js 16.3 and later include the bundled documentation and managed agent instructions. Next.js 16.2 includes the bundled documentation but does not generate the instructions automatically.
...
Automatic agent feedback is experimental and disabled by default. Enable it with [`experimental.agentFeedback`](/docs/app/api-reference/config/next-config-js/agentFeedback) in `next.config`. When enabled, coding agents keep track of potential Next.js issues without interrupting the current user request. Immediately before the final response, the agent evaluates those feedback candidates and opens each qualifying issue as a separate review form in your default browser.
