...
To understand the value for a specific task, compare the same task with the bundled docs alone and with the Skill:

| Compare                 | What it shows                                              |
| ----------------------- | ---------------------------------------------------------- |
| Full-task success       | Whether the agent completed the intended outcome           |
| Reliability             | Whether the outcome holds across repeated runs and models  |
| Durable verification    | Whether the work leaves tests or other regression coverage |
| Execution time and cost | Whether the improvement justifies the added workflow       |

A Skill adds value when it improves the completed outcome or leaves reusable verification that offsets its execution time and model cost. When the bundled docs produce the same result reliably, the Skill adds little value for that task and model.
