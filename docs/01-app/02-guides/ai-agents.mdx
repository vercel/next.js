...
It needs a route that already builds with [Cache Components](/docs/app/getting-started/caching). It can also verify a client-navigation entry point when shared layouts produce a different instant shell. For URL-specific content that should be ready before a click, use the [`next-partial-prefetching-optimizer`](#next-partial-prefetching-optimizer) instead.
