...
The [`next-partial-prefetching-optimizer`](https://www.skills.sh/vercel/next.js/next-partial-prefetching-optimizer) Skill tunes what one client navigation prepares before the click:

1. Agrees on the UI the navigation should prefetch, what should wait for navigation, and whether the extra prefetch should begin in the viewport or after intent.
...
```prompt
Make the navigation from /products to /products/[id] prefetch the product title so it is part of the instant UI. Keep the rest of the product page streaming. Use the next-partial-prefetching-optimizer Skill.
```
