...
On Next.js `v16.2.0-canary.37` or later, just run `next dev` from your Next.js app directory (in a monorepo, the sub-package that declares `next`). When an AI coding agent is detected in the environment and the managed block is missing, Next.js auto-generates `AGENTS.md` and `CLAUDE.md` at the project root. It upserts into an existing `AGENTS.md` or `CLAUDE.md`, or creates both if neither exists:
...
You can also add the block by hand with the exact content above.

### Opting out

To disable the auto-generation, delete the files and set `agentRules` to `false` in your config:
