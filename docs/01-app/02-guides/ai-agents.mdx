...
```prompt
When navigating to a product, make its title available immediately, but don't prefetch the rest of the product page. Use the next-partial-prefetching-optimizer Skill.
```
