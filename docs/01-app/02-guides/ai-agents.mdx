...
The [`next-cache-components-optimizer`](https://www.skills.sh/vercel/next.js/next-cache-components-optimizer) Skill takes one navigation that isn't instant, or one where you want more of the destination on screen at click time:
...
Then give the agent this prompt like:
...
1. Audits the existing `<Link prefetch={true}>` calls and preserves the selected legacy-prefetched UI.
2. Turns the flag on and resolves the insights it surfaces.
3. Marks only additional URL-specific UI outside that preserved contract for a later decision.
...
The experimental [`next-partial-prefetching-optimizer`](https://www.skills.sh/vercel/next.js/next-partial-prefetching-optimizer) Skill adds the UI you want one link to carry into a specific route before the click:

1. Writes a failing [`instant()`](/docs/app/guides/instant-navigation#prevent-regressions-with-e2e-tests) test for that link's prefetched result.
2. Adds only the prefetching needed to pass it.
3. Commits the passing test with the change.

It needs [Partial Prefetching](/docs/app/guides/adopting-partial-prefetching) already adopted. A runtime prefetch costs a server invocation per link, so the shared App Shell stays the baseline for other links.
...
```prompt
Make the navigation from the projects list to /projects/123 instant using the next-partial-prefetching-optimizer Skill. The project name and status should be part of the instant UI.
```
