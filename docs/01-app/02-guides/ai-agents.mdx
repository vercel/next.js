...
On Next.js `v16.2.0-canary.37` or later, run the codemod to install the agent rules automatically:
...
On version 16.1 and earlier, the codemod downloads the docs to `.next-docs/` in the project root instead of using the bundled `node_modules/next/dist/docs/`, and the generated agent files will point at that directory:
