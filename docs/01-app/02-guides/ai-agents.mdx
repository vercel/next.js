...
`next dev` also writes its PID, port, and URL to `.next/dev/lock`. This metadata lives alongside the incremental compilation caches in `.next`.

Resetting `.next` discards the running server metadata and incremental compilation caches used across edits and reloads, so it is a recovery step for corrupt or incompatible generated output rather than routine troubleshooting.
