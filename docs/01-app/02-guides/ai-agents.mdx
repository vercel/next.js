...
The [`next-browser-initial-load-optimizer`](https://www.skills.sh/vercel/next.js/next-browser-initial-load-optimizer) Skill uses the [bundle analyzer](/docs/app/guides/optimizing-browser-initial-load) to reduce JavaScript, CSS, fonts, and assets loaded before a route needs them:

1. Ranks client sources and packages that are synchronously reachable for the route.
2. Inspects application importers for safe lazy-loading, server-rendering, narrower-import, and package-deduplication opportunities.
3. Applies one change at a time and verifies either lower client attribution or an `initial` to `async` scope transition.
