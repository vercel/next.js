...
When you install `next`, React and Next.js documentation is bundled inside the package:
...
│   ├── 03-architecture/
│   └── index.md
└── react-docs/                # React API reference
    ├── useActionState.md
    ├── useState.md
    ├── useEffect.md
    └── ...
...
The default `AGENTS.md` contains a single, focused instruction: **read the bundled docs before writing code**. This is intentionally minimal — the goal is to redirect agents from stale training data to the accurate, version-matched documentation.
...
The bundled docs include:

- **React reference** (`dist/react-docs/`) — API reference for React hooks like `useActionState`, `useState`, `useEffect`, and others. Helps agents use modern React patterns instead of outdated ones.
- **Next.js guides and API reference** (`dist/docs/`) — Guides, API references, and file conventions for the App Router and Pages Router.

When an agent encounters a task involving routing, data fetching, or any other React or Next.js feature, it can look up the correct API in the bundled docs rather than relying on potentially outdated training data.
