...
### `next-browser-initial-load-optimizer`

The [`next-browser-initial-load-optimizer`](https://github.com/vercel/next.js/tree/canary/skills/next-browser-initial-load-optimizer) Skill is an experimental, CLI-free workflow for investigating browser bundle costs. It teaches agents to inspect the raw `experimental-analyze --output` artifacts, choose graph analyses when useful, and verify one change at a time. Initial request timing still requires browser evidence; the raw artifact does not determine it by itself.
