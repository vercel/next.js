...
- **React API reference** (`dist/react-docs/`) — Covers hooks, components, and APIs from the React library. Agents can look up correct signatures and usage patterns instead of relying on training data.
...
When an agent encounters a task involving any React or Next.js feature, it can consult the bundled docs for up-to-date API details rather than relying on potentially outdated training data.
