...
The [`next-partial-prefetching-optimizer`](https://www.skills.sh/vercel/next.js/next-partial-prefetching-optimizer) Skill optimizes one client navigation:

1. Turns the requested user-visible result into an explicit contract for what is prefetched, what waits for navigation, and when prefetching begins.
