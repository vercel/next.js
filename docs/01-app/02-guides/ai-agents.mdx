...
- **Adoption skills** are user-facing migrations tied to framework docs and development insights. They explain app-wide behavior changes, work through surfaced issues, and show the user the result.
- **Optimizer skills** are unattended, test-driven loops. After the user sets the product goal, they use `instant()` to verify and guard the result.

### `next-dev-loop`

The [`next-dev-loop`](https://github.com/vercel/next.js/tree/canary/skills/next-dev-loop) runtime foundation verifies each change against a running dev server using the [MCP server](/docs/app/guides/mcp) and browser.
...
The [`next-cache-components-adoption`](https://github.com/vercel/next.js/tree/canary/skills/next-cache-components-adoption) adoption skill turns on [Cache Components](/docs/app/getting-started/caching) and resolves each blocking route until the app builds.
...
The [`next-cache-components-optimizer`](https://github.com/vercel/next.js/tree/canary/skills/next-cache-components-optimizer) optimizer maximizes the meaningful UI available on an exact navigation and guards it with an `@next/playwright` `instant()` test.
...
The [`next-partial-prefetching-adoption`](https://github.com/vercel/next.js/tree/canary/skills/next-partial-prefetching-adoption) adoption skill turns on [Partial Prefetching](/docs/app/guides/adopting-partial-prefetching) and resolves its insights until every link reuses a shared App Shell.
...
### `next-partial-prefetching-optimizer` (experimental)

The experimental [`next-partial-prefetching-optimizer`](https://github.com/vercel/next.js/tree/canary/skills/next-partial-prefetching-optimizer) optimizer selects per-link UI and cost, then optimizes the exact `<Link>` navigation and guards it with an `@next/playwright` `instant()` test.
...
```prompt
Optimize Partial Prefetching in this project using the next-partial-prefetching-optimizer skill.
```
