...
The experimental [`agentFeedback`](/docs/app/api-reference/config/next-config-js/agentFeedback) option adds a separate managed block that lets detected AI coding agents prepare de-identified reports about significant Next.js friction. The report opens as a short, first-person account grouped into what the agent was doing, what went wrong, and what would have helped. You can select a point to edit it or remove individual points before choosing whether to submit it. Agents never submit reports on your behalf.
