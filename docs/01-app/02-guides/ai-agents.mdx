...
| [`next-cache-components-optimizer`](https://github.com/vercel/next.js/tree/canary/skills/next-cache-components-optimizer)     | Drives a route to instant navigation with a test-driven loop: it encodes the goal as a failing `@next/playwright` `instant()` test, works it to green for both first load and client-side navigation, and ships the test as a regression guard. |
