...
The [`next-partial-prefetching-optimizer`](https://www.skills.sh/vercel/next.js/next-partial-prefetching-optimizer) Skill implements and verifies a prefetch contract for one client navigation:

1. Starts from the source link, destination, prefetched UI, navigation-only UI, and prefetch trigger you specify. It asks when a missing choice would change cost, freshness, authorization, or user-visible behavior.
2. Encodes that exact source-link and destination contract in an [`instant()`](/docs/app/guides/instant-navigation#prevent-regressions-with-e2e-tests) test.
3. Applies the [Optimizing prefetching](/docs/app/guides/optimizing-prefetching) patterns until the test passes, then keeps it as regression coverage.
...
```prompt
Optimize the navigation from the featured product on /products to /products/[id]. The product title should be prefetched. Related products should stay reusable but wait for navigation, and inventory should stay fresh. Prefetch on intent. Use the next-partial-prefetching-optimizer Skill.
```
