...
On Next.js `v16.2.0-canary.37` or later, upgrading via the codemod automatically scaffolds the agent-rules block as a post-upgrade step. Run it from your Next.js app directory (in a monorepo, the sub-package that declares `next`):

```bash
npx @next/codemod@canary upgrade
```

The codemod upserts into an existing `AGENTS.md` or `CLAUDE.md`, or creates both if neither exists:
