...
The [`next-cache-components-adoption`](https://www.skills.sh/vercel/next.js/next-cache-components-adoption) adoption Skill turns on [Cache Components](/docs/app/getting-started/caching) and resolves blocking routes until the app builds.
...
The [`next-cache-components-optimizer`](https://www.skills.sh/vercel/next.js/next-cache-components-optimizer) optimizer maximizes the meaningful UI available on an exact navigation and guards it with an `@next/playwright` `instant()` test.
...
The [`next-partial-prefetching-adoption`](https://www.skills.sh/vercel/next.js/next-partial-prefetching-adoption) adoption Skill turns on [Partial Prefetching](/docs/app/guides/adopting-partial-prefetching) and resolves its insights until all routes reuse a shared App Shell.
...
The experimental [`next-partial-prefetching-optimizer`](https://www.skills.sh/vercel/next.js/next-partial-prefetching-optimizer) optimizer selects per-link UI and cost, then optimizes the exact `<Link>` navigation and guards it with an `@next/playwright` `instant()` test.
