...
The experimental [`agentFeedback`](/docs/app/api-reference/config/next-config-js/agentFeedback) option adds a separate managed block that lets detected AI coding agents prepare de-identified reports about reproducible Next.js issues. Each report contains a generic setup, reproduction steps, observed facts, an expected result, and an optional control or workaround. You can edit the report or remove individual details before choosing whether to submit it. Agents never submit reports on your behalf.
...
The option is off by default and is independent of `agentRules`. Turning it off removes only the managed agent-feedback block the next time `next dev` runs.
