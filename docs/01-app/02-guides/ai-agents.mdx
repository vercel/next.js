...
On Next.js `v16.2.0-canary.37` or later, run the codemod to generate both files automatically:

```bash
npx @next/codemod@latest agents-md
```

The codemod detects the bundled docs at `node_modules/next/dist/docs/` and writes `AGENTS.md` and `CLAUDE.md` at the project root:
...
`CLAUDE.md` uses the `@` import syntax to include `AGENTS.md`, so [Claude Code](https://docs.anthropic.com/en/docs/claude-code) users get the same instructions without duplicating content. Running the codemod again is a no-op when the files already contain the managed block. You can also create both files by hand with the exact content above.
