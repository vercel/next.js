...
For the lowest possible latency, the static shell can be served directly from an edge location rather than from the origin server. The platform reads the shell from edge storage (for example, a KV store populated during `onBuildComplete`) and streams it to the client immediately, while routing the resume POST to the Next.js server for the dynamic render.

This is purely a platform architecture decision about where to serve the shell from. It does not require any changes to the Next.js application or its runtime configuration.
