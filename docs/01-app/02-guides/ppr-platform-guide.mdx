...
> **Good to know:** In standard `next start`, the server handles both the shell and dynamic render in a single pass automatically. The resume protocol is useful for CDN-to-origin architectures and adapter-based deployments that want to serve the shell separately from the dynamic render.
