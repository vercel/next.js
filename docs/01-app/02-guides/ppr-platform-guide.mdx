...
In the [adapter output](/docs/app/api-reference/config/next-config-js/adapterPath#output-types), PPR routes are identified by `renderingMode: 'PARTIALLY_STATIC'` in the prerenders section. The `postponedState` is available at `outputs.prerenders[route].fallback.postponedState`.
