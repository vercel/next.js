...
Build Single-Page Applications (SPAs) with client-side navigation and data fetching while progressively adopting Next.js server features. Existing SPAs can migrate to Next.js without a full rewrite.
...
You can pass a single Promise as a prop and unwrap it with `use()`, or pair it with a React context provider so any Client Component can read the value through a custom hook. For general client-side data fetching, a library like [SWR](/docs/app/guides/single-page-applications/swr) can help.
...
Interactivity often requires writing data. A Client Component can call a [Server Action](/docs/app/guides/server-actions) to run the mutation on the server. A client data-fetching library can instead write through a Route Handler and update its browser cache with SWR's [`mutate`](https://swr.vercel.app/docs/mutation) or TanStack Query's [`setQueryData`](https://tanstack.com/query/latest/docs/reference/QueryClient#queryclientsetquerydata). The rest of this section uses Server Actions directly.
...
The sections above use React, Server Actions, and the Next.js cache directly. You can also pair Next.js with a client data-fetching library such as [SWR](https://swr.vercel.app), [TanStack Query](https://tanstack.com/query), or [Apollo Client](https://www.apollographql.com/docs/react) when a Client Component needs client-side features such as focus revalidation, interval polling, or request deduplication.

When you seed a client cache from a Server Component, three cache layers can hold related data:
...
| Next.js client cache         | React Server Component payloads for prefetched routes      | `cacheLife` [`stale`](/docs/app/api-reference/functions/cacheLife#stale)                        |
...
[Prefetching](/docs/app/guides/prefetching) can place the next route's server-rendered view in the Next.js client cache before navigation. Seeding copies its initial data into the library cache, which then owns the browser value and its revalidation.

With Cache Components, React Activity also [preserves recently visited route state](/docs/app/guides/preserving-ui-state). Effects restart when a route becomes visible, so the data-fetching library can apply its normal mount revalidation without adding another cache layer.

You do not have to seed every client query from a Server Component. For browser-driven interactions such as autocomplete, fetch directly from a Client Component. The library caches the result after hydration and a client request. The Route Handler can still call a cached server function, so server seeding and server data caching remain separate choices.

Seed a client cache from a Server Component when you want the initial data to benefit from server rendering, streaming, and the Next.js cache while still using the library's client-side features. These libraries follow the same shape when server seeding is useful, even though each library's API differs:

- **Optionally seed the client cache from a Server Component**, so the library has first-render data without a client-side waterfall.
- **Scope data to the segment that owns it**, not a global config, so unrelated keys stay separate.
- **Define cache identities once**, so server tags, client keys, hydration, and mutations cannot drift apart.
- **Coordinate the server and client caches on mutation**, updating the client cache for an instant change and refreshing the server data for the next render.

The cache layers keep independent freshness policies and do not need matching durations. Only their identities and mutation invalidation must stay coordinated, and an optimistic update should restore the previous value if the write fails.

The same shape applies to any client cache, so a library not listed here follows it too. Seeding from a Server Component lets the same Client Component read data from the client, the server, or both:

|                      | Client library      | RSC                 | RSC + client library |
| -------------------- | ------------------- | ------------------- | -------------------- |
...
For the library-specific setup and code, see:

- [Client-side data fetching with SWR](/docs/app/guides/single-page-applications/swr)
- [Client-side data fetching with TanStack Query](/docs/app/guides/single-page-applications/tanstack-query)

If a Client Component only needs to read server data once, pass a Promise to it and unwrap it with [`use()`](#using-reacts-use-within-a-context-provider) instead. That avoids adding a data-fetching library for data that never revalidates on the client.
