...
If you have an existing SPA, you can migrate to Next.js without large changes to your code. Next.js then lets you progressively add server features as needed.
...
You do not have to seed every client query from a Server Component. For interactions whose input is only known in the browser, such as autocomplete, you can fetch directly from a Client Component. The library still caches the result in the browser, but the initial result waits for client JavaScript and a client-side request instead of being included in the server-rendered response. The API route can still call a cached server function, so server seeding and server data caching are separate choices.
...
The server value seeds the client cache's initial snapshot. After hydration, the client library owns its browser cache and revalidation. The two caches keep independent freshness policies and do not need matching durations; only their identities and mutation invalidation must stay coordinated, and an optimistic update should restore the previous value if the write fails.
