...
related:
  ...
  links:
    - app/guides/single-page-applications/swr
    - app/guides/single-page-applications/tanstack-query
...
These libraries follow the same shape with Next.js, so the concepts are shared even though each library's API differs:

- **Seed the client cache from a Server Component.** Fetch on the server and hand the result to the client on first render, so the library has data without a client-side waterfall. Point the client's fetch key at an [API route](/docs/app/api-reference/file-conventions/route) (a GET) so the library can revalidate and poll from the browser.
- **Scope data to the segment that owns it.** Seed the data on the route segment that reads it, not a global config, so each segment starts its own server-side requests and unrelated keys stay separate.
- **Coordinate the server and client caches on mutation.** When the Next.js cache and the client cache hold different state (for example, a server-rendered feed and a client-owned unread badge), a mutation updates the client cache for an instant change, then writes through an API route that calls [`revalidateTag`](/docs/app/api-reference/functions/revalidateTag) to refresh the server render for the next visit.

Seeding the client cache from a Server Component lets the same Client Component read data from the client, the server, or both. Server Components provide the first-render data, and the library owns revalidation and polling from there:
...
The same concepts apply to any client cache, so a library not listed here (such as Apollo Client) follows the same shape: seed from a Server Component, scope per segment, and coordinate caches on mutation.
