...
You do not have to seed every client query from a Server Component. For browser-driven interactions such as autocomplete, fetch directly from a Client Component. The library caches the result after hydration and a client request. The Route Handler can still call a cached server function, so server seeding and server data caching are separate choices.

Seed a client cache from a Server Component when you want the initial data to benefit from server rendering, streaming, and the Next.js cache while still using the library's client-side features. When server seeding is useful, keep three responsibilities separate:

- **Server Components provide the initial data**, scoped to the segment that owns it.
- **The data-fetching library owns the browser value**, using a shared cache identity for reads and hydration.
- **Mutations coordinate both sides**, updating the browser cache immediately and refreshing the server data for the next render.
