...
> **Good to know:**
>
> - This environment variable must be available at both **build time** and **runtime** with the same value.
> - Learn more in the [Data Security guide](/docs/app/guides/data-security#overwriting-encryption-keys-advanced).
...
Version skew occurs when clients running an older version of your application try to communicate with servers running a newer version (or vice versa). This commonly happens during rolling deployments when some instances are updated before others.

<details>
  <summary>What is version skew?</summary>

In web applications, version skew typically happens when a user's browser has cached JavaScript or CSS from an older deployment while the server is running a newer version.

For Next.js applications, this can cause issues such as:

- **Missing assets**: The client requests JavaScript or CSS files that no longer exist on the server
- **Server Function mismatches**: Each Server Function has a unique ID generated at build time. When the client calls a Server Function using an old ID, the server cannot find it, resulting in "Failed to find Server Action" errors
- **Navigation failures**: Prefetched page data from an old deployment may be incompatible with the new server
...
You can also set the deployment ID at runtime using the `NEXT_DEPLOYMENT_ID` environment variable.
