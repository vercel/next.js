...
> **Good to know:**
>
> - This variable is used at **build time** and saved to the build cache. The **server** reads the key from the build cache at startup. If your deployment does not preserve the build cache (for example, standalone output), the server falls back to reading this variable from the environment.
