...
You can pass a single Promise as a prop and unwrap it with `use()`, or pair it with a React context provider so any Client Component can read the value through a custom hook. For general client-side data fetching, a library like [SWR](#spas-with-swr) can help.
...
Interactivity means writing data, and that usually happens through a [Server Action](/docs/app/guides/server-actions): a Client Component calls the action to run the mutation on the server. If you use a client data-fetching library like SWR or TanStack Query (covered below), you can instead write through an API route and revalidate with SWR's [`mutate`](https://swr.vercel.app/docs/mutation) or TanStack Query's [`invalidateQueries`](https://tanstack.com/query/latest/docs/framework/react/guides/invalidations-from-mutations). The rest of this section uses Server Actions directly.
...
[SWR](https://swr.vercel.app) is a React library for data fetching.

With SWR 2.3.0 and React 19, you can add Server Component data to an existing SWR setup without moving all data fetching to the server. This abstracts the [`use()` pattern](#using-reacts-use-within-a-context-provider), so the same Client Component can read data from the client, the server, or both:

- **Client-only:** `useSWR(key, fetcher)`
- **Server-only:** `useSWR(key)` + RSC-provided data
- **Mixed:** `useSWR(key, fetcher)` + RSC-provided data

Use SWR when the Client Component needs client-side features, such as focus revalidation, interval revalidation, [`mutate`](https://swr.vercel.app/docs/mutation), or request deduplication across components. If a Client Component only needs to read server data once, pass a Promise to it and unwrap it with [`use()`](#using-reacts-use-within-a-context-provider). That avoids adding a data-fetching library for data that never revalidates on the client.
...
The examples above seed the client cache from the server, so both caches hold the same data flowing one direction. Some apps keep the two caches for different jobs instead:

- The **Next.js cache** owns the server-rendered route tree: pages, layouts, and cached server queries. It decides what the server sends for a route.
- The **SWR cache** lives only in the browser. It owns small client reads that need instant updates, such as an unread badge in the app chrome.

The two caches meet at a mutation. The client updates its own cache first so the badge changes instantly, then writes through an [API route](/docs/app/api-reference/file-conventions/route) that calls [`revalidateTag`](/docs/app/api-reference/functions/revalidateTag) to mark the server render stale for the next visit.

Updating the client cache before the request resolves is SWR's [optimistic update](https://swr.vercel.app/docs/mutation#optimistic-updates) pattern. Use [`mutate`](https://swr.vercel.app/docs/mutation) with `revalidate: false` to set the badge without triggering a client refetch, then send the write:
...
The query key connects the server prefetch and client hook, the same way the matching `fallback` and `useSWR` keys do above.
...
The `dehydrate()` call records the current time (`Date.now()`) in its output. Reading the current time is not allowed while prerendering, so calling `dehydrate()` directly fails with a [current-time prerender error](/docs/messages/blocking-prerender-current-time). Wrapping the helper in [`"use cache"`](/docs/app/api-reference/directives/use-cache) moves that read into the cache, where it runs once when the entry fills instead of on every render:

```tsx filename="app/projects/[id]/project-state.ts" switcher
...
```js filename="app/projects/[id]/project-state.js" switcher
import { dehydrate, QueryClient } from '@tanstack/react-query'
import { cacheTag } from 'next/cache'
import { getProject } from './data'

export async function getProjectState(id) {
  'use cache'
  cacheTag(`project:${id}`)
...
Then await the helper in the segment that owns the data and pass the result to `<HydrationBoundary>`, the same `ProjectData` boundary shown above:
...
Scoping the helper to that boundary keeps the cached payload next to the component that reads it, the same way a per-segment `<SWRConfig>` does. The payload is now cached and revalidates through its tag.

TanStack Query coordinates the two caches on a mutation the same way as [SWR](#coordinating-the-server-and-client-caches-on-mutation): update the client cache for an instant change, then write through an API route that calls `revalidateTag` to refresh the server render for the next visit. Update the badge in `useMutation`'s [`onMutate`](https://tanstack.com/query/latest/docs/framework/react/guides/optimistic-updates) callback, which runs before the request:

```tsx filename="app/activity/mark-read.tsx" switcher
'use client'

import { useMutation, useQueryClient } from '@tanstack/react-query'
import { activityKeys } from './activity-query'
...
export function useMarkActivityRead() {
  ...
  return useMutation({
    ...
    onMutate: () => queryClient.setQueryData(activityKeys.unread, { count: 0 }),
  })
}
```
