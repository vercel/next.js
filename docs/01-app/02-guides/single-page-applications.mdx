...
The `dehydrate()` call records the current time (`Date.now()`) in its output. Reading the current time is not allowed while prerendering, so calling `dehydrate()` directly throws. Wrapping the helper in [`"use cache"`](/docs/app/api-reference/directives/use-cache) moves that read into the cache, where it runs once when the entry fills instead of on every render:
