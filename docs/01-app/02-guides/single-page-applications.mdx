...
In SWR 2.5 beta, [`preload`](https://swr.vercel.app/docs/prefetching) can fill the SWR cache with `SWRConfig` `cacheData`:
...
The preloaded value is stored under the same key that `useSWR(key)` and `mutate(key)` use later. To read `cacheData` during render, enable Suspense in the Client Component that reads the key:
...
export function Profile() {
  const { data } = useSWR(USER_KEY, { suspense: true })
