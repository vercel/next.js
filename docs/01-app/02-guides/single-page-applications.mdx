...
Because the value is a real cache entry, `mutate(key)`, revalidation, and deduplication treat it like any fetched data, so the server seed and later client mutations share one source of truth. Reach for `preload` when the seeded value is part of your client data flow and gets mutated or revalidated. Use `fallback` when the data only needs to render once, since it is stable and needs no beta API.
...
Define the query key and query function in one shared helper:
...
A cache scope captures runtime values, such as the current time, at fill time instead of reading them during the dynamic render. That is the key to caching `dehydrate()`, which reads the current time for its own bookkeeping. Run it inside a [`"use cache"`](/docs/app/api-reference/directives/use-cache) scope so that read happens at fill time, and seed the query with `setQueryData` and `updatedAt: 0` so the per-query timestamp is deterministic across fills:
...
export async function getProjectState(id: string) {
  ...
  queryClient.setQueryData(['project', id], project, { updatedAt: 0 })
