...
Add or merge the following into your `.buildkite/pipeline.yml`:
...
Note that [cache volumes](https://buildkite.com/docs/pipelines/hosted-agents#using-cache-volumes) are only available on Buildkite macOS and Linux hosted agents. 

For self-hosted agents, use the [Cache plugin](https://buildkite.com/resources/plugins/buildkite-plugins/cache-buildkite-plugin/).
