...
Cannot read properties of undefined (reading 'hash')
    ...
    at <anonymous> (src/ai/flows/automated-humanized-floor-plan-generation-flow.ts:99:40)

## Code Frame
   97 |   async (input) => {
   98 |     // Step 1: Analyze the floor plan and generate detailed description + image generation prompt
>  99 |     const { output: analysisOutput } = await humanizationAnalysisPrompt(input);
      |                                        ^
  100 |
  101 |     if (!analysisOutput) {
  102 |       throw new Error('Failed to get analysis and image generation prompt.');
