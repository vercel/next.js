...
If you're running Next.js through a package manager script (e.g. `npm run dev`), you need to use `--` to forward flags to the `next` CLI:
...
> **Good to know**: With `npm`, flags after `npm run <script>` are consumed by npm itself unless separated with `--`. This is not required for `yarn` or `pnpm`, which forward flags directly to the script.
