...
Server Components can call `getCurrentUser()` directly. To reach Client Components without prop drilling, create the promise once, pass it through context, and unwrap it with [`use()`](https://react.dev/reference/react/use). For the general pattern, see [Using React's `use` within a Context Provider](/docs/app/guides/single-page-applications#using-reacts-use-within-a-context-provider). One auth-specific detail: because `getCurrentUser` reads the request, create its promise inside the Suspense boundary, not at the top of a layout.
...
Call it from a Server Component that reads the user for its own rendering. Awaiting `getCurrentUser()` here is fine because it needs `user.id`, and the private cache means this reuses the session rather than reading it again:
