...
This guide shows how to read the user's session, show authenticated UI without slowing down the rest of the page, and cache data derived from the session, all with [Cache Components](/docs/app/getting-started/caching) enabled.
...
This guide assumes you're comfortable with Cache Components. If not, read [Caching](/docs/app/getting-started/caching) first. It also assumes you already have a session set on login. See [How to implement authentication in Next.js](/docs/app/guides/authentication) for sign-up, login, and session creation.

## Step 1: Read the current user

Reading the current user reads the session cookie, then looks the user up. Because a request read can't be part of the static shell, it always sits behind a [`<Suspense>`](/docs/app/api-reference/file-conventions/loading) boundary, where it streams in on demand.

That works, but it means streaming the section to the client on every navigation. Giving the read a cache lifetime lets the framework prefetch this UI ahead of time instead.
...
Server Components can call `getCurrentUser()` directly. To reach Client Components without prop drilling, create the promise once and pass it through context, then unwrap it with [`use()`](https://react.dev/reference/react/use). This is the [context and `React.cache` pattern](/docs/app/getting-started/fetching-data#sharing-data-with-context-and-reactcache) from data fetching, with one auth-specific adjustment: because `getCurrentUser` reads the request, create its promise inside the Suspense boundary, not at the top of a layout.
...
The Server Component behind the boundary creates the promise, awaits it for its own rendering, and passes the same promise to the provider:
...
Client Components read the user with `useUser()`, no prop drilling and no second session read. Keep the read behind a `<Suspense>` boundary, since `use()` suspends until the promise resolves:
...
Call it from a Server Component that already has the user:
...
When a [Server Action](/docs/app/getting-started/updating-data) changes a user's data, call [`updateTag`](/docs/app/api-reference/functions/updateTag) with the same tag to refresh the cached entry. Re-read the session inside the action so it authorizes itself rather than trusting the client.
...
- A route that _also_ depends on the URL (a `params` or `searchParams` value) needs one more opt-in: set [`prefetch = 'allow-runtime'`](/docs/app/api-reference/file-conventions/route-segment-config/prefetch) so the router resolves that per-link data at prefetch time too.

```tsx filename="app/notes/[id]/page.tsx"
export const prefetch = 'allow-runtime'
```

A runtime prefetch fires as each link enters the viewport, one server invocation per link, so a page with many links to the same route (a sidebar of `/chat/[id]` links) triggers a prefetch per link. The [`partialPrefetching`](/docs/app/api-reference/config/next-config-js/partialPrefetching) flag changes this, fetching one [App Shell](/docs/app/glossary#app-shell) per route instead. See [Adopting Partial Prefetching](/docs/app/guides/adopting-partial-prefetching).
