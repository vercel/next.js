...
A plain `use cache` scope can't read `cookies()`, so the exported function resolves the user and passes the id inward:
...
Keep `getNotesByUserId` unexported. An exported getter that takes a user id trusts its caller to pass the right one, and the wrong id returns another user's data. See [Data Access Layer](/docs/app/guides/data-security#data-access-layer).
