...
Every major CDN has the infrastructure primitives needed to build full Next.js support: edge compute, key-value storage, blob storage, and the ability to resume PPR rendering. Most community adapters today deploy Next.js as a Node.js server without leveraging these CDN primitives, but no architectural blockers prevent deeper integration. See the [Deploying](/docs/app/getting-started/deploying#adapters) page for the current list of adapters.
