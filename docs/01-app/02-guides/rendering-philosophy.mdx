...
Every major CDN has the infrastructure primitives needed to build full Next.js support: edge compute, key-value storage, blob storage, and the ability to resume PPR rendering. Platform adapters exist for several providers, though not all adapters leverage the full CDN infrastructure yet. See the [Deploying](/docs/app/getting-started/deploying#adapters) page for the current list.
