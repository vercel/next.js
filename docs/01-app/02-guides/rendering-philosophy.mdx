...
Most web frameworks place the static/dynamic boundary at the **route level**: each route is either fully static or fully dynamic. This keeps infrastructure simple, since static routes are just files on a CDN and dynamic routes are requests to a server.

Next.js places the boundary at the **component level**: static and dynamic content coexist within a single streaming response. This gives developers more flexibility but requires more from the hosting platform: streaming, cache coordination, and cache consistency.

That is a deliberate trade-off. A finer-grained rendering boundary transfers complexity from application code into infrastructure. The infrastructure requirements described below exist because of this choice.
