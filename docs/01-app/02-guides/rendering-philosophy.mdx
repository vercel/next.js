...
- **Cache consistency** matters because revalidation regenerates both the HTML response and the RSC payload (React Server Components payload). If these get out of sync across instances, users may see inconsistent data during navigation. See [How Revalidation Works](/docs/app/guides/how-revalidation-works) for consistency requirements and the role of [`deploymentId`](/docs/app/api-reference/config/next-config-js/deploymentId).
