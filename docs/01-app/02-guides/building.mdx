...
This works because the params and data are fully known at build time, so Next.js can prerender each page in full. But if the page also uses request-time APIs like `cookies()`, `headers()`, or `searchParams`, the build fails again — those values are not available during prerendering.
...
With the loading fallback in place:
...
Large or unbounded slug sets (for example, millions of product pages) aren't practical to prerender fully at build time. List the ones worth prerendering in `generateStaticParams` — they appear as their own `○` rows — and the dynamic segment's `◐` row covers the rest, served as a shell and upgraded after the first visit. [Incremental Static Regeneration with Cache Components](/docs/app/guides/incremental-static-regeneration-cache-components) explains that model in full.
