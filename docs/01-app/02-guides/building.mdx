...
`next dev` keeps its active server state and incremental compilation caches in `.next/dev`. Avoid deleting or moving `.next` while the development server is running. If you need isolated production output, configure a separate [`distDir`](/docs/app/api-reference/config/next-config-js/distDir) for the build.
