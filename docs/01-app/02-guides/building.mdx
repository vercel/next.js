...
The `○` on `├ ○ /products/[id]` means that route is **Static** (see the legend). The indented lines name each concrete URL Next.js built for the ids you returned from `generateStaticParams`, such as `/products/1`, `/products/2`, and `/products/3`.
