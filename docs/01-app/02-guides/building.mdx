...
The `◐` on `/products/[id]` means the route is **Partial Prerender**: the loading fallback is served as a static shell, and the page content streams in at request time.
...
### Pre-render known params with `generateStaticParams`
...
The concrete paths (`/products/1`, `/products/2`, `/products/3`) are `○` (Static) because their params were known at build time. The `◐` row on `/products/[id]` is the shell for unlisted params. When there are more paths than fit in the preview, Next.js prints a short list and ends with `[+N more paths]`.
