...
The error's `[stream]` option prerenders fallback UI while the params and product data load at request time. Add a [`loading.js`](/docs/app/api-reference/file-conventions/loading) file:
...
The unmarked `/products/[id]` row groups the fallback and generated product paths. Each indented row shows its own build result. The generated product paths stay `◐` because their IDs are known during the build, but the product lookup is still uncached. Each page serves the static shell and streams the product data.
...
Suppose the product route will be refactored in a later migration pass. Keep `generateStaticParams`, remove the `loading` file and `use cache`, and choose the error's `[block]` option by setting `instant = false`:
...
The `instant = false` export opts the segment out of instant-navigation validation. When it is the highest `instant` config in the route tree, it also disables static-shell validation, allowing the build to accept an empty HTML prerender. The route continues to use PPR.

Next.js attempts to prerender each path from `generateStaticParams` and the fallback for unlisted IDs. The uncached product request blocks the whole page, so every prerender attempt produces empty HTML:
...
The indented `/products/[id]` row is the fallback for IDs not returned by `generateStaticParams`. The other indented rows are the generated product paths. Every row uses `ƒ` because its prerender is empty.

Use `instant = false` when a route is intentionally allowed to wait for request-time work, or when you want to defer its refactor while [adopting Cache Components incrementally](/docs/app/guides/migrating-to-cache-components#adopting-incrementally).
