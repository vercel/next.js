...
After a successful build, the route table lists each route with a **leading symbol** on its main row. When a dynamic route expands into concrete paths (from [`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params)), those nested lines show the resolved URLs **without** a second symbol. The legend at the bottom only includes symbols that actually appeared in the tree.
...
With [Cache Components](/docs/app/api-reference/config/next-config-js/cacheComponents), app `page` routes are labeled `○` or `◐`. The `ƒ` symbol appears for API routes, [Proxy](/docs/app/api-reference/file-conventions/proxy) routes, dynamic [metadata](/docs/app/api-reference/functions/generate-metadata) (such as `icon` or `opengraph-image`), and other server-rendered handlers. Without Cache Components (or on the Pages Router), you can still see `●` (SSG) and `ƒ` (Dynamic) for prerendered versus on-demand pages.

[Partial Prerendering](/docs/app/getting-started/caching#how-rendering-works) is the default rendering model with Cache Components. For a dynamic route with `generateStaticParams`, the **parent** row carries `○` or `◐` depending on whether the page is fully static at build time or includes streamed content. Nested rows are the individual prerendered paths.
...
If `generateStaticParams` returns more paths than fit in the preview, Next.js lists the first few and ends with a `[+N more paths]` row instead of showing every URL.
