...
The unmarked `/products/[id]` row groups the fallback and generated product paths. Each indented row shows its own build result. The generated product paths stay `◐`. Next.js knows their IDs during the build, so it can prerender each page's static shell. The product lookup still runs at request time, and its result streams into the shell.
...
Suppose this product route needs to remain blocking for now. Keep `generateStaticParams`, remove the `loading` file and the `use cache` directive, and set `instant = false`:
...
[`instant`](/docs/app/api-reference/file-conventions/route-segment-config/instant) is scoped by the route tree. Because this page is the only segment that sets it, `instant = false` opts the product route out of instant-navigation validation and lets the build accept an empty static shell. Setting `false` on a parent layout would extend the static-shell opt-out to its descendants, so place it as low as possible. PPR remains enabled because the flag changes validation, not the rendering model.

Next.js still attempts to prerender each path from `generateStaticParams` and the fallback for unlisted IDs. For the listed IDs, the product lookup blocks before the page produces HTML. For the fallback, `params` blocks first. Every prerender attempt is therefore empty:
