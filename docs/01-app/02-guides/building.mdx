...
After a successful build, the route table puts a **symbol on each main route row**. If a dynamic route lists concrete paths from [`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params), the nested lines show those URLs **without** a second symbol. The legend at the bottom lists only symbols that appeared in the tree.
...
[Partial Prerendering](/docs/app/getting-started/caching#how-rendering-works) is the default rendering model with Cache Components. For a dynamic route that uses `generateStaticParams`, the **parent** row is `○` or `◐` depending on whether the page is fully static at build time or includes streamed content. Nested rows are the prerendered paths.
...
By default, Next.js prerenders one page per value you return from `generateStaticParams`, and the route table lists those paths under the dynamic segment.
