...
This works because the params and data are fully known at build time, so Next.js can prerender each page in full. But there are two things `generateStaticParams` alone does not cover:

1. **Request-time APIs.** If the page also uses `cookies()`, `headers()`, or `searchParams`, the build fails again — those values are not available during prerendering.
2. **Unlisted params.** Param values not returned by `generateStaticParams` have no prerendered shell, so they wait for a full server render.

Adding a loading fallback solves both.

### Add a loading fallback

Add a [`loading.js`](/docs/app/api-reference/file-conventions/loading) so Next.js can prerender a static shell and stream the page content when it's ready:
...
`next build` now prerenders each listed param alongside a shell for unlisted params:
...
When a user visits an unlisted param, Next.js serves the loading fallback immediately while the page content streams in. The result is cached for subsequent visits.

When there are more paths than fit in the preview, Next.js prints a short list and ends with `[+N more paths]`.

> **Good to know**: The shell for unlisted params is available from Next.js 16.3, where [partial fallbacks](/docs/app/api-reference/config/next-config-js/partialFallbacks) is enabled by default. Set `experimental: { partialFallbacks: false }` to opt out and fall back to the previous behavior where unlisted params wait for a full server render before the response is sent.
