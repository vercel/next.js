...
This guide will show you how to read the build output, including the route table and prerender-blocking errors, which are the most common build failure.
...
Run the build with your package manager:
...
With Cache Components, most routes show `○` or `◐`. [Partial Prerendering](/docs/app/getting-started/caching#static-cached-and-streaming) is the default rendering model, which splits each route into a static shell, prerendered at build time, and dynamic parts that stream in at request time. A route shows `ƒ` when it has nothing to prerender, such as [Route Handlers](/docs/app/api-reference/file-conventions/route) that depend on the request, Proxy (Middleware), and dynamic [metadata](/docs/app/api-reference/functions/generate-metadata) like `icon` or `opengraph-image`.

The symbol reflects what the route does at prerender time, not which validation configs (such as [`instant`](/docs/app/api-reference/file-conventions/route-segment-config/instant)) it exports. Without Cache Components, or on the Pages Router, prerendered pages can show `●` (SSG) and on-demand pages can show `ƒ` (Dynamic).
...
Run `next build`. It fails with a prerender-blocking error because the uncached lookup runs outside a `<Suspense>` boundary:

```bash filename="Terminal"
Error: Route "/products/[id]": Next.js encountered uncached data during prerendering.
...
That disables minification, turns on source maps for server bundles, and continues past the first failure, so remaining issues surface in one run. This helps with any error thrown during prerendering, not only blocking ones. The stack now points at the lookup in the page:

```bash filename="Terminal"
Error: Route "/products/[id]": Next.js encountered uncached data during prerendering.
  ...
    at Page (app/products/[id]/page.tsx:5:19)
  3 | export default async function Page(props: PageProps<'/products/[id]'>) {
  4 |   const { id } = await props.params
> 5 |   const product = await db.product.find(id)
    |                   ^
  6 |   return <div>{product.name}</div>
```
...
Next.js compiles and prerenders only the matching routes, and skips the rest of the app. The option accepts comma-separated paths and glob patterns, and combines with `--debug-prerender` while debugging. Our store only has a few routes, so we'll keep building the whole app.
