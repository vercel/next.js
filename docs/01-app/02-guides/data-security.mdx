...
The key must be a base64-encoded value with a valid AES key length (16, 24, or 32 bytes). Next.js generates 32-byte keys by default. You can generate one using your platform's cryptographic tools, for example:

```bash
openssl rand -base64 32
```

> **Good to know:** This variable must be available at **build time** and is saved to the build cache. For details on how the key is resolved at runtime, see the [Self-Hosting guide](/docs/app/guides/self-hosting#server-functions-encryption-key).
