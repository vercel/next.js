...
## Choosing a data fetching approach

Start by choosing a data fetching approach that fits your project:
...
We recommend avoiding mixing approaches. This makes it clear for both developers working in your code base and security auditors what to expect.

### External HTTP APIs

If you're adopting Server Components in an existing project, treat them as untrusted by default. Instead, follow a **Zero Trust** model. Call your custom API endpoints such as REST or GraphQL from Server Components using [`fetch`](/docs/app/api-reference/functions/fetch), just as you would from the client and passing cookies as needed.
...
- You already have secure API layers and security practices in place.
- Backend teams use other languages.

Meanwhile, you still gain the benefits of Server Components by sending less code to the client and inherent data waterfalls can execute with low latency.

> If your project uses `getStaticProps` or `getServerSideProps` to connect directly to a database, consider moving that logic into a Route Handler. This consolidates data access into a single, auditable pattern.

### Data Access Layer

For new projects, we recommend creating a dedicated Data Access Layer (DAL) within your JavaScript codebase.

This is a internal library that acts similar to HTTP endpoints and control how and when data is fetched, and what gets returned to the UI.

Each function in a DAL should:

- Only run on the server.
- Perform access checks.
- Return safe, minimal **Data Transfer Objects (DTOs)** ready to be consumed by the client.

This centralizes all data access logic, making it easier to enforce consistent data access and reduces the risk of authorization bugs. You also get the benefit of sharing an in-memory cache across different parts of a request.
...
This approach makes it easier to accidentally expose private data in the client, for example:
...
Instead, the data should be sanitized before passing it to the client:
...
This prevents the tainted objects or values from being passed to the client. However, it's an additional layer of protection, you should still filter and sanatize the data before passing it to React's render context.
