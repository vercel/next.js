...
Common approaches to this problem each have tradeoffs. A Client Component that re-renders with client values causes a hydration error. Deferring to `useEffect` avoids the error but introduces a visible flash. And rendering only on the server means giving up on client-specific formatting entirely.
