...
**Resolve data in a Server Component.** When the page resolves request-time data itself, its heading and other independent UI cannot render until that work finishes:
...
Moving data access into smaller regions can expose loading states that a previously blocked page never showed. [`<Suspense>` lets those regions stream](/docs/app/guides/streaming#granular-streaming-with-suspense), but its boundaries also determine which content appears together and what replaces it while it waits.
...
**Refine an existing `loading.tsx`.** A route may already have a [`loading.tsx` file](/docs/app/api-reference/file-conventions/loading), which automatically wraps the segment's page and children in a Suspense boundary:
...
The file can stay as the segment's outer loading state. As you push data access down, add focused boundaries where completed UI should remain visible or smaller regions should reveal independently. This is often when a route that previously showed one broad loading state needs more deliberate fallbacks.
