...
The parent no longer needs to know about the editor's internal state. This keeps components encapsulated and reduces drilling props.
...
When using Activity with [Cache Components](/docs/app/getting-started/caching) navigation, some patterns that relied on component unmounting may behave differently. See [Activity with Cache Components](/docs/app/guides/activity-cache-components) for handling common patterns like dropdowns, dialogs, and testing.
