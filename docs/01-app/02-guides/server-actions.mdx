...
A **Server Action** is a [React Server Function](https://react.dev/reference/rsc/server-functions) dispatched inside a [transition](https://react.dev/reference/react/startTransition). React wraps the call automatically when the function is passed to a `<form action>` or a `<button formAction>`.
...
{/* i think we can somehow merge these two paragraphs */}
...
In Next.js, it typically maps to a data mutation: writing to a database, updating cookies, or invalidating cached data.
...
- **Sequential dispatch still applies.** The returned UI lands in the same one-at-a-time queue described in [Sequential dispatch](#sequential-dispatch-on-the-client), so back-to-back submissions render in order, not in parallel.
