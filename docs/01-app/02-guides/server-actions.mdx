...
> **Next.js server actions are only meant for mutations.**
>
> While they can be convenient for data fetching, they are only meant to be used for mutations. They do not support caching, and client dispatch is sequential, which can affect performance if used for fetching data.
