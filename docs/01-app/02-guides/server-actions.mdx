...
- **Client Components in the returned tree must already be reachable from the route's [module graph](/docs/app/glossary#module-graph).** A Client Component imported _only_ inside a Server Action is not traced into the React Client Manifest, and returning it fails with `Could not find the module in the React Client Manifest`. Make sure the same Client Component is rendered or imported elsewhere in the route, or wrap it in a Server Component that the action returns.
