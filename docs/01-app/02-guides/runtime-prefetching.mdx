...
App Shells are on by default with Cache Components. Pairing with `partialPrefetching: true` activates the full prefetch ladder:
...
Every Cache Components route has an instant floor. N links to the same route share one prefetched App Shell, and rendering a `<Link>` is effectively free. `<Link prefetch={true}>` adds a per-link prefetch of the route's static and cached content for that specific URL; dynamic data is excluded. Opting a route into [`prefetch = 'allow-runtime'`](/docs/app/api-reference/file-conventions/route-segment-config/prefetch#allow-runtime) further adds a runtime prerender that includes dynamic data.
