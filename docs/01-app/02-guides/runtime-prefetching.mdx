...
Runtime prefetching is for URL data. Session data is handled separately: a route that reads `cookies()` or `headers()` gets an App Shell that includes its session data, cached per session on the client. That content is ready on navigation without `allow-runtime`.

Derived data needs one more step. Take a dashboard nav that reads a cookie, then looks up content based on it:
...
The cookie itself is session data, which the App Shell already knows. But the lookup it drives is a data read, and for that to land in the shell instead of streaming in after navigation, it needs a cache lifetime. Because `"use cache"` can't read `cookies()` inside the cached function, two patterns bridge it:
