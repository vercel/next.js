...
1. **Proxy generates a nonce**: Your proxy creates a unique nonce for the request and sets the `Content-Security-Policy` header on both the request and the response. The request copy is what Next.js reads while rendering; the response copy is what the browser enforces. The `x-nonce` header is for your own code to read.
2. **Next.js extracts the nonce**: During rendering, Next.js parses the `Content-Security-Policy` or `Content-Security-Policy-Report-Only` header from the request and extracts the nonce using the `'nonce-{value}'` pattern. Report-only lets you trial a policy while nonces are still applied.
