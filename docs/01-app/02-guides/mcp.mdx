...
During `next dev`, Next.js DevTools exposes two [WebMCP](https://developer.chrome.com/docs/ai/webmcp/imperative-api) tools in supporting browsers and agent harnesses. An agent can discover these tools when it opens your application, without configuring `next-devtools-mcp`:

| Tool         | Behavior                                                                                                                        |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------- |
| `pause_hmr`  | Hold incoming hot updates, build errors, and automatic reloads while the agent edits files. The current page stays interactive. |
| `resume_hmr` | Resume updates after editing. If updates arrived while paused, reload once to load the latest files.                            |

Both tools take no arguments. The agent should pause before its first edit and resume after its final edit. Intermediate versions are discarded, so temporary syntax errors or module errors from incomplete edits do not replace the current page.

Resuming with pending changes performs a full reload and resets client state. Resuming without pending changes preserves the page. If the final source still contains errors, they appear after resuming.

Pausing applies to the current document. It does not cancel updates already in progress, stop server compilation or application requests, or prevent manual navigation. Reloading the page also ends the pause.

> **Good to know**: WebMCP is experimental. Browsers without this API keep normal hot-update behavior, and these tools are only available during development.
