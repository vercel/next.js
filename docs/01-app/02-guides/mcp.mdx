...
| `resume_hmr` | Resume updates after editing, applying buffered updates together through normal HMR.                                                                                 |

Both tools take no arguments. The agent should await `pause_hmr` before its first edit and call `resume_hmr` after its final edit. While paused, temporary errors from incomplete edits do not replace the current page. If the final source still contains errors, they appear after resuming.
