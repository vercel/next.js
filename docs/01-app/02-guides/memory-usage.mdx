...
If your application builds with webpack, see [reducing webpack memory usage](/docs/webpack/memory) for options specific to that bundler.
...
> **Good to know**: This feature is not compatible with the [Webpack build worker](/docs/webpack/memory#webpack-build-worker) option, which is auto-enabled unless you have a custom webpack config.
