...
Since `v15`, when the Next.js server starts, it preloads page's JavaScript modules to memory, rather than at request time.

This optimization allows for faster response times when requests arrive, in exchange for a larger initial memory footprint.

To disable this optimization, use the `experimental.preloadEntries` flag, set to `false`. We will soon move this flag out of the `experimental` scope.
...
Next.js does not ever unload these JavaScript modules, which means that, even with this optimization disabled, the memory footprint of your Next.js server will eventually be the same.
