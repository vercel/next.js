...
Large or unbounded param sets, such as millions of product pages, aren't practical to prerender fully at build time. List the params worth prerendering in [`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params) so those pages are prerendered in full. Visits to any other param get the App Shell instantly, while Next.js renders the full page in the background. Later visits to the same URL get the completed page from the cache.
