...
When a [`<Link prefetch={true}>`](/docs/app/api-reference/components/link#prefetch) to an unlisted URL enters the viewport, Next.js starts the background upgrade before the click, just as a first visit would, so navigation lands on the upgraded result.

Links to these pages need `prefetch={true}`. A default `<Link>` prefetches only the App Shell, which links to the same route share.
...
### Deferring expensive content

Because links to these pages use `prefetch={true}`, each prefetch also prepares content that depends on `cookies` or `headers`. Consider showing each price in the visitor's currency. The currency comes from a cookie, and converting the price calls an exchange-rate service, so every prefetch of a product link would run that conversion. Use [`await navigation()`](/docs/app/api-reference/functions/navigation) before it. The prefetch leaves the converted price out, and it renders when the user navigates.

```tsx filename="app/[category]/[product]/page.tsx" highlight={3-5,7-16,32-34}
import { Suspense } from 'react'
import Link from 'next/link'
import { cookies } from 'next/headers'
import { navigation } from 'next/cache'
import { getProduct, getExchangeRate } from '../../lib/data'

async function LocalPrice({ price }: { price: number }) {
  await navigation()
  const currency = (await cookies()).get('currency')?.value ?? 'USD'
  const rate = await getExchangeRate(currency)
  return (
    <p>
      {(price * rate).toFixed(2)} {currency}
    </p>
  )
}
...
The prefetch still includes the product name and description. The converted price renders when the user navigates.

The exchange rate comes from a new function in `app/lib/data.ts`, cached per currency by the file's `'use cache'` directive:
...
export async function getExchangeRate(currency: string) {
  const res = await fetch(`https://rates.example.com/latest?to=${currency}`)
