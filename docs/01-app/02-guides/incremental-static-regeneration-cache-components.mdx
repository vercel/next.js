...
The first visit to `/tops/overshirt`. The product is unknown, but the category `tops` was prerendered. Next.js serves the App Shell with the category header already rendered. The product streams in.

The first visit to `/shoes/basketball-shoes`. Neither param was prerendered. Next.js serves the generic App Shell. Both the category and the product stream in.
...
- If some data is uncached or runtime APIs (`cookies`, `headers`) are accessed behind `<Suspense>` fallbacks, the upgrade produces a **cached result** with the params resolved but the dynamic parts still streaming.
