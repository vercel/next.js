...
```tsx filename="app/[category]/[product]/page.tsx" highlight={1-2,4,6-24,32-34}
import { cookies } from 'next/headers'
import { navigation } from 'next/cache'
import { Suspense } from 'react'
import { getProduct, getExchangeRate } from '../../lib/data'

async function LocalPrice({
  params,
}: Pick<PageProps<'/[category]/[product]'>, 'params'>) {
  await navigation()
  ...
  return (
    <p>
      {(data.price * rate).toFixed(2)} {currency}
    </p>
  )
}
  ...
  return (
    <div>
      ...
      <Suspense fallback={<p>Loading price...</p>}>
        <LocalPrice params={props.params} />
      </Suspense>
    </div>
  )
}
