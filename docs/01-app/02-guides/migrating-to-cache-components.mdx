...
- **Forms after submission** — input values persist when returning. Reset with a callback ref or `useLayoutEffect` cleanup.
- **Action state** — success or error messages from `useActionState` persist. Dispatch a reset action when the component is hidden.
