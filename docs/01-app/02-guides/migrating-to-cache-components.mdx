...
**Replace with `cacheLife`.** Use the [`cacheLife`](/docs/app/api-reference/functions/cacheLife) function with a [preset profile](/docs/app/api-reference/functions/cacheLife#using-preset-profiles) instead of the route segment config. For example, `revalidate = 3600` often lines up with the `'hours'` profile. Pick the profile that matches how often your data changes.
...
**Still supported.** [`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params) works with [dynamic routes](/docs/app/api-reference/file-conventions/dynamic-routes) when Cache Components is enabled.
...
Exact `Route (app)` lines can differ slightly by Next.js version. The snippets below show a representative `/products/[id]` route with `generateStaticParams`. When Cache Components is on, you may see **`◐` (Partial Prerender)** on the dynamic segment and **`○` (Static)** on prerendered param paths instead of **`●` (SSG)**. Use this as a quick legend for your own `next build` output, not a checklist to match line for line.
...
In the legend, **`◐`** means a static prerendered shell with dynamic content streamed on the request. That behavior is [Partial Prerendering (PPR)](/docs/app/guides/public-static-pages#partial-prerendering). See [Building](/docs/app/guides/building) for the full `next build` output reference.
