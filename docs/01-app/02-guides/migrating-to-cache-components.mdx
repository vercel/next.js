...
## Use the migration skill (recommended)

The [`next-cache-components-adoption`](https://github.com/vercel/next.js/tree/canary/skills/next-cache-components-adoption) skill lets an agent drive the migration with you. It runs in two steps:

1. **Get the build green.** The agent runs a codemod that adds `export const instant = false` to every page and layout. Each opt-out gets a `// TODO: Cache Components adoption` comment as a marker. You can merge this as a standalone PR and tackle step 2 incrementally.
2. **Remove the opt-outs one at a time.** For each route, the agent walks the subtree top-down, applies the fix the dev overlay points at (cache the data with [`use cache`](/docs/app/api-reference/directives/use-cache), wrap the dynamic part in [`<Suspense>`](https://react.dev/reference/react/Suspense), or leave the route opted out with a comment explaining why), and checks in with you before moving on.
