...
2. Replace the route segment configs it flags (`dynamic`, `revalidate`, `fetchCache`) with their Cache Components equivalents. The per-key sections below cover each.
3. [Follow the validation insights and errors](#following-validation), caching uncached data with [`use cache`](/docs/app/api-reference/directives/use-cache) or wrapping runtime data in [`<Suspense>`](https://react.dev/reference/react/Suspense).

If a route needs more work than you want to take on right now, [opt it out of validation](#opting-out-of-validation) and come back to it later. To migrate gradually rather than all at once, see [Adopting incrementally](#adopting-incrementally).
