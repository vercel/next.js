...
Routes that used `dynamic = 'force-static'` or `dynamic = 'error'` should remain prerendered. Replacing either config with `instant = false` can move rendering to request time and reduce how much UI a link can prefetch, even when the route's data remains cached.
...
**Replace with `use cache`.** Remove the config, then add `use cache` and `cacheLife` where needed to keep the route prerendered. Replacing it with `instant = false` allows the route to block instead.
