...
When self-hosting with `next start`, make sure `cacheComponents` resolves to the same value at build time and runtime. This is especially important for multi-stage Docker images. See [Cache Components configuration](/docs/app/guides/self-hosting#cache-components-configuration).
