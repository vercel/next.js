...
Cache Components changes how navigation works. Instead of unmounting pages when navigating away, Next.js preserves them using React's [`<Activity>`](https://react.dev/reference/react/Activity) component. Pages are hidden with `display: none` but remain in the DOM, so React state, form input values, scroll position, and other DOM state are preserved when the user navigates back.

This is a significant behavior change. Code that relied on components unmounting on navigation — such as cleanup logic, analytics events, or state resets — may behave differently. See [Preserving UI state across navigations](/docs/app/guides/preserving-ui-state) for patterns on choosing what to preserve and what to reset.
