...
Before enabling the flag, remove route segment configs such as `export const dynamic`, `export const revalidate`, and `export const fetchCache` from your route segments. Under `cacheComponents`, these are no longer compatible, and pages that still export them will fail to build. The sections below cover the replacements (`cacheLife` and `'use cache'`).
