...
The `Route (app)` section at the end of `next build` shows how each route was prerendered. With Cache Components, the table differs from a build without it. Compare the examples below. In the legend, **`◐`** means a static prerendered shell with dynamic content streamed on the request—that is [Partial Prerendering (PPR)](/docs/app/guides/public-static-pages#partial-prerendering). For more information, see [Building](/docs/app/guides/building).
