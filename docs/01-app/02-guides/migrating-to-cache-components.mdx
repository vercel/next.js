...
To have an agent walk you through this guide one subtree at a time, use the [`next-cache-components-adoption`](https://github.com/vercel/next.js/tree/canary/skills/next-cache-components-adoption) skill. It blankets `instant = false` to get the build green, then works each opt-out back off with you, checking in before each step.
