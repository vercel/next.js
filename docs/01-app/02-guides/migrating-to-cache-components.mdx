...
> **Good to know**: if the value drives an attribute on the `<html>` element itself (`lang`, `dir`, `data-theme`, etc.) set in the root layout, reading it on the server makes the whole subtree request-bound — there's no child to wrap in `<Suspense>`. An inline `<script>` in `<head>` that sets the attribute before paint keeps the shell static; see [Preventing flash before hydration](/docs/app/guides/preventing-flash-before-hydration) for the pattern.
