...
With Cache Components, Partial Prerendering applies to eligible routes, so returning `[]` from `generateStaticParams` triggers a [build error](/docs/messages/empty-generate-static-params) because the build needs at least one path to validate the prerendered shell. Return at least one sample param (`After` below), or remove `generateStaticParams` and let params resolve at request time (see [dynamic routes with Cache Components](/docs/app/api-reference/file-conventions/dynamic-routes#with-cache-components)).

See [`generateStaticParams` with Cache Components](/docs/app/api-reference/functions/generate-static-params#with-cache-components) for more on Suspense boundaries, placeholder params, and the full contract.
