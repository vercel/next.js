...
[`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params) works with [dynamic routes](/docs/app/api-reference/file-conventions/dynamic-routes) when Cache Components is enabled. Returning `[]` from `generateStaticParams` now triggers a [build error](/docs/messages/empty-generate-static-params). Return at least one param, or remove `generateStaticParams` and let params resolve at request time.
...
> **Good to know**: If the route has a [`loading.js`](/docs/app/api-reference/file-conventions/loading) or `<Suspense>` boundary, unlisted params (values not returned by `generateStaticParams`) are served with the prerendered shell while content streams in, and the result is cached for subsequent visits. This is the default behavior in Next.js 16.3+, where [partial fallbacks](/docs/app/api-reference/config/next-config-js/partialFallbacks) is enabled by default. Without a loading fallback, unlisted params perform a blocking prerender.

The `Route (app)` section from `next build` also changes. With Cache Components, **`●` (SSG)** is replaced by **`○` (Static)** or **`◐` (Partial Prerender)**:
...
**With Cache Components (and a `loading.js`):**
...
See [Building](/docs/app/guides/building) for more on `next build` output, symbols, and when a loading fallback is needed. See [`generateStaticParams` with Cache Components](/docs/app/api-reference/functions/generate-static-params#with-cache-components) for Suspense boundaries, placeholder params, and the full contract.
