...
Code that relied on unmounting to reset state — such as analytics events or transient UI like dropdowns — may need adjustment. See [Preserving UI state across navigations](/docs/app/guides/preserving-ui-state) for patterns on choosing what to preserve and what to reset.
