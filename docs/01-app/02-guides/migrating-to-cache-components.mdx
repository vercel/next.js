...
> **Good to know**: if the cookie or header drives an attribute on the `<html>` element itself (a `data-theme`, `lang`, or `dir` set in the root layout), reading it on the server makes the whole subtree request-bound — there's no child to wrap in `<Suspense>`. For themes, an inline `<script>` in `<head>` that sets the attribute before paint keeps the shell static; see [Preventing flash before hydration](/docs/app/guides/preventing-flash-before-hydration#themes).
