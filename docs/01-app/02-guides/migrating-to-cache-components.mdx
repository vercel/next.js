...
Your existing `fetch` caching keeps working as a separate layer, so let the insights and errors guide what to change.
...
When a component would block a navigation, the dev overlay surfaces a **blocking-route** insight that names the component and points at two fixes: cache it, or stream it behind `<Suspense>`.
...
Work through the insights and errors until they're gone. For the full validation workflow, the DevTools, and CI testing, see the [instant navigation guide](/docs/app/guides/instant-navigation).

## Opting out of validation

A validation insight or error tells you a route won't render instantly. Resolve it by following the recommendation: cache the data with [`use cache`](/docs/app/api-reference/directives/use-cache) or wrap it in [`<Suspense>`](https://react.dev/reference/react/Suspense). If you don't want to deal with it right now, set the [`instant`](/docs/app/api-reference/file-conventions/route-segment-config/instant) config to `false` on the segment that raised it (a layout, page, or parallel slot), then come back to it later.
...
Similarly to the `fetch` cache, `unstable_cache` persists cached values across deployments and across serverless instances.

`use cache` defaults to in-memory storage, so its entries are discarded when the serverless instance is destroyed and are scoped to a single deployment. Use [`use cache: remote`](/docs/app/api-reference/directives/use-cache-remote) or a [cache handler](/docs/app/api-reference/config/next-config-js/cacheHandlers) for storage that survives instance teardown. Even with durable storage, expect cached values to recompute after a new deployment.

> **Good to know:** Like `unstable_cache`, a `use cache` function cannot read runtime data such as `cookies()` or `headers()`. Read it outside the cached function and pass what you need in as an argument.
...
**Wrap runtime data access in `<Suspense>`.** Without Cache Components, reading [`cookies()`](/docs/app/api-reference/functions/cookies), [`headers()`](/docs/app/api-reference/functions/headers), or [`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional) opts the whole route into dynamic rendering. With Cache Components, accessing them outside a [`<Suspense>`](https://react.dev/reference/react/Suspense) boundary raises [`Uncached data was accessed outside of <Suspense>`](/docs/messages/blocking-route). Move the access into a component wrapped in `<Suspense>` so the rest of the page prerenders as a static shell and the dynamic part streams in at request time.
...
`params` and `searchParams` arrive as props on your page, and both are promises. Apply the same pattern: pass the promise straight through to the `<Suspense>`-wrapped component as a prop and `await` it there, rather than at the top of the page.
...
> **Good to know:** Reading uncached or runtime data in a `GET` handler bails out of prerendering by **throwing**. A `try/catch` you already have around other operations will catch that bail-out. This is harmless (prerendering has already stopped and the value isn't used), but a `catch` that logs the error produces noisy build-time logs. To quiet them, either set `experimental.hideLogsAfterAbort: true` to hide logs emitted after a bail-out, or call [`unstable_rethrow`](/docs/app/api-reference/functions/unstable_rethrow) at the top of your `catch` so the framework's control-flow error propagates instead of being logged.
