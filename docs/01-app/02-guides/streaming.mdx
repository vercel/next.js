...
See [Sharing data with context and React.cache](/docs/app/getting-started/fetching-data#sharing-data-with-context-and-reactcache) for a full example.
...
2. **The static shell** is everything above and outside those boundaries: layouts, navigation, fallback UI, and cached component output. With Cache Components, this shell is prerendered at build time. It is sent in the first HTML chunk.
...
4. **Caching directives** like [`"use cache"`](/docs/app/api-reference/directives/use-cache) let you push work back into the shell. With [Cache Components](/docs/app/getting-started/cache-components), cached components are prerendered at build time and served from the edge. Their output becomes part of the static shell. Uncached components stream at request time. This means you control the boundary between instant and streamed content by choosing which components use `"use cache"`.
