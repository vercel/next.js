...
React's server renderer produces HTML in chunks aligned to `<Suspense>` boundaries. Next.js wires this into the App Router so streaming works out of the box.
...
The component payload is a serialized representation of the component tree that React uses to [hydrate](/docs/app/04-glossary#hydration) the page and handle client-side updates. On initial load, it arrives embedded in the HTML stream (as described above). On **client-side navigation**, only the component payload is fetched (with an `rsc: 1` request header) and no HTML is transferred at all. React uses it to update the component tree in place.

### The static shell

Everything that renders before any async work resolves is called the **static shell**: your layouts, navigation, and the fallback UI defined by your `<Suspense>` boundaries. It is sent immediately, giving the user something to see and interact with while dynamic content streams in. With [Cache Components](/docs/app/getting-started/cache-components), the static shell is prerendered at build time and served instantly from the edge.
...
For pages where some sections can render independently, you may prefer granular `<Suspense>` boundaries instead. Pushing fallbacks down into specific sections lets the static shell include more real content rather than a full-page skeleton.
...
While `loading.js` streams an entire page, `<Suspense>` lets you control exactly which parts of the page stream independently. This is important when a page combines content with different loading characteristics.

### Parallel streaming with sibling boundaries

Wrap each independent section in its own `<Suspense>` boundary. They resolve and stream in whatever order they finish. The fast ones don't wait for the slow ones:
...
| **Best for**   | Pages with a single data source | Pages with multiple independent sections |
...
Server Components can pass promises to Client Components, allowing the client to wait for data without blocking the server's HTML stream. The Client Component uses React's [`use`](https://react.dev/reference/react/use) API to read the promise value:
...
The Server Component starts the fetch and passes the unresolved promise to the Client Component. The HTML for the `<Suspense>` fallback is sent immediately. When the promise resolves, React streams the completed component to the client.

### Sharing a promise across the client tree

When multiple Client Components need the same streamed data, you can store the promise in a React context. A Server Component creates the promise once (in a layout, for example) and passes it to a context provider. Any Client Component in the subtree can then call `useContext` to get the promise and `use()` to resolve it. The data is fetched once and the same promise is shared with every consumer through context.

See [Sharing data with context and React.cache](/docs/app/getting-started/server-and-client-components#sharing-data-with-context-and-reactcache) for a full example.
...
When a `<Suspense>` fallback renders or a Server Component suspends, the server must commit to `200 OK` in order to start sending the HTML stream. If a [`notFound()`](/docs/app/api-reference/functions/not-found) fires mid-stream, Next.js cannot go back and change the status to 404. Instead, it injects `<meta name="robots" content="noindex">` into the streamed HTML so that search engines don't index the page. Similarly, a [`redirect()`](/docs/app/api-reference/functions/redirect) mid-stream becomes a client-side redirect rather than a 301/302 header.

### When does streaming start?

The response body begins streaming when a Suspense fallback renders (for example, a `loading.tsx`) or when a Server Component suspends under a `<Suspense>` boundary. To get a real HTTP status code for errors, place `notFound()` **before** any `await` or `<Suspense>` boundary:
...
## Streaming through the infrastructure stack

Any layer between your server and the client that buffers the response will defeat streaming. The HTML may be fully generated progressively on the server, but if a proxy, CDN, or even the client itself collects all the chunks before rendering them, the user sees a single delayed response instead of progressive rendering.
...
const res = await fetch('http://localhost:3000/your-page', {
...
Run with `node stream-observer.mjs`. You will see the static shell arrive in the first chunk, then each Suspense boundary resolve in subsequent chunks with timestamps showing the delay.
...
The trigger is **your code**. The framework requires explicitness about three kinds of work:

- **Asynchronous work** - database queries, API calls, file reads. These suspend the component while they resolve.
- **Non-deterministic output** - `new Date()`, `Math.random()`. [Cache Components](/docs/app/getting-started/cache-components) track these and ask you to decide: cache the result with [`"use cache"`](/docs/app/api-reference/directives/use-cache), or make it per-request by accessing it in a dynamic scope (under [`connection`](/docs/app/api-reference/functions/connection), for example).
- **Runtime data** - `cookies()`, `headers()`, `searchParams`. These are inherently request-scoped and cannot be known at build time.

When the prerenderer encounters any of these, it cannot resolve that subtree statically. It walks up the component tree looking for the nearest `<Suspense>` boundary to use as a fallback. From there:

1. **`<Suspense>` boundaries** are the catch. They tell React where to stop and what fallback to show while the subtree resolves. `loading.js` is a file convention that creates one automatically, wrapping the page and sibling slots inside the layout.
2. **The static shell** is everything above and outside those boundaries: layouts, navigation, and fallback UI. It is sent in the first HTML chunk.
3. **Streaming** delivers the static shell immediately over HTTP. As each boundary resolves, React streams the completed HTML along with a small script that swaps it into the page in-place, without waiting for the full JavaScript bundle.
4. **Caching directives** like [`"use cache"`](/docs/app/api-reference/directives/use-cache) let you push work back into the shell. A cached database query can be prerendered at build time and served instantly from the edge, enlarging the static shell and reducing what needs to stream at request time.

The key decisions are **what to cache** and **where to place Suspense boundaries**. Cache what you can to grow the static shell. Push dynamic access down to the components that need it, and wrap those components in `<Suspense>`. Everything else becomes part of the shell.

{/* TODO: update URL once repo is public */}

## Example

The companion [streaming demo](https://github.com/vercel-labs/streaming-demo) lets you see each concept from this guide in action:
