...
If your LCP element (a hero image, a main heading, a product photo) is inside a Suspense boundary, it can't paint until that boundary's content is swapped in. This is fundamentally a problem because the LCP element no longer depends on your initial server response time, but on the server and the work it does to eventually render that element, plus the browser's own work to reveal it: React streams a small inline script alongside the boundary's HTML, and the content is only on screen once that script runs.

Data fetching is not the only reason an LCP element might not render immediately. A boundary with enough HTML is held back on size alone, because sending that HTML takes time. See [what activates a Suspense boundary](https://react.dev/reference/react/Suspense#what-activates-a-suspense-boundary).
...
- For non-image LCP elements (text, headings), make sure they are not wrapped in a Suspense boundary.
