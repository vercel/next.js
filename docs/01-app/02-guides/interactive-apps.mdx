...
The per-id tag `task-${id}` gives a single task its own handle. The broader `tasks` tag covers the list. See the [`'use cache'` reference](/docs/app/api-reference/directives/use-cache) for cache scoping (`'use cache: private'`, `'use cache: remote'`) and tag patterns.
...
Every Server Function follows the same shape: run the write, then call `updateTag` for the cached reads that changed.
...
- [`useTransition`](https://react.dev/reference/react/useTransition) and [`startTransition`](https://react.dev/reference/react/startTransition) mark async updates as transitions and expose `isPending`.
