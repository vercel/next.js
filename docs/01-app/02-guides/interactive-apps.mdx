...
In this demo, the comment thread stays dynamic because readers expect the current discussion to update immediately on the open task page. Adding or deleting a comment calls `refresh()`, which reruns the dynamic comment read for the current route while leaving the task caches in place. See [Caching](/docs/app/getting-started/caching) for tag patterns and time-based revalidation, and [Revalidating the cache](/docs/app/getting-started/mutating-data#revalidating-the-cache) for on-demand invalidation.
