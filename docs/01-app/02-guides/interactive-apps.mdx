...
Under [Partial Prefetching](/docs/app/guides/adopting-partial-prefetching), [`<Link>`](/docs/app/api-reference/components/link) prefetches the destination's [App Shell](/docs/app/glossary#app-shell) by default, which carries the route's static, cached, and session output. [URL data](/docs/app/glossary#url-data) like [`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional) and [`params`](/docs/app/api-reference/file-conventions/page#params-optional) varies per link, so the shell can't carry it. To resolve URL data ahead of the click, opt the link into [runtime prefetching](/docs/app/guides/runtime-prefetching) with `prefetch={true}`:
