...
Whether that costs a server request is an internal optimization. Next.js attempts a static prefetch first and serves statically renderable pages from the static cache; only a page that reads non-static data is prefetched at runtime, one server invocation per link.
