...
The page stays synchronous by using `params.then()` inline in JSX. Each `<Suspense>` boundary wraps one section, so the task header and comments stream in independently as their data resolves.
