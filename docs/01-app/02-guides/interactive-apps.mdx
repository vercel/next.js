...
Replace the direct call with [`useOptimistic`](https://react.dev/reference/react/useOptimistic) and [`startTransition`](https://react.dev/reference/react/startTransition). `useOptimistic` provides a value to render in place of the stale prop. `startTransition` runs the Server Function as part of a transition. The optimistic value applies for as long as that transition is pending:
...
If the transition ends without revalidating to new data, the optimistic value reverts. The standalone `startTransition` import does not bubble errors to error boundaries. If you need errors to reach an error boundary, use [`useTransition`](https://react.dev/reference/react/useTransition) instead (shown in Step 3).
...
`useOptimistic(priority)` updates to match the `priority` prop on each render. When `refresh()` delivers fresh data, the component shows it. `useState(priority)` captures the initial value on mount and ignores subsequent prop changes, so after a mutation it shows stale data.
...
Consider a form that calls the Server Function as a form action:
...
If you type a comment and press Send, the form action runs, but the input stays filled and the list does not change until the server responds.
...
If you drag a card from "Todo" to "In Progress", the card stays in the old column while the server processes the update, then jumps to the new column after the response arrives.
...
If you fill out the form and click "Create Task", the button stays enabled with no progress text. The form fields keep their values. The dialog closes before the board shows the new task, because state updates after the `await` are not automatically part of the surrounding transition.
...
Pick the API that matches the situation you're solving:

| Situation                                                     | Use                                                                                                                                                                                   |
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Slow data should stream in without blocking the page shell    | [`<Suspense>`](https://react.dev/reference/react/Suspense)                                                                                                                            |
| One value should update before the server responds            | [`useOptimistic`](https://react.dev/reference/react/useOptimistic) + [`startTransition`](https://react.dev/reference/react/startTransition)                                           |
| A URL change triggers a new server render                     | [`useTransition`](https://react.dev/reference/react/useTransition) + `data-pending`                                                                                                   |
| A reusable component owns part of the pending UI              | Callback prop named `action` or suffixed with `Action`                                                                                                                                |
| A new item should appear before it exists on the server       | [`useOptimistic([])`](https://react.dev/reference/react/useOptimistic) with an empty initial array                                                                                    |
| A collection needs a temporary structural change              | [`useOptimistic`](https://react.dev/reference/react/useOptimistic) with a reducer                                                                                                     |
| A form needs pending state, reset behavior, and result state  | [`useActionState`](https://react.dev/reference/react/useActionState)                                                                                                                  |
| A parent should style itself while a child action is pending  | [`useOptimistic(false)`](https://react.dev/reference/react/useOptimistic) + `data-pending` attribute                                                                                  |
| A reusable read should survive across renders and navigations | [`'use cache'`](/docs/app/api-reference/directives/use-cache) + [`cacheTag`](/docs/app/api-reference/functions/cacheTag) + [`updateTag`](/docs/app/api-reference/functions/updateTag) |
| A cached destination should be filled before the user clicks  | [`unstable_prefetch = 'force-runtime'`](/docs/app/api-reference/file-conventions/route-segment-config/prefetch)                                                                       |

For more on each API, see:

- [Server Functions](/docs/app/getting-started/mutating-data) for the server side of any mutation
- [`refresh()`](/docs/app/api-reference/functions/refresh) for revalidating dynamic work after a write
