...
The page stays synchronous by using `params.then()` inline in JSX. The outer `<Suspense>` covers the params resolution. Once `id` is available, `TaskDetail` renders immediately (with its own data fetch), and a nested `<Suspense>` lets comments stream in independently.
