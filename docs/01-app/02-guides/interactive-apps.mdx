...
When a user interaction triggers server-side work, the result isn't available immediately. Network requests take an unknown amount of time and can succeed or fail. Without feedback during that wait, users see stale data and can't tell whether anything is happening.
...
Replace the direct call with [`useOptimistic`](https://react.dev/reference/react/useOptimistic) and [`useTransition`](https://react.dev/reference/react/useTransition). `useOptimistic` provides a value to render in place of the stale prop, and `startTransition` runs the Server Function as part of a transition. The optimistic value applies for as long as that transition stays pending:
...
> **Good to know:** This is available in Next.js 16.3 and later with [`partialPrefetching: true`](/docs/app/api-reference/config/next-config-js/partialPrefetching) set in `next.config.ts`. See the [Runtime Prefetching guide](/docs/app/guides/runtime-prefetching) and [Instant Navigation guide](/docs/app/guides/instant-navigation) for the full picture.
...
- [`useTransition`](https://react.dev/reference/react/useTransition) and [`startTransition`](https://react.dev/reference/react/startTransition) mark async updates as transitions and expose `isPending`. `useTransition` also catches errors thrown inside the async callback and bubbles them to the nearest error boundary.
