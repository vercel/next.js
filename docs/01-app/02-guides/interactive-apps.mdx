...
Enabling Cache Components applies prerender validation across every route, not only the ones in this step. A route that reads request data such as `cookies()`, `headers()`, or `searchParams` outside [`<Suspense>`](https://react.dev/reference/react/Suspense) now blocks prerendering and needs the Step 1 treatment. To turn it on in an existing app, see [Migrating to Cache Components](/docs/app/guides/migrating-to-cache-components).
...
Most patterns mix two or more of these. Reach for whichever one fits the constraint you're solving.
...
- [Designing loading UI](/docs/app/guides/loading-ui) for fallbacks, pending indicators, and keeping loading states from flashing or shifting
