...
Under [Partial Prefetching](/docs/app/guides/adopting-partial-prefetching), [`<Link>`](/docs/app/api-reference/components/link) prefetches the destination's [App Shell](/docs/app/glossary#app-shell) by default. The shell carries the route's static and cached output, including UI gated behind session data such as [`cookies()`](/docs/app/api-reference/functions/cookies). The shell cannot carry the route's [URL data](/docs/app/glossary#url-data) ([`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional), and [`params`](/docs/app/api-reference/file-conventions/page#params-optional) not resolved by [`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params)), because URL data varies per link.

To resolve URL data ahead of the click, opt the link into [runtime prefetching](/docs/app/guides/runtime-prefetching) with `prefetch={true}`:
...
Whether the prefetch costs a server request is an internal optimization. Next.js attempts a static prefetch first and serves statically renderable pages from the static cache. Only a page that reads non-static data is prefetched at runtime, one server invocation per link.
