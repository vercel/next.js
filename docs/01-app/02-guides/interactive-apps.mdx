...
`LabelFilter` decides _where_ the app navigates. It holds no pending state of its own.

#### Handling pending feedback internally
...
The consumer passes a callback (`changeAction`), and the design component runs it inside a transition. By convention, props named `action` or suffixed with `Action` signal this. See [Exposing `action` props from components](https://react.dev/reference/react/useTransition#exposing-action-props-from-components) in the React docs for this pattern.

The `data-pending` attribute on `ChipGroup`'s root element lets ancestor components react via CSS without any coordination. The board on the home page uses `group-has-data-pending:opacity-50` to dim while the transition runs, without replacing the board with a skeleton. Since `ChipGroup` renders inside the `group` element, the attribute is accessible to any ancestor for styling.
...
This step uses [Cache Components](/docs/app/api-reference/config/next-config-js/cacheComponents), introduced in Next.js 16. Steps 1–7 work without it. If you are not using Cache Components, see [Caching without Cache Components](/docs/app/guides/caching-without-cache-components).
...
> Available in v16.3 and later with [`partialPrefetching: true`](/docs/app/api-reference/config/next-config-js/partialPrefetching) set in `next.config.ts`. See the [Runtime Prefetching guide](/docs/app/guides/runtime-prefetching) and [Instant Navigation guide](/docs/app/guides/instant-navigation) for the full picture.

By default, [`<Link>`](/docs/app/api-reference/components/link) prefetches the destination's [App Shell](/docs/app/glossary#app-shell). Setting `prefetch={true}` extends the prefetch to include the destination's cached content. For destinations that also read cookies, headers, or `params`, opt the route into [runtime prefetching](/docs/app/guides/runtime-prefetching) with the [`prefetch`](/docs/app/api-reference/file-conventions/route-segment-config/prefetch) segment export so private-cached reads come along too:
