...
If we navigate to a task now, the page shell paints immediately, the task header streams in when `getTask` resolves, and the comments section streams in after that when `getComments` resolves.
