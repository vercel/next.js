...
Under [Partial Prefetching](/docs/app/guides/adopting-partial-prefetching), [`<Link>`](/docs/app/api-reference/components/link) prefetches the destination's [App Shell](/docs/app/glossary#app-shell) by default. The App Shell is a per-route prerender of everything that doesn't depend on [URL data](/docs/app/glossary#url-data), including content behind session reads like [`cookies()`](/docs/app/api-reference/functions/cookies). Resolving [`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional) or [`params`](/docs/app/api-reference/file-conventions/page#params-optional) would tie the shared shell to a single link, so URL data is excluded by definition. To resolve it ahead of the click, opt the link into [runtime prefetching](/docs/app/guides/runtime-prefetching) with `prefetch={true}`:
...
Next.js still attempts a static prefetch first, and only a page that reads non-static data is prefetched at runtime, one server invocation per link.

A prefetch only pays off when the destination's reads are cached. The tags from the previous subsections give the prefetched output a stable slot in the [Client Cache](/docs/app/glossary#client-cache), so a later navigation reuses it instead of refetching.

In the companion app, each task card is a [`<Link>`](/docs/app/api-reference/components/link) to the task detail page. The cards prefetch as they scroll into view, so the destination paints instantly by the time the user clicks.
