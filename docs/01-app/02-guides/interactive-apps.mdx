...
The consumer passes a callback (`changeAction`). The design component handles the transition and optimistic state. By convention, props named `action` or suffixed with `Action` signal that the component may call them inside a transition. See [Exposing `action` props from components](https://react.dev/reference/react/useTransition#exposing-action-props-from-components) in the React docs for this pattern.
...
The parent comment card uses Tailwind's `has-data-pending:` variant to fade itself when any descendant has the `data-pending` attribute set. The button is the only component that knows when deletion is pending, so exposing it as a CSS attribute lets any ancestor dim itself without lifting state or threading callbacks:
...
Prefetch only sticks if the destination reads are cached. The tags from the previous subsections give the prefetched output a stable slot in the [Client Cache](/docs/app/glossary#client-cache). In the companion app, each task card is a [`<Link>`](/docs/app/api-reference/components/link) that navigates to the task detail page. The same element is also `draggable`, and the browser distinguishes a click from a drag natively. Because cards are links, the detail page prefetches as they scroll into view. By the time the user clicks, the destination paints instantly. See the [Prefetching guide](/docs/app/guides/prefetching) for prefetch modes, and the [Instant Navigation guide](/docs/app/guides/instant-navigation) for validating that navigations stay instant in production.
