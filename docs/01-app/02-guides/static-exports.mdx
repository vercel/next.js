...
- **Param-dependent components** (those that `await params`) suspend at the nearest [Suspense boundary](/docs/app/api-reference/components/suspense), showing your `loading.tsx` or `<Suspense fallback>` UI.
