...
When [Cache Components](/docs/app/getting-started/caching) are enabled, dynamic routes in the App Router can use a static export without enumerating every param in [`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params).

During `next build`, Next.js renders a shared fallback shell for the route. Param-independent layouts and Server Components are prerendered into the shell, while param-dependent parts of the route render on the client after navigation or page load.

If you export `generateStaticParams`, the returned params are still fully prerendered at build time. Any other param values fall back to the shared shell.
...
To serve unenumerated params on a static host, configure the host's 404 or error document to serve `/_fallback.html` from the export output. For example:
...
> - Direct requests for unenumerated params may initially return a 404 status from the static host before the fallback shell loads.
