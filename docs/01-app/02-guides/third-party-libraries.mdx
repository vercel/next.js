...
All supported third-party libraries from Meta can be imported from `@next/third-parties/meta`.

### Meta Pixel

The `MetaPixel` component can be used to easily integrate the [Meta (Facebook) Pixel](https://developers.facebook.com/docs/meta-pixel/) into your Next.js application. By default, it tracks a `PageView` event when the page loads.

<AppOnly>

To load Meta Pixel for all routes, include the component directly in your root layout and pass in your Pixel ID:
...
To load Meta Pixel for all routes, include the component directly in your custom `_app`:
...
To load Meta Pixel for a single route, include the component in your page file:
...
The `sendMetaPixelEvent` helper function can be used to track both standard and custom events dynamically. For this function to work, ensure the `<MetaPixel />` component has been included in the app.
...
Here are the standard events supported by Meta Pixel:
...
For more details, see the [Meta Pixel documentation](https://developers.facebook.com/docs/meta-pixel/reference#standard-events).

### Options

Options to pass to the `<MetaPixel>` component:

| Name         | Type     | Description                                                                    |
| ------------ | -------- | ------------------------------------------------------------------------------ |
| `pixelId`    | Required | Your Meta Pixel ID. Usually a string of numbers.                               |
| `trackEvent` | Optional | The standard event to track when the component mounts. Defaults to `PageView`. |
