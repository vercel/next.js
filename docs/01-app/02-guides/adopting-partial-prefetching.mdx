...
1. Before you enable the global flag, [audit every `<Link prefetch={true}>`](#auditing-link-prefetchtrue-calls) and record the UI its full prefetch currently includes. You can also [capture this behavior programmatically](#preserve-existing-prefetched-ui). A new app that has never shipped without Partial Prefetching can skip this step.
...
Do not include the default value, `prefetch="auto"`, or `prefetch={false}`. The default and `auto` modes don't request the legacy full prefetch, while `false` disables prefetching. This workflow preserves the dynamic UI that `prefetch={true}` requested before Partial Prefetching. Because each full prefetch can contain different UI, inspect the navigation and record the meaningful UI that appears before dynamic data streams in. Then use the matching pattern below to preserve it.
...
| Baseline content                                                         | How to preserve it                                                                                          |
| ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- |
...
You can use [`instant()`](/docs/app/guides/instant-navigation#prevent-regressions-with-e2e-tests) to turn the UI you recorded into a programmatic regression check. This is optional: you can adopt the route from your manual audit and add the tests later.

1. With `partialPrefetching` disabled, write a client-navigation `instant()` test for each audited `<Link prefetch={true}>`. Assert the meaningful destination UI from its completed legacy full prefetch.
2. Adopt the destinations without changing the assertions, then rerun the suite. Failures identify prefetched UI that the migration removed.
3. Apply the matching pattern above for each failure. Cache the smallest coherent data or rendered subtree that restores the baseline, then rerun the suite.
...
`instant()` measures which UI is available, not elapsed navigation time. For a client navigation, it models a completed, warm prefetch for the clicked `<Link>`. Base assertions on the UI available inside `instant()` and the destination code. If you don't complete the tests during adoption, keep the original expectations documented so you can restore them later. If UI was never present in the baseline, treat adding it as a separate optimization.
...
1. After you record the existing prefetched UI, adopt one destination. Add `export const prefetch = 'partial'` to its page or layout and apply the matching [preservation pattern](#auditing-link-prefetchtrue-calls). If you wrote an `instant()` test, rerun it to see whether any UI still needs to be restored. The route export clears the insight for every link pointing to that route:
