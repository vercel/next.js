...
After enabling the flag, every `<Link>` prefetches its destination's App Shell, and `<Link prefetch={true}>` no longer includes the route's dynamic content. A new project has no legacy links to audit and is done here.
...
A legacy full prefetch can include more UI than the navigation needs. The migration only needs to preserve content that should be available immediately when the navigation starts. This is usually the primary, above-the-fold content. Secondary, below-the-fold, or frequently changing content can stream after navigation.
...
To add regression coverage, use [`instant()` tests](/docs/app/guides/instant-navigation#prevent-regressions-with-e2e-tests) to verify that the migration preserves the prefetched UI you selected. A preservation test covers a subset of the legacy full prefetch, not an exact copy. If you defer these tests, record the expected UI before enabling Partial Prefetching.

1. With `partialPrefetching` disabled, write a client-navigation `instant()` test for each audited `<Link prefetch={true}>`. Use positive assertions for the UI that is present in the legacy full prefetch and should remain prefetched. Other prefetched UI can remain unasserted. Confirm the test passes before migrating.
...
`instant()` checks which UI is available from a completed, warm prefetch, not the navigation duration. Treat UI that wasn't part of the legacy prefetch as a separate optimization. If existing UI should stop being prefetched, verify that behavior separately after migration.
