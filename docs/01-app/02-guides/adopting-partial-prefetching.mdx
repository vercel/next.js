...
Content behind [`cookies()`](/docs/app/api-reference/functions/cookies) or [`headers()`](/docs/app/api-reference/functions/headers) varies per session, not per link, and the App Shell for a session-reading route is cached per session, so cached session content still gets included. Read the session value outside the cached function and pass it in, then remove `prefetch={true}` from the links:
