...
1. Before you enable the global flag, [audit every `<Link prefetch={true}>`](#auditing-link-prefetchtrue-calls) and choose which prefetched UI to preserve. You can use [`instant()` tests](#preserve-existing-prefetched-ui) to add regression coverage. When the adoption is large enough that reviewers need smaller diffs, or adopted routes should reach users before the rest, [adopt incrementally](#adopting-incrementally) with the flag off, guided by the [dynamic data during prefetching](/docs/messages/instant-link-prefetch-partial) insight.
...
After enabling the flag, every `<Link>` prefetches its destination's App Shell. A link with `prefetch={true}` can also resolve cached URL-specific content, but it no longer includes uncached dynamic content from the legacy full prefetch. A new project has no legacy links to audit and is done here.
...
Only `<Link>` elements whose effective `prefetch` value is `true` use the legacy full prefetch. This includes `prefetch={true}`, the bare `prefetch` prop, and wrapper or conditional props that resolve to `true`. The default value, `prefetch="auto"`, and `prefetch={false}` don't use the legacy full prefetch.

A legacy full prefetch can include more UI than the navigation needs. Preserve the content that should be ready when the user navigates. This is usually the primary content visible at the top of the destination. Secondary content, content farther down the page, or frequently changing content can stream after navigation.
...
To add regression coverage, use [`instant()` tests](/docs/app/guides/instant-navigation#prevent-regressions-with-e2e-tests) to verify that the migration preserves the prefetched UI you selected. A preservation test can cover any subset of the legacy full prefetch. If you defer these tests, record the expected UI before enabling Partial Prefetching.

1. With `partialPrefetching` disabled, write a client-navigation `instant()` test for each audited `<Link prefetch={true}>`. Use positive assertions for the selected UI already present in the legacy full prefetch. You don't need to assert other prefetched UI. Confirm the test passes before migrating.
2. Enable Partial Prefetching and rerun the tests without changing the assertions. Use failures to identify prefetched UI that the migration removed, then cache only the data or rendered subtree needed to restore it. When adopting routes incrementally with the global flag off, enable the flag in the test configuration for this verification; a temporary `prefetch = 'partial'` export alone doesn't change the legacy full prefetch.
3. After removing the temporary route exports, rerun the tests under the final global configuration and keep them as regression coverage.

The `instant()` helper holds back dynamic content while assertions run against the prefetched UI. It doesn't measure navigation duration. Add new prefetched UI as a separate optimization, and verify any deliberate removals after migration.

These tests verify which UI is available from the prefetch. When preserving that UI changes caching or invalidation, follow the project's existing verification approach. Extend an applicable test suite to cover behavior such as fresh data after a mutation or cache isolation between users. If the project doesn't test this type of behavior, verify it manually and record the expected result.
