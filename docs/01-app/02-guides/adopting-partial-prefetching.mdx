...
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Fully static, or content already cached                                                                                                                | The output is already in the App Shell. Remove the now-redundant `prefetch={true}`.                                                                                                                                 |
| Delivered uncached content you want kept ahead of the click                                                                                            | Cache it with [`use cache`](/docs/app/api-reference/directives/use-cache) so it joins the App Shell, then remove `prefetch={true}`.                                                                                 |
| Delivered content that depends on [`cookies()`](/docs/app/api-reference/functions/cookies) or [`headers()`](/docs/app/api-reference/functions/headers) | The App Shell is cached per session, so this content can join it. Pass the session value as an argument into a [`use cache`](/docs/app/api-reference/directives/use-cache) function, then remove `prefetch={true}`. |
...
| Delivers real-time content that must stay fresh per request                                                                                            | A prefetch would be stale by the click, so there is nothing to preserve. Remove `prefetch={true}` and let it stream in after navigation.                                                                            |
