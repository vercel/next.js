...
The `instant()` helper checks which UI is available from a completed, warm prefetch, while navigation duration requires separate measurement. New prefetched UI is a separate optimization, and any UI that should stop being prefetched requires post-migration verification.
