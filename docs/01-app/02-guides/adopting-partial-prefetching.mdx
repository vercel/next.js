...
1. Before you enable the global flag:
   1. [Audit every `<Link prefetch={true}>`](#auditing-link-prefetchtrue-calls) and choose which prefetched UI to preserve.
   2. Optionally, use [`instant()` tests](#preserve-existing-prefetched-ui) to add regression coverage.
   3. When reviewers need smaller diffs, or adopted routes should reach users before the rest, [adopt incrementally](#adopting-incrementally) with the flag off, guided by the [dynamic data during prefetching](/docs/messages/instant-link-prefetch-partial) insight.
...
> **Good to know**: In an existing app, [audit your `<Link prefetch={true}>` calls](#auditing-link-prefetchtrue-calls) first. The flag-off state is what those links deliver today.
...
An [`instant()`](/docs/app/guides/instant-navigation#prevent-regressions-with-e2e-tests) test locks in what a navigation shows immediately. With `partialPrefetching` off, write a client-navigation test for each audited `<Link prefetch={true}>` that asserts the UI you chose to preserve, and confirm it passes. If you don't add tests, record the expected UI before enabling Partial Prefetching.

Then adopt the destination and rerun the same test unchanged. A failure identifies UI the App Shell no longer carries, so you can cache the data or subtree that restores it. A passing test stays as regression coverage.
