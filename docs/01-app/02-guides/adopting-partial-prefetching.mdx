...
> **Good to know**: In [`next dev`](/docs/app/api-reference/cli/next#next-dev-options), [Cache Components](/docs/app/api-reference/config/next-config-js/cacheComponents) raises the [`instant-link-prefetch-partial`](/docs/messages/instant-link-prefetch-partial) Insight on each `<Link prefetch={true}>` that points at a route without Partial Prefetching. Use it to find the calls to audit.
