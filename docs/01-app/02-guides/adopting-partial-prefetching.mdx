...
The App Shell is shared across every link to a route, so it can't contain data that belongs to a single URL. When a route reads [`params`](/docs/app/api-reference/file-conventions/page#params-optional) or [`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional) outside of a [`<Suspense>`](https://react.dev/reference/react/Suspense) boundary, Next.js can't extract the shell, and navigations to the route may not be instant. In [`next dev`](/docs/app/api-reference/cli/next#next-dev-options), the dev overlay surfaces a [**URL data outside of Suspense while extracting a reusable shell** insight](/docs/messages/instant-shell-link-data) for each route where this happens, so you can work through them after enabling Partial Prefetching.

Unlike the [`<Link prefetch={true}>` audit](#auditing-existing-link-prefetchtrue-calls), this audit runs with the flag on. Finish that audit first: enabling the flag marks every route as adopted, which stops its warning from firing and removes the per-link signal.
