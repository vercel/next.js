...
The App Shell is shared across every link to a route, so it can't contain data that belongs to a single URL. [`params`](/docs/app/api-reference/file-conventions/page#params-optional) and [`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional) are [URL data](/docs/app/glossary#url-data), and reading them outside of a [`<Suspense>`](https://react.dev/reference/react/Suspense) boundary ties the shell to the link's URL. Next.js can no longer share the prefetch across links, so a click arrives with nothing prefetched, and the navigation waits on the network instead of being instant.

In [`next dev`](/docs/app/api-reference/cli/next#next-dev-options), the dev overlay surfaces an [**instant-shell-link-data** insight](/docs/messages/instant-shell-link-data) for each route that reads URL data this way, so you can work through them one at a time. The insight appears when you load or navigate to the route, regardless of any link's [`prefetch`](/docs/app/api-reference/components/link#prefetch) prop.

Unlike the [`<Link prefetch={true}>` audit](#auditing-existing-link-prefetchtrue-calls), this audit runs once Partial Prefetching is enabled. Finish that audit first: enabling the flag marks every route as adopted, which stops its warning from firing and removes the per-link signal.
...
After applying the fix, load or navigate to the route again and confirm the insight is gone and the shell renders real content. A `<Suspense>` boundary around the entire page body can pass validation with an empty shell, which defeats the point of an instant navigation.

> **Good to know**: [`cookies()`](/docs/app/api-reference/functions/cookies) and [`headers()`](/docs/app/api-reference/functions/headers) don't tie the prefetch to a URL. They vary per user, not per link, so the App Shell is still shared. Only `params` and `searchParams` surface this insight, and a read inside [`generateMetadata`](/docs/app/api-reference/functions/generate-metadata) surfaces as [URL data in `generateMetadata()`](/docs/messages/blocking-prerender-metadata-runtime) instead.
