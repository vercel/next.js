...
2. Adopt the destinations with `prefetch = 'partial'`, or enable Partial Prefetching globally, and rerun the tests without changing the assertions. Use failures to identify prefetched UI that the migration removed, then cache only the data or rendered subtree needed to restore it.
