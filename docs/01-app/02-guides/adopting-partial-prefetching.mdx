...
The App Shell is shared across every link to a route, so it can't contain data that belongs to a single URL. When a route reads [`params`](/docs/app/api-reference/file-conventions/page#params-optional) or [`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional) outside of a [`<Suspense>`](https://react.dev/reference/react/Suspense) boundary, its prefetch is tied to one URL and can no longer be shared. In [`next dev`](/docs/app/api-reference/cli/next#next-dev-options), the dev overlay surfaces a [**URL data while extracting a reusable shell** insight](/docs/messages/instant-shell-link-data) for each route where this happens, so you can work through them after enabling Partial Prefetching.
