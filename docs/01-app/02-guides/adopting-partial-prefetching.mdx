...
1. Before you enable the global flag, [audit every `<Link prefetch={true}>`](#auditing-link-prefetchtrue-calls) and choose which prefetched UI to preserve. You can use [`instant()` tests](#preserve-existing-prefetched-ui) to add regression coverage.
...
New apps that have always used Partial Prefetching can skip the first step.
...
For an existing app, complete the [`prefetch={true}` audit](#auditing-link-prefetchtrue-calls) before enabling Partial Prefetching.
...
With the flag enabled, each prefetching `<Link>` loads its destination's App Shell. The `prefetch={true}` prop no longer includes all dynamic content from the legacy full prefetch.
...
Audit each `<Link>` whose effective `prefetch` value is `true`. This includes `prefetch={true}`, the bare `prefetch` prop, and wrapper or conditional props that resolve to `true`.

Skip the default value, `prefetch="auto"`, and `prefetch={false}`. The default and `auto` modes don't request the legacy full prefetch, while `false` disables prefetching.

Legacy full prefetching may include more UI than the navigation needs. Choose the content that should be available immediately when the navigation starts. This is usually the primary, above-the-fold content. Secondary, below-the-fold, or frequently changing content can stream after navigation.
...
| Content to preserve                                                    | Migration                                                                            |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| [Fully static, or content already cached](#static-or-cached-content)   | Keep it in the App Shell. Remove `prefetch={true}` if the route no longer needs it.  |
| [Uncached content](#uncached-content)                                  | Cache it with `use cache`. Remove `prefetch={true}` if the route no longer needs it. |
| [Content that depends on `cookies()` or `headers()`](#session-content) | Cache the lookup using the session value.                                            |
| [URL-dependent content](#url-data)                                     | Cache it using the resolved URL value, then add or keep `prefetch={true}`.           |
| [Real-time content](#real-time-content)                                | Choose between a cached prefetch and streaming fresh content at request time.        |

### Preserve existing prefetched UI

To add regression coverage, use [`instant()` tests](/docs/app/guides/instant-navigation#prevent-regressions-with-e2e-tests) to verify that the migration preserves the prefetched UI you selected. If you defer these tests, record the expected UI before enabling Partial Prefetching.

1. With `partialPrefetching` disabled, write a client-navigation `instant()` test for each audited `<Link prefetch={true}>`. Assert the UI you want to preserve and confirm the test passes against the legacy full prefetch.
2. Adopt the destinations and rerun the tests without changing the assertions. For missing UI, cache the smallest coherent data or rendered subtree that restores it.
3. After enabling the global flag and removing the temporary route exports, rerun the tests and keep them as regression coverage.

`instant()` checks which UI is available from a completed, warm prefetch, not the navigation duration. Treat UI that wasn't part of the legacy prefetch as a separate optimization.
...
[URL data](/docs/app/glossary#url-data) (`params`, `searchParams`) varies per link, so the shared App Shell can't include content that depends on it. The content streams after navigation from behind its `<Suspense>` boundary. To prefetch this content, cache it using the resolved value and add or keep `prefetch={true}`. See [Prefetching URL data](#prefetching-url-data) for an example.

### Real-time content

Prefetching real-time content can make it stale before the navigation starts. Keep the prefetch with an explicit cache policy when lower latency outweighs freshness. Otherwise, remove `prefetch={true}` and stream fresh content from behind its `<Suspense>` boundary.
...
1. Choose the UI to preserve for one destination and apply the matching [preservation pattern](#auditing-link-prefetchtrue-calls). Add `export const prefetch = 'partial'` to the page or layout, then rerun its `instant()` test if you added one. The route export clears the insight for every link pointing to that route:
...
If you added `instant()` tests, run them after enabling the global flag and removing the per-route exports.
