...
Each `<Link prefetch={true}>` used to prefetch its destination's dynamic content along with the page. With the flag on it loads the App Shell like every other link, which can be thinner than the old full prefetch. Go through each one and work out what the destination should still prefetch:

> **Good to know**: `cookies()` and `headers()` don't tie a prefetch to a URL. They vary per session, not per link, so the App Shell still carries session content. Only `params` and `searchParams` are [URL data](/docs/app/glossary#url-data), which varies per link and can't join the shared App Shell.

| Destination                                                                                                                                            | Recommendation                                                                                                                                                                                                    |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Fully static, or content already cached                                                                                                                | The output is already in the App Shell. Drop the now-redundant `prefetch={true}`.                                                                                                                                 |
...
| Delivered content that depends on [`cookies()`](/docs/app/api-reference/functions/cookies) or [`headers()`](/docs/app/api-reference/functions/headers) | The App Shell is cached per session, so this content can join it. Pass the session value as an argument into a [`use cache`](/docs/app/api-reference/directives/use-cache) function, then drop `prefetch={true}`. |
| Reads [URL data](/docs/app/glossary#url-data) (`params`, `searchParams`), or has cached content that depends on it                                     | URL data varies per link, so this content can't join the shared App Shell and streams in after navigation. Keep `prefetch={true}` to [prefetch it later](#prefetching-url-data).                                  |
| Delivers real-time content that must stay fresh per request                                                                                            | A prefetch would be stale by the click, so there is nothing to preserve. Drop `prefetch={true}` and let it stream in after navigation.                                                                            |
...
Cache it with `use cache` so it joins the App Shell, then remove `prefetch={true}` from the links:
...
Content behind `cookies()` or `headers()` joins the per-session App Shell once cached. Read the session value outside the cached function and pass it in, then remove `prefetch={true}` from the links:
...
Content that reads `params` or `searchParams` streams in after navigation from behind its `<Suspense>` boundary. The links keep `prefetch={true}`, and [Prefetching URL data](#prefetching-url-data) covers prefetching the content ahead of the click.

### Real-time content

A prefetch would be stale by the click. Remove `prefetch={true}` and let the content stream in from behind its `<Suspense>` boundary.

## Adopting incrementally

Adoption doesn't have to ship as one change. [`prefetch = 'partial'`](/docs/app/api-reference/file-conventions/route-segment-config/prefetch) is the global flag scoped to one route, so with the global flag still off, each destination can adopt and ship on its own.
...
2. Ship it. Links to the adopted destination load the App Shell, and the insight keeps firing for the destinations you haven't reached.
