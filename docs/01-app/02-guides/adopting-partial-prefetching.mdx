...
5. Optionally, [defer cached content until navigation](/docs/app/guides/optimizing-prefetching#defer-cached-content-until-navigation) on routes where including it in a prefetch would add unnecessary work.
...
Content behind [`cookies()`](/docs/app/api-reference/functions/cookies) or [`headers()`](/docs/app/api-reference/functions/headers) varies per session, not per link, and [session data resolves in the App Shell](/docs/app/guides/optimizing-prefetching#include-session-specific-content-in-the-app-shell), so cached session content still gets included. Read the session value outside the cached function and pass it in, then remove `prefetch={true}` from the links:
