...
Unlike the [`<Link prefetch={true}>` audit](#auditing-existing-link-prefetchtrue-calls), this audit runs with the flag on. Finish that audit first: enabling the flag marks every route as adopted, which silences its warning and removes the per-link signal.
