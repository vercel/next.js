...
Create an `mdx-components.tsx` (or `.js`) file in the root or src folder of your project to define global MDX Components. For example, at the same level as `pages` or `app`, or inside `src` if applicable.
