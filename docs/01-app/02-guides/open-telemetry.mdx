...
This span represents a function callback passed to [`after`](/docs/app/api-reference/functions/after). It starts when the callback runs after the response is finished and may therefore outlive its parent request span. The callback runs with the trace context captured when `after()` was called, so spans created inside it are children of this span. Promises passed directly to `after()` are not traced because they begin executing before `after()` receives them.
