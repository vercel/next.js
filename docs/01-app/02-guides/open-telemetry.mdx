...
This span represents the execution of a Server Action. It covers the action
function itself, excluding request decoding, validation, revalidation, and the
render that follows the action. The stable span name is
`AppRender.executeServerAction`; `next.span_name` provides the human-readable
`run Server Action [action name]` label.
...
- `next.server_action.file` (only in `next dev`)

For a progressive-enhancement form submission that runs before hydration or
without JavaScript, the action name is `<action>` and
`next.server_action.file` is omitted.

Client-dispatched functions marked with `use cache` do not emit this Server
Action span.
