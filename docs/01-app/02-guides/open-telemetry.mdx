...
This span represents one invocation of a function marked with [`use cache`](/docs/app/api-reference/directives/use-cache). It includes the cache lookup, any wait for a concurrent invocation, and generation on a miss. Spans created while generating the cached value are children of this span. The span reports only fixed cache metadata; cache keys, arguments, tags, headers, cookies, and return values are never included.
...
- `next.cache.outcome`: `hit`, `miss`, `stale`, `bypass`, or `wait`; `wait` means this invocation waited for another invocation to generate the value
...
- `next.cache.background_refresh`: `true` when a stale value is returned while a refresh runs in the background
