...
### `use cache [next.cache.name]`

- `next.span_type`: `UseCache.execute`

This span represents one invocation of a function marked with [`use cache`](/docs/app/api-reference/directives/use-cache). It includes the cache lookup, any wait for a concurrent invocation, and foreground generation on a miss. Spans created during foreground generation are children of this span. Background revalidation runs separately. The span reports only fixed cache metadata; cache keys, arguments, tags, headers, cookies, and return values are never included.

Attributes:

- `next.span_name`: `use cache [next.cache.name]`
- `next.span_type`: `UseCache.execute`
- `next.span_category`: `application`
- `next.cache.name`: the cached function or component name, or `<cache>` when no stable name is available
- `next.cache.file`: the project-relative source file in development, when available
- `next.cache.kind`: `public` (including custom cache kinds) or `private`
- `next.cache.outcome`: `hit`, `miss`, `stale`, `bypass`, or `wait`; `wait` means this invocation waited for another invocation to generate the value. A failed invocation reports an error span status and may omit this attribute if it failed before reaching an outcome.
- `next.cache.source`: `resume-data`, `handler`, or `generated`, when the source is known
- `next.cache.joined`: `intra-request` or `cross-request` when the invocation waits for another invocation
- `next.cache.handler`: `default`, `custom`, or `none`
- `next.cache.reason`: `forced`, `draft-mode`, `dev-rewarm`, `stale`, `runtime-data`, `prerender-ended`, `omitted-from-shell`, or `joined-pending`, when applicable
- `next.cache.background_refresh`: `true` when this invocation schedules a refresh in the background

### `revalidate use cache [next.cache.name]`

- `next.span_type`: `UseCache.revalidate`

This span represents cache regeneration that runs after a stale value was returned, or a development cache entry was scheduled for rewarming. It is a child of the `UseCache.execute` span that scheduled it and may finish after both that span and the request span. Spans created while regenerating the value are children of this span. It ends after generation and cache-handler writes settle. Concurrent invocations that reuse an existing background refresh do not create another revalidation span.

Attributes:

- `next.span_name`: `revalidate use cache [next.cache.name]`
- `next.span_type`: `UseCache.revalidate`
- `next.span_category`: `application`
- `next.cache.name`: the cached function or component name, or `<cache>` when no stable name is available
- `next.cache.file`: the project-relative source file in development, when available
- `next.cache.kind`: `public` (including custom cache kinds) or `private`
- `next.cache.handler`: `default`, `custom`, or `none`
- `next.cache.reason`: `stale` or `dev-rewarm`

### `run Server Action [next.server_action.name]`

- `next.span_type`: `AppRender.executeServerAction`

This span represents the execution of a Server Action. It covers only the user function call; decoding the request, loading the action module, revalidation, and rendering the response remain outside the span. Server Functions marked with [`use cache`](/docs/app/api-reference/directives/use-cache) are excluded from this span.
...
- `next.span_type`
- `next.span_category`: `application`
- `next.server_action.name`: the exported action name, `<inline action>` for an inline action, or `<action>` when the name is unavailable
- `next.server_action.file`: the project-relative source file in development; omitted in production

### `after callback`

- `next.span_type`: `After.executeCallback`

This span represents a function callback passed to [`after`](/docs/app/api-reference/functions/after). It starts when the queued callback runs, rather than when `after()` is called, and may outlive its parent request span. The callback runs with the trace context captured when `after()` was called, so spans created inside it are children of this span. Promises passed directly to `after()` are not traced because they begin executing before `after()` receives them.
