...
- **[Streaming](/docs/app/api-reference/file-conventions/loading):** Use Loading UI and React Suspense to progressively send UI from the server to the client, and prevent the whole route from blocking while data is being fetched. See [Designing loading UI](/docs/app/guides/loading-ui) for sizing fallbacks and ordering reveals so the streamed page does not shift or flash.
