...
Protect the database module with `import 'server-only'` as well. The guard prevents client imports of server code; selecting props controls which data reaches the browser. A successful build alone does not verify that props contain no private data.
