...
Keep database clients, database schemas, and other server implementation behind an `import 'server-only'` in the module that owns that code. Importing a module from a Server Component does not prevent a Client Component from importing it later. The guard makes that accidental client import a build error, even when the module contains no private environment variables.

Keep shared values and validators in environment-neutral modules that Client Components can import directly. If a client imports a shared value through a server module, fix that import and guard the server module. The import change removes the server dependency from the client graph; the guard prevents future client imports. See [Preventing environment poisoning](/docs/app/getting-started/server-and-client-components#preventing-environment-poisoning) for an example.
