...
Adding `'use client'` changes how a component receives data. A record passed between Server Components stays on the server. Passing that same record to a Client Component sends all its fields to the browser, including fields the component never renders.

Review the component's callers when moving this boundary. Select the fields needed by the UI on the server, after checking that the user may access them. For example, a product record may include an internal supplier cost. Pass the public name and price to an interactive product card instead of the complete record:
...
Also add this import to the existing module that owns your database client or queries. It protects against future client imports, even when the current client imports are safe:

```ts filename="lib/db.ts" switcher
import 'server-only'
```

```js filename="lib/db.js" switcher
import 'server-only'
```

Next.js handles `server-only` imports internally, so [installing the package is optional](/docs/app/getting-started/server-and-client-components#preventing-environment-poisoning). The guard prevents client imports of server code; selecting props controls which data reaches the browser. A successful build alone does not verify that props contain no private data.
