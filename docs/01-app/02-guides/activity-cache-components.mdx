...
This preserves component state and DOM state, which is great for back/forward navigation. But it also means you need to decide which state should persist and which should reset - patterns that relied on unmounting to clear state now need explicit choices.
...
**When to reset it:** A dropdown menu or popover triggered by a button click - these are transient interactions, not persistent view state. Returning to a page with a dropdown already open feels wrong.
...
**When to keep it:** A ticket redemption form showing "Ticket redeemed successfully", or a settings form showing "Changes saved". Seeing the result of a previous action when returning to the page is useful confirmation - the user can see what happened.
...
You can think of `useActionState` as a `useReducer` that allows side effects. It doesn't have to only handle form submissions - you can dispatch any action to it. Adding a `RESET` action gives you a clean way to clear state when Activity hides the component:
