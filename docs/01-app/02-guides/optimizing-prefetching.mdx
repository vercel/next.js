...
If only some links should use per-link prefetching, leave the others on the default `auto` behavior. A conditional such as `prefetch={isFeatured}` becomes `prefetch={false}` when the condition is false, which disables prefetching entirely. Use `prefetch={isFeatured ? true : 'auto'}` instead.
...
Only put reusable work inside the cache boundary. If an existing data function calls [`connection()`](/docs/app/api-reference/functions/connection) to wait for a request, don't leave that call inside a new `"use cache"` scope. When the result can be reused, remove the request-time gate and cache the lookup. When only part of the function is reusable, split that work into a cached helper and keep request-specific work outside it. Data that must wait for a real request should stay uncached and stream from its `<Suspense>` boundary.
...
Unlike [`connection()`](/docs/app/api-reference/functions/connection), `unstable_navigation()` does not make the subtree request-dependent. This lets reusable content keep its cache lifetime while waiting for navigation. See [How `unstable_navigation()` differs from `connection()`](/docs/app/api-reference/functions/navigation#how-unstable_navigation-differs-from-connection) for the full comparison.

## Verify the prefetched UI

Use an [`instant()`](/docs/app/guides/instant-navigation#prevent-regressions-with-e2e-tests) test to verify both what the selected link includes in its prefetch and what still waits for navigation.

All assertions about the prefetched result belong inside the `instant()` callback. Assert that the intended content is visible and navigation-only content is absent. After the callback releases the navigation, assert that the deferred content eventually renders:
...
The assertion after `instant()` distinguishes intentionally deferred content from content that is missing or broken. Run the locked test before changing the implementation to capture the current result, then run it against a production build after the optimization and keep the passing test as regression coverage.
