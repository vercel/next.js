...
`prefetch={false}` disables all prefetching. When a condition enables per-link prefetching for some links, return `'auto'` for the others to preserve the default behavior:
...
[`connection()`](/docs/app/api-reference/functions/connection) marks the code that follows as request-time work, while `"use cache"` marks a scope as reusable. Because these behaviors conflict, `connection()` cannot be called inside a cached scope. A data function that needs both behaviors can use a cached helper for the reusable lookup and keep its request-specific work outside the cache boundary, where it can stream from `<Suspense>`.
