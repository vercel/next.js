...
If only some links should resolve URL data before navigation, return `'auto'` for the rest to keep the default App Shell prefetch:
...
If prefetched data has write paths, follow the [cache lifecycle guidance](/docs/app/getting-started/caching#keep-mutable-data-fresh) to connect them to invalidation and verify a write followed by a fresh read.
