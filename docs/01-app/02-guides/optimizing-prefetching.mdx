...
Optimizing a prefetch also means excluding content from it. [Cache Stages](/docs/app/glossary#cache-stages) are the control mechanism for choosing how much of a route each prefetch includes. Some subtrees are expensive enough to produce that you would rather not pay for them on every prefetch of the route.
