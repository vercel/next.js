...
The two sections above add content to a prefetch. Sometimes the useful move is the opposite: a subtree that is expensive to produce, and that you would rather not include in every prefetch of the route.

Two experimental functions defer a subtree past a prefetch stage. The fallback of the nearest [`<Suspense>`](https://react.dev/reference/react/Suspense) boundary takes its place, and the deferred content renders at the stage you chose:

- [`unstable_prefetch()`](/docs/app/api-reference/functions/prefetch) keeps the content out of the App Shell. It renders on a per-link prefetch when the link requests one with `prefetch={true}`, and otherwise on an actual navigation.
- [`unstable_navigation()`](/docs/app/api-reference/functions/navigation) keeps it out of per-link prefetches too, so it renders only on navigation.

Await the function above the work to defer. Take the dashboard again, where a ranking keyed on the session is too expensive to include in the App Shell that every link into the route prefetches:
...
The session is read above the call, so it resolves in the shell like any other session data. Only the ranking is left out, and it keeps its cache lifetime.

Without these functions, keeping a subtree out of a prefetch means leaving it uncached, which gives up that lifetime on the navigation too. Neither call can sit inside a cached scope, so put the caching directive on a function called below the call, as `rank` does here.
...
[Excluding content](#exclude-content-from-a-prefetch) is one more control on top of these. The trade is the `<Suspense>` fallback on arrival against producing that content ahead of the click, in a prefetch that may never be used.
