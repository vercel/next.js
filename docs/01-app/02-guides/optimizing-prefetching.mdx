...
Test a link whose `href` points directly to the destination. A redirect prevents the link from prefetching the final route tree, so the test measures the redirect instead of the intended destination.
