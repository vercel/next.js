...
Awaiting [`unstable_navigation()`](/docs/app/api-reference/functions/navigation) above the work leaves it out of every prefetch of the route, so it is produced when the user navigates. The nearest [`<Suspense>`](https://react.dev/reference/react/Suspense) fallback is what the user sees on navigation, while the content streams in. Take the dashboard again, where a ranking keyed on the session is too expensive to produce for every link into the route:
...
Without it, keeping a subtree out of a prefetch means leaving it uncached, which gives up that lifetime on the navigation too. The call cannot sit inside a cached scope, so put the caching directive on a function called below it, as `rank` does here.

To keep a subtree that would otherwise sit in the App Shell out of it, while a `<Link prefetch={true}>` can still resolve it before the click, use [`unstable_prefetch()`](/docs/app/api-reference/functions/prefetch) instead.
