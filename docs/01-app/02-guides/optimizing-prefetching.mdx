...
If only some links should use full prefetching, leave the others on the default `auto` behavior. A conditional such as `prefetch={isFeatured}` becomes `prefetch={false}` when the condition is false, which disables prefetching entirely. Use `prefetch={isFeatured ? true : 'auto'}` instead.
