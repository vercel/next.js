...
Build the SWR key from stable request inputs. A value created during render, such as `Date.now()` or `new Date()`, produces a different key after the component remounts. SWR then treats the read as new and shows its loading state instead of reusing the previous result when the user navigates away and back. If the request uses a rolling time range, update the range on the intended refresh interval rather than on every mount.
