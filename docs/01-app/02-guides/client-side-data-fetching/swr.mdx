...
Use the same key for the fallback, the Client Component read, and the mutation. With SWR, pass the mutation to [`mutate`](https://swr.vercel.app/docs/mutation) and provide `optimisticData`. SWR shows the optimistic value immediately and rolls it back if the mutation fails. This example keeps the optimistic value after the action succeeds because the final value is known:
