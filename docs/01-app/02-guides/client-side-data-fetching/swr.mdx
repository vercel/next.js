...
Use stable request inputs in an SWR key. Values created during render, such as `Date.now()` or `new Date()`, produce a new cache entry after a remount. For a rolling time range, update the key on the intended refresh interval.
