...
On mutation, use `useMutation`'s [`onMutate`](https://tanstack.com/query/latest/docs/framework/react/guides/optimistic-updates) callback to update the cache immediately and restore the previous value in `onError` if the mutation fails. This example keeps the optimistic value after the action succeeds because the final value is known:
