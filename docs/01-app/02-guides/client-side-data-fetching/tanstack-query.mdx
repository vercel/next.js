...
> **Good to know:** TanStack Query's `dehydrate()` reads the current time during Cache Components prerendering. Use the [prerenderable hydration helper](#build-a-prerenderable-hydration-state) for cached server seeds.
...
The optimistic query value updates the current screen. The `{ expire: 0 }` option ensures the next cached server read returns fresh activity.
