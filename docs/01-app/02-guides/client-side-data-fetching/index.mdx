...
An optimistic update should restore the previous browser value if the mutation fails. If the server read is not cached, there is no server tag to invalidate.
