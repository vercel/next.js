...
To do manual prefetching, import the `useRouter` hook from `next/navigation`.
...
```tsx
<button onMouseEnter={() => router.prefetch('/pricing')}>
  View Pricing
</button>
```

If the intent is to prefetch a URL when a component loads, see the [example](#extending-or-ejecting-link) in the extending or ejecting link section.
