...
> **Good to know:** During the initial navigation, the browser fetches the HTML, RSC Payload, and JS Bundle. For subsequent navigations, the browser will fetch the RSC Payload for Server Components and JS bundle for Client Components.
...
Next.js tries to do the right prefetching by default but power users can tweak the knobs to do more or less prefetching based on their needs. You have the control between performance and resource consumption.
...
You can also use [`useRouter`](/docs/app/api-reference/functions/use-router) to recreate some of the some of the native `<Link>` behavior, this allows you to build your custom prefetching strategy. But be aware this opts you into maintaining prefetching, cache invalidation, and accessibility concerns.
