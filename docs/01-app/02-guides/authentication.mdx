...
2. [**Database**](#database-sessions): Session data is stored in a database, with the user's browser only receiving a signed token that contains the session ID. This method is more secure, but can be complex and use more server resources.
...
2. Write logic to sign and verify session data using a session management library.
...
Signing prevents undetected changes to the session data, but it does not hide that data from the user. Do not include sensitive information in the session payload.
...
export default async function handler(
  ...
  const signedSessionData = await sign(sessionData)
...
3. Sign a session payload containing the session ID before storing it in the user's browser, and ensure the database and cookie stay in sync (this is optional, but recommended for optimistic auth checks in [Proxy](#optimistic-checks-with-proxy-optional)).
