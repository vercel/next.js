...
A top-level `await` on `cookies()` or the DAL in a layout delays streaming for the segment, and Client Components cannot import the DAL, so you need a deliberate way to get session data to the shell or the client.

If the layout reads `cookies()` or the DAL for shell UI, see [Streaming](/docs/app/guides/streaming#push-dynamic-access-down). For session in Client Components, see [Sharing data with context and `React.cache`](/docs/app/getting-started/fetching-data#sharing-data-with-context-and-reactcache) and [Context Providers](#context-providers).
