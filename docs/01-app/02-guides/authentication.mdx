...
A [layout](/docs/app/api-reference/file-conventions/layout) can stay mounted across navigations ([Partial Rendering](/docs/app/getting-started/linking-and-navigating#client-side-transitions)), so treat it as shell UI: enforce access in your [DAL](#creating-a-data-access-layer-dal) and on each page, Server Action, or Route Handler. For session-dependent nav or menus, use [pushing dynamic access down](/docs/app/guides/streaming#push-dynamic-access-down) in the [Streaming](/docs/app/guides/streaming) guide—nested `<Suspense>` around the shell, not `{children}` behind verify + `<Suspense>` as a fake gate.

The DAL is server-only. When auth libraries or client fetchers need the session in the browser tree, wire them the same way as [Fetching Data](/docs/app/getting-started/fetching-data): start from a cached server read and share it with clients ([Sharing data with context and `React.cache`](/docs/app/getting-started/fetching-data#sharing-data-with-context-and-reactcache)), then read [Context Providers](#context-providers) for how providers interact with Server Components and client-side fetching.
