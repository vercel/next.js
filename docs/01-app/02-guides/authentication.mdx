...
> - [Proxy](/docs/app/api-reference/file-conventions/proxy) uses the Node.js runtime by default. Check if your auth library and session management library are compatible.
...
A top-level `await` on `cookies()` or the DAL in a layout delays streaming for the segment. If only the shell needs that data (for example a user menu), move the async work into a nested Server Component and wrap it in `<Suspense>` so `{children}` are not blocked:
...
The layout itself stays synchronous. Only `<UserNav>` suspends while it reads the session, and `{children}` stream immediately. For more on this pattern, see [Push dynamic access down](/docs/app/guides/streaming#push-dynamic-access-down).

Client Components cannot import the DAL. Run `verifySession()`, `getUser()`, or similar in a parent Server Component, then pass data to client children as props or through context. See [Context Providers](#context-providers) or [Sharing data with context and `React.cache`](/docs/app/getting-started/fetching-data#sharing-data-with-context-and-reactcache).
