...
[Partial Rendering](/docs/app/getting-started/linking-and-navigating#client-side-transitions) means layout code does not always run again when the user navigates. Put authorization in your [Data Access Layer (DAL)](#creating-a-data-access-layer-dal) and keep checking on each page, Server Action, and Route Handler (see below). Calling `getUser()` from a layout is fine if the DAL does the check.

Top-level `await` on `cookies()` or the DAL in a layout delays streaming; for shell-only work (like a nav), use nested `<Suspense>` as in [Streaming](/docs/app/guides/streaming#push-dynamic-access-down). The DAL is server-only—pass values from Server parents, or use [context and `React.cache`](/docs/app/getting-started/fetching-data#sharing-data-with-context-and-reactcache) and [Context Providers](#context-providers) when you need session on the client.
