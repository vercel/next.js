...
A top-level `await` on `cookies()` or the DAL in a layout delays streaming for the segment. If only the shell needs that data (for example a user menu), move the `await` into a nested Server Component and wrap it in `<Suspense>` so `{children}` are not blocked. See [Push dynamic access down](/docs/app/guides/streaming#push-dynamic-access-down) for a full example.
