...
> **Good to know:** With [Cache Components](/docs/app/getting-started/caching) enabled, reading the session and caching per-user data follows specific rules. See [Authentication with Cache Components](/docs/app/guides/authentication-with-cache-components).
...
> **With Cache Components:** read the session in a [`use cache: private`](/docs/app/api-reference/directives/use-cache-private) function kept behind a `<Suspense>` boundary. See [Authentication with Cache Components](/docs/app/guides/authentication-with-cache-components#step-1-read-the-current-user).
...
> **With Cache Components:** the same applies. Keep the session read behind `<Suspense>` so the static shell streams first. See [Show the user without blocking the page](/docs/app/guides/authentication-with-cache-components#step-2-show-the-user-without-blocking-the-page).
...
> **With Cache Components:** pass a `Promise<User>` through context and unwrap it with [`use()`](https://react.dev/reference/react/use) instead of client-side fetching. See [Share the user across Components](/docs/app/guides/authentication-with-cache-components#step-3-share-the-user-across-components).
