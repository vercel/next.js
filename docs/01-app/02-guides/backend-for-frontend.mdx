...
For example, a documentation site might serve HTML pages to browsers and raw Markdown to tools like `curl -H "Accept: text/markdown"` or LLM agents—all from the same `/docs/…` URLs.
...
When a request to `/docs/getting-started` includes `Accept: text/markdown`, the rewrite transparently routes it to `/docs/md/getting-started`—a Route Handler that returns Markdown. Browsers and other clients that do not send `text/markdown` in their `Accept` header continue to receive the normal HTML page.
...
> - For more advanced negotiation logic—such as handling multiple content types or `q`-value priorities—you can use [`proxy`](/docs/app/api-reference/file-conventions/proxy) instead of rewrites for more flexibility.
