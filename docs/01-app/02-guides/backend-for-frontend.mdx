...
With `experimental.agentRoutes` enabled, App Router pages can also expose
agent-friendly outputs from the same URL via `Accept` negotiation:

- `Accept: text/markdown`
- `Accept: application/json`

Use page exports to opt in:

```ts filename="app/docs/page.tsx"
...
export async function generateAgent(): Promise<AgentRoute.Document> {
  return {
    title: 'Docs',
    sections: [{ content: 'Agent-focused route summary.' }],
  }
}
```

If `generateAgent` is exported without `agent`, Next.js defaults to markdown
when `Accept: text/markdown` is explicitly preferred.
