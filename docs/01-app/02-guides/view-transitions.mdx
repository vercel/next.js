...
> [!NOTE]
> React's integration requires the second-generation View Transitions API (Chromium 125+, Safari 18.2+, Firefox 144+), though some animations may behave differently in Safari. Without browser support, your application works normally, the transitions simply do not animate.
...
The morph plays when the destination content renders in the same commit as the navigation — the case with prefetched (cached) pages. If the destination suspends into a fallback first, no pair forms, and the content animates with its enter animation instead when it arrives.
...
Apply the same props to both sides of the pair. `default="none"` keeps each named image from running its own cross-fade on every unrelated transition — without it, every named `<ViewTransition>` animates whenever any transition runs on the page. When you add `default="none"` to a named pair, keep the explicit `share`: with `default="none"` and no `share` prop, the pair silently stops morphing.
...
This restores interactivity for unnamed content. Named participants — like the anchored header — are skipped by hit-testing for the transition's duration regardless, so keep transitions short and avoid naming elements the user clicks rapidly.
