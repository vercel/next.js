...
On page load, a well-sized skeleton prevents layout shift. But without a transition, the swap from skeleton to content is still instant. If we refresh the page, the skeleton slides down and fades out, and a moment later the real content slides up and fades in. The slide animation makes the handoff visible.
...
- [React View Transitions skill](https://github.com/vercel-labs/agent-skills/tree/main/skills/react-view-transitions) — additional patterns, gotchas, and implementation guidance for AI-assisted development
