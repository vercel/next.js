...
Then give the agent a prompt like this:

```prompt
Add view transitions to this app using the vercel-react-view-transitions skill.
```

You don't have to use this exact prompt. Ask for whatever you want to achieve, from a single shared-element morph to a full set of route transitions, and the skill guides the agent through the patterns to get there.
