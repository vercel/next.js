...
App Router uses [React canary releases](https://react.dev/blog/2023/05/03/react-canaries) built-in, which include all the stable React changes, and newer features, such as `ViewTransitions`. You do not need to install `react@canary` yourself.
