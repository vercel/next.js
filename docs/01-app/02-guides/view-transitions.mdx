...
When a user clicks a photo thumbnail and a detail page loads, the thumbnail disappears and a larger version appears somewhere else on screen. Nothing connects the two visually, so the user has to scan the page to confirm they clicked the right thing. Building these transitions typically requires animation libraries that manage mount/unmount lifecycles, track element positions across routes, and coordinate timing manually.
...
> [!NOTE]
> Next.js bundles its own React canary build internally. The `import { ViewTransition } from 'react'` statement works because the Next.js bundler rewrites it to use this internal copy. You do not need to install `react@canary` yourself.
...
The gallery displays photos in a grid. Clicking a photo opens a detail page with a larger version of the same image. We want the thumbnail to visually transform into the hero image so the user sees continuity instead of a hard cut.
...
Wrap the Suspense fallback in a `ViewTransition` with an exit animation, and the content in a `ViewTransition` with an enter animation. The `default="none"` on the content prevents it from also animating during unrelated transitions, like the shared element morph from Step 1:
...
The `var(--duration-exit)` delay on the enter animation means the new content waits for the old content to finish leaving before it appears. If we navigate to a photo, the skeleton slides down and fades out, then the real content slides up and fades in.
...
The same pattern works for any navigation. For example, previous/next arrows on the detail page use the same types:

```tsx filename="app/photo/[id]/photo-nav.tsx"
<Link href={`/photo/${prevId}`} transitionTypes={['nav-back']}>← Previous</Link>
<Link href={`/photo/${nextId}`} transitionTypes={['nav-forward']}>Next →</Link>
```
...
Browser-initiated back navigations (the back button or swipe gestures) do not carry a transition type. Since the `enter` and `exit` objects use `default: "none"`, no directional slide plays for these navigations. The shared element morph from Step 1 still applies if both pages have matching `name` props, because shared element transitions use the `share` trigger, which is separate from `enter`/`exit`.
