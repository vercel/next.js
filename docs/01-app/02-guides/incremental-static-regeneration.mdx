...
- When running multiple instances, the default file-system cache is per-instance. Without a shared [cache handler](/docs/app/api-reference/config/next-config-js/cacheHandlers), each instance revalidates independently: on-demand revalidation only invalidates the instance that receives the call, and other instances learn about invalidations via [`refreshTags()`](/docs/app/api-reference/config/next-config-js/cacheHandlers#refreshtags) polling. For near-instant propagation, implement a shared tag service. See [How Revalidation Works](/docs/app/guides/how-revalidation-works) for the architecture.
...
- You can use the `x-nextjs-cache` response header to observe cache behavior. Values are `HIT` (served from cache), `STALE` (served from cache, revalidating in background), `MISS` (not in cache, rendered fresh), or `REVALIDATED` (regenerated via on-demand revalidation). This header is not set when minimal mode is enabled.
