...
- When running multiple instances, the default file-system cache is per-instance. Without a shared [cache handler](/docs/app/api-reference/config/next-config-js/cacheHandlers), each instance revalidates independently, and on-demand revalidation (via `revalidateTag()` or `revalidatePath()`) only directly invalidates the instance that receives the call.
- Other instances learn about tag invalidations via the [`refreshTags()`](/docs/app/api-reference/config/next-config-js/cacheHandlers#refreshtags) polling mechanism. For near-instant propagation across instances, implement a shared tag service. See [How Revalidation Works](/docs/app/guides/how-revalidation-works) for the architecture.
...
- You can use the `x-nextjs-cache` response header to observe cache behavior. Values are `HIT` (served from cache), `STALE` (served from cache, revalidating in background), `MISS` (not in cache, rendered fresh), or `REVALIDATED` (regenerated via on-demand revalidation). This header is set when using `next start`; adapter-based deployments in minimal mode do not emit it.
