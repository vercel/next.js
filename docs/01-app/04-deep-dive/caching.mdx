...
Some content, such as user-specific information (based on `cookies`), is _cacheable_ but not practical to prerender at build time or store in a persistent server cache.

In the previous static-or-dynamic model, this type of content would be treated as dynamic, re-rendering on every request. But, in Next.js, you can [mark it as cacheable](), allowing the content to be prefetched and cached on the client.

This means more of the route is ready before navigation, and only truly dynamic or frequently updated content is refetched and rendered on navigation.
...
> The difference between `revalidate` and `expire` is that `revalidate` immediately responds with a stale entry and revalidates in the background, whereas `expire` blocks the response until the new entry is generated. For this reason, the `revalidate` time should be shorter than `expire`.
...
Each route segment is cached independently. This means their cache lifetimes are evaluated in isolation. For instance, a `layout` can use the `days` profile, while its corresponding `page` uses the `hours` profile:
...
For components and functions nested deeper within the route segment, the shortest lifetime takes precedence. For example, if the `page` has an `hours` profile but a component inside it has a `minutes` profile, the cache duration for the `page`will be `minutes`:
...
[`cacheTag`](/docs/app/api-reference/functions/cacheTag) is a function that lets you mark one or more cache entries to be explicitly revalidated based on an event, such as a mutation or webhook. For example, you can tag a component to be revalidated:

```tsx filename="app/blog/post/[id]/page.tsx" switcher
...
```jsx filename="app/blog/post/[id]/page.js" switcher
import { unstable_cacheTag as cacheTag } from 'next/cache'
...
And trigger the revalidation using `revalidateTag` in a Server Action or Route Handler:
...
TODO: Explain how `revalidateTag` works.

### Nesting tags

Cache tags are composable. A parent will inherit the tags its children. For example, `page` will revalidate when tag `a` or `b` is updated:
