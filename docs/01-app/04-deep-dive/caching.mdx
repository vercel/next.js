...
- Next.js determines the most appropriate caching layer based on your declared semantics.
...
1. **Dynamic route segments** - which can potentially create a large number of unique pages. Trying to prerender or cache all of them on the server can be time-consuming and server-intensive.
...
In Next.js, rather than caching entire pages, you can cache:

- Layouts (e.g. navigation, static shell)
...
- [`"use cache"`](/docs/app/api-reference/directives/use-cachee): Defines a cache boundary for a component or function. Cache boundaries are tied to serialized arguments and function identity.
- [`cacheLife()`](/docs/app/api-reference/functions/cacheLife): Defines the cache lifetime for a component or function. You can specify `stale`, `revalidate`, and `expire` values, or use presets like `'minutes'`, `'hours'`, `'days'`.
- [`cacheTag()`](/docs/app/api-reference/functions/cacheTag): Defines a cache tag across multiple boundaries to be invalidated together. This allows coordination between nested boundaries or external triggers (e.g. a CMS update).
