...
- [`"use cache"`](#use-cache): Defines a cache **boundary** for an async component or function.
- [`cacheLife()`](#cacheLife): Defines the cache lifetime for a component or function. You can specify `stale`, `revalidate`, and `expire` values, or use profiles like `'minutes'`, `'hours'`, `'days'`.
- [`cacheTag()`](#cacheTag): Defines a cache tag across multiple boundaries to be invalidated together. This allows coordination between boundaries or external triggers.
