...
The Next.js `<Image />` component extends the HTML `<img>` element for automatic image optimization with best practices built-in.
...
By default, it's a Client Component that runs in the browser. Using the component signals to the [Next.js Image Loader](#the-image-loader) to optimize and cache the image on the server.
...
The optimized output is served as `webp` and `avif` images where supported.
