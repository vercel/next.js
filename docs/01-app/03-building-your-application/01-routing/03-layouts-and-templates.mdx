...
- **Location**: Must be placed **directly in the `app` directory** (e.g., `app/layout.js|tsx`).
- **No nesting**: Subfolders (e.g., `app/dashboard/layout.js|tsx`) **do not** count as root layouts.
- **Only one root layout**: Your app can only have **one root layout**.
- **HTML structure**: Must include `<html>` and `<body>` tags (unlike nested layouts).
...
├── page.js
└── dashboard/
    ├── layout.js      # ← Nested layout (extends root)
    └── page.js
```
...
### How Nested Layouts Interact with the Root Layout

- Nested layouts (e.g., `app/dashboard/layout.js|tsx`) **automatically inherit** the root layout.
- They **do not replace** the root layout unless you explicitly redefine `<html>` or `<body>` tags.
