...
> Note that the `(..)` convention is based on _route segments_, not the filesystem.
...
In the above example, the path to the `photo` segment can use the `(..)` matcher since `@modal` is a slot and **not** a segment. This means that the `photo` route is only one segment level higher, despite being two filesystem levels higher.
