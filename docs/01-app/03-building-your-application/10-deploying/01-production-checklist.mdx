...
- **[Static Rendering](/docs/app/getting-started/caching-and-revalidating#static-rendering):** Next.js statically renders Server and Client Components on the server at build time and caches the rendered result to improve your application's performance. You can opt into [Dynamic Rendering](/docs/app/getting-started/caching-and-revalidating#dynamic-rendering) for specific routes, where appropriate. {/* TODO: Update when PPR is stable */}
...
- **[Dynamic APIs](/docs/app/getting-started/caching-and-revalidating#dynamic-apis):** Be aware that Dynamic APIs like [`cookies`](/docs/app/api-reference/functions/cookies) and the [`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional) prop will opt the entire route into [Dynamic Rendering](/docs/app/getting-started/caching-and-revalidating#dynamic-rendering) (or your whole application if used in the [Root Layout](/docs/app/building-your-application/routing/layouts-and-templates#root-layout-required)). Ensure Dynamic API usage is intentional and wrap them in `<Suspense>` boundaries where appropriate.
