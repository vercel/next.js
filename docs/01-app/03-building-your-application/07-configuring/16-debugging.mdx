...
With the launch.json shown above, could occur an error like this, if jou start "Next.js: debug full stack"-Debugging:

```
Uncaught SyntaxError [workspaceFolder]\node_modules\.bin\next:2
...
To fix the problem, use the Windows version of Next by replacing the "program" line in the "Next.js: debug full stack" configuration with:
```json filename='launch.json'
