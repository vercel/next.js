...
> **Good to know**:
>
> - Middleware is built as its own bundle and runs in an isolated [runtime](#runtime). It does not share the same JavaScript global environment or module instances as your server or rendering code.
> - You cannot share in-memory objects, singletons, or other mutable state between Middleware and rendering. To pass information from Middleware to your application, use headers, cookies, rewrites, redirects, or the URL.
