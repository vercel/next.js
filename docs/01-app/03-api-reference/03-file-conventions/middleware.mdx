...
> **Good to know**:
>
> - Middleware is built as a separate entry.
>   - Edge runtime: runs in an isolated environment and does not share `globalThis` or module instances with your server code.
>   - Node.js runtime: in `next dev` and `next start`, the process may be shared (same `globalThis`). Middleware is compiled as a separate bundle, so module-level singletons aren't shared. Middleware may still be deployed separately (at the edge) by your hosting provider. Avoid relying on in-memory sharing.
