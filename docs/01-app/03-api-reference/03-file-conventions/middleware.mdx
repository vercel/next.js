...
> **Note**:  
> By default, Middleware files are named `middleware.js` or `middleware.ts`.  
> If you’ve customized [`pageExtensions`](/docs/app/api-reference/config/next-config-js/pageExtensions) (e.g. to `.page.ts` / `.page.js`),  
> you must name your file `middleware.page.ts` or `middleware.page.js`.  
