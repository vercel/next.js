...
This example reads a local image from the file system and renders it in an Open Graph image.

> **Path note:** Place your asset relative to the **project root** (e.g. `logo.png` next to `app/`). If you keep assets in `public/`, change the path to `join(process.cwd(), 'public', 'logo.png')`.

**A) Base64 (TypeScript-friendly, recommended)**
...
export default async function Image() {
  // If your file is in /public, use: join(process.cwd(), 'public', 'logo.png')
...
**B) ArrayBuffer / Typed Array (runtime-supported; TS suppression needed)**
...
export default async function Image() {
  // If your file is in /public, use: join(process.cwd(), 'public', 'logo.png')
  const logoData = await readFile(join(process.cwd(), 'logo.png'))
  // Satori accepts ArrayBuffer/typed arrays at runtime, but TS doesn't model this yet.
...
Why two versions? Base64 keeps TypeScript happy without suppressions. The ArrayBuffer/typed array form is supported by Satori at runtime but currently needs a small @ts-expect-error in TypeScript.

## Version History

| Version   | Changes                                                                                                    |
| --------- | ---------------------------------------------------------------------------------------------------------- |
| `v15.x`   | Added two approaches for Node.js runtime local assets: `Base64` (TS-friendly) and `ArrayBuffer` (runtime). |
