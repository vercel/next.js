...
### Semantic sitemap outputs (experimental)

With `experimental.agentRoutes` enabled, sitemap routes can opt into semantic
outputs at:

- `/sitemap.md`
- `/sitemap.json`
- `/sitemap.xml` with `Accept: text/markdown`
- `/sitemap.xml` with `Accept: application/json`

You can opt in by exporting `agent` from `app/sitemap.ts`:
...
export default function sitemap(): MetadataRoute.Sitemap {
  return [{ url: 'https://acme.com' }]
}
```

For custom semantic summaries, export `semanticSitemap`:
  ...
  return [
    {
      url: 'https://acme.com',
      title: 'Home',
      summary: 'Primary entry point for the site.',
    },
  ]
}
```

If `semanticSitemap` is exported without `agent`, Next.js defaults to markdown
for `/sitemap.md` and when `/sitemap.xml` is requested with
`Accept: text/markdown`.

> **Good to know**
>
> - Semantic outputs are additive. `/sitemap.xml` still returns XML by default.
> - `generateSitemaps` cannot be combined with `agent` or
>   `semanticSitemap`.
