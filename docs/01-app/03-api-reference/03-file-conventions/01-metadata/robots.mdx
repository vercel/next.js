...
[Content Signals](https://contentsignals.org/) declare how crawlers may use content **after** they access it: search indexing, AI input (RAG / grounding), and AI training. They do not replace `Allow` / `Disallow`.
...
Path-specific (per-link) signals, including the [discussion #85382](https://github.com/vercel/next.js/discussions/85382) shape:
...
A rule-level `contentSignal` **replaces** the file-level default for that user-agent group (keys are not merged). `contentSignal: {}` emits no `Content-Signal` line for that group. Set `contentSignalsPolicy: true` to prepend the CC0 policy comment from [contentsignals.org](https://contentsignals.org/).

> **Good to know**: Content Signals are usage preferences. Not every crawler honors them. Combine them with `Allow` / `Disallow` and, where needed, bot controls.
