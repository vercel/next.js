...
- **`'manual-warning'`**: Only segments with an explicit `unstable_instant` are validated, at warning level (dev only).

Setting `unstable_instant = false` on a segment opts it out of validation entirely.

> **Good to know**:
>
> - Two additional levels are available behind the experimental prefix: `'experimental-error'` (validates all navigations in development and at build time) and `'experimental-manual-error'` (the same, but only for segments with an explicit `unstable_instant`). These turn warnings into build failures and are subject to change. Use with caution.
...
| Version   | Changes                                                      |
| --------- | ------------------------------------------------------------ |
| `v16.3.0` | `unstable_instant` export introduced (Cache Components only) |
