...
The `prefetch` route segment config controls how a segment is prefetched during client-side navigation. The default (`'auto'`) lets the framework manage the strategy based on the app's [`partialPrefetching`](/docs/app/api-reference/config/next-config-js/partialPrefetching) setting. The other options let you override the strategy per segment. Use them with care, since they can affect both navigation latency and prefetch cost.
...
Lets the framework manage prefetching for this segment based on the app's [`partialPrefetching`](/docs/app/api-reference/config/next-config-js/partialPrefetching) setting. To override per route, use [`'partial'`](#partial), [`'allow-runtime'`](#allow-runtime), or [`'force-disabled'`](#force-disabled).
...
Opts this segment into Partial Prefetching. The prefetch contains the route's cached content; uncached parts and link-specific data (such as `params` and `searchParams`) stream in after navigation.

This is the per-segment equivalent of enabling [`partialPrefetching`](/docs/app/api-reference/config/next-config-js/partialPrefetching) at the app level. To include link data in the prefetch, use [`'allow-runtime'`](#allow-runtime), which is a superset of `'partial'`.

```tsx filename="page.tsx"
export const prefetch = 'partial'
```

> **Good to know**: When `<Link prefetch={true}>` navigates to a route that hasn't opted into Partial Prefetching, a dev console error suggests enabling it. Setting `prefetch = 'partial'` here is the per-route fix; `partialPrefetching: true` is the app-wide alternative.
...
> **Good to know**: `prefetch = 'force-static'` is distinct from [`dynamic = 'force-static'`](/docs/app/api-reference/file-conventions/route-segment-config/dynamic). The `dynamic` config controls how a segment renders at request time; the `prefetch` config controls how it's prefetched ahead of navigation. They're independent.
