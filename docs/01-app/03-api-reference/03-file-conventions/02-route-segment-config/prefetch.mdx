...
> **Good to know**: `prefetch = 'force-static'` is distinct from [`dynamic = 'force-static'`](/docs/app/guides/caching-without-cache-components#dynamic). The `dynamic` config controls how a segment renders at request time; the `prefetch` config controls how it's prefetched ahead of navigation. They're independent.
