...
> - A `page` or `layout` that also calls the [`prefetch()`](/docs/app/api-reference/functions/prefetch) function needs to import it under another name, for example `import { prefetch as prefetchStage } from 'next/cache'`.
