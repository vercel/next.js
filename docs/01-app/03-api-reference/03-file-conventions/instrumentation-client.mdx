...
A conditional `import()` here is fire-and-forget and may land after hydration, so for anything loaded on demand, prefer polyfilling in the code that uses the feature. See [Custom Polyfills](/docs/architecture/supported-browsers#custom-polyfills) for that and other strategies.
