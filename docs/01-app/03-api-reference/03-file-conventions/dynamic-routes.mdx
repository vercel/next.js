...
> - In layouts, avoid awaiting `params` at the top level. Doing so prevents the layout from being prerendered. Instead, pass the params promise down to the component that needs it and await there. See [Creating a meaningful loading experience](/docs/app/getting-started/caching#creating-a-meaningful-loading-experience) for examples.
