...
Dynamic segment values are typed as `string` or `string[]`. If your app needs narrower value types, use one of the following approaches.

#### Override `params`

```tsx filename="/app/[lng]/page.tsx"
type PageWithLocale = Omit<PageProps<'/[lng]'>, 'params'> & {
  params: Promise<{ lng: Locale }>
}

export default async function Page(props: PageWithLocale) {
...
export default async function Page(props: PageProps<'/[lng]'>) {
  const { lng } = await props.params // lng is typed as string
  assertValidLocale(lng)
  // lng is now typed as Locale
}
```
