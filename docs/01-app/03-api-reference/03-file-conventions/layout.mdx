...
A layout-level fallback shows on the first load but not on client navigations, because React re-enters the tree at the layout. See [Designing loading UI](/docs/app/guides/loading-ui#competing-loading-signals) for what that means when a route also has a `loading.js` or its own page-level fallbacks.
