...
### Agent route paths (experimental)

When `experimental.agentRoutes` is enabled, page agent responses are negotiated
on the page's normal URL. Explicit route handlers at paths like `/agent.md`
remain ordinary user-defined routes.

For example, this explicit route continues to own `/agent.md`:

```ts filename="app/agent.md/route.ts"
export function GET() {
  return new Response('custom agent output\n', {
    headers: {
