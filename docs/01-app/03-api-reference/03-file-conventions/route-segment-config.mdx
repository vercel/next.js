...
```tsx filename="layout.tsx | page.tsx" switcher
export const staleTime = 300 // 5 minutes in seconds
```
...
> **Good to know:**
>
> - Unlike `revalidate` which uses the minimum value across layouts, `staleTime` uses the nested-most value (page overrides layout).
...
> - Setting `staleTime = 0` causes the client to refetch on every navigation.
