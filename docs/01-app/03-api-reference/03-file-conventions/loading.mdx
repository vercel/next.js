...
A fallback that doesn't match the size of the content replacing it shifts the page when it resolves. See [Skeletons that don't match the final geometry](/docs/app/guides/loading-ui#skeletons-that-dont-match-the-final-geometry) for sizing one.
