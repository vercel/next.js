...
> - **With [Cache Components](/docs/app/getting-started/cache-components):** Uncached or runtime data access in the layout must be explicitly wrapped in `<Suspense>`, otherwise Next.js errors at build time. The static shell streams first, and the uncached content fills in.
>
