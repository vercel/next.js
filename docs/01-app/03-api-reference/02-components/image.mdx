...
| `v16.0.0`  | `qualities` default configuration changed to `[75]`, `minimumCacheTTL` default changed from `60` to `14400`, `imageSizes` default changed to `[32, 48, 64, 96, 128, 256, 384]` (removed `16`), `localPatterns.search` configuration required for local images with query strings, `preload` prop added, `priority` prop deprecated, `dangerouslyAllowLocalIP` config added, `maximumRedirects` config added.                                                                                 |
