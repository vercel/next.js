...
- Without `sizes`, on an image with a `width`: Next.js generates a limited `srcset` (e.g. 1x, 2x), suitable for fixed-size images.
- Without `sizes`, on a `fill` image: Next.js generates a `srcset` from [`deviceSizes`](#devicesizes) only, as if the image spans the full viewport.
...
`deviceSizes` allows you to specify a list of device width breakpoints. These widths are used when the `next/image` component uses the [`sizes`](#sizes) prop, or the [`fill`](#fill) prop without `sizes`, to ensure the correct image is served for the user's device.
...
`minimumCacheTTL` allows you to configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](/docs/app/getting-started/images#local-images) which will automatically hash the file contents and cache the image with a long-lived `immutable` `Cache-Control` header.
