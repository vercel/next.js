...
> **Good to know**:
>
> - These scripts are appended to the `head` and executed by the Next.js runtime before hydration, so where you place them inside the root layout does not affect when they run.
> - They run once per document load. A client-side navigation does not run them again, including one that only changes a root param, such as `/en` to `/fi`, since the root layout stays the same.
