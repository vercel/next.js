...
If a route has a dynamic parameter that is not known during prerendering, the active state may appear after hydration. See [Unknown dynamic parameters](#unknown-dynamic-parameters).
...
When Cache Components is enabled, the pathname is not known at build time for a route prerendered with a dynamic param not covered by [`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params). `NavLink` renders inactive in that prerender and applies the active class on the client after hydration. This is a first-paint flicker, not a missing boundary. To apply the class before hydration, set it from an inline script as shown in [Preventing flash before hydration](/docs/app/guides/preventing-flash-before-hydration).
