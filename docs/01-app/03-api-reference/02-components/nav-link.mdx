...
> **Good to know:** `<NavLink>` is available in the App Router only. On routes with unknown dynamic parameters, active styling may appear after hydration. See [Cache Components](#cache-components) for details and an inline-script workaround.
