...
`NavLink` updates its active state after hydration when a dynamic route's parameter value isn't known during prerendering. See [Unknown dynamic parameters](#unknown-dynamic-parameters).
...
With Cache Components, Next.js cannot determine the pathname while prerendering a route whose parameter value is not included in [`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params). `NavLink` renders its inactive state during prerendering and updates it after hydration. This may briefly show inactive styling or content. For CSS-based active styling, set `data-active` from an inline script before hydration. See [Preventing flash before hydration](/docs/app/guides/preventing-flash-before-hydration).
