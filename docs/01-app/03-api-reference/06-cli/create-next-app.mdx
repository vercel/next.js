...
| `--oxlint`                              | Initialize with Oxlint config                                         |
| `--oxfmt`                               | Initialize with Oxfmt formatter config                                |
...
Which linter would you like to use? ESLint / Biome / Oxlint / None
...
**Oxlint**: A fast, Rust-based linter built on the Oxc compiler stack. Provides comprehensive rules with type-aware linting for TypeScript projects and includes built-in Next.js plugin support. When Oxlint is selected, you'll be prompted to also enable **Oxfmt**, the companion formatter that's approximately 30x faster than Prettier.
