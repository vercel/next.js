...
Use `--ai-upgrade security`, `--ai-upgrade latest`, or `--ai-upgrade future` to choose a policy explicitly. Use `--no-ai-upgrade` to write `false` to the config. These options cannot be combined.
...
These options apply to built-in templates, including `--example default`. Downloaded examples keep their existing configuration and do not accept AI upgrade options.
