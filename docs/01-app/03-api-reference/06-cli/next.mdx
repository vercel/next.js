...
>
> We recommend migrating to invoking ESLint directly (or using Biome). A [codemod](/blog/next-15-5#next-lint-deprecation) is available to migrate scripts to the ESLint CLI.
>
> `next lint` generates a `next-env.d.ts` file, which is often needed during type-check. To ensure `next-env.d.ts` is present run [`next typegen`](#next-typegen-options) before type-checking. `next dev` and `next build` also generate the `next-env.d.ts` file, but in it is often undesirable to run these just to type-check.
>
> ```bash filename="Terminal"
