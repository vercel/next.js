...
#### Agentic upgrades (experimental)

`next update` and `next up` are aliases for `next upgrade`. Use `--agentic`
(alias for `--experimental-agentic`) to hand migration work to a coding agent:

```bash filename="Terminal"
next upgrade --agentic=security
next upgrade --agentic=16.0.0
next upgrade apps/web --agentic=16.0.0
next upgrade apps/web --agentic
next upgrade --experimental-agentic-dry-run=16.0.0
```

The positional directory selects the app in both modes, defaulting to the current
directory. Pass `--agentic=<target>` to select `security`, an exact version or an
npm tag. Bare `--agentic` reads `experimental.agenticAutoUpgrade` from the app's
configuration. Without a configured policy, provide an explicit target to proceed.
`--experimental-agentic-dry-run` accepts the same optional target.
Use only one agentic option; `--revision` is reserved for traditional upgrades.

Explicit agentic targets do not require `experimental.agenticAutoUpgrade`.
Security mode chooses a safe stable release from advisory evidence.
Security upgrades and reminders currently assess stable installed versions only.
For a prerelease install, request an explicit version instead; it does not carry
a security assessment. Prerelease security support is deferred.

An exact version is an explicit upgrade request and does not require an applicable vulnerability.
Version and tag targets exit if the resolved version is installed and reject
downgrades. All targets check Node.js compatibility before handoff.
