...
| [`analyze`](#next-analyze-options)     | Analyzes bundle output using Turbopack. Does not produce build artifacts.                                     |
...
| `--webpack`                              | Use the bundled Webpack instead of the default [Turbopack](/docs/app/api-reference/turbopack) bundler for development.                               |
| `--custom-webpack`                       | Use the project-installed `webpack` peer dependency (`^5.98.0`) for development.                                                                     |
...
| `--turbopack`                      | Force enable [Turbopack](/docs/app/api-reference/turbopack) (enabled by default). Also available as `--turbo`.                                |
| `--webpack`                        | Build using the bundled Webpack.                                                                                                              |
| `--custom-webpack`                 | Build using the project-installed `webpack` peer dependency (`^5.98.0`).                                                                      |
| `--analyze`                        | Generate bundle analysis data for the interactive analyzer during a Turbopack build.                                                          |
...
### `next analyze` options

`next analyze` analyzes your application's bundle output using [Turbopack](/docs/app/api-reference/turbopack). This command helps you understand the size and composition of your bundles, including JavaScript, CSS, and other assets. This command doesn't produce an application build.
...
The following options are available for the `next analyze` command:

| Option                    | Description                                                                                                                                   |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `-h, --help`              | Show all available options.                                                                                                                   |
| `[directory]`             | A directory on which to analyze the application. If not provided, the current directory will be used.                                         |
| `--experimental-app-only` | Analyzes only App Router routes.                                                                                                              |
| `--no-mangling`           | Disables [mangling](https://en.wikipedia.org/wiki/Name_mangling). This may affect performance and should only be used for debugging purposes. |
| `--profile`               | Enables production [profiling for React](https://react.dev/reference/react/Profiler). This may affect performance.                            |
| `--snapshot-name <name>`  | Names the snapshot created by this run. The name appears in the comparison UI instead of the Git branch and commit.                           |
...
You can combine `--debug-build-paths` with `--debug-prerender` for more detailed prerender diagnostics.
