...
| `--ai, --experimental-ai [type]` | Prepare a coding agent upgrade with `security`, `latest`, or `future`. Cannot be combined with `--revision`.                                       |

Without a type, `--ai` uses `experimental.agentUpgrade` if configured, or `security` otherwise. See [Upgrade with AI](/docs/app/guides/upgrading/agentic-upgrade) for the upgrade workflow and policy behavior.
