...
directory. `--ai security` explicitly selects security, while bare `--ai`
defaults to security. `--experimental-agentic-dry-run` accepts the same optional
upgrade type. Use only one AI option; `--revision` is reserved for traditional
upgrades.

Security mode chooses the closest supported major whose latest stable release
clears known advisories. Security upgrades currently assess stable installed
versions only. Prerelease security support is deferred. The target's Node.js
compatibility is checked before handoff.
