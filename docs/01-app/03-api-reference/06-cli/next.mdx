...
configuration, falling back to the CLI's release-channel default when no policy
is configured. `--experimental-agentic-dry-run` accepts the same optional target.
...
Security mode chooses a safe stable release from advisory evidence. An exact
version is an explicit upgrade request and does not require an applicable vulnerability.
