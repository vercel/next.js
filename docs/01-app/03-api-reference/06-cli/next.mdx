...
#### AI upgrades (experimental)

`next update` and `next up` are aliases for `next upgrade`. Use `--ai` to hand
security, latest-stable, or Future Defaults migration work to a coding agent:

```bash filename="Terminal"
next upgrade --ai
next upgrade --ai security
next upgrade --ai latest
next upgrade --ai future
next upgrade apps/web --ai
next upgrade apps/web --ai security
next upgrade apps/web --ai latest
next upgrade apps/web --ai future
next upgrade --experimental-agentic-dry-run=security
```

The positional directory selects the app in both modes, defaulting to the current
directory. `--ai security`, `--ai latest`, and `--ai future` explicitly select an
upgrade type.
Bare `--ai` reads
`experimental.agenticAutoUpgrade` from the app's configuration and defaults to
security when the policy is absent or `false`. The `false` value disables automatic
nudges, not an explicitly requested upgrade. `--experimental-agentic-dry-run`
accepts the same optional upgrade type. Use only one AI option; `--revision` is
reserved for traditional upgrades.

Security mode chooses the closest supported major whose latest stable release
clears known advisories. Latest mode resolves npm's stable `latest` tag once,
keeps that exact release throughout the migration, and includes the security
assessment for stable installed versions. If the installed version is equal to
or newer than the latest stable release, it exits without changes.

Future mode includes the security and latest checks, then adopts Future Defaults
available in the selected stable release. It never downgrades an app already on
a newer stable release. Next.js prepares temporary, version-matched instructions
for each available default and falls back to its bundled migration guide when
additional instructions cannot be prepared.

After verification, the agent sets `experimental.agenticAutoUpgrade` to the
selected upgrade type so future nudges are enabled.
Security upgrades and reminders currently assess stable installed versions only.
Prerelease security support is deferred. The target's Node.js compatibility is
checked before handoff.

With `experimental.agenticAutoUpgrade: 'security'`, `next dev` and `next build`
check the running Next.js version for known vulnerabilities when a coding agent
is detected. The advisory lookup runs in the background without delaying startup.
An affected version logs a warning with advisory counts by severity, the source endpoint, and the
`next upgrade --ai` command. Development and builds continue.
The agent decides how to proceed under its existing permissions. No upgrade is
launched automatically. If advisory lookup fails, the command warns and continues.
Ordinary terminals and `next start` are unaffected.

Set `experimental.agenticAutoUpgrade: 'latest'` for a gentle reminder when a
newer stable major or minor release is available. Patch-only updates do not
trigger reminders; explicit `--ai latest` upgrades still include them. The
latest policy checks security first and shows only the security warning when the
installed version is affected.
In agent-driven `next dev` and `next build`,
Next.js prints the running and latest versions with an upgrade command. The
check runs without delaying startup, stays silent on lookup failure, and does
not launch an upgrade.

Set `experimental.agenticAutoUpgrade: 'future'` to compose all three checks.
Security warnings take priority, followed by newer stable release reminders,
then available Future Defaults. The reminder lists defaults that are available
for the installed stable version but are not enabled in the app. Run the shown
bare `next upgrade --ai` command to use the configured Future policy.

The command delegates to the published Next.js canary CLI for current migration
instructions. It continues in an existing coding agent or offers an installed
Codex/Claude session in an interactive terminal. Dry-run still edits, verifies
and creates local commits, but stops before pushing or creating a PR.
