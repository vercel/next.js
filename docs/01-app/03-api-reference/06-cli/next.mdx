...
> **Good to know**: If your routes are inside [route groups](/docs/app/building-your-application/routing/route-groups), include the group folder in the path. For example, a page at `app/(marketing)/about/page.tsx` must be referenced with the full path including the route group:
>
> ```bash filename="Terminal"
