...
`next build` creates an optimized production build of your application. The output displays information about each route. See the [Building guide](/docs/app/guides/building) for how to read the output, pre-render dynamic routes, and debug build errors. For example:
