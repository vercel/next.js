...
security migration work to a coding agent:
...
directory. `--ai security` explicitly selects security. Bare `--ai` reads
...
clears known advisories. After verification, the agent sets
`experimental.agenticAutoUpgrade: 'security'` so future nudges are enabled.
...
`next upgrade --agentic` command. Development and builds continue.
