...
```typescript
{
  ...
  route: string           // Source route matcher aligned with the filesystem route, keeping dynamic segments (e.g. /blog/[slug] for the prerendered path /blog/first). On a dynamic route, the prerender whose pathname equals route is the fallback template entry; others are concrete prerendered paths
  resuming?: boolean      // Whether serving this prerender involves per-request compute (the PPR prerender postponed and resumes on the server); false: prerendered fully static at build time, undefined: does not apply (pages router, route handlers, blocking templates)
  htmlSize?: number       // Byte size of the prerendered HTML shell for app pages; 0 means an empty shell (everything postponed). Only set on the HTML prerender output; RSC/data/segment, pages router, and route handler outputs leave it undefined. htmlSize > 0 && resuming === false means the prerender was complete (fully static) at build time
  pprChain?: {
