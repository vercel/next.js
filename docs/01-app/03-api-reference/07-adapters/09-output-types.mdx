...
```typescript
{
  ...
  hasPostponed?: boolean  // Whether the build-time prerender of a PPR app page postponed (suspended on dynamic data); false: prerendered without postponing, undefined: does not apply (pages router, route handlers, blocking templates)
  hasFallback?: boolean   // Whether a dynamic route template has a static fallback shell generated during build; false: blocking or omitted template, undefined: does not apply (concrete prerenders)
  ...
  isDynamicRoute?: boolean  // true: prerender originates from a dynamic route template (dynamicRoutes in the prerender manifest), false: concrete prerendered path (routes)
  pprChain?: {
