...
> - `useSelectedLayoutSegments` reflects the file-system route **not** the pathname, therefore in a [Catch-all](/docs/app/api-reference/file-conventions/dynamic-routes#catch-all-segments) route it will return an array with all segments in a single string instead of each segment separately.
