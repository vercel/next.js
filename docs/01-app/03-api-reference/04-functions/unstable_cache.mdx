...
> We recommend opting into [Cache Components](/docs/app/getting-started/caching) and replacing `unstable_cache` with the `use cache` directive. See the [migration guide](/docs/app/guides/migrating-to-cache-components#unstable_cache) for step-by-step instructions, including when to prefer [`use cache: remote`](/docs/app/api-reference/directives/use-cache-remote).
