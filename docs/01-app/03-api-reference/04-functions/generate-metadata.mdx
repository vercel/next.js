...
> - `generateMetadata` is now non-blocking and streamable, so it can serve the initial UI faster, and when it resolves, it will be streamed in, unlike in prior versions where it was blocking and not serving any page until it resolves.
