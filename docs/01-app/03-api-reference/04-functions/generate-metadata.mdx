...
For **HTML-limited bots** that can’t execute JavaScript (e.g. `Twitterbot`, `Slackbot`, `Bingbot`), metadata continues to block page rendering. The resulting metadata will be available in the `<head>` tag.
