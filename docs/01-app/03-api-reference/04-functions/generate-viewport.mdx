...
Alternatively, opt the segment out of instant-navigation validation with [`unstable_instant = false`](/docs/app/api-reference/file-conventions/route-segment-config/instant). The route renders on every request and the navigation blocks until the render completes:
...
Setting `unstable_instant = false` on a layout exempts every route in the subtree from instant-navigation validation, not only the layout itself. Audit child routes before opting a shared layout out.

See [Next.js encountered runtime data in `generateViewport()`](/docs/messages/blocking-prerender-viewport-runtime) for the full set of fix options and trade-offs.
