...
If viewport genuinely requires runtime data, opt the segment out of instant-navigation validation with [`unstable_instant = false`](/docs/app/api-reference/file-conventions/route-segment-config/instant). The route renders on every request. There is no static shell.

```tsx filename="app/layout.tsx" highlight={1,3}
...
Alternatively, wrap the document `<body>` in a Suspense boundary to signal that the entire route should be dynamic. This approach works on all versions of Next.js with Cache Components, but there is no static shell or content to send immediately when a request arrives:
...
See [Next.js encountered runtime data in `generateViewport()`](/docs/messages/blocking-prerender-viewport-runtime) for full fix options and trade-offs.

Setting `unstable_instant = false` on a layout exempts every route in the subtree from instant-navigation validation. Caching is preferred because it allows static shell generation.
