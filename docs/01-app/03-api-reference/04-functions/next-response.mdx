...
This forwards `newHeaders` upstream and does not expose them to the client. While this pattern is useful to pass data upstream, it should be used with caution because the headers containing this data can be further shared with external services.

In contrast, the `NextResponse.next({ headers })` pattern makes these headers available to the client. This is **NOT** good practice and should be avoided. Additionally, setting response headers like `content-type` here can override framework expectations (for example, the `Content-Type` used by Server Actions), leading to failed submissions or broken streaming responses.
...
In general, avoid copying all incoming request headers, as doing so can leak sensitive data back to the client, or upstream to external services.

Prefer a defensive approach, by creating a subset of incoming request headers upstream using an allow-list. For example, you might discard custom `x-*` headers and only forward known-safe headers:
