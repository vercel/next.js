...
> - `useSelectedLayoutSegment` reflects the file-system route **not** the pathname, therefore in a [Catch-all](/docs/app/api-reference/file-conventions/dynamic-routes#catch-all-segments) route it will return all segments as a single string instead of only the segment one level down.
