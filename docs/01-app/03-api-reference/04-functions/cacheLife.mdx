...
- **`max`** - Rarely changes (legal pages, documentation)

Import `cacheLife` and pass a profile name:

```tsx filename="app/blog/page.tsx" highlight={1,5} switcher
'use cache'
...
```jsx filename="app/blog/page.js" highlight={1,5} switcher
'use cache'
import { cacheLife } from 'next/cache'
...
> **Good to know**: The `use cache` directive must be placed at the top of the function or component, and `cacheLife` must be called within its scope.
...
- **`expire`**: Maximum time before cached content must be regenerated

All values are in seconds. Let's look at each property in detail:

#### `stale`

How long the client can use cached data without checking the server.

- During this time, the client serves cached content immediately without a network request
- After this period expires, the client checks with the server for updates
- Setting to `0` means the client always checks with the server
...
Maximum time before cached content must be regenerated.
...
- When undefined or set to a very large value, content doesn't expire

```tsx
cacheLife({ expire: 3600 }) // 1 hour
```

> **Relationship to Cache-Control**: These properties roughly map to: `Cache-Control: max-age=[stale], s-maxage=[revalidate], stale-while-revalidate=[expire-revalidate]`

### Preset cache profiles

Now that you understand the three properties, here are the preset profiles Next.js provides:
...
| `max`       | Stable content that rarely changes     | 5 minutes  | 30 days      | Never    |
...
> **Good to know**: Profile names are semantic labels. They don't enforce specific timeframes, but help you organize and understand your cache strategy.
...
export async function GET() {
  // Custom timing for limited-time offer
  cacheLife({
    stale: 60, // 1 minute
  ...
  const offer = await fetchLimitedOffer()
