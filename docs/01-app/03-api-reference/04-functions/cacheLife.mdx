...
> - If used, `cacheLife` should be called directly in the function where the `use cache` directive appears
...
When you nest components with different `use cache` scopes, the parent cache behavior depends on whether it has an explicit `cacheLife`:
...
export default async function Dashboard() {
  'use cache'
  cacheLife('hours') // Explicit parent cache lifetime
...
```tsx filename="app/dashboard/widget.tsx"
import { cacheLife } from 'next/cache'

export async function Widget() {
  'use cache'
  cacheLife('minutes') // Child cache lifetime
...
**With explicit parent `cacheLife`**: The `Dashboard` cache uses its own lifetime (hours), regardless of the child's lifetime. When the Dashboard cache hits, it returns the complete cached output including the widget's data. The parent's explicit `cacheLife` always takes precedence - it can be longer or shorter than the children's lifetimes.

**Without explicit parent `cacheLife`**: If the `Dashboard` has `use cache` but no `cacheLife` call, it uses the `default` profile, and nested children can only reduce this default value to their shorter lifetimes. The parent cannot be extended beyond the default by children with longer lifetimes.

> **Good to know**: It is recommended to specify an explicit `cacheLife` for each `use cache` directive. With explicit values, you can inspect a component and immediately know its cache behavior regardless of its children. Without explicit values, the behavior becomes dependent on nested cache lifetimes, making it harder to reason about.
...
This pattern is useful when different outcomes need different cache durations, such as errors vs. success, or missing vs. available content.

#### Using dynamic cache lifetimes from data

For dynamic cache behavior based on external data (like CMS settings), use inline objects instead of profile names to avoid validation complexity:
...
> **Good to know**: Invalid profile names passed to `cacheLife` will throw runtime errors. When using external data, inline objects with fallback values are recommended.
