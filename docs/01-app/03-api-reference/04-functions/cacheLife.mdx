...
> **Good to know**:
>
> - `cacheLife` must be called inside a function body, not at module scope. Calling it at the top level of a file (outside any function) will throw an error, even when the file has `'use cache'` at the file level. Always place `cacheLife` inside the function whose output is being cached.
