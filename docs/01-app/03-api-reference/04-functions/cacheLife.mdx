...
A `stale` under 30 seconds is used as written, but the entry is [excluded from prerenders](#prerendering-behavior) because a prefetch would expire before the user could click. The value you set is never rewritten.
