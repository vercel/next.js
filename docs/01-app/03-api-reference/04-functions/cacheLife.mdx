...
export default async function Page() {
  'use cache'
  // Explicit cacheLife prevents the error and keeps this prerendered
  cacheLife('default') // or any profile with sufficient revalidate/expire
  return <ShortLivedWidget />
}
```

**If you want the outer cache to be dynamic**, explicitly set a short cache lifetime to confirm this is intentional. Wrap the component in a `<Suspense>` boundary to provide a fallback while the dynamic content loads:

```tsx filename="app/page.tsx" highlight={6,15}
