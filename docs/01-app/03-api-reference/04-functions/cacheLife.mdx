...
> **Note:** If a nested `use cache` has a zero revalidate time (e.g., `cacheLife({ revalidate: 0 })`) and the outer `use cache` does not have an explicit `cacheLife`, Next.js will throw an error during prerendering. This prevents accidental misconfigurations where the outer cache would silently become dynamic. To fix this, add an explicit `cacheLife()` call to the outer cache to choose whether it should be prerendered (with non-zero revalidate) or remain dynamic (with zero revalidate).
