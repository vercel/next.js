...
`cacheLife` itself must be called inside an async function or component, it cannot be used at module scope. Calling it at the top level of a file will throw an error.

> **Good to know**:
>
> - Call `cacheLife` inside the async function or component whose output is cached.
