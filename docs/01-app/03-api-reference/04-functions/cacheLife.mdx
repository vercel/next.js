...
> **Good to know**:
>
> - `cacheLife` must not run at module scope. Calling it at the top level of a file (outside any function) throws an error, even when the file has `'use cache'` at file level.
