...
- If `expire` is omitted, the cache will never expire. This is equivalent to the behavior of the `default` preset profile.
...
The example above caches for 14 days, checks for updates daily, and expires the cache after 14 days. If you omit `expire`, the cache will never expire, similar to the `default` profile. You can then reference this profile throughout your application by its name:
