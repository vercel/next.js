...
**With explicit parent `cacheLife`**: The `Dashboard` cache uses its own lifetime (hours). When the Dashboard cache hits, it returns the complete cached output including the widget's data. The parent cache extends the child's output lifetime, the widget won't refresh on its own schedule while served from the parent cache.

**Without explicit parent `cacheLife`**: If you **remove** `cacheLife('hours')` from the `Dashboard`, the parent cache adopts the shortest child lifetime. In this case, the `Dashboard` would revalidate every few minutes to match the `widget` timing.

> **Good to know**: It is recommended to specify an explicit `cacheLife` for each `use cache` directive. Relying on implicit lifetimes (where a parent adopts the shortest child lifetime) makes cache behavior harder to understand and maintain, especially as your component tree grows.

### Conditional cache lifetimes

You can call `cacheLife` conditionally to set different cache durations based on your data or application logic:
...
async function getPostContent(slug: string) {
  ...
  // Use cache lifetime configured by the content author
  cacheLife(post.cacheProfile)
...
- Different outcomes need different cache durations (errors vs. success)
- Cache behavior depends on data properties (author preferences, content type)
- Content might not be available yet, requiring a shorter cache duration to check back sooner while still reducing database load
