...
> **Good to know**:
>
> - The "stale" property does not set the `Cache-control: max-age` header. It instead controls the client-side router cache. The server sends this value to the client via the `x-nextjs-stale-time` response header (in seconds).
> - **The client enforces a minimum stale time of 30 seconds**. This ensures that prefetched data remains usable long enough for users to actually click on links after they've been prefetched. Without this minimum, very short stale times would cause prefetched data to expire before it could be used, making prefetching ineffective.
