...
To capture the value in the [static shell](/docs/app/glossary#static-shell), wrap it in [`"use cache"`](/docs/app/api-reference/directives/use-cache). To keep it out of the static shell, use `await io()`, which suspends during the build-time prerender.

The `io` function returns a promise that suspends during the build-time prerender. Everywhere else, it resolves immediately and does nothing: at request time, inside a cache, in the browser, and in apps without Cache Components (including the Pages Router).

## Usage

In a Server Component, call `await io()` before reading a per-request value like `new Date()`, `Math.random()`, `crypto.randomUUID()`, or a synchronous database driver such as [`node:sqlite`](https://nodejs.org/api/sqlite.html):
...
The example wraps `CurrentTime` in a `<Suspense>` boundary, so `io()` suspends and the fallback ships in the static shell. If `CurrentTime` were rendered inside a [`"use cache"`](/docs/app/api-reference/directives/use-cache) scope instead, `io()` would be a no-op, so the value is captured when the cache fills and no `<Suspense>` boundary is required.
