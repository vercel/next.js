...
However, you cannot use these request APIs inside `after` in [Server Components](/docs/app/building-your-application/rendering/server-components). This is because `after` runs after React's rendering lifecycle. Accessing the request APIs would interfer with [Partial Prerendering](/docs/app/building-your-application/rendering/partial-prerendering) and [Streaming](/docs/app/getting-started/data-fetching-and-streaming#streaming).
