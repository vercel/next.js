...
It's useful when a component doesn't use [Runtime APIs](/docs/app/glossary#runtime-apis), but you want it to be rendered at runtime and not prerendered at build time. This usually occurs when you access external information that you intentionally want to change the result of a render, such as `Math.random()` or `new Date()`.
...
- The function is only necessary when dynamic rendering is required and common Runtime APIs are not used.
