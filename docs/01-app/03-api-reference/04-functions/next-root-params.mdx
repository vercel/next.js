...
> - `next/root-params` is only available in Server Components. It cannot be used in Client Components, Server Actions, or Route Handlers.
...
Root parameters are the [dynamic segments](/docs/app/glossary#dynamic-segment) that appear in the path up to the root layout file. Other dynamic segments deeper in the route tree are accessed through the [`params` prop](/docs/app/api-reference/file-conventions/page#params-optional) as usual.
...
Support for Route Handlers is planned for a future version of Next.js. In the meantime, use the [`params`](/docs/app/api-reference/file-conventions/route#context) from the route context:

```tsx filename="app/[lang]/api/route.ts"
import { NextRequest } from 'next/server'

export async function GET(req: NextRequest, ctx: RouteContext<'/[lang]/api'>) {
