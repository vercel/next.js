...
> - `next/root-params` can be used in Server Components and [Route Handlers](/docs/app/api-reference/file-conventions/route). It cannot be used in Client Components or Server Actions.
...
> - With Cache Components, [`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params) must return at least one value for each root parameter. This is required for prerendering the fallback shell.

## Root parameters vs dynamic parameters

Root parameters are the [dynamic segments](/docs/app/glossary#dynamic-route-segments) that appear in the path up to the root layout file. Other dynamic segments deeper in the route tree are accessed through the [`params` prop](/docs/app/api-reference/file-conventions/page#params-optional) as usual.
...
Only `lang` is a root parameter. `slug` is a regular dynamic parameter:
...
> **Good to know:** To prevent blocking routes during prerendering, [`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params) must return at least one value for each root parameter. See [Why root parameters are special](#why-root-parameters-are-special) for more on how root parameters differ from other dynamic APIs.
...
Root parameter getters can be used in [Route Handlers](/docs/app/api-reference/file-conventions/route):

```tsx filename="app/[lang]/api/route.ts" highlight={1,5} switcher
import { lang } from 'next/root-params'
...
```jsx filename="app/[lang]/api/route.js" highlight={1,5} switcher
import { lang } from 'next/root-params'
...
Root parameter getters can only be used in Server Components and [Route Handlers](/docs/app/api-reference/file-conventions/route):
...
The [root layout](/docs/app/api-reference/file-conventions/layout#root-layout) is the top-level rendering boundary. Dynamic segments before it are shared by all routes under that root layout, which is what makes them safe to access from any Server Component in that tree. Parameters deeper in the route vary depending on which child page is being rendered, so they are only available through the [`params` prop](/docs/app/api-reference/file-conventions/page#params-optional) in the page or layout that defines them.
...
app/
  [lang]/              ← root parameter (shared by all routes below)
    ...
    blog/
      [slug]/          ← regular dynamic segment
        page.tsx
    store/
      [...slug]/       ← catch-all segment with the same name
        page.tsx
```

In this example, `lang` is the only root parameter. The `page.tsx` directly under the root layout has no `slug` at all. The blog and store branches both define `slug`, but with different segment types (`[slug]` vs `[...slug]`). Each route only knows about its own parameters. The same name can have different types, different meanings, or not exist at all depending on the route. This is why parameters below the root layout are accessed through the `params` prop rather than as global getters.

The exception is [multiple root layouts](#multiple-root-layouts). When different root layouts define different parameters, the getter functions include `undefined` in their return type to account for routes where a given parameter may not exist.

## Version History

| Version   | Changes                                                |
| --------- | ------------------------------------------------------ |
| `v16.2.0` | `next/root-params` introduced as experimental feature. |
