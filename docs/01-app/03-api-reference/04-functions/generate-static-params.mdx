...
When using [Cache Components](/docs/app/getting-started/caching) with dynamic routes, `generateStaticParams` controls which params are fully prerendered at build time.
...
When using [`output: 'export'`](/docs/app/guides/static-exports), `generateStaticParams` becomes optional for dynamic routes when `cacheComponents: true` is enabled. If you omit it, Next.js emits a fallback shell for the route and resolves param-dependent content on the client. If you provide it, the returned params are prerendered and all other params use the fallback shell.
