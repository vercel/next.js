...
The `unauthorized` function allows you to render the [`unauthorized.js` file](/docs/app/api-reference/file-conventions/unauthorized) to handle authentication errors. To start using `unauthorized`, enable the experimental [`authInterrupts`](/docs/app/api-reference/config/next-config-js/authInterrupts) configuration option in your `next.config.js` file:
...
export default async function DashboardPage() {
  ...
  return (
    <main>
      ...
      <p>Hi, {session.user.name}! You have successfully logged in.</p>
    </main>
  )
}
```
...
export async function updateProfile(data: FormData) {
  ...
  // Proceed with the update for authenticated users
  const { userId } = session
  const newName = data.get('name')
  await db.users.update({ where: { id: userId }, data: { name: newName } })
