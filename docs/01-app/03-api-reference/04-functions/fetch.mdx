...
- **`auto no cache`** (default): Next.js fetches the resource from the remote server once and prerenders the page with the fetched data. If [Dynamic APIs](/docs/app/building-your-application/rendering/server-components#dynamic-rendering) are detected on the route, the data will be fetched on every request.
