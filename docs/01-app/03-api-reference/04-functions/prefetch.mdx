...
`unstable_prefetch()` only keeps the code that follows out of the App Shell. It can resolve during a per-link prefetch, without making that work request-dependent. This means a function called after `await unstable_prefetch()` can use [`"use cache"`](/docs/app/api-reference/directives/use-cache) and keep its cache lifetime while avoiding the cost of generating its result for every link into the route.
