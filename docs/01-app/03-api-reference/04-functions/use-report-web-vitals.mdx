...
description: Measure and report Core Web Vitals (LCP, CLS, INP, FCP, TTFB) to any analytics provider with the `useReportWebVitals` hook.
---
...
Next.js reports these custom metrics to the `reportWebVitals` function exported from
`pages/_app`, and reports the [Web Vitals](#web-vitals) listed above to `useReportWebVitals`.
Each entry point reports its own set, so export `reportWebVitals` alongside the hook to
receive both:
