...
- `name`: The name of the performance metric. Possible values include names of [Web Vitals](#web-vitals) metrics (TTFB, FCP, LCP, CLS, INP) specific to a web application.
...
- `navigationType`: Indicates the navigation type that triggered metric collection. Values are derived from [PerformanceNavigationTiming.type](https://developer.mozilla.org/docs/Web/API/PerformanceNavigationTiming/type) and may include `"navigate"`, `"reload"`, `"prerender"`, `"back-forward"` (normalized from `"back_forward"`), `"back-forward-cache"` (BFCache restore), `"restore"` (page restored after discard), and `"soft-navigation"` (see [Soft navigations](#soft-navigations)).
- `navigationURL`: The URL the metric was measured for. Metrics are often reported after the fact, so use this instead of the current URL when attributing a metric to a page.
...
The Core Web Vitals (LCP, CLS, and INP) are also measured for soft navigations, i.e. client-side
navigations that update the URL without a full page load. This requires browser support for the
[Soft Navigations API](https://developer.chrome.com/docs/web-platform/soft-navigations) (Chrome 151
and newer). In browsers without support, the Core Web Vitals are measured for the initial page load
only.

Metrics that were measured for a soft navigation have a `navigationType` of `"soft-navigation"`.
Since a metric can be reported after another navigation already happened, use `navigationURL` to
attribute a metric to the page it was measured for:

```js
const logWebVitals = (metric) => {
  console.log(metric.navigationURL, metric.name, metric.value)
}
```

> **Good to know**: Enabling soft navigation measurement also changes how the initial page load is
> measured, because its metrics are finalized as soon as the first soft navigation happens, instead
> of when the page is unloaded.
