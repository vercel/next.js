...
The [`connection()`](/docs/app/api-reference/functions/connection) function waits for a real request. It excludes the code that follows from prerendering and prefetching, and makes that work request-dependent.

`unstable_navigation()` only keeps the code that follows out of route prefetches. It resolves when the user navigates without making that work request-dependent. This means a function called after `await unstable_navigation()` can use [`"use cache"`](/docs/app/api-reference/directives/use-cache) and keep its cache lifetime while avoiding the cost of generating its result during prefetching.
