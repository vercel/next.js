...
With Partial Prefetching enabled, Next.js can prepare different amounts of a route before navigation:

1. By default, `<Link>` prefetches the [App Shell](/docs/app/glossary#app-shell), which holds the part of the route that does not depend on the URL.
2. [`<Link prefetch={true}>`](/docs/app/api-reference/components/link#prefetch) can also prefetch content that depends on the link's URL.
3. When the user navigates, Next.js renders anything that remains.

The control mechanism that determines how much of this work a prefetch includes is called [Cache Stages](/docs/app/glossary#cache-stages).
...
- A call outside a `<Suspense>` boundary surfaces a [validation insight](/docs/messages/instant-cache-stage) in development. It is reported even on a page that is currently prerendered in full, because the same page starts deferring content as soon as it switches to a runtime shell.
