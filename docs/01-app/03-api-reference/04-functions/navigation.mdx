...
With Partial Prefetching enabled, Next.js can prepare progressively more of a route before and during navigation:

1. A default `<Link>` prefetch includes the [App Shell](/docs/app/glossary#app-shell), which holds the part of the route that does not depend on the URL.
2. A [`<Link prefetch={true}>`](/docs/app/api-reference/components/link#prefetch) prefetch can also include content that depends on the link's URL.
3. A navigation renders any content that remains.

This control mechanism is called [navigation stages](/docs/app/glossary#navigation-stages).
...
## How `unstable_navigation()` differs from `connection()`
...
`unstable_navigation()` only excludes the code that follows from route prefetches. It resolves when the user navigates without making that work request-dependent. A function called after `await unstable_navigation()` can therefore use [`"use cache"`](/docs/app/api-reference/directives/use-cache) and keep its cache lifetime without generating its result during prefetching.

Use `connection()` when the work must run for a real request. Use `unstable_navigation()` when the work is reusable but should wait until the user navigates.
...
- A call outside a `<Suspense>` boundary surfaces a [validation insight](/docs/messages/instant-navigation-stage) in development. It is reported even on a page that is currently prerendered in full, because the same page starts deferring content as soon as it switches to a runtime shell.
