...
### 
oPropertyAccessFromIndexSignature`r

Setting 
oPropertyAccessFromIndexSignature: true in your 	sconfig.json will cause TypeScript errors when accessing process.env with dot notation (e.g. process.env.NEXT_PUBLIC_*). Since Next.js uses webpack's DefinePlugin which only replaces dot-access patterns, you should not enable this option.

If you need this option enabled, use bracket notation for environment variables: process.env["NEXT_PUBLIC_..."]. Note that bracket access is not inlined at build time.
## Version Changes
