...
| `outputHashSalt`    | A string salted into content-addressed output filenames to force hash changes across deployments.                                        |
...
Turbopack incorporates a configurable salt string into every content-addressed output filename (chunks, assets). Changing this value forces all output hashes to change, which is useful for invalidating cached assets across deployments without modifying source files.

To configure the output hash salt, use the `outputHashSalt` field in `next.config.js`:
...
When the `NEXT_HASH_SALT` environment variable is also set, the two values are **concatenated** (`outputHashSalt + NEXT_HASH_SALT`) to form the effective salt. This lets you combine a per-project salt baked into the config with a per-deployment salt injected at build time.

## Version History

| Version  | Changes                                              |
| -------- | ---------------------------------------------------- |
| `16.3.0` | `turbopack.outputHashSalt` was added.                |
