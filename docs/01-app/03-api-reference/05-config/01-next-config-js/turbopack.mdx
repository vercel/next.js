...
| `workerPublicPath`  | Custom URL prefix for Web Worker entrypoints and their module chunks, overriding [`assetPrefix`](/docs/app/api-reference/config/next-config-js/assetPrefix). Mirrors webpack's `output.workerPublicPath`. |
...
When [`assetPrefix`](/docs/app/api-reference/config/next-config-js/assetPrefix) points to a cross-origin CDN, Web Workers created via `new Worker(new URL(...))` fail to load because browsers reject cross-origin Worker construction. Use `workerPublicPath` to route Worker entrypoints and their module chunks through a same-origin prefix while keeping other assets on the CDN. This mirrors webpack's [`output.workerPublicPath`](https://webpack.js.org/configuration/output/#outputworkerpublicpath).
...
When unset, Worker URLs use the regular chunk base path (`assetPrefix`). Setting `workerPublicPath` only affects URLs loaded via `new Worker(...)`; other chunks, stylesheets, and static assets continue to use `assetPrefix`.

## Version History

| Version  | Changes                                              |
| -------- | ---------------------------------------------------- |
| `16.3.0` | `turbopack.workerPublicPath` was added.              |
