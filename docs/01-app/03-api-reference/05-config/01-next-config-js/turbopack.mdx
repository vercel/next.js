...
You can apply Turbopack loaders to individual imports using the `with` clause (import attributes). This uses the same loader syntax as the `turbopack.rules` configuration, but is specified per-import rather than globally.
...
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `turbopackUse`        | Array of loaders to apply. Uses the same format as `turbopack.rules[].loaders`.                                               |
...
Loaders with options use the same object format as in the `rules` configuration:

```tsx filename="app/page.tsx"
import value from '../data.js' with { turbopackUse: [
  {
    loader: 'string-replace-loader',
    options: { search: 'PLACEHOLDER', replace: 'replaced value' },
  },
] }
```

> **Good to know**: Import attributes with `turbopackUse` are Turbopack-specific and are not supported by webpack. This feature requires the `with` keyword (not `assert`) in your import statements.
...
| Version  | Changes                                              |
| -------- | ---------------------------------------------------- |
| `16.2.0` | `turbopackUse` import attributes were added.         |
