...
> **Good to know**: If you use `<Link prefetch={true}>` without Partial Prefetching enabled for the destination route, a dev console error explains that the prefetch included dynamic data and suggests enabling `partialPrefetching` app-wide or `prefetch = 'partial'` on the segment.
