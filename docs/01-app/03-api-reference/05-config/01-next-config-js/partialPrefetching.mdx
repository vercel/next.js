...
> **Good to know**: Routes that read `cookies()` or `headers()` produce a personalized App Shell that the framework auto-detects (no opt-in needed). Personalized shells are cached per session on the client, not shared across users.

A link can ask for more than the App Shell with [`<Link prefetch={true}>`](/docs/app/api-reference/components/link#prefetch). The per-link payload is then capped by the destination's [`prefetch` segment config](/docs/app/api-reference/file-conventions/route-segment-config/prefetch): `'partial'` keeps it to the shell, `'allow-runtime'` lets it include runtime data like cookies, headers, and search params.

> **Good to know**: If you use `<Link prefetch={true}>` to a route that hasn't opted into Partial Prefetching, a dev console error suggests enabling `partialPrefetching` app-wide or `prefetch = 'partial'` on the segment. See [the dev warning Insight](/docs/messages/instant-link-prefetch-partial) for the full fix options.
