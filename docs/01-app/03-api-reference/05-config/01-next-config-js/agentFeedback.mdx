...
The generated instructions let the agent retain potential Next.js friction without interrupting the task. Before sending a final response, after the requested work is complete, the agent prepares each distinct qualifying issue as a separate report.
