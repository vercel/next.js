...
The agent attempts to open each draft once in a separate tab in your default browser using the browser-opening capability available in the agent environment. If the browser does not open, the agent continues the task without retrying, investigating the failure, or changing your system. You can edit a report, remove details, close the form without sending, or select **Send feedback**. Nothing is sent when a form opens.

The generated instructions tell the agent to attempt to anonymize each report by replacing customer and project details with generic descriptions and omitting source code, logs, paths, URLs, secrets, and personal information. Review the draft and remove anything you don't want to include before sending it.
