...
> **Good to know:** This option only has an effect in `next dev` sessions when the [FileSystem Cache](/docs/app/api-reference/config/next-config-js/turbopackFileSystemCache) is enabled, since eviction relies on data already being persisted to disk.
...
| Version   | Changes                               |
| --------- | ------------------------------------- |
| `v16.3.0` | `turbopackMemoryEviction` introduced. |
