...
In the [prerenders output type](#prerenders-outputsprerenders), `pprChain.headers` contains the headers needed for the [resume protocol](#implementing-ppr-in-an-adapter). Specifically, it contains `{ 'next-resume': '1' }`.
