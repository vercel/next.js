...
Next.js provides an experimental adapter API that allows you to create custom adapters to hook into the build process. This is useful for deployment platforms or custom build integrations that need to modify the Next.js configuration or process the build output.
...
/** @type {import('next/src/build/adapter/build-complete').NextAdapter} */
const adapter = {
