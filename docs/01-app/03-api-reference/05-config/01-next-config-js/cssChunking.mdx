...
When using `graph` mode, CSS is grouped into shared chunks to cut requests. You can shift that balance by passing an object with two factors:
...
- **`requestCost`** (default `100000`): the estimated cost, in bytes, of each additional CSS request. Larger values bias toward fewer, larger shared chunks, and fewer requests overall.
- **`weightDistribution`** (default `0.1`): controls how a shared chunk's cost is distributed across the routes that load it, weighted by how much CSS each route imports. `0` weights every route equally; higher values give more weight to routes that import less CSS, so the algorithm ships them less unused CSS at the expense of more requests overall.

### Example

Merging CSS into shared chunks is what the default (`true`) already does; `graph` just lets you control where it draws the line between merging and splitting. Take two routes that share a stylesheet:
...
Here, `/b` never imports `only-a.css`. Merging `only-a.css` into the same chunk as `shared.css` lets `/a` load all its CSS in one request instead of two, but the price is that `/b`, which loads that shared chunk, now downloads `only-a.css` even though it never imported it. Splitting `only-a.css` into its own chunk spares `/b` those bytes, at the cost of `/a` making two requests. The default makes this decision with a fixed heuristic.

In `graph` mode, that decision becomes the `requestCost` option: how much un-imported CSS one route will carry so another route can save a request. It keeps `only-a.css` merged while `only-a.css` is well under `requestCost`, and splits it out once it grows large enough to outweigh the extra request. What matters is the size of the un-imported CSS a merge would push onto a route, more than the size of the shared chunk it joins. With the default of about 100 KB, small stylesheets stay merged and only a substantial amount earns its own request.

<details>
  <summary>How the graph algorithm groups CSS</summary>
