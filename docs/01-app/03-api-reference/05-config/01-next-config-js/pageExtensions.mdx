...
You can extend the default Page extensions (`.tsx`, `.ts`, `.jsx`, `.js`) used by Next.js. Inside your Next.js config file, add the `pageExtensions` config:
...
You can colocate test files or other files used by components in the `pages` directory. Inside your Next.js config file, add the `pageExtensions` config:
