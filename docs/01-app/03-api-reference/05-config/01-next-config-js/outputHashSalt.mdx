...
To configure the output hash salt, set the `outputHashSalt` field inside `turbopack` in `next.config.js`:
...
The `NEXT_HASH_SALT` environment variable can also be used for the same purpose. When both are set, the values are **concatenated** (`turbopack.outputHashSalt + NEXT_HASH_SALT`) to form the effective salt. This lets you combine a per-project salt baked into the config with a per-deployment salt injected at build time via environment variable.

```bash filename="Terminal"
NEXT_HASH_SALT=my-deployment-salt next build
```

> **Good to know**: `experimental.outputHashSalt` is supported as an alias for backward compatibility but is deprecated in favour of `turbopack.outputHashSalt`.
