...
Open your Next.js config and add the following `exportPathMap` config:

```ts filename="next.config.ts" highlight={4-15} switcher
...
```js filename="next.config.mjs" highlight={5-16} switcher
/**
...
To switch back and add a trailing slash, enable the `trailingSlash` config:
