...
Configures how Lightning CSS processes [CSS Modules](https://lightningcss.dev/css-modules.html): the naming pattern used for scoped class names and which features are scoped. Only supported by Turbopack.
...
const nextConfig: NextConfig = {
  experimental: {
    lightningCssModules: {
      // Custom naming pattern for scoped class names
      pattern: 'my-company-[hash]-[local]',
      // Keep authored @keyframes names instead of scoping them
      animation: false,
    },
  },
}
...
| -------------- | --------- | --------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `pattern`      | `string`  | `'[name]__[hash]__[local]'` | Naming pattern for scoped names. Supported placeholders: `[name]`, `[local]`, `[hash]` and `[content-hash]`. |
| `animation`    | `boolean` | `true`                      | Scope `@keyframes` names.                                                                                    |
| `grid`         | `boolean` | `false`                     | Scope CSS grid line names.                                                                                   |
| `customIdents` | `boolean` | `true`                      | Scope `custom-ident` values.                                                                                 |
| `dashedIdents` | `boolean` | `false`                     | Scope dashed idents such as custom property names.                                                           |
| `container`    | `boolean` | `false`                     | Scope `@container` names.                                                                                    |

> **Good to know**: Disabling feature scoping (for example `animation: false`) matches the behavior of webpack's `css-loader`, which can be useful when migrating from webpack to Turbopack.

## Version History

| Version  | Changes                                                                                                                                                                                      |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `16.3.0` | `lightningCssModules` added.                                                                                                                                                                 |
