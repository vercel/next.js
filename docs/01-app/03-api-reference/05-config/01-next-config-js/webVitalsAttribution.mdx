...
Valid attribution values are all `web-vitals` metrics specified in the [`NextWebVitalsMetric`](https://github.com/vercel/next.js/blob/canary/packages/next/src/shared/lib/utils.ts) type: `CLS`, `FCP`, `INP`, `LCP`, and `TTFB`.
