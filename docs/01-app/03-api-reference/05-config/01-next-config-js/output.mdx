...
> **Good to know**:
>
> - If your project needs to listen to a specific port or hostname, you can define `PORT` or `NEXT_STANDALONE_HOSTNAME` environment variables before running `server.js`. For example, run `PORT=8080 NEXT_STANDALONE_HOSTNAME=0.0.0.0 node server.js` to start the server on `http://0.0.0.0:8080`.
