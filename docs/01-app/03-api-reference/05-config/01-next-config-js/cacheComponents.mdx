...
If your project still sets `experimental.useCache` or `experimental.cacheComponents`, switch to top-level `cacheComponents: true` instead. Those experimental flags are deprecated and may be removed in a future major release.

Before migrating, your configuration may look like this:
...
After migrating, use top-level `cacheComponents`:
