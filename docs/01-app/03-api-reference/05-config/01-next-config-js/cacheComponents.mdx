...
The top-level `cacheComponents` option replaces `experimental.useCache` and `experimental.cacheComponents`. If your config uses either experimental flag, move to top-level `cacheComponents: true`.

Deprecated configuration under `experimental` used either `useCache: true` or `cacheComponents: true`, not both at once:
...
`experimental.useCache` and `experimental.cacheComponents` are deprecated. Use top-level `cacheComponents` instead; the experimental options may be removed in a future major release.
