...
The top-level `cacheComponents` replaces two earlier experimental flags. If your config uses either of these, update it to use `cacheComponents: true` instead:
...
`experimental.useCache` and `experimental.cacheComponents` are legacy options still supported for backwards compatibility, but top-level `cacheComponents` is preferred.
