...
module.exports = {
  ...
  async updateTags(tags, durations) {
    // This example invalidates immediately in both cases. See
    // [`updateTags()`](#updatetags) for how `durations` separates immediate
    // invalidation from stale-while-revalidate.
    const now = Date.now()
