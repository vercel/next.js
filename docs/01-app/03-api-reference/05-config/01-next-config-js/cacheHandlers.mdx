...
> **Good to know**: Cache handler files must be JavaScript (`.js`) or ESM (`.mjs`) files. They are loaded at runtime via dynamic `import()`, so TypeScript files need to be pre-compiled to JavaScript before use. See the **Using TypeScript** section below for a setup guide.
...
Cache handlers are loaded at runtime by Node.js, which doesn't natively support TypeScript. To use TypeScript, compile your handler to JavaScript before building your Next.js app.

**1. Write your handler in TypeScript:**

```ts filename="cache-handlers/my-handler.ts"
import type {
  CacheEntry,
  CacheHandler,
...
```bash
npx tsc cache-handlers/my-handler.ts --outDir cache-handlers/dist --module nodenext --moduleResolution nodenext
```

Or add a build script using [tsup](https://tsup.egoist.dev/) for a simpler setup:

```bash
npx tsup cache-handlers/my-handler.ts --outDir cache-handlers/dist --format esm
```

**3. Reference the compiled file in your config:**
...
const nextConfig: NextConfig = {
  cacheHandlers: {
    default: require.resolve('./cache-handlers/dist/my-handler.js'),
  },
}
...
**4. Add the compile step to your scripts in `package.json`:**

```json filename="package.json"
{
  "scripts": {
    "build:cache-handler": "tsup cache-handlers/my-handler.ts --outDir cache-handlers/dist --format esm",
    "dev": "npm run build:cache-handler && next dev",
    "build": "npm run build:cache-handler && next build"
  }
}
```
