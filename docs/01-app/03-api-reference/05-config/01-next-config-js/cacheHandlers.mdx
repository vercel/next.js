...
A complete Redis wiring of both cache APIs lives in the [`cache-handler-redis` example](https://github.com/vercel/next.js/tree/canary/examples/cache-handler-redis). Production community handlers are listed in that example's README.
