...
| `softTags` | `string[]` | Implicit tags derived from the route path. See [Understanding Soft Tags](#understanding-soft-tags) for how to use them. |
...
- **`set()` failure**: the response is still served to the user, but the cache entry is lost. The next request to that route will trigger a fresh render.
- **`get()` failure**: treated as a cache miss. The route is server-rendered fresh.
