...
> **Deprecated**: This option has been moved to [`logging.browserToTerminal`](/docs/app/api-reference/config/next-config-js/logging#browser-console-logs). Please update your configuration to use the new location.
...
The new `logging.browserToTerminal` option supports the same log level values (`true`, `'verbose'`, `'warn'`, `'error'`). Note that the object configuration options (`depthLimit`, `edgeLimit`, `showSourceLocation`) are no longer available in the new configuration.
...
| `v16.2.x` | Deprecated in favor of `logging.browserToTerminal`   |
