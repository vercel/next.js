...
> **Good to know:** Because private output is stored in the Client Cache, calling [`revalidateTag`](/docs/app/api-reference/functions/revalidateTag) from a Route Handler does not evict output already delivered to a browser. For read-your-own-writes, call [`updateTag`](/docs/app/api-reference/functions/updateTag) from a Server Action. Other tabs and browsers can keep the previous output until its stale time expires.
