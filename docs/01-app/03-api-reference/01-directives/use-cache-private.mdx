...
Functions marked with `'use cache: private'` run on the server, but results are **never stored on the server**. Instead, they're cached in the browser's memory and do not persist across page reloads. Without a server-side cache, the function executes on every server render.

When using this directive, functions can access `cookies()`, `headers()`, and `searchParams`, which excludes the function from running during [static shell](/docs/app/getting-started/cache-components#how-rendering-works-with-cache-components) generation.

Use this as an escape hatch when:

- Your function requires access to runtime request APIs and refactoring is not practical
...
async function getRecommendations(productId: string) {
  ...
  cacheLife({ stale: 60 }) // At least 30 seconds stale time required for runtime prefetch
