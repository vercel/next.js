...
When possible, read runtime values outside the cached scope and [pass them as arguments](/docs/app/getting-started/cache-components#with-runtime-data) to regular `use cache` functions. Only reach for `'use cache: private'` when:
