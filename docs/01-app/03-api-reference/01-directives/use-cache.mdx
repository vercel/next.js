...
Only the arguments explicitly passed to the cached function become part of the cache key. Values from parent scope are NOT automatically captured—you must pass them as arguments to include them in the cache key.

```tsx filename="lib/data.ts"
const userId = '123' // NOT part of cache key

async function getCachedDataWrong(options: { filter?: string }) {
  'use cache'
  // Cache key = buildId + functionId + hash(options)
  // userId is NOT captured, even though it's used!
  return fetch(`/api/users/${userId}/data?filter=${options.filter}`)
}

// Correct: Pass all values that should affect caching as arguments
async function getCachedData(userId: string, options: { filter?: string }) {
  'use cache'
  // Cache key = buildId + functionId + hash(userId, options)
  return fetch(`/api/users/${userId}/data?filter=${options.filter}`)
}

// Different cache entries:
getCachedData('123', { filter: 'active' })
getCachedData('456', { filter: 'active' })
getCachedData('123', { filter: 'archived' })
```
...
async function UserCard({ user }: { user: UserClass }) {
  'use cache'
...
On the **server**, cache entries are stored in-memory and respect the `revalidate` and `expire` times from your `cacheLife` configuration.
