...
> **Good to know:** A `use cache` with a zero `revalidate` or a short `expire` (under 5 minutes), nested inside another `use cache` that has no explicit `cacheLife`, is not allowed during prerendering and fails the build with [`nested-use-cache-no-explicit-cachelife`](/docs/messages/nested-use-cache-no-explicit-cachelife). Add [`cacheLife()`](/docs/app/api-reference/functions/cacheLife) to the **outer** `use cache` to choose whether it prerenders or stays dynamic.
