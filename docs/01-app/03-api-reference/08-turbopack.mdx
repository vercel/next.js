...
> **Good to know**: `import.meta.env` requires Turbopack. Custom `VITE_*` variables, Vite custom modes, `envPrefix`, `envDir`, and configurable `BASE_URL` values are not supported. `BASE_URL` does not reflect the Next.js `basePath` configuration.
