...
Every match is imported as a module. Turbopack can process JavaScript, TypeScript and image files as-is; anything else, such as `.md`, needs a [loader](/docs/app/api-reference/config/next-config-js/turbopack#configuring-webpack-loaders).

Patterns resolve relative to the file that calls `import.meta.glob()`. To reach a directory elsewhere, such as content kept outside `app/`, pass `base`:

```js filename="app/blog/page.js"
// Matches the .md files in content/posts/ at the root of the project
const posts = import.meta.glob('*.md', { base: '../../content/posts' })
```

Files above the Turbopack filesystem root are not matched. The root defaults to the directory holding your `next.config.js` and is configurable with [`turbopack.root`](/docs/app/api-reference/config/next-config-js/turbopack#root-directory).

Avoid broad patterns. The `node_modules` directory is included in matching and something like `**/*.md` at the project root walks every installed package.

Keys are the matched file paths, and their order is stable: the same on every build and in both development and production. They are compared character by character, so `'./10-post.md'` sorts before `'./2-post.md'`, uppercase before lowercase, and nested files interleave with their siblings. Sort the keys before rendering if the order is user-facing.
...
Use the `query` option to append a query string to every import request. Match it with a loader [`condition`](/docs/app/api-reference/config/next-config-js/turbopack#advanced-webpack-loader-conditions) to process globbed files differently from the same files imported directly:

```js
const modules = import.meta.glob('./dir/*.svg', { query: '?svgr' })
```
...
| `base`          | `string`                                      | `undefined` | Directory to resolve patterns against. Keys stay relative to the calling file. |
| `caseSensitive` | `boolean`                                     | `true`      | Match glob patterns case-sensitively. Set to `false` to ignore ASCII case.     |

> **Note:** The `as` option (deprecated in Vite 5) is not supported. To load matched files as strings, register a [loader](/docs/app/api-reference/config/next-config-js/turbopack#configuring-webpack-loaders) for the extension. The legacy `import.meta.globEager()` API is also not supported — use `import.meta.glob('...', { eager: true })` instead.
