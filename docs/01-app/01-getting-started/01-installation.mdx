...
> **Note**
>
> Depending on how you set up your Next.js project, you may see different ESLint configuration files:
>
> - If you use `create-next-app`, your project will use the new `eslint.config.mjs` file for configuration ([learn more](https://eslint.org/docs/latest/use/configure/configuration-files#configuring-eslint)).
> - If you set up ESLint manually (by running `npm run lint` and following the prompts), your project will use a `.eslintrc.json` file.
>
> Both files serve the same purpose—configuring ESLint for your project—but use different formats. The new `eslint.config.mjs` format is recommended for new projects created with `create-next-app`.
