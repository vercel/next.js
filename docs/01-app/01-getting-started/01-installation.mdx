...
If either of the two configuration options are selected, Next.js will automatically install `eslint` and `eslint-config-next` as dependencies in your application and create an `eslint.config.mjs` file in the root of your project that includes your selected configuration.
