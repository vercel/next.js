...
Next.js supports linting with either ESLint, Oxlint or Biome. Choose a linter and run it directly via `package.json` scripts.
...
- Use **Oxlint** (fast linter with comprehensive rules) and **Oxfmt** (fast formatter):
