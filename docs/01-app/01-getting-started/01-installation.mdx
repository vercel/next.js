---
title: Installation
description: Learn how to create a new Next.js application with the create-next-app CLI or by hand, and set up TypeScript, ESLint, and Module Path Aliases.
related:
  title: Next Steps
  description: Learn more about browser support, local development, and the CLI commands mentioned on this page.
  links:
    - app/guides/local-development
...
A Next.js project has the `next` package installed and an `app` folder that defines your routes. This page walks you through creating one with `create-next-app` or setting it up manually, including TypeScript, linting, and import aliases.
...
See [Upgrading](/docs/app/guides/upgrading) for version guides and manual upgrade steps, or the [preview docs](https://preview.nextjs.org) to explore features before they ship in a stable version.
