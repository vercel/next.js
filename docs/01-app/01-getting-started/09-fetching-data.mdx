...
This page will walk you through how you can fetch data in [Server and Client Components](/docs/app/getting-started/server-and-client-components). As well as how to [stream](#streaming) content that depends on data.
...
> **Good to know:** `fetch` responses are not cached by default. You can cache individual requests using the `{ cache: 'no-store' }` option. See the [`fetch` API Reference](/docs/app/api-reference/functions/fetch).
