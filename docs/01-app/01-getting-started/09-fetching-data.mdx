...
However, within _any_ component, multiple`async`/`await` requests can still be sequential if placed after another. For example, `getAlbums` will be blocked until `getArtist` is resolved:
...
You can initiate requests in parallel by defining them outside the components that use the data, and resolving them together with `Promise.all`:
...
You can use React's taint APIs, [`taintObjectReference`](https://react.dev/reference/react/experimental_taintObjectReference) and [`taintUniqueValue`](https://react.dev/reference/react/experimental_taintUniqueValue), to prevent whole data objects or sensitive values from being passed to the client.
