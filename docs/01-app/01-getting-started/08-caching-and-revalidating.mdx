...
This page describes how you can cache and revalidate routes, components, and functions. As well as how to [stream](#optimizing-dynamic-routes-with-streaming) dynamic content.
...
Using a [Dynamic API](#dynamic-apis) or a [fetch](/docs/app/api-reference/functions/fetch) request with the `{ cache: 'no-store' }` option will make a route dynamic, meaning it'll be rendered at runtime and the result won't be cached.
...
Slow data requests can block the whole route from rendering. To improve the initial load time and user experience, you can use streaming to break up the route's HTML into smaller _chunks_ and progressively send those chunks from the server to the client.

With streaming, the static parts of the page are sent immediately to the client, alongside a loading state (also known as a fallback) for the dynamic content. The loading state is then swapped for the dynamic content once fetching and rendering is complete.
...
You can create a [`loading.js`](/docs/app/api-reference/file-conventions/loading) file in the same folder as your page to stream the **entire page** while the data is being fetched. For example, to stream `app/blog/page.js`, add the file inside the `app/blog` folder.
...
On navigation, the user will immediately see the layout and the loading state while the page is being rendered. The loading state is then swapped for the page once rendering is complete.
...
For the best user experience, we recommend designing loading states that are meaningful and help users understand the app is responding. For example, skeletons, spinners, or a small but meaningful part of future screens such as a cover photo, title, etc.

In development, you can preview and inspect the loading state of your components using [React Devtools](https://react.dev/learn/react-developer-tools).
...
Following the example above, let's say you want to cache the blog list rather than stream it. To ensure the `<BlogList />` component is cached, make it an **asynchronous** function and add the [`'use cache'`](/docs/app/api-reference/directives/use-cache) directive:
...
To use `cacheLife`, import it from `next/cache` and nest it within the **scope** of component or function. For example, to revalidate the blog list page after one hour:
...
The client cache has an **automatic invalidation period** that depends on how the route was [prefetched](/docs/app/api-reference/components/link#prefetch), and if the route was [statically generated](/docs/app/getting-started/caching-and-revalidating#static-routes-default):
