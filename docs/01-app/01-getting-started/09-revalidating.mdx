...
Cache data that doesn't depend on [runtime data](/docs/app/getting-started/caching#work-with-runtime-apis) and that you're OK serving from cache for a period of time. Use `use cache` with `cacheLife` to describe that behavior.
