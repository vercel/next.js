...
> This page covers revalidation with [Cache Components](/docs/app/api-reference/config/next-config-js/cacheComponents). If you're not using Cache Components, see [Caching without Cache Components](/docs/app/guides/caching-without-cache-components).
...
Cache data that doesn't depend on [runtime data](/docs/app/getting-started/caching#runtime-data) and that you're OK serving from cache for a period of time. Use `use cache` with `cacheLife` to describe that behavior.
