...
Alternatively, you can stream data from a Server Component to a Client Component with the [`use` Hook](https://react.dev/reference/react/use). See an [example](/docs/app/getting-started/fetching-data#with-the-use-hook).
...
You can pass Server Components as a prop to a Client Component. This allows you to visually nest UI rendered on the server within the UI rendered on the client.
