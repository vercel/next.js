...
A Server Function is an asynchronous function that is executed on the server.

Server Functions invoked as part of an `action` are Server Actions.

By convention, an `action` is an asynchronous function passed to `startTransition`.

Server Functions are automatically wrapped with `startTransition` when:

- Passed to a `<form>` using the `action` prop,
- Passed to a `<button>` using the `formAction` prop
- Passed to `useActionState`

> **Good to know**: As of September 2024, the React team started to use the term, Server Functions. Initially these were called Server Actions. For the purpose of this document, when used to mutate server side data, we use the term Server Action.

Server Functions are inherently asynchronous because they are invoked by the client using a network request.
