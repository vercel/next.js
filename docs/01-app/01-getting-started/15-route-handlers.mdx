...
Route Handlers are not cached by default. Other supported HTTP methods (`POST`, `PUT`, `PATCH`, `DELETE`, `HEAD`, `OPTIONS`) are **not** cached, even if they are placed alongside a `GET` method that is cached, in the same file.

#### With Route Segment Config

You can opt into caching for `GET` methods using a [route config option](/docs/app/api-reference/file-conventions/route-segment-config#dynamic) such as `export const dynamic = 'force-static'` in your Route Handler file.
