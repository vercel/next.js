...
A user who loses connectivity mid-submit does not lose their work. Server Actions stay pending until the network returns and complete on their own. Use the [`useOffline`](/docs/app/api-reference/functions/use-offline) hook to make the pending state offline-aware.

> **Good to know**: Reloading the page while offline cancels any pending Server Action. The retry only works while the page stays open.

See [retry Server Actions after the network returns](/docs/app/guides/offline-support#retry-server-actions-after-the-network-returns).
