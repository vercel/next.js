...
In Partial Prerendering, using the APIs throws a special React error that informs Next.js the component cannot be statically rendered, causing a build error. You can use wrap the error in [Suspense](#suspense) to defer rendering until runtime.
