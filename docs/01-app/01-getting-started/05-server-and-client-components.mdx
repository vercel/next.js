...
Here, the RSC Payload contains the rendered `<Cart>`, plus a placeholder for `<Modal>` with a reference to its JavaScript file. On the client, React shows the prerendered HTML, reconciles the server and client trees from the RSC Payload, and uses `<Modal>`'s JavaScript during hydration and for ongoing interactivity. See [How do Server and Client Components work in Next.js?](#how-do-server-and-client-components-work-in-nextjs) for the full rendering flow.
