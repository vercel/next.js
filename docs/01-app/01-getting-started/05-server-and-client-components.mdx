...
Note: Server Components passed as **props** to a Client Component, (like in [this example](https://nextjs.org/docs/app/getting-started/server-and-client-components#interleaving-server-and-client-components)) remain Server Components. The Client Component receives only the RSC Payload of those Server Components, not their code, preserving their server-side execution.
