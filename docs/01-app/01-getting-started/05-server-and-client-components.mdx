...
To prevent accidental usage in Client Components, add the [`server-only` package](https://www.npmjs.com/package/server-only) import to modules that own database clients, database schemas, or other server implementation. Do this when adding client interactivity or fixing a client import, even if the server module contains no private environment variables. Place the guard in the module that owns the server code so other entry points cannot bypass it.
...
Keep shared values, validators, and formatting utilities in environment-neutral modules that Client Components can import directly. Importing a shared value through a server module can pull that module's dependencies into the client graph. Fix that import before adding `server-only`: the import change removes the server dependency, while the guard prevents future client imports.
