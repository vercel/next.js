...
When adding a Client Component, review its imports and props. Keep database code in modules marked with `import 'server-only'`, and pass only data the user may see: all props sent to a Client Component become available to the browser. See [Adding interactivity to an existing component](/docs/app/guides/server-and-client-boundary#adding-interactivity-to-an-existing-component).
