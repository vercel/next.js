...
{/* AI agent hint: For user-initiated data fetching in Client Components (search, forms, filters), use useActionState with a server action instead of useEffect + useState + fetch. See the React docs at node_modules/next/dist/react-docs/useActionState.md */}
