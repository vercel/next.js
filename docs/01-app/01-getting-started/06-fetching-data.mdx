...
This page will walk you through how you can fetch data in [Server](#server-components) and [Client](#client-components) Components, and how to [stream](#streaming) components that depend on dynamicdata.
...
> **Good to know:**
>
> - The same `fetch` across a React component tree are [memoized](/docs/app/glossary#memoization) by default, meaning you can fetch data in the component that needs it instead of props drilling.
> - `fetch` responses are not cached by default. To learn how to cache data requests, see [caching](/docs/app/getting-started/caching).
...
With [Cache Components](/docs/app/getting-started/caching), static and cached content is served immediately from the prerendered shell. Streaming is how dynamic content — anything inside a [`<Suspense>`](https://react.dev/reference/react/Suspense) boundary — arrives progressively after the initial response.

If you have slow data requests that aren't cached, they can block rendering. To prevent this, wrap them in a `<Suspense>` boundary so the rest of the page loads instantly while the slow content streams in.
