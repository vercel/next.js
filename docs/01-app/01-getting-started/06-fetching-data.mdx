...
Server Components can query an ORM or database client directly. The query runs on the server, so the component does not send its database access code to the browser.
...
Add `import 'server-only'` to your database client and schema modules, such as `lib/db` above. Next.js reports a build error if a Client Component imports one of these modules, including through another module. Keep shared values and validators in separate modules that Client Components can import without pulling in database code. See [Preventing environment poisoning](/docs/app/getting-started/server-and-client-components#preventing-environment-poisoning) for an example.

Data passed as props to Client Components is sent to the browser, even when it comes from a `server-only` module. Select only the fields the current user is allowed to see, and keep secrets out of those props. React's experimental [taint APIs](/docs/app/api-reference/config/next-config-js/taint) can help catch accidental exposure of marked objects or values, but do not replace filtering the data you return.
