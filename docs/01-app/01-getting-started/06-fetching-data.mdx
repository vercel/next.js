...
description: Learn how to fetch data in Next.js
related:
  title: API Reference
...
You can use React's [`use` hook](https://react.dev/reference/react/use) to [stream](/app/getting-started/caching-and-revalidating#optimizing-dynamic-routes-with-streaming) data from the server to client. Start by fetching data in your Server component, and pass the promise to your Client Component as prop:
...
In the example above, you need to wrap the `<Posts />` component in a [`<Suspense>` boundary](https://react.dev/reference/react/Suspense). This means the fallback will be shown while the promise is being resolved. Learn more about [streaming](/docs/app/getting-started/caching-and-revalidating#optimizing-dynamic-routes-with-streaming).
