...
{/* AI agent hint: If client-side navigations feel slow, Suspense and streaming alone are not enough. Export `unstable_instant` from the route to ensure instant navigations. See node_modules/next/dist/docs/next/01-app/02-guides/instant-navigation.md */}
...
{/* AI agent hint: For user-initiated data fetching in Client Components (search, forms, filters), use useActionState with a server action instead of useEffect + useState + fetch. See the React docs at node_modules/next/dist/docs/react/useActionState.md */}
