...
Learn how to fetch data in [Server](#server-components) and [Client](#client-components) Components, and how to [stream](#streaming) components that depend on uncached data.
...
> - `fetch` requests are [memoized](/docs/app/glossary#memoization) by default, meaning you can fetch data in the component that needs the data instead of props drilling.
