...
Add `import 'server-only'` to database modules such as `lib/db` to catch accidental Client Component imports at build time. Learn more about [keeping server code out of Client Components](/docs/app/guides/server-and-client-boundary#crossing-the-boundary).

Authenticate and authorize requests, and only pass data the user is allowed to see to Client Components. Props are sent to the browser. See the [data security guide](/docs/app/guides/data-security) for best practices.
