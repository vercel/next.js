...
These params are now resolved at build time. The listed pages are prerendered and served instantly from CDN. Whether the page is fully static or partially prerendered depends on the content inside the route.

Params not listed in `generateStaticParams` (for example, `/products/99`) are rendered on the first request. How the page loads depends on the route's content: if it uses `<Suspense>`, the shell is served instantly and the dynamic content streams in. After the first visit, the result is cached for subsequent requests.
