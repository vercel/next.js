...
These params are now resolved at build time. The static shell is served instantly from CDN. If the entire page is cached, it can be fully served from CDN, but the build output still shows `◐`.

Params not listed in `generateStaticParams` (for example, `/products/99`) are rendered on the first request. The static shell is still served instantly, and any dynamic content streams in. After the first visit, the result is cached for subsequent requests.
