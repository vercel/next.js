...
The listed params are resolved ahead of time. Unlisted params (for example, `/products/99`) are server-rendered on the first request, then cached. Without [`partialFallbacks`](#high-cardinality-routes), unlisted params block until the server finishes rendering.
