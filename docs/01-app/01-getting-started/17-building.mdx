...
For example, `await params` in a Page without `generateStaticParams` blocks the entire page from prerendering, because the param values are not known at build time:
