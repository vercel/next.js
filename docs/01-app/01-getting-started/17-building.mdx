...
These params are now resolved at build time. If the route has no other dynamic content, it becomes fully static (`○`). If it also contains streamed content, it stays `◐` but the params no longer block prerendering.
