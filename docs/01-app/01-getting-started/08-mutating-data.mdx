...
{/* TO DO: Create form example page */}

{/* See the [Forms](/docs/app/getting-started/examples/forms) docs for more examples of how to use forms in Next.js. */}
...
export function Button() {
  ...
  return (
    <button disabled={pending} onClick={async () => action()}>
      Create
    </button>
  )
}
...
After performing a mutation, you can revalidate a cache entry by calling [`revalidatePath`](/docs/app/api-reference/functions/revalidatePath) or [`revalidateTag`](/docs/app/api-reference/functions/revalidateTag) within the Server Function:
