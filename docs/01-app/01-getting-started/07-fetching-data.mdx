...
<Image
  alt="Sequential and Concurrent Data Fetching"
...
Concurrent data fetching happens when data requests in a route are eagerly initiated and executed at the same time.
...
You can initiate requests concurrently by defining them outside the components that use the data, and resolving them together, for example, with [`Promise.all`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/all)(or any of the 4 [`Promise` concurrency methods](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise#promise_concurrency), depends on your use case):
