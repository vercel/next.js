...
Initiate multiple requests by calling fetch and await them together with [`Promise.all`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/all). Requests start immediately when fetch is called. `Promise.all` only waits for all of them to resolve, if any rejects `Promise.all` rejects too. If some failures are acceptable, use [`Promise.allSettled`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/allSettled) to handle successes and errors together.
