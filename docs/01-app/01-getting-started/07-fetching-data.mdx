...
> **Good to know**: `loading.js` does not wrap the layout itself. If the layout is dynamic (e.g. uses `cookies()` or `headers()`), `loading.js` will not show a fallback for that work — without [Cache Components](/docs/app/getting-started/cache-components), navigation blocks until the layout finishes; with Cache Components, the layout's dynamic work must be explicitly wrapped in `<Suspense>`. In both cases, move dynamic work to `page.js` or wrap it in its own [`<Suspense>`](#with-suspense) boundary. See [`loading.js`](/docs/app/api-reference/file-conventions/loading) for details.
