...
When prefetching a route, Next.js fetches two things:

1. **Route tree**: A lightweight description of which segments make up the route. This tells the client router what layouts and pages are needed.
2. **Segment data**: The prerendered content for each segment (the [static shell](/docs/app/getting-started/cache-components) when using Cache Components).

For routes you've already visited, Next.js can often skip the route tree request entirely. It learns the route structure from previous navigations and predicts it for similar URLs—for example, after visiting `/blog/post-1`, it knows `/blog/post-2` uses the same `[slug]` route structure.

This incremental learning means subsequent navigations to similar routes can skip the server roundtrip for route resolution, going directly to fetching segment data.
