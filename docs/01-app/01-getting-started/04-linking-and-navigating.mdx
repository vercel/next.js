...
- Keeps any shared layouts and UI.
- Replaces the current page with the prefetched loading state or a new page if available.
...
We recommend adding `loading.tsx` to enable partial prefetching, trigger immediate navigation, and display a loading UI while the route renders.
