...
Instead of unmounting the previous route when you navigate away, Next.js hides it with `display: none`. This means:
...
As a consequence of routes being hidden rather than unmounted, portals created via [`createPortal`](https://react.dev/reference/react-dom/createPortal) need special handling. Since portals render outside their parent component's DOM tree, they bypass the CSS cascade and remain visible even when the parent route is hidden.
