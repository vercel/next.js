...
The trade-off of server rendering is that the client must wait for the server to respond before the new route can be shown. Next.js addresses this delay by [pretching](#what-is-prefetching) routes the user is likely to visit and performing [client-side transitions](#what-is-client-side-transition).
