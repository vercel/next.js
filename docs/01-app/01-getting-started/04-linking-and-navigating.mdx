...
Next.js handles connectivity drops gracefully: users can keep navigating between prefetched routes, and any route that needs server data completes once the network returns. Use the [`useOffline`](/docs/app/api-reference/functions/use-offline) hook to surface that state in your UI. See the [offline support guide](/docs/app/guides/offline-support).

> **Good to know**: This applies to soft navigations from the current page. A full reload while offline still fails because the browser needs the network to fetch the HTML. For that, see the [Progressive Web Apps guide](/docs/app/guides/progressive-web-apps).
