...
related:
  title: Next Steps
  description: Learn more about data-fetching patterns and the APIs mentioned on this page.
  links:
...
> - `fetch` requests are not cached by default and will block the page from rendering until the request is complete. Use the [`use cache`](/docs/app/api-reference/directives/use-cache) directive to cache results, or wrap the fetching component in [`<Suspense>`](/docs/app/getting-started/fetching-data#streaming) to stream fresh data at request time. See [caching](/docs/app/getting-started/caching) for details.
...
Behind the scenes, `loading.js` will be [nested inside `layout.js`](/docs/app/guides/project-structure#component-hierarchy), and will automatically wrap the `page.js` file and any children below in a `<Suspense>` boundary.
