...
Following the example above, the blog list route is now dynamic because `cookies` is nested in the blog's layout. However, the blog list doesn't need to update on every request. To ensure this page is cached, make the component **asynchronous**, and add the [`'use cache'`](/docs/app/api-reference/directives/use-cache) directive:
...
With the above approach, whenever `getPosts` function is called in your application, the same cached value will be reused as long as the arguments passed to the function are the same. Learn more about `use cache` in the [API Reference](/docs/app/api-reference/directives/use-cache).
