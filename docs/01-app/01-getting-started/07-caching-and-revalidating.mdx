...
For example, the `<User>` component uses the `cookies` API to fetch the current user's information. Using it anywhere in a route will make it dynamic and block it from being sent to the client until the data is fetched.
