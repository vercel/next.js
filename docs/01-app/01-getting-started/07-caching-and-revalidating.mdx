...
The client-side cache can be invalidated with [`cacheLife` function](/docs/app/api-reference/functions/cacheLife) or [`cacheTag` function](/docs/app/api-reference/functions/cacheTag).
...
- **Full Prefetching** (`prefetch={true}` or `router.prefetch`): 5 minutes for both static & dynamic routes.
