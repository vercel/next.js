...
By default, routes are prerendered at build time (static). During development, you can check if a route is static by running `next dev` and opening the [Next.js Dev Tools](/docs/app/api-reference/config/next-config-js/devIndicators) in your local host.

### Dynamic routes

Using **request-time APIs** will make a route dynamic, meaning it'll be rendered on every request at runtime and the result won't be cached.

These request-time APIs are:
...
> During development, you can check if a route is dynamic by running `next dev` and opening the [Next.js Dev Tools](/docs/app/api-reference/config/next-config-js/devIndicators) in your local host.

### Optimizing dynamic routes with Streaming

To optimize the initial loading performance of dynamic routes, you can use React's [`<Suspense>` boundary](https://react.dev/reference/react/Suspense) to show loading UI and progressively stream dynamic content to the client.

For example, the `<User>` component uses the `cookies` API to fetch the current user's information. Since `<User>` is nested inside `<Navigation>` and `<Navigation>` is nested inside the layout, the `cookies` API opts the whole route into dynamic rendering.

To prevent the whole route from being blocked while all data is being fetched, you can wrap `<User>` in a `Suspense` boundary. This means the `<Navigation>` component and a fallback will be sent to the client first, while the `<User>` component will be streamed when it's ready.
...
### Optimizing dynamic routes with Caching

In addition to streaming, you can optimize dynamic routes by caching the output of nested components or functions.

Using the Dashboard route example above, although the route is fully dynamic, you can cache parts of it. Within the `DashboardPage` component, the `<AnalyticsChart>` doesn't need to be recomputed on every request.

```tsx filename="app/dashboard/page.tsx" switcher
import { AnalyticsChart } from '@/app/ui/AnalyticsChart'
...
To cache it, add the `'use cache'` directive to the component:

```tsx filename="app/dashboard/components/AnalyticsChart.tsx" switcher
'use cache'
...
export async function AnalyticsChart() {
  ...
  return (
    <div>
      ...
      <div>
        {data.map((point) => (
          <div key={point.id} style={{ height: `${point.value}%` }} />
        ))}
      </div>
    </div>
  )
...
While the Dashboard route will remain dynamic, the `<AnalyticsChart>` render output of the component will be cached separately and reused for each request.

You can verify a component is cached by running `next build && next start`.

TODO: Talk about functions
