...
related:
  title: Next Steps
  description: Learn more about composing across the boundary, and the APIs mentioned on this page.
  links:
    - app/guides/server-and-client-boundary
    - app/api-reference/directives/use-client
---

By default, every component you write is a [Server Component](https://react.dev/reference/rsc/server-components) and runs on the server. You add the [`use client`](https://react.dev/reference/rsc/use-client) directive to the components that need browser features, such as state, effects, and event handlers.

This page explains that boundary, when to cross it, and how to compose the two kinds of component together.

## Start with Server Components

You don't opt in to Server Components. Write a component and it is one already.

Because it runs on the server, it can be `async`, and it can read data directly rather than through an endpoint you have to write:
...
Database credentials and API keys used here stay on the server, and none of this component's code is sent to the browser. [Fetching Data](/docs/app/getting-started/fetching-data) covers reads in depth.

You can build a lot this way. Keep going until you hit something the server cannot do.

## Add interactivity where you need it

Eventually you want a button that responds to a click, a value that persists between renders, or something only the browser knows. None of that can happen on the server, because there is no user in front of it yet.

That is the moment to reach for a Client Component. Put `use client` at the top of the file:
...
You will know you need one when you find yourself wanting:

- **State or an event handler**, such as `useState`, `onClick`, or `onChange`
- **Lifecycle logic**, such as `useEffect`
- **A browser-only API**, such as `localStorage`, `window`, or `navigator.geolocation`
- **A custom hook** that uses any of the above

Nothing else has to change. The rest of your tree stays on the server, and a Server Component can render a Client Component as a child, passing data down as props:
...
`<Page>` fetches on the server and hands `likes` to `<LikeButton>`, which handles the click in the browser.

Two consequences are worth knowing early, because they are the ones that surprise people:

- **`use client` applies to the whole file, and to what it imports.** It marks an entry point into the browser bundle, not a single component.
- **Props crossing the boundary must be serializable.** A number or an object is fine; a function is not, apart from a [Server Function](/docs/app/getting-started/mutating-data), which crosses as a reference.

> **Good to know:** "Client" describes where code _may_ run, not where it does run. A Client Component is still rendered on the server for the initial HTML, then hydrated in the browser. See [The Server and Client Boundary](/docs/app/guides/server-and-client-boundary).

## When to use Server and Client Components?

A quick reference once you know the model. The two environments have different capabilities, so each component runs where its work belongs.

| Use a **Client Component** for               | Use a **Server Component** for                                                               |
| -------------------------------------------- | -------------------------------------------------------------------------------------------- |
| State and event handlers                     | Fetching data close to its source                                                            |
| Lifecycle logic such as `useEffect`          | Using API keys, tokens, and other secrets without exposing them                              |
| Browser-only APIs                            | Keeping JavaScript out of the browser bundle                                                 |
| Custom hooks that depend on any of the above | Improving [First Contentful Paint](https://web.dev/fcp/) and streaming content progressively |

When both would work, prefer a Server Component, and move the boundary as far down the tree as you can. [Reducing JS bundle size](#reducing-js-bundle-size) shows what that buys you.
...
You do not need this section to build with Server and Client Components. It is here for when the boundary behaves in a way you want to explain, and for the vocabulary that appears in error messages and tooling.
