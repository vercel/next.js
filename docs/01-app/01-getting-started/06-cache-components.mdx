...
Add `use cache` to any **async** Server Component to make it cached and include it in the pre-rendered shell. Utility functions can also be marked with `use cache` and be called from within Server Components. Runtime APIs cannot be used from inside a cached component.
