...
If your route is intentionally fully dynamic and you don't need granular `<Suspense>` boundaries, you can wrap the document body in a `<Suspense>` boundary in your Root Layout. This signals to Next.js that the entire page should defer to request time.

```tsx filename="app/layout.tsx" highlight={1,9,13}
import { Suspense } from 'react'
...
Be aware that this disables prerendering for the route. There is no static shell to send immediately, so every request blocks until the page is fully rendered. To limit the impact, use [multiple root layouts](/docs/app/api-reference/file-conventions/layout#root-layout) to isolate fully dynamic routes from the rest of your application.

> **Good to know**: This same pattern applies when `generateViewport` or `generateMetadata` access uncached dynamic data. See [Viewport with Cache Components](/docs/app/api-reference/functions/generate-viewport#with-cache-components) for a detailed example.
