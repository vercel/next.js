...
Cache Components lets you mix static, cached, and streaming content in a single route, giving you the speed of static sites with the flexibility of dynamic rendering.

Traditionally, server-rendered applications force a choice between static pages (fast but can't show personalized data) and dynamic pages (fresh but slow). With Cache Components, Next.js treats all routes as dynamic by default and eliminates this tradeoff by prerendering a static HTML shell at build time.

During prerendering, as long as components don't access network resources, certain system APIs, or require an incoming request to render, their output is **automatically added to the static shell**. Otherwise, you must choose how to handle them:

- Defer to request time, showing fallback UI until the content is ready, or
- Cache the result to include it in the static shell (if no request data is needed)

Caching can be applied at the component or function level, while fallback UI can be defined around any subtree, which means you can compose static, cached, and streaming content within a single route.
...
This rendering strategy is called **Partial Prerendering**.

## Prerendered content

During prerendering, components that only use immediately resolved promises, synchronous file system reads, module imports, and pure computations are rendered completely.

Their rendered output becomes part of the **static shell**: prerendered HTML for initial page loads and a serialized [RSC Payload](/docs/app/getting-started/server-and-client-components#on-the-server) for client-side navigation. The browser receives fully rendered content instantly, whether the user navigates directly to the URL or transitions from another page.

In the snippet below, the `Page` component requires no special handling. Because all operations complete at build time, the page's rendered output is included in the static shell. When both the layout and page prerender successfully, the entire route is the static shell.
...
When your components make network requests, use async file operations or timeouts, access request data (cookies, headers), or generate random values, you must explicitly choose how to handle it:

- **Render at request time**: [Define `<Suspense>` fallback UI](#rendering-at-request-time) to be included in the static shell while the actual content streams in
- **Prerender and include in static shell**: [Use the `use cache` directive](#during-prerendering) for content that doesn't change frequently
...
When your components execute operations that depend on external systems (network requests, database queries) or use async file operations, their content cannot be included in the static shell and must be resolved at request time:
...
During prerendering, Next.js stops at the `fetch` request. The request itself is not started, and any code after it is not executed.
