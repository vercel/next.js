...
- Dynamic parts must be explicitly wrapped in `<Suspense>` (enforced via [error](#missing-suspense-boundaries))
...
Work that can complete during the build or prerender phase:
...
### Dynamic (Defers to Request Time)

Operations that perform external I/O or access non-deterministic values. These create dynamic holes (deferred UI parts requiring `<Suspense>`) during prerendering unless wrapped in [`use cache`](/docs/app/api-reference/directives/use-cache):
...
These operations can be completed with [`use cache`](/docs/app/api-reference/directives/use-cache), making them part of the static shell, or wrapped in [`<Suspense>`](https://react.dev/reference/react/Suspense) to include the fallback UI in the static shell.
...
async function User() {
  // Reads the user's session cookie
...
export default function Page() {
  return (
    // Suspense shows static shell while waiting for runtime data
...
Runtime data and [`use cache`](/docs/app/api-reference/directives/use-cache) cannot be used in the same scope. However, you can pass derived values from runtime APIs into a function using the `use cache` directive.
...
Content outside the boundary, including the fallback UI, is prerendered as a static shell, while content inside the boundary streams in when ready.
...
At build time, Next.js prerenders the static content and the `fallback` UI, while the dynamic content is postponed until a user requests the route.
...
You must not pass dynamic or runtime data into `use cache` functions unless you avoid introspecting them. Passing values from `cookies()`, `headers()`, or other runtime APIs as arguments will cause errors, as the cache key cannot be determined at prerender time.

#### Runtime caching considerations

While `use cache` is designed primarily to include dynamic data in the static shell, it can also cache data at runtime. However, the in-memory LRU storage has limitations - as older entries are evicted, you may unnecessarily repeat data fetching or calculations.

If the in-memory cache isn't sufficient, [`use cache: remote`](/docs/app/api-reference/directives/use-cache-remote) allows platforms to provide a dedicated cache handler (like a KV database). This can help reduce hits against data sources not scaled to your total traffic, though it comes with its own costs (storage, network latency, platform fees).

For compliance requirements where certain data must never be stored on the server (even ephemerally), or when a deeply nested component needs to access runtime APIs (like `cookies()`) and restructuring would be disruptive, use [`use cache: private`](/docs/app/api-reference/directives/use-cache-private). This allows runtime API access while preventing the data from being cached.
...
`GET` and `HEAD` Route Handlers follow the same model as normal UI routes in your application. They are dynamic by default, can be prerendered when deterministic, and you can `use cache` to include more dynamic data in the cached response. Other HTTP methods (POST, PUT, DELETE, PATCH, OPTIONS) are always dynamic and never prerendered.
...
A handler that returns only static data will be prerendered at build time:
...
> - Using runtime APIs like [`cookies()`](/docs/app/api-reference/functions/cookies) or [`headers()`](/docs/app/api-reference/functions/headers), or calling [`connection()`](/docs/app/api-reference/functions/connection), always defers to request time (no prerendering).
...
When accessing runtime APIs like `cookies()`, Next.js will only prerender the fallback UI above this component.
...
Now we have a `Suspense` boundary around our User component we can prerender the Page with a Skeleton UI and stream in the `<User />` UI when a specific user makes a request
