...
Next.js attempts to pre-render everything during the build phase. When it encounters work that cannot complete during pre-rendering (dynamic I/O, runtime APIs), that work is deferred to request time:

1. **Pre-rendering starts**: Next.js tries to render the entire page
...
### Static (Pre-renderable)

Work that can complete during the build or pre-render phase:
...
Operations that perform external I/O or access non-deterministic values. These create dynamic holes (deferred UI parts requiring `<Suspense>`) during pre-rendering unless wrapped in [`use cache`](/docs/app/api-reference/directives/use-cache):
...
> **Good to know:**
>
> - Cache Components is an opt-in feature. Enable it by setting the `cacheComponents` flag to `true` in your Next config file. See [Enabling Cache Components](#enabling-cache-components) for more details.
> - For technical details on serialization, cache keys, and constraints, see the [Cache Components API Reference](/docs/app/api-reference/Cache-Components).
...
`GET` and `HEAD` Route Handlers follow the same model as normal UI routes in your application. They are dynamic by default, can be pre-rendered when deterministic, and you can `use cache` to include more dynamic data in the cached response. Other HTTP methods (POST, PUT, DELETE, PATCH, OPTIONS) are always dynamic and never pre-rendered.
...
- **Route-level controls**: Switches like `dynamic`, `revalidate`, `fetchCache` controlled caching for the whole page (see [how these are replaced](/docs/app/api-reference/Cache-Components#effect-on-route-segment-config))
