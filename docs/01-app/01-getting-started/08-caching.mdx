...
{/* consider a callout for remote or private directives, short, x-refs to the API reference for those */}
...
{/* Perhaps here we need a takeway, something to notice, the cookies presence doesn't opt the route into a fully dynamic space, as the previous model, rather this is contained by the Suspense fallback, allowing everything else, static and cached content to be part of the initial HTML */}
...
{/* End with, you don't need to memorize which APIs do this, the framework provides feedback and insights when these type of APIs are used -how to properly use them rather, depending your intention */}

## Synchronous I/O and pure computations

{/* As said before, we need to make the contrast, these are predictable - the pure computation is a good hint already, but maybe we need more info? */}
...
{/* bullet point for unstable/unpredicatable APIs random timestamps */}
...
{/* same, does this need a tiny bit more info? */}
Every route producing a static shell makes direct navigations instant.

{/* we need a connector like, However... */}
In a client-side transition, only the components below the shared layout re-render. To keep that transition instant, data accessed in the re-rendered subtree needs either a `use cache` lifetime or a local `<Suspense>` boundary. A boundary higher up covers the page load but not the transition.

To validate every navigation is instant, the framework provides:

{/* we are missing the global config story */}

- The [`unstable_instant`](/docs/app/api-reference/file-conventions/route-segment-config/instant) route segment config opts a layout or page into validation. Next.js then verifies navigation into the route, including client transitions, in development.
...
- DevTools to visualize loading states: the **Navigation Inspector** in the Next.js DevTools (available when Cache Components is enabled), complemented by the [React DevTools Suspense panel](https://react.dev/learn/react-developer-tools).
...
{/* prob need to be explicity that private cache executes server side, no code leaking to the client, subtle and succint */}

- [`use cache`](#usage) called with values extracted from runtime APIs (passed as arguments) joins the runtime shell
- [`use cache: private`](/docs/app/api-reference/directives/use-cache-private) reads runtime data directly and caches the result in the browser, joining the runtime shell
- [`<Suspense>`](#streaming-uncached-data) fallbacks stay in the runtime shell while uncached content streams at request time
  {/* For this last bulletpoint, once agian, as long as uncached data or random/timestamps via connection... mmm maybe too much nuance */}

This generates a **runtime shell**: a prerender that extends past the static shell with content the user's request unlocks. Because it happens during the prefetch, the navigation has nothing to wait on. The cost is a server invocation per prefetchable link.
...
On a direct visit, `<Stats>` streams in behind the fallback. When a user navigates to `/dashboard` from another route, the framework prefetches with their session cookie. `getStats` is cached, so its result joins the runtime shell before the click.
