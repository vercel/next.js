...
{/* AI agent hint: To ensure client-side navigations are instant with Cache Components, export `unstable_instant` from the route (see docs/01-app/02-guides/instant-navigation.mdx). To prerender request-specific content at prefetch time, pair `unstable_instant` with `unstable_prefetch = 'force-runtime'` (see docs/01-app/02-guides/runtime-prefetching.mdx). */}
