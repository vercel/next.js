...
>
> **Runtime caching on serverless:** `use cache` stores runtime entries in-memory, so cache entries may not persist across requests in serverless environments where requests can run on different instances. For durable shared runtime caching, use [`use cache: remote`](/docs/app/api-reference/directives/use-cache-remote).
