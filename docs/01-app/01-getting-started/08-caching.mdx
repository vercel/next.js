---
title: Caching
description: Learn how to cache data and UI with the use cache directive.
related:
  title: Next Steps
...
Cached content can be revalidated in two ways: automatically based on the cache lifetime using [`cacheLife`](/docs/app/api-reference/functions/cacheLife), or on-demand using tags. See [Revalidating](/docs/app/getting-started/revalidating) for details.
...
If data doesn't depend on [runtime data](#runtime-data), you can use `use cache` to include it in the static HTML shell. Use [`cacheLife`](/docs/app/api-reference/functions/cacheLife) to control how long the cached data is valid. See [Revalidating](/docs/app/getting-started/revalidating) for details on cache lifetimes and on-demand revalidation.
