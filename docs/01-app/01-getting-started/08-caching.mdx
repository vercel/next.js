...
Reading `cookies()` here doesn't push the whole route into dynamic rendering, the way the previous rendering model did. The Suspense boundary contains the runtime access, so static and cached content still ship in the initial HTML.
...
You don't need to memorize which operations behave this way. The framework surfaces insights when prerendering hits one, guiding you to the pattern that matches your intent.
...
Client-side transitions are different. Only the components below the shared layout re-render, so to keep that transition instant, data accessed in the re-rendered subtree needs either a `use cache` lifetime or a local `<Suspense>` boundary. A boundary higher up covers the page load but not the transition.
...
- [`use cache: private`](/docs/app/api-reference/directives/use-cache-private) executes on the server, reads runtime data directly, and caches the result in the browser, joining the runtime shell
