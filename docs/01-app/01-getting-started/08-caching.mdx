...
If your route is intentionally fully dynamic and you don't need granular `<Suspense>` boundaries, you can wrap the document body in a `<Suspense>` boundary in your Root Layout. This signals to Next.js that the entire page should defer to request time. To limit the impact to specific routes, use [multiple root layouts](/docs/app/api-reference/file-conventions/layout#root-layout).
