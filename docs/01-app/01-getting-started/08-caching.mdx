...
If you want to start using `use cache` in parts of your app without converting every dynamic component to use `<Suspense>` boundaries, you can wrap the document body in a `<Suspense>` boundary in your Root Layout. This catches any uncached dynamic data access that would otherwise require its own boundary, letting you adopt caching incrementally. To limit the impact to specific routes, use [multiple root layouts](/docs/app/api-reference/file-conventions/layout#root-layout).
...
Be aware that this disables prerendering for the route. There is no static shell to send immediately, so every request blocks until the page is fully rendered.
