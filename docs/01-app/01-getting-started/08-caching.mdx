...
## Creating a meaningful loading experience

To produce a maximally prerenderable UI, move access to data that cannot be prerendered as deep in the component tree as is reasonable. This applies to all [runtime APIs](#working-with-runtime-apis) and async operations like data fetches.
...
However, it is often possible to read the parameter value further down the tree. Instead of awaiting at the layout level, narrow the promise to the specific param you need and pass it down without awaiting:

```tsx filename="app/shop/[slug]/layout.tsx" highlight={3,10,15-16}
import { Suspense } from 'react'
...
async function SlugHeading({ slug }: { slug: Promise<string> }) {
  return <h1>{await slug}</h1>
}
```
...
### Client Components that read the current time

Client Components are also prerendered on the server. If a Client Component reads `Date.now()`, `new Date()`, or `Date()` during prerender, Next.js cannot include it in the static shell because the value would be stale by the time a user sees it.

Wrap these components in `<Suspense>` so the fallback is prerendered and the time-dependent content renders at request time:

```tsx filename="app/article.tsx"
'use client'
...
export default function Article({ publishedAt }: { publishedAt: number }) {
  return (
    <article>
      <h1>Article title</h1>
...
Alternatively, move the time access into a `useEffect` so it only runs in the browser, or cache the value with `use cache` on the server if a stale time is acceptable (for example, a copyright year). See the [current time in Client Components](/docs/messages/next-prerender-current-time-client) error reference for all available patterns.
