...
When a segment's content all depends on runtime data (cookies, headers, or fresh-on-every-request queries) and none of it can be cached, there's nothing meaningful to put in a static shell. Set [`unstable_instant = false`](/docs/app/api-reference/file-conventions/route-segment-config/instant) on just that layout or page:

```tsx filename="app/dashboard/layout.tsx"
export const unstable_instant = false
```

The navigation blocks on the server for that segment, but the rest of the app keeps its static shells.
...
To achieve instant navigations, wrap uncached or runtime data in a `<Suspense>` boundary as close as possible to where access happens. If every component in the tree either uses `use cache`, is static, or follows this rule, your application gets instant navigations out of the box.

However, during migration to Cache Components, developing new features, or refactoring, Suspense boundaries can drift higher up the component tree into shared layouts or even the root layout. This results in less meaningful loading states and, at worst, blocking client-side transitions.

Client-side navigations re-render from a shared layout down. A `<Suspense>` boundary above that shared layout will not help because it is above the re-render point. If a page reads uncached data without a local Suspense boundary, the server-side render must complete before any UI is shown, blocking the navigation.

> **Good to know:** Cold caches are typically a one-time cost, not a permanent block. The documentation on instant navigations assumes caches are warm.

### Inspecting loading states

Open the Next.js DevTools and select **Instant Navs**. The **Page load** option refreshes the page and freezes it at the initial static UI that was generated for the route. The **Client navigation** option makes every link click show the prefetched UI instead of the full result. Page loads and client navigations can produce different shells because client-side hooks like `useSearchParams` resolve synchronously on client navigations but suspend on page loads.

### Preventing Suspense drift

The [`instant`](/docs/app/api-reference/file-conventions/route-segment-config/instant) route segment config opts a route into validation for instant navigations. Next.js checks, during development and at build time, that the caching structure produces an instant shell at every possible entry point into the route.

In the example below, there is no `generateStaticParams`, so `id` is only known at request time. Awaiting `params` suspends, so each component needs its own `<Suspense>` boundary. Product details rarely change and are cached as a function of `id`. Inventory must be fresh and streams behind a fallback:

```tsx filename="app/products/[id]/page.tsx" highlight={1-3}
...
The product name is cached and part of the instant shell. The inventory count streams behind a loading state. If either component would block navigation, `instant` flags it during development and at build time.

To prevent regressions, the `@next/playwright` package includes an [`instant()`](/docs/app/api-reference/file-conventions/route-segment-config/instant#testing-instant-navigation) helper that lets you write e2e assertions for the fallback states that appear during instant navigations.

### Static and runtime prefetching

By default, components that read cookies or headers must be behind `<Suspense>`. They cannot be prefetched without the user's session data.

Opt into runtime prefetching with the [`unstable_prefetch`](/docs/app/api-reference/file-conventions/route-segment-config/prefetch) export. Next.js will then send prefetch requests with the user's actual cookies and headers, caching the response in the browser. Personalized content like a user's name or cart count can become part of the instant shell instead of streaming behind a loading state:
...
See the [`instant` API reference](/docs/app/api-reference/file-conventions/route-segment-config/instant) for all configuration options, including incremental adoption with `instant = false` and the `instant()` testing API.
