...
Cache Components lets you mix static and dynamic content in a single route, giving you the speed of static sites with the flexibility of dynamic rendering.
...
To cache an asynchronous functions that fetches data, add the `use cache` directive at the top of the function body:
...
When you need fresh data for each request (like real-time feeds or personalized content). Instead of adding `use cache`, you can wrap the component that access the data in [`<Suspense>`](https://react.dev/reference/react/Suspense) and provide fallback UI. At request time, the fallback UI will be shown first, while the content streams in:
...
> **Good to know**: For dynamic content that doesn't change frequently, you can use `use cache` to cache it instead of streaming it at request time.

## Working with runtime APIs

Runtime APIs requires request context that isonly available when a user makes a request. These include:
...
## Working with pure computations
...
- [`use cache`](#caching-data-with-use-cache): the result is cached and included in the static shell
- [`<Suspense>`](#streaming-uncached-data): fallback UI is included in the static shell while the content streams at request time
- [Synchronous operations](#working-with-synchronous-operations): like pure computations and module imports are automatically included in the static shell
...
> You can verify that a route was fully prerendered by checking the [build output summary](/docs/app/api-reference/cli/next-build#output). Alternatively, see what content was added to the static shell of any page by viewing the page source in your browser.
...
During prerendering, the header (static) and blog posts (cached with `use cache`) become part of the static shell along with the fallback UI for user preferences. Only the personalized preferences stream in at request time.
