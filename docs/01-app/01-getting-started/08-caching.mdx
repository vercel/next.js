...
When you need fresh data for each request (like real-time feeds or personalized content), wrap the component that accesses the data in [`<Suspense>`](https://react.dev/reference/react/Suspense) and provide fallback UI instead of adding `use cache`. At request time, the fallback UI will be shown first, while the content streams in:
...
- [`<Suspense>`](#stream-uncached-data): fallback UI is included in the static shell while the content streams at request time
- [Synchronous operations](#work-with-synchronous-operations): like pure computations and module imports are automatically included in the static shell
