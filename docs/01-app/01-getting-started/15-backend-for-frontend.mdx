...
`export` mode outputs a static site without a runtime server. Features that require the Next.js runtime are unsupported.

You can generate static responses for Route Handlers `GET` requests, which are compatible with `export` mode, as long as these don't access dynamic APIs.

- [Unsupported features](/app/guides/static-exports#unsupported-features)
