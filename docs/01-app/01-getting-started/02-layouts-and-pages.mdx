...
You can use the [`<Link>` component](/docs/canary/app/api-reference/components/link) to navigate between routes. `<Link>` is a built-in Next.js component that extends the HTML `<a>` tag to provide [prefetching](#2-prefetching) and [client-side navigation](#client-side-navigation).
