...
If metadata isn’t immediately ready, Next.js renders and sends the initial UI. As soon as `generateMetadata` resolves its results are streamed to the client. The streamed metadata tags are appended to the `<body>`.

Bots that can’t execute JavaScript are **opted out of this behavior**. Next.js waits for `generateMetadata` before sending the initial UI. The HTML received by these bots contains metadata in the `<head>` tag.

This improves perceived performance by reducing [TTFB](https://developer.mozilla.org/docs/Glossary/Time_to_first_byte) and often [LCP](https://developer.mozilla.org/docs/Glossary/First_contentful_paint) for users and JS-capable bots.

Statically rendered pages don’t use streaming since metadata is resolved at build time.

You can customize which user agents receive blocking metadata, or **disable streaming metadata** completely, with the [`htmlLimitedBots`](/docs/app/api-reference/config/next-config-js/htmlLimitedBots#disabling) option in your Next.js config file.
