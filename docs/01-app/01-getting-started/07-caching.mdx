---
title: Caching
description: Learn how Cache Components uses cache lifetimes to reuse data and UI and how reusable and request-time content combine in one route.
related:
  title: Next Steps
...
Every result you cache has a lifetime, which tells Next.js how long it may be reused. You can set that lifetime with [`cacheLife`](#cachelife) or use the default profile. Next.js can use it to include the result in a [prerendered shell](/docs/app/getting-started/rendering-model) or a prefetch, supporting direct visits and client navigation without repeating the work. Uncached asynchronous work remains request-time work and streams behind Suspense.

This page walks you through choosing what to cache, setting cache lifetimes, working with runtime data, and understanding how cached results are used during prerendering, prefetching, and navigation.
