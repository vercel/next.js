...
The control mechanism for choosing how much of a [Cache Components](#cache-components) route each prefetch includes. With [Partial Prefetching](#partial-prefetching), [`unstable_prefetch()`](/docs/app/api-reference/functions/prefetch) excludes subsequent work from the [App Shell](#app-shell), while [`unstable_navigation()`](/docs/app/api-reference/functions/navigation) excludes it from every prefetch. The work remains cacheable.
