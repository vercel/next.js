...
Next.js improves your application's performance and reduces costs by caching the output of functions and rendering work. This page provides an in-depth look at how caching works in Next.js.
...
| ----------------------------------------- | -------------------- | ------ | --------------------------------------- | ------------------------------- |
| [Data Cache](#data-cache)                 | Data                 | Server | Cache the return value of data requests | Persistent (can be revalidated) |
| [ISR Cache](#isr-cache)                   | HTML and RSC payload | Server | Store the render output of component    | Persistent (can be revalidated) |
| [Client Cache](#client-side-router-cache) | RSC Payload          | Client | Reduce server requests on navigation    | Browser session or time-based   |

By default, Next.js will cache as much as possible to improve performance and reduce cost. This means routes are **prerendered** and data requests are cached unless you opt out. The diagram below shows the default caching behavior: when a route is statically rendered at build time and when a static route is first visited.
...
The Data Cache is a granular cache that allows you to store the return value of functions across incoming **server requests** and **deployments**.
