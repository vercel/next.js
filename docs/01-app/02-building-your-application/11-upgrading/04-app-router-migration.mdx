...
Moving to the App Router may be the first time using React features that Next.js builds on top of such as Server Components, Suspense, and more. When combined with new Next.js features such as [special files](/docs/app/building-your-application/routing#file-conventions) and [layouts](/docs/app/getting-started/layouts-and-pages#layouts), migration means new concepts, mental models, and behavioral changes to learn.
...
- `pages/_app.js` and `pages/_document.js` have been replaced with a single `app/layout.js` root layout. [Learn more](/docs/app/getting-started/layouts-and-pages#root-layout-required).
...
Create a new `app/layout.tsx` file inside the `app` directory. This is a [root layout](/docs/app/getting-started/layouts-and-pages#root-layout-required) that will apply to all routes inside `app`.
...
Next.js recommended adding a [property to Page components](/docs/pages/building-your-application/routing/pages-and-layouts#layout-pattern#per-page-layouts) to achieve per-page layouts in the `pages` directory. This pattern can be replaced with native support for [nested layouts](/docs/app/getting-started/layouts-and-pages#layouts) in the `app` directory.
