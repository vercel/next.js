...
A Next.js [App Router](/docs/app) application must include a [root layout](/docs/app/getting-started/layouts-and-pages#root-layout-required) file, which is a [React Server Component](/docs/app/building-your-application/rendering/server-components) that will wrap all pages in your application. This file is defined at the top level of the `app` directory.
