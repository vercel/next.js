...
As below, you can add a [Suspense Boundary](/docs/app/building-your-application/routing/loading-ui-and-streaming) to break up the rendering work and show part of the result as soon as possible.

```tsx filename="app/ui/albums.tsx" switcher
async function getAlbums(username: string) {
  const res = await fetch(`https://api.example.com/artist/${username}/albums`)
...
```jsx filename="app/ui/albums.js" switcher
async function getAlbums(username) {
  const res = await fetch(`https://api.example.com/artist/${username}/albums`)
  if (!res.ok) throw new Error('Failed to fetch albums')
  const data = await res.json()
...
export default async function Albums({ username }) {
  const albums = await getAlbums(username)
  return (
    <ul>
      {albums.map((album) => (
...
```tsx filename="app/ui/artist.tsx" switcher
async function getArtist(username: string) {
  const res = await fetch(`https://api.example.com/artist/${username}/artist`)
  if (!res.ok) throw new Error('Failed to fetch artist')
  const data = await res.json()
  return data
}

export default async function Artist({ username }: { username: string }) {
  const artist = await getArtist(username)
  return <h1>{artist.name}</h1>
}
```

```jsx filename="app/ui/artist.js" switcher
async function getArtist(username) {
  const res = await fetch(`https://api.example.com/artist/${username}/artist`)
  if (!res.ok) throw new Error('Failed to fetch artist')
  const data = await res.json()
...
export default async function Artist({ username }) {
  const artist = await getArtist(username)
  return <h1>{artist.name}</h1>
}
```
...
export default async function Page({
  ...
  return (
    <>
      <Suspense fallback={<div>Artist Loading...</div>}>
...
Separate the `getArtist` and `getAlbums` functions, and initiate both requests in parallel. Wrap each function's result in its own Suspense boundary, which provides the user with the fastest possible display of results and a distinct loading state.
