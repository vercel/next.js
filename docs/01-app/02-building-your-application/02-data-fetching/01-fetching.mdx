...
Alternatively, you use [`<Suspense>`](https://react.dev/reference/react/Suspense) to break up the rendering work and stream albums and artists separately. Start by calling `getArtist` and `getAlbums` in their respective components. Then, wrap each component in its own Suspense boundary.
