...
- Enabling [nested layouts](/docs/app/getting-started/layouts-and-pages) in the same route segment level:
...
To create multiple [root layouts](/docs/app/getting-started/layouts-and-pages#root-layout-required), remove the top-level `layout.js` file, and add a `layout.js` file inside each route group. This is useful for partitioning an application into sections that have a completely different UI or experience. The `<html>` and `<body>` tags need to be added to each root layout.
