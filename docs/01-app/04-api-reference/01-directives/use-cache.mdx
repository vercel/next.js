...
- Any non-serializable arguments, props, or closed-over values won't be part of the cache key. They will be assigned opaque references that will act as placeholders in the stored cache entry, to be filled in at request time when the cache entry is read.
- The return value of the cacheable function must also be serializable, but can include non-serializable values found in the list of arguments or closed-over values, to which opaque references have been assigned. Non-serializable values found in the cached return value must have been passed through directly from an argument or closed-over value, they cannot be mutated from or be a nested property of an argument or closed-over value. This ensures that the cached data can be stored and retrieved correctly.
- For example, a cached function can take in JSX as a `children` prop and can return `<div>{children}</div>`, but it cannot return a mutated version or a nested property of the children object.
