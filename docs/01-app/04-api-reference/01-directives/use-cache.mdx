...
- Any non-serializable arguments, props, or closed-over values read from React's component rendering scope will turn into opaque references inside the cached function, and can be only passed through and not inspected nor modified. These non-serializable values will be filled in at the request time and won't become a part of the cache key.
  ...
  - Closed-over values from outside React's component rendering scope won't be turned into opaque references nor be part of the cache key. This allows to pass functions or objects needed for the cached function's computation, such as your db connection.
