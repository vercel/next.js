...
It's useful when a component doesn’t use [Dynamic APIs](/docs/app/getting-started/caching-and-revalidating#dynamic-apis), but you want it to be dynamically rendered at runtime and not statically rendered at build time. This usually occurs when you access external information that you intentionally want to change the result of a render, such as `Math.random()` or `new Date()`.
