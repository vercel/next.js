...
| ----------- | --------- | ------------ | -------- | ------------------------------------------------------------------------ |
| `default`   | 5 minutes | 15 minutes   | never    | Default profile, suitable for content that doesn't need frequent updates |
...
| `max`       | 5 minutes | 30 days      | never    | For very stable content that rarely needs updating                       |
...
> **Good to know:** The [`staleTimes`](/docs/app/api-reference/config/next-config-js/staleTimes) and [`expireTime`](/docs/app/api-reference/config/next-config-js/expireTime) config options change the `stale` and `expire` properties of the `default` profile.
