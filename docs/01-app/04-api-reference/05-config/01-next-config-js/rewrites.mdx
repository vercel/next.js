...
3. [Middleware](/docs/app/building-your-application/routing/middleware) (`rewrites`, `redirects`, etc.), if a `middleware.ts` file is present
4. `beforeFiles` rewrites are checked/applied
5. static files from the [public directory](/docs/app/building-your-application/optimizing/static-assets), `_next/static` files, and non-dynamic pages are checked/served
