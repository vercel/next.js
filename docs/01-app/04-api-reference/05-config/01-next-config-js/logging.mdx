...
By default all the incoming requests will be logged in the console during development. You can use this `incomingRequests` option and decided which requests to ignore.
Since it's only logging for development so this option doesn't affect the production build.
...
Or you can disable only incoming requests logging by setting `incomingRequests` to `false`.
