...
> - External URLs won't trigger `onNavigate` since it's only for SPA-like navigations.
> - Links with the `download` attribute will call `onClick` but not `onNavigate` since downloads use browser navigation.
