...
If upgrading to Next.js 13 generates errors related to `useRouter`, refer to the upgrade guide with the new export of the router, [`next/compat/router`](/docs/pages/api-reference/functions/use-router#the-next/compat/router-export).

This allows developers to have components that access router information in both `pages` and `app` directories.
