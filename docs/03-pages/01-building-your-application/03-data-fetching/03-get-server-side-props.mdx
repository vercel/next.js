...
When a custom `App` component is used, one must be careful with conditional logic and the rendering of `Component`.
...
In the above snippet, incoming requests, will not render the page `Component`, and only see `Loading` in the initial HTML. Depending on your use case, this might be what you want, but in other cases, it will affect what users with disabled JavaScript, as well as bots, see.

The page `Component` will be rendered, on the client, after hydration takes place.
