...
After Next.js 13, a new export `next/compat/router` is exposed to maintain compatibility with the old `next/router` API.
...
This allows developers to convert components to support running in both `app` and `pages` as they update to Next.js 13 or transition to the `app` router.
