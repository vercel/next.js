...
## `key` property to avoid duplicated tags
...
## No `body` or `head` tags

You cannot use `<Head>` to set attributes on `<head>` or `<body>` body. Doing so will result in an `next-head-count is missing` error.
