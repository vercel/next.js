...
Next.js can automatically transpile and bundle dependencies from local packages (like monorepos) or from external dependencies (`node_modules`). This replaces the `next-transpile-modules` package and is useful when working with packages that use modern JavaScript features not supported by your target browsers.
...
The `define` option allows you to statically replace variables in your code at build-time. This is useful for environment-specific configuration, feature flags, or injecting constants that should be available at runtime.
