...
Next.js supports legacy decorators and the December 2021 and March 2022 versions of the decorators proposal. Use the `compiler.decoratorVersion` option in `next.config.js` to select the transform:
...
| Value       | Description                                                                                                             |
| ----------- | ----------------------------------------------------------------------------------------------------------------------- |
| `'legacy'`  | Use the legacy decorator transform. This is the default for existing applications that enable `experimentalDecorators`. |
| `'2021-12'` | Use the December 2021 decorators proposal.                                                                              |
| `'2022-03'` | Use the March 2022 decorators proposal, which reached Stage 3.                                                          |
| `'2023-11'` | Use the November 2023 decorators proposal, which reached Stage 3.                                                       |

Setting `compiler.decoratorVersion` automatically enables decorator parsing. You do not need to set `experimentalDecorators` in `jsconfig.json` or `tsconfig.json`.

For compatibility with existing applications, Next.js also detects `experimentalDecorators` when `compiler.decoratorVersion` is not configured and uses the legacy decorator transform:
...
Legacy decorators are commonly used with older versions of libraries such as `mobx`. They are supported for compatibility with existing applications, but are not recommended for new applications.
