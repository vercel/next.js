...
module TodoList = {
  ...
  let complete = (list, target) =>
    list->Belt.Array.map(todo => todo->Todo.isSame(target) ? todo->Todo.complete : todo)
  ...
  let add = (list, todo) =>
    if !(list->contains(todo)) {
      ...
      Js.Exn.raiseError(j`Could not add todo $id, it already exists.`)
    }

  let remove = (list, todo) => list->Belt.Array.keep(current => !(todo->Todo.isSame(current)))
}
...
let useTodoReducer = () => React.useReducer(reducer, appState.contents)
