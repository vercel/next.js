...
readline.question(`Please provide the FaunaDB server key\n`, (serverKey) => {
    ...
    const options = { model: 'merge', 'uri': 'https://graphql.fauna.com/import', headers: { 'Authorization':`Bearer ${serverKey}`}} 
    const stream = fs.createReadStream('./schema.gql')
    ...
    streamToPromise(stream)
        .then((res) => {
            ...
            if(readableResult.startsWith('Invalid authorization header')){
                ...
                console.error('The secret you have provided is not valid, closing. Try again')
                return readline.close()
            }
        ...
        .then((res) => {
            ...
            .then((res) => {
                console.log('2. Successfully created role to read and write guestbook entries')
            })
            .catch((err) => {
        ...
        .then((res) => {
            ...
            .then((res) => {
                ...
                console.log('Replace the < GRAPHQL_SECRET > placehold in next.config.js with:')
