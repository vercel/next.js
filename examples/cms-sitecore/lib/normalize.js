  ...
  if (postsFromEdge4CH) {
    const posts = postsFromEdge4CH.map((p) => {
      let imageUrl = p.contentAssets?.assets?.[0]?.urls
        ? p.contentAssets?.assets?.[0]?.urls
        : null
      if (
        imageUrl === null &&
        p.contentAssets?.assets?.[0]?.assetToPublicLink?.results?.[0]
          ?.relativeUrl &&
        p.contentAssets?.assets?.[0]?.assetToPublicLink?.results?.[0]
          ?.versionHash
      ) {
        ...
        let versionHash =
          p.contentAssets?.assets?.[0]?.assetToPublicLink?.results?.[0]
            ?.versionHash
        imageUrl =
          process.env.SITECORE_CONTENTHUB_PUBLIC_URL +
          '/api/public/content/' +
          relativeUrl +
          '?v=' +
          versionHash
      ...
      let normalizedPost = {
        ...
        promo: {
          // This example to show how you can relate article to some promo
