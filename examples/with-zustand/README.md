...
To illustrate SSG, go to `/ssg` and to illustrate SSR go to `/`, those pages are using Next.js server side rendering with `revalidate = 0`, so each request will trigger a new data fetching from the server
