...
Choose this fix when the route can't provide an App Shell and you accept that navigations to it may not be instant. Setting [`instant`](https://preview.nextjs.org/docs/app/api-reference/file-conventions/route-segment-config/instant) to `false` opts the route out of instant-navigation validation.
...
Without an App Shell, navigations to this route may not be instant. Use this only when the route genuinely can't provide one.
