...
Next.js validates the route on both initial loads and client navigations, but this insight applies to the App Shell used for client-side navigation. On initial load, the same read surfaces as [runtime data during prerendering](/docs/messages/blocking-prerender-runtime) while Next.js validates the route's [static shell](/docs/app/glossary#static-shell). For URL data read through client hooks like [`useSearchParams`](/docs/app/api-reference/functions/use-search-params), see [URL data in a Client Component outside of Suspense](/docs/messages/blocking-prerender-client-hook).
...
Choose this fix when the route needs URL data high in the tree and cannot provide a shared App Shell. Navigations to the route will not be instant. Setting [`instant`](/docs/app/api-reference/file-conventions/route-segment-config/instant) to `false` allows the segment to block, so it renders per navigation instead of reusing a shared prefetch.
