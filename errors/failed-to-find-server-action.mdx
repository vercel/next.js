...
The key must be a base64-encoded value with a valid AES key length (16, 24, or 32 bytes). This environment variable must be available at both **build time** and **runtime** with the same value.
