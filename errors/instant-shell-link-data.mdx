---
title: Next.js encountered URL data outside of Suspense while extracting a reusable shell
kind: insight
---
...
Partial Prefetching extracts one reusable shell from the route ahead of the click, so every link to it reuses the same prefetch instead of fetching a fresh one per URL. `params` and `searchParams` are [URL data](https://preview.nextjs.org/docs/app/glossary#url-data): they're specific to a single URL, so reading them outside a boundary ties the shell to one link. Next.js can no longer reuse it across links, so navigations to this route may not be instant.
...
Choose this fix when the route can't share a prefetch across links and you accept that its links prefetch per URL instead. Setting [`instant`](https://preview.nextjs.org/docs/app/api-reference/file-conventions/route-segment-config/instant) to `false` opts the route out of instant-navigation validation.
...
Without a reusable shell, navigations to this route may not be instant. Use this only when the route genuinely can't provide one.
...
- `instant = false` disables all instant-navigation checks for the route, not only this one. That includes [runtime data during prerendering](/docs/messages/blocking-prerender-runtime) errors and [unrendered segment](/docs/messages/instant-unrendered-segment) warnings for the route.
- This export does not disable [Partial Prefetching](https://preview.nextjs.org/docs/app/glossary#partial-prefetching) or prefetching. It only disables instant-navigation validation for the segment.

## Verifying the fix

After applying a fix, navigate to the route and confirm the insight no longer appears in the dev overlay and the [static shell](https://preview.nextjs.org/docs/app/glossary#static-shell) renders real content. A [`<Suspense>`](https://react.dev/reference/react/Suspense) boundary around the whole page body can pass validation with an empty shell, which defeats the point of an instant navigation.
