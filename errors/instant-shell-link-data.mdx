---
title: Next.js encountered URL data while extracting a reusable shell
kind: insight
---
...
Partial Prefetching extracts one reusable shell from the route ahead of the click, so every link to it reuses the same prefetch instead of fetching a fresh one per URL. `params` and `searchParams` are specific to a single URL, so reading them outside a boundary ties the shell to one link. Next.js can no longer reuse it across links, which may cause a blocking navigation and slower, more expensive prefetches.
