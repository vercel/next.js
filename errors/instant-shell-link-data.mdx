...
Choose this fix when the URL-specific content can render after the navigation. A [`<Suspense>`](https://react.dev/reference/react/Suspense) boundary keeps the read out of the reusable shell, so every link still shares the same prefetch and only the wrapped region [streams](https://preview.nextjs.org/docs/app/glossary#streaming) in after the navigation.
...
Choose this fix when the route can't provide a reusable shell and you accept that navigations to it may not be instant. Setting [`instant`](https://preview.nextjs.org/docs/app/api-reference/file-conventions/route-segment-config/instant) to `false` opts the route out of instant-navigation validation.

### Patterns

#### Opt the page out

Add the export to the page that reads the URL data. Only that route opts out of validation.
...
When a shared layout reads the URL data, set [`instant`](https://preview.nextjs.org/docs/app/api-reference/file-conventions/route-segment-config/instant) to `false` on the layout. That layout segment opts out while descendant segments remain independently validated.
