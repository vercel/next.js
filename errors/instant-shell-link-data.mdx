...
- `instant = false` disables all instant-navigation checks for that segment, not only this one. If the same read also fails prerendering, its [runtime data during prerendering](/docs/messages/blocking-prerender-runtime) error is silenced too, along with [unrendered-segment](/docs/messages/instant-unrendered-segment) and other insights for the segment.
