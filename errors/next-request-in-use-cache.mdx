...
`unstable_prefetch()` and `unstable_navigation()` are also unavailable inside cached functions. Move the stage boundary outside the cached function, then call the cached function below it so only the deferred work is cached.
