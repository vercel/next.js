...
If used in the `pages` directory, try to update the export to [`next/compat/router`](/docs/pages/api-reference/functions/use-router#the-nextcompatrouter-export). This is the same `useRouter` hook but does not throw this error, intented to be used while migrating to Next.js 13 or from `pages` to `app`.
...
- [next/compat/router](/docs/pages/api-reference/functions/use-router#the-nextcompatrouter-export)
