...
If used in the `pages` directory, try to update the export to `next/compat/router`. This is the same `useRouter` hook but does not throw this error, intented to be used while migration from `pages` to `app`.
...
- [next/compat/router](https://github.com/vercel/next.js/pull/42502)
