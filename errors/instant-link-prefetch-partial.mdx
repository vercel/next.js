...
The route's dynamic data isn't included in the prefetch. The user sees the App Shell as soon as the link is hovered or visible, and the dynamic content streams in after navigation. The dynamic content arrives later than it would with a full prefetch; how much later depends on how long the dynamic data takes to fetch.
...
Choose this fix when you set `prefetch={true}` to warm up a frequently-visited route and you can accept fetching the dynamic data on navigation instead of ahead of time. Removing the prop lets the link use Next.js's default prefetch strategy, which under Cache Components prefetches the [App Shell](/docs/app/glossary#app-shell) only.

If you specifically want the dynamic data warmed up too, the setup is two steps:

1. Make the dynamic data cacheable with [`"use cache"`](/docs/app/api-reference/directives/use-cache). For request-bound reads like `cookies()` or `headers()`, use the [extract-and-pass pattern](/docs/app/guides/runtime-prefetching#extract-and-pass) (or [`"use cache: private"`](/docs/app/api-reference/directives/use-cache#use-cache-private) for per-user reads) so the read stays outside the cache scope.
2. Opt the route into [runtime prefetching](/docs/app/guides/runtime-prefetching) with [`prefetch = 'allow-runtime'`](/docs/app/api-reference/file-conventions/route-segment-config/prefetch). The prerender then advances past the App Shell into your newly-cached content before the click.
...
The link no longer forces a full prefetch. The user gets the App Shell on hover or when the link becomes visible, and the dynamic data is fetched at navigation time. This is the default behavior under Cache Components. If the per-visit fetch cost is unacceptable, use the two-step setup above to make the dynamic data prefetchable.
