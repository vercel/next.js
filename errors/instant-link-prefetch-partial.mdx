...
Opting in prefetches the App Shell, which already includes the route's static and cached content. To also prefetch the per-link runtime data (`params`, `searchParams`, and the full URL) and the cached content behind it, opt the route into [runtime prefetching](https://preview.nextjs.org/docs/app/guides/runtime-prefetching) with `prefetch = 'allow-runtime'`. The [prefetching guide](https://preview.nextjs.org/docs/app/guides/prefetching) covers the link-level options end to end.
