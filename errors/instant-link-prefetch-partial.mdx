...
To warm up the dynamic data without the legacy full prefetch, follow these two steps:
...
The link no longer forces a full prefetch. The user gets the App Shell on hover or when the link becomes visible, and the dynamic data is fetched at navigation time. This is the default behavior under Cache Components. To make the dynamic data prefetchable instead, follow the two-step setup above.
