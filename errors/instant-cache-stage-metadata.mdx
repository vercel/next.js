...
During prerendering or a client-side navigation, [`generateMetadata()`](/docs/app/api-reference/functions/generate-metadata) called a [Navigation Stage API](/docs/app/glossary#navigation-stages). Navigation Stage APIs include [`unstable_prefetch()`](/docs/app/api-reference/functions/prefetch) and [`unstable_navigation()`](/docs/app/api-reference/functions/navigation). The API can exclude subsequent metadata from an earlier prefetch while the rest of the route remains available.
...
Choose this fix when the metadata should be available before the point selected by the Navigation Stage API. Replace [`generateMetadata()`](/docs/app/api-reference/functions/generate-metadata) with a static [`metadata`](/docs/app/api-reference/functions/generate-metadata#metadata-object) export.
...
The route is no longer fully static. The metadata waits until the point selected by the Navigation Stage API, while the rest of the page can remain available from the earlier result.
