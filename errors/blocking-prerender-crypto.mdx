...
If you'd rather not tie the read to an HTTP request context, use [`io()`](/docs/app/api-reference/functions/io) from `next/cache`. It signals a request-time IO read and works the same way as `connection()` in the streaming case, plus inside `"use cache"` scopes and Client Components where `connection()` throws.
