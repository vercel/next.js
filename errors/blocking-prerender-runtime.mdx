...
> **Good to know**: With [Partial Prefetching](https://preview.nextjs.org/docs/app/api-reference/config/next-config-js/partialPrefetching) enabled, a `params` or `searchParams` read outside `<Suspense>` also surfaces at navigation time as [URL data outside of Suspense](/docs/messages/instant-shell-url-data). The same fix clears both.
