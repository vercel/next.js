...
During prerendering or a client-side navigation, [`generateViewport()`](/docs/app/api-reference/functions/generate-viewport) called a [Navigation Stage API](/docs/app/glossary#navigation-stages). Navigation Stage APIs include [`unstable_prefetch()`](/docs/app/api-reference/functions/prefetch) and [`unstable_navigation()`](/docs/app/api-reference/functions/navigation). The API can exclude subsequent viewport metadata from an earlier prefetch. Viewport metadata must be ready before Next.js can create the route's App Shell, so it cannot stream behind [`<Suspense>`](https://react.dev/reference/react/Suspense).

For a route with a runtime [App Shell](/docs/app/glossary#app-shell), `unstable_prefetch()` defers subsequent viewport work until a `<Link prefetch={true}>` prefetch or navigation. The `unstable_navigation()` function defers it until navigation. Both functions have no effect on initial loads or [static prerenders](/docs/app/glossary#prerendering), but prevent Next.js from completing the App Shell.
...
Choose this fix when the viewport should be available before the point selected by the Navigation Stage API. Replace [`generateViewport()`](/docs/app/api-reference/functions/generate-viewport) with a static [`viewport`](/docs/app/api-reference/functions/generate-viewport#the-viewport-object) export.
...
Choose this fix when the viewport is intentionally excluded from an earlier prefetch and no static value is correct. Setting [`instant`](/docs/app/api-reference/file-conventions/route-segment-config/instant) to `false` exempts the segment from instant-navigation validation.
...
Don't use this to dismiss the insight. Choose [Use static viewport](#use-static-viewport) when feasible.
