...
Static viewport values cannot depend on work intentionally deferred to a per-link prefetch or navigation. If the viewport must follow that lifecycle, allow the route to block.
...
Add the export to the layout that owns `generateViewport()`. This example defers the viewport until navigation. Use `unstable_prefetch()` instead when a per-link prefetch should include it.
