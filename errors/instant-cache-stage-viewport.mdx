...
Next.js reported this Insight because [`generateViewport()`](/docs/app/api-reference/functions/generate-viewport) called [`unstable_prefetch()`](/docs/app/api-reference/functions/unstable_prefetch) or [`unstable_navigation()`](/docs/app/api-reference/functions/unstable_navigation). Viewport metadata must be ready before Next.js can create the route's shell, so it cannot wait for a later cache stage or stream behind [`<Suspense>`](https://react.dev/reference/react/Suspense).

For a route with a runtime [App Shell](/docs/app/glossary#app-shell), `unstable_prefetch()` defers subsequent viewport work until a `<Link prefetch={true}>` prefetch or navigation. The `unstable_navigation()` function defers it until navigation. Both functions have no effect on initial loads or [static prerenders](/docs/app/glossary#prerendering), but prevent Next.js from completing the earlier runtime shell.
...
After applying a fix, navigate to the route and confirm the Insight no longer appears in the dev overlay. Check that the document contains the expected viewport metadata. If you use a static viewport, the route should continue to produce a shell. If you allow the route to block, confirm that behavior is intentional.
