...
For the same error in the page body or metadata code, see [Cache Stage API outside of Suspense](/docs/messages/instant-cache-stage) or [Cache Stage API in `generateMetadata()`](/docs/messages/instant-cache-stage-metadata).
...
Choose this fix when the viewport should be available before the point selected by the Cache Stage API. Replace [`generateViewport()`](/docs/app/api-reference/functions/generate-viewport) with a static [`viewport`](/docs/app/api-reference/functions/generate-viewport#the-viewport-object) export.
