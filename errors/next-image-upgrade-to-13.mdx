...
Run the [next-image-to-legacy-image](/docs/advanced-features/codemods#next-image-to-legacy-image) codemod to automatically change `next/image` imports to `next/legacy/image`, for example:
...
- [`next/legacy/image` Documentation](/docs/api-reference/next/legacy/image)
