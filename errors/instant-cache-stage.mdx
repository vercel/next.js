...
During prerendering or a client-side navigation, a Server Component called a [Cache Stage API](/docs/app/glossary#cache-stages) outside of [`<Suspense>`](https://react.dev/reference/react/Suspense). Cache Stage APIs include [`unstable_prefetch()`](/docs/app/api-reference/functions/prefetch) and [`unstable_navigation()`](/docs/app/api-reference/functions/navigation). Without a boundary, Next.js has no fallback to render when the API excludes the subtree from a prefetch.
...
The earlier prefetch contains the fallback instead of the deferred content. A per-link prefetch adds speculative server work for content below `unstable_prefetch()`, while content below `unstable_navigation()` does not begin rendering until navigation.
