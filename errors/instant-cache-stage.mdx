...
The earlier stage contains the fallback instead of the deferred content. A per-link prefetch adds speculative server work for content below `unstable_prefetch()`, while content below `unstable_navigation()` does not begin rendering until navigation.
...
Navigations through this segment are allowed to block. When they do, the destination UI waits for the server render instead of updating immediately. Use this only when that latency is the deliberate cost of the route's purpose.
