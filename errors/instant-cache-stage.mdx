...
Next.js reported this Insight because a Server Component called [`unstable_prefetch()`](/docs/app/api-reference/functions/unstable_prefetch) or [`unstable_navigation()`](/docs/app/api-reference/functions/unstable_navigation) outside of [`<Suspense>`](https://react.dev/reference/react/Suspense). Each function pauses the component tree until a later cache stage. Without a boundary, Next.js has no fallback to render before that stage.

For a route with a runtime [App Shell](/docs/app/glossary#app-shell), `unstable_prefetch()` defers subsequent content until a `<Link prefetch={true}>` prefetch or navigation. The `unstable_navigation()` function defers it until navigation. Both functions have no effect on initial loads or [static prerenders](/docs/app/glossary#prerendering), and content below them remains cacheable.
...
After applying a fix, navigate to the route and confirm the Insight no longer appears in the dev overlay. Before the deferred content resolves, the route should paint meaningful UI, with each `<Suspense>` fallback covering only the region that streams in. A [`<Suspense>`](https://react.dev/reference/react/Suspense) boundary around the whole page body can pass validation with an empty shell, which defeats the purpose of an instant navigation.
