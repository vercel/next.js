...
Next.js reported this Insight because a Server Component called a [Cache Stage API](/docs/app/glossary#cache-stages)—either [`unstable_prefetch()`](/docs/app/api-reference/functions/prefetch) or [`unstable_navigation()`](/docs/app/api-reference/functions/navigation)—outside of [`<Suspense>`](https://react.dev/reference/react/Suspense). Without a boundary, Next.js has no fallback to render when the API excludes the subtree from a prefetch.
...
For the same error in metadata or viewport code, see [Cache Stage API in `generateMetadata()`](/docs/messages/instant-cache-stage-metadata) or [Cache Stage API in `generateViewport()`](/docs/messages/instant-cache-stage-viewport).
...
- The segment intentionally has no meaningful UI before the point selected by the Cache Stage API.
