...
If your project configures custom [`pageExtensions`](/docs/app/api-reference/config/next-config-js/pageExtensions) in `next.config.js`, provide the same value to this rule so it can recognize your page files. When using this option, pass an object with optional `pagesDir` and `pageExtensions` properties.
