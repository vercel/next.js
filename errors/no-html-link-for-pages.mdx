...
If you're working in a monorepo, we recommend configuring the [`rootDir`](/docs/pages/api-reference/configuration/eslint#specifying-a-root-directory-within-a-monorepo) setting in `eslint-plugin-next`, which `pagesDir` will use to locate your `pages` directory.
