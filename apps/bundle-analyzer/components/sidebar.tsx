...
function SelectionDetails({
  ...
  const specialModuleType = getSpecialModuleType(analyzeData, selectedSourceIndex)

  const selectedSource =
    selectedSourceIndex != null ? analyzeData.source(selectedSourceIndex) : undefined

  const hasChildModules =
    selectedSourceIndex != null && analyzeData.sourceChildren(selectedSourceIndex).length > 0
  ...
  const { size, compressedSize } = analyzeData.getRecursiveSizes(selectedSourceIndex, filterSource)

  const chunks = selectedSourceIndex != null ? analyzeData.sourceChunks(selectedSourceIndex) : []
        ...
        {selectedSourceIndex != null && analyzeData.source(selectedSourceIndex) ? (
          <div className="text-xs">
            <div>
              ...
              <span className="text-muted-foreground ml-1">compressed (estimated)</span>
              <InlineHelpTooltip>
                Estimated compressed size. Modules are compressed in isolation which may differ from
                their size in the final chunk.
              </InlineHelpTooltip>
            </div>
            <div>
              ...
              <InlineHelpTooltip>
                Uncompressed modules may still be minified, tree-shaken, and dead-code eliminated.
                They just don't account for general compression like gzip.
              </InlineHelpTooltip>
            </div>
      ...
      {selectedSourceIndex != null &&
        ...
        (specialModuleType === SpecialModule.POLYFILL_MODULE ||
          ...
          <dl className="flex items-center gap-2">
            ...
            <dd className="text-xs text-muted-foreground">Next.js built-in polyfills</dd>
          </dl>
        )}
      ...
      {selectedSourceIndex != null &&
        ...
        !hasChildModules && (
          <>
            ...
            {chunks.length > 0 ? (
              <div className="mt-2">
                <p className="text-xs font-semibold text-foreground">Output Chunks</p>
