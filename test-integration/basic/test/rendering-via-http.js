...
test('renders a stateless component', async () => {
  const html = await server.fetchHTML('/stateless')
...
test('renders styled jsx', async () => {
  const $ = await server.fetch$('/styled-jsx')
...
test('should render 404 for _next routes that do not exist', async () => {
  const res = await server.fetch('/_next/abcdef')
  expect(res.status).toBe(404)
})

test('should expose the compiled page file in development', async () => {
  await server.fetch('/stateless') // make sure the stateless page is built
  const clientSideJsRes = await server.fetch('/_next/development/static/development/pages/stateless.js')
  ...
  const serverSideJsRes = await server.fetch('/_next/development/server/static/development/pages/stateless.js')
...
test('error-in-the-global-scope', async () => {
  const $ = await server.fetch$('/error-in-the-global-scope')
...
it('should set Cache-Control header', async () => {
  ...
  // build dynamic page
  await server.fetch('/dynamic/ssr')

  const buildManifest = require(join(__dirname, '../.next', BUILD_MANIFEST))
  const reactLoadableManifest = require(join(__dirname, '../.next', REACT_LOADABLE_MANIFEST))
  const resources = []
  ...
  const responses = await Promise.all(resources.map((resource) => server.fetch(resource)))
...
describe('Url prop', () => {
  ...
  it('should override props.url, even when getInitialProps returns url as property', async () => {
    const $ = await server.fetch$('/url-prop-override')
...
describe('with the HOC based router', () => {
  it('should navigate as expected', async () => {
    const $ = await server.fetch$('/nav/with-hoc')
