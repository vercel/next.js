...
impl SingleModuleGraph {
    ...
    fn traverse_cycles<'l>(
        ...
        let mut index = 0;
        let mut scc = Vec::new();
        // The nodes of `scc`, in the same order, so that the entry points can be matched back to
        // the modules once the component's membership is known.
                    ...
                    VisitStep::AfterVisit(node) => {
                        ...
                        if node_state.lowlink == node_state.index {
                            ...
                            if scc.len() > 1 || node_has_self_loop {
                                ...
                                for (module, &scc_node) in scc.iter().zip(scc_nodes.iter()) {
