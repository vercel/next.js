...
impl MergedModuleInfo {
    ...
    pub fn should_replace_module(
        ...
        Ok(turbo_tasks::read!(self.replacements.get(&module))?.as_deref().copied())
    }
    }
...
fn compute_merged_modules_span() -> tracing::Span {
    tracing::info_span!(
...
pub async fn compute_merged_modules(module_graph: Vc<ModuleGraph>) -> Result<Vc<MergedModuleInfo>> {
    let span_outer = compute_merged_modules_span();
    let span = span_outer.clone();
    compute_merged_modules_inner(module_graph, &span)
        .instrument(span_outer)
        .await
}

#[cfg(feature = "sync")]
pub fn compute_merged_modules(module_graph: Vc<ModuleGraph>) -> Result<Vc<MergedModuleInfo>> {
    let span_outer = compute_merged_modules_span();
    let span = span_outer.clone();
...
turbo_tasks::dual_fn! {
fn compute_merged_modules_inner(
    ...
    let mergeable_candidates = module_graph
        .iter_reachable_modules()?
        .filter_map(ResolvedVc::try_downcast::<Box<dyn MergeableModule>>)
        .collect::<Vec<_>>();
    #[cfg(not(feature = "sync"))]
    let mergeable_flags = async {
        turbo_tasks::parallel!(mergeable_candidates.iter().map(|m| m.is_mergeable()))
    }
    ...
    let mergeable = mergeable_candidates
        .iter()
        .zip(mergeable_flags)
        .filter_map(|(&module, flag)| {
            (*flag).then_some(ResolvedVc::upcast::<Box<dyn Module>>(module))
        })
        .collect::<FxHashSet<_>>();
    ...
    let mergeable_list = mergeable.iter().copied().collect::<Vec<_>>();
    #[cfg(not(feature = "sync"))]
    let async_flags = async {
        mergeable_list
            .iter()
            .map(|&module| async_module_info.is_async(module))
            .try_join()
            .await
    }
    .instrument(inner_span)
    ...
    let async_modules: FxHashSet<_> = mergeable_list
        .iter()
        .zip(async_flags)
        .filter_map(|(&module, is_async)| is_async.then_some(module))
        .collect();
        ...
        lists
            ...
            .map(|list| {
                merge_module_list(
                    list,
                    &intra_group_references_rev,
                    &exposed_modules_imported,
                    &exposed_modules_namespace,
                )
            })
            .try_join()
            .await
...
turbo_tasks::dual_fn! {
/// Merges one execution-ordered list of mergeable modules into a single replacement module
/// (via `MergeableModule::merge`). Returns `None` for lists with fewer than two entries,
/// otherwise `(original_entry, replacement, included_modules)`.
fn merge_module_list(
    list: Vec<ResolvedVc<Box<dyn MergeableModule>>>,
    intra_group_references_rev: &FxIndexMap<
        ResolvedVc<Box<dyn Module>>,
        FxIndexSet<ResolvedVc<Box<dyn Module>>>,
    >,
    exposed_modules_imported: &FxHashSet<ResolvedVc<Box<dyn Module>>>,
    exposed_modules_namespace: &FxHashSet<ResolvedVc<Box<dyn Module>>>,
) -> Result<MergedGroup> {
    if list.len() < 2 {
        // Nothing to merge
        return Ok(None);
    ...
    let result = turbo_tasks::read!(entry
        .merge(
