...
impl ModuleExportUsage {
    ...
    pub async fn all() -> Result<Vc<Self>> {
        Ok(Self {
            export_usage: turbo_tasks::read!(ModuleExportUsageInfo::all().to_resolved())?,
...
impl BindingUsageInfo {
    ...
    pub fn used_exports(
        ...
        let is_circuit_breaker =
            turbo_tasks::read!(self.export_circuit_breakers.contains_key(&module))?;
        let Some(exports) = turbo_tasks::read!(self.used_exports.get(&module))? else {
            // There are some module that are codegened, but not referenced in the module graph,
            let ident = turbo_tasks::read!(module.ident_string())?;
...
pub async fn compute_binding_usage_info(
    ...
    #[cfg(not(feature = "sync"))]
    return compute_binding_usage_info_inner(graph, remove_unused_imports, &span)
        .instrument(span_outer)
        .await;
    #[cfg(feature = "sync")]
    {
        let _guard = span_outer.enter();
        compute_binding_usage_info_inner(graph, remove_unused_imports, &span)
    }
}

turbo_tasks::dual_fn! {
fn compute_binding_usage_info_inner(
    graph: OperationVc<ModuleGraph>,
    remove_unused_imports: bool,
    span: &tracing::Span,
) -> Result<Vc<BindingUsageInfo>> {
    ...
    let side_effect_free_modules = if remove_unused_imports {
        let side_effect_free_modules = turbo_tasks::read!(compute_side_effect_free_module_info(graph))?;
    ...
    #[cfg(debug_assertions)]
    {
        ...
        if *PRINT_UNUSED_REFERENCES {
            let entries = debug_unused_references_name.iter().collect::<Vec<_>>();
            let source_idents =
                turbo_tasks::parallel!(entries.iter().map(|(s, _, _)| s.ident_string()))?;
            let target_idents =
                turbo_tasks::parallel!(entries.iter().map(|(_, _, t)| t.ident_string()))?;
            println!(
                "unused references: {:#?}",
                entries
                    .iter()
                    .zip(source_idents.into_iter().zip(target_idents))
                    .map(|((_, e, _), (s, t))| (s, e, t))
                    .collect::<Vec<_>>()
            );
        }
        ...
        if *PRINT_USED_EXPORTS {
            let entries = used_exports.iter().collect::<Vec<_>>();
            let idents = turbo_tasks::parallel!(entries.iter().map(|(m, _)| m.ident_string()))?;
            println!(
                "used exports: {:#?}",
                idents
                    .into_iter()
