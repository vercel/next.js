...
use turbo_tasks::{FxIndexMap, FxIndexSet, MappedReadRef, ReadRef, ResolvedVc, Vc};
...
/// The per-item lookup result of [`prepare_chunk_item`]: the chunk groups of a chunk
/// item (or the read batch, whose `chunk_groups` are used directly).
enum Prepared {
...
turbo_tasks::dual_fn! {
/// Per-item body of the `prepared` fan-out in [`make_production_chunks`].
fn prepare_chunk_item(
    item: &ChunkItemOrBatchWithInfo,
    module_chunk_groups: Vc<ModuleToChunkGroups>,
    merged_modules: &MergedModuleInfo,
) -> Result<Prepared> {
    Ok(match item {
        &ChunkItemOrBatchWithInfo::ChunkItem {
            ...
            if let Some(module_chunk_groups) =
                turbo_tasks::read!(module_chunk_groups.get(&ResolvedVc::upcast(module)))?
            {
                module_chunk_groups
            } else {
                ...
                let original_module =
                    turbo_tasks::read!(merged_modules.get_original_module(ResolvedVc::upcast(module)))?
                        .context("every module should have a chunk group")?;
                turbo_tasks::read!(module_chunk_groups.get(&original_module))?
                    .context("every module should have a chunk group")?
            },
        ),
        &ChunkItemOrBatchWithInfo::ChunkItem {
        ...
        ChunkItemOrBatchWithInfo::Batch { batch, .. } => {
            Prepared::Batch(turbo_tasks::read!(batch)?)
        }
    })
}
}
...
pub fn make_production_chunks(
    ...
    let merged_modules = turbo_tasks::read!(module_graph.merged_modules())?;
    ...
    let prepared = turbo_tasks::read!(parallel_reads(
        chunk_items
            .iter()
            .copied()
            .map(|item| prepare_chunk_item(item, module_chunk_groups, &merged_modules))
    ))?;
    ...
    let batch_group_read_refs = turbo_tasks::parallel!(batch_groups.iter())?;
                ...
                if let Some(smallest) = heap.peek() {
                    ...
                    let too_many_chunks = max_chunk_count_per_group != 0
                        && heap.len() + chunks_to_merge_size / merge_threshold + 1
                            > max_chunk_count_per_group;
                    let too_small_chunk =
                        min_chunk_size != 0 && smallest.size < min_chunk_size;
            ...
            while chunks_to_merge.len() > 1 {
                ...
                while let Some(candidate) = chunks_to_merge.pop() {
                    ...
                    for (i, other) in selection.iter().enumerate() {
                        ...
                        // If the candidate is already big enough, avoid shrinking the
                        // sharing
                        if is_big_candidate && overlap != candidate.chunk_groups_len() {
                            continue;
                        }
                        if other.size > merge_threshold
                            && overlap != other.chunk_groups_len()
                        {
                            continue;
                        }
                        ...
                        */
                            ...
                            best_combination.as_mut()
                        {
                            if (overlap.cmp(best_overlap))
                                .then_with(|| value.cmp(best_value))
                ...
                for unused in selection {
                    ...
                    if unused.size > merge_threshold
                        && unused.chunk_groups_len() > best_overlap
                    {
