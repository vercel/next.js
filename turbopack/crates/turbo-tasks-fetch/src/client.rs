...
/// HTTP fetch in the no-tokio `sync` build. `reqwest` is built on hyper+tokio, so
/// rather than reimplement an HTTP client we keep the async `reqwest` path and
/// drive it to completion on a small dedicated runtime via `block_on` — the same
/// tokio-at-the-I/O-edge bridge the node-eval pool uses. The turbo-tasks core
/// stays synchronous; only the socket work runs on this runtime, and the
/// surrounding `Vc` construction happens on the calling (contexted) thread.
///
/// A one-shot `next build` does not track dependencies, so the async build's
/// TTL-timer / cross-session cache machinery (its `fetch_inner`/`fetch` two-task
/// split) is unnecessary here — this is a single request that builds the result.
#[cfg(feature = "sync")]
mod sync_impl {
    ...
    /// Dedicated tokio runtime hosting `reqwest` at the I/O edge (see module docs).
    fn edge_rt() -> &'static tokio::runtime::Runtime {
        static EDGE_RT: std::sync::OnceLock<tokio::runtime::Runtime> = std::sync::OnceLock::new();
        // A current-thread runtime is enough for a leaf HTTP request and avoids
        // requiring tokio's `rt-multi-thread` feature (reqwest only pulls `net`/
        // `time`/`rt`). `block_on` drives the request on the calling thread.
        EDGE_RT.get_or_init(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("failed to build turbo-tasks-fetch edge runtime")
        })
    }

    impl FetchClientConfig {
        fn try_get_cached_reqwest_client(
    ...
    impl FetchClientConfig {
        /// Fetches `url` and returns the response, driving the async `reqwest`
        /// request on the edge runtime. `block_on` polls on the calling
        /// sync-engine thread (which holds the turbo_tasks context), so the `Vc`
        /// construction below resolves correctly.
        ...
        pub fn fetch(
            ...
            // `block_in_place` keeps the sync worker pool from silently draining while this
            // worker parks on the HTTP round-trip (see `turbo_tasks::block_in_place`).
            let outcome: reqwest::Result<(u16, Vec<u8>)> = turbo_tasks::block_in_place(|| {
                edge_rt().block_on(async move {
                    let client = this.try_get_cached_reqwest_client()?;
                    let mut builder = client.get(url_ref.as_str());
...
mod async_impl {
    ...
    impl FetchClientConfig {
        /// Returns a cached instance of `reqwest::Client` it exists, otherwise constructs a new
        /// one.
        ///
        /// The cache is bound in size to prevent accidental blowups or leaks. However, in
        /// practice, very few clients should be created, likely only when the bundler
    ...
    struct FetchInnerResult {
        ...
        /// Invalidator for the `fetch_inner` task. Used by the outer `fetch` to set up a timer
        /// that triggers re-fetching when the Cache-Control max-age expires.
        ...
        /// instead of a relative duration so that the remaining TTL is correct on warm cache
        /// restore.
    ...
    #[turbo_tasks::value_impl]
    impl FetchClientConfig {
        /// Performs the actual HTTP request. This task is `network` but NOT `session_dependent`,
        /// so its cached result survives restarts. The outer `fetch` task (which IS
        /// `session_dependent`) reads the cached invalidator and sets up a timer for TTL-based
        /// re-fetching.
        ...
        async fn fetch_inner(
            ...
            match response_result {
                ...
                Err(err) => {
                    ...
                    // restore. This ensures transient errors (network down, DNS failure) are
                    // retried on the next session without a timer or busy-loop.
                    Completion::session_dependent().await?;
                    Ok(FetchInnerResult {
        ...
        /// - `fetch` (session_dependent): always re-executes on restore, reads the cached inner
        ///   result, and spawns a timer for mid-session TTL expiry.
        /// - `fetch_inner` (network, NOT session_dependent): performs the actual HTTP request and
        ///   stays cached across restarts. Returns an `Invalidator` that the outer task uses to
        ///   trigger re-fetching when the TTL expires.
        #[turbo_tasks::function(network, session_dependent)]
        pub async fn fetch(
            self: Vc<FetchClientConfig>,
            url: RcStr,
                ...
                && let (Some(deadline_secs), Some(invalidator)) = (deadline_secs, invalidator)
            {
                ...
                if let Ok(now) = SystemTime::now().duration_since(SystemTime::UNIX_EPOCH) {
                    let remaining =
                        Duration::from_secs(deadline_secs.saturating_sub(now.as_secs()));
                    // NOTE: in the case where the deadline is expired on session start this
                    // timeout will immediately invalidate and race with us returning.  This is
                    ...
                    // end up with multiple timers.  Currently there is no known case where this
                    // could happen, if it somehow does we could end up with redundant
                    // invalidations and re-fetches.  The solution is to detect this with a
                    // mutable hash map on FetchClientConfig to track outstanding timers and
                    // cancel them.
