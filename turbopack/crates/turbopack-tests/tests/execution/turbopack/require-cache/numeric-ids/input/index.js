// Same suite as `../basic`, but built with numeric module ids (see
// options.json), which is what production builds use. Property keys reach a
// `Proxy` trap as strings, so the shim has to map `"123"` back onto the `123`
// key the `Map` actually holds.

it('should be running with numeric module ids', () => {
  // Guards against this suite silently degrading into a copy of `../basic`
  // if the id strategy ever stops applying.
