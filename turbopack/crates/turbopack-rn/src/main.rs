...
use turbopack_rn::arguments::Arguments;
...
async fn main_inner(args: Arguments) -> Result<()> {
    ...
    match args {
        Arguments::Build(args) => turbopack_rn::build::build(&args).await,
        Arguments::Dev(args) => turbopack_rn::dev::start_server(&args).await,
    }
}
