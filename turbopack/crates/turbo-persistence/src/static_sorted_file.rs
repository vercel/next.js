...
impl quick_cache::Weighter<(u32, u16), ArcBytes<'static>> for BlockWeighter {
    fn weight(&self, _key: &(u32, u16), val: &ArcBytes<'static>) -> u64 {
        if val.is_mmap_backed() {
            // The mapping owns the data; only account for the cached slice metadata.
            64
        } else {
            val.len() as u64 + 8
        }
...
/// Prevents eviction of cache entries still referenced outside the cache.
#[derive(Clone, Default)]
...
impl Lifecycle<(u32, u16), ArcBytes<'static>> for BlockCacheLifecycle {
    ...
    fn is_pinned(&self, _key: &(u32, u16), value: &ArcBytes<'static>) -> bool {
        value.is_shared_arc()
    }
...
impl ValueBlockCache for Option<(u16, ArcBytes<'static>)> {
    fn get_or_read(
        &mut self,
        sst: &StaticSortedFile,
        block_index: u16,
...
impl StaticSortedFile {
    ...
    pub fn lookup<K: QueryKey, const FIND_ALL: bool>(
        ...
        let mut value_block_cache = value_block_cache;
        loop {
    ...
    fn lookup_key_block<K: QueryKey, const FIND_ALL: bool>(
        ...
        key: &K,
        has_hash: bool,
        reader: &mut impl ValueBlockCache,
    ) -> Result<SstLookupResult> {
    ...
    fn lookup_fixed_key_block<K: QueryKey, const FIND_ALL: bool>(
        ...
        let header_type = block.read_u8()?;
        let (value_type, val_size) = if header_type == FIXED_KEY_BLOCK_MIXED_VALUE_TYPE {
            let value_size = block.read_u8()? as usize;
            (None, value_size + 1)
        } else {
            (Some(header_type), entry_val_size(header_type)?)
        };
    ...
    fn handle_key_match(
        ...
        value_block_cache: &mut impl ValueBlockCache,
    ) -> Result<LookupValue> {
        Ok(match ty {
            ...
            ty if ty >= KEY_BLOCK_ENTRY_TYPE_KEY_VALUE_DELETED_MIN => {
                let value = unsafe { key_block_arc.slice_from_subslice(val) }.into_static();
    ...
    fn enter_block(&mut self, block_index: u16) -> Result<()> {
        ...
        match block_type {
            ...
            BLOCK_TYPE_FIXED_KEY_WITH_HASH | BLOCK_TYPE_FIXED_KEY_NO_HASH => {
                ...
                let key_size = block.read_u8()? as usize;
                let header_type = block.read_u8()?;
                let layout = fixed_value_layout(&block_arc, header_type)?;
                let stride = hash_len as usize + key_size + layout.val_size;
                let entries_range = layout.header_size..block_arc.len();
                let entries = block_arc.slice(entries_range);
                self.current_key_block = Some(CurrentKeyBlock {
...
fn get_fixed_key_entry<'l>(
    ...
    let (ty, val_start) = match value_type {
        Some(ty) => (ty, key_end),
        None => (entries[key_end], key_end + 1),
    };
    Ok(GetKeyEntryResult {
