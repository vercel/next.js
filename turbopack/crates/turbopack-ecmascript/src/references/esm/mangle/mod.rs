...
/// on one of them would collide with it.
///
/// Only `__esModule` qualifies: `esm()` in the runtime does
/// `defineProp(exports, '__esModule', …)` for every ESM module, so that property is always there,
/// whatever the module's own exports are called. `default` is *not* here — nothing defines a
/// `default` property on a module that reaches this point, since the runtime paths that look it up
/// by name (the CommonJS interop in `esmNamespaceObject`, the proxy traps behind
/// `ensureDynamicExports`) only see CommonJS targets and modules with dynamic re-exports, and both
/// of those keep their original names via the checks below.
const RESERVED_KEYS: &[&str] = &["__esModule"];
...
pub async fn mangled_export_names(
    ...
    // name is just an ordinary export. Only `__esModule` stays in `RESERVED_KEYS`, because the
    // runtime defines that property on the exports object regardless; see there.
    let names = expanded
        .exports
        .iter()
        .map(|(name, _)| name)
