...
use std::{collections::hash_map::Entry, fmt::Display, mem::take};
    ...
    F: 'l + Fn(JsValue<'a>) -> RF + Sync,
{
    ...
    let (val, steps) = turbo_tasks::read!(link_internal_iterative(
        arena,
        graph,
        val,
        early_visitor,
...
/// Sync twin of the async `link` above (hand-written pair because `dual_fn!` does not
/// parse lifetimes/`where` clauses). The visitor return types are bounded by
/// [`SyncRead`](turbo_tasks::macro_helpers::SyncRead) instead of [`Future`], so callers
/// pass plain closures returning `Result` (which implements `SyncRead` by identity).
/// Visits happen inline and sequentially under sync.
#[cfg(feature = "sync")]
pub fn link<'a, 'l, B, RB, F, RF>(
    arena: &'a ThreadLocal<Bump>,
    graph: &'l VarGraph<'a>,
    mut val: JsValue<'a>,
    early_visitor: &'l B,
...
where
    RB: 'l + turbo_tasks::macro_helpers::SyncRead<Output = Result<(JsValue<'a>, bool)>> + Send,
    B: 'l + Fn(JsValue<'a>) -> RB + Sync,
    RF: 'l + turbo_tasks::macro_helpers::SyncRead<Output = Result<(JsValue<'a>, bool)>> + Send,
    F: 'l + Fn(JsValue<'a>) -> RF + Sync,
{
    val.normalize(arena.get_or_default());
    let (val, steps) = turbo_tasks::read!(link_internal_iterative(
    ...
    while let Some(step) = work_queue_stack.pop() {
        ...
        match step {
            ...
            Step::EarlyVisit(mut val) => {
                ...
                let (mut val, visit_modified) = turbo_tasks::read!(early_visitor(val))?;
                val.debug_assert_total_nodes_up_to_date();
...
/// Sync twin of the async `link_internal_iterative` above (hand-written pair; see
/// `link`). Identical body — every await point is a `read!` that reads inline under
/// sync.
#[cfg(feature = "sync")]
pub(crate) fn link_internal_iterative<'a, 'l, B, RB, F, RF>(
    arena: &'a ThreadLocal<Bump>,
    graph: &'l VarGraph<'a>,
    val: JsValue<'a>,
    early_visitor: &'l B,
    ...
    F: 'l + Fn(JsValue<'a>) -> RF + Sync,
{
    *total_nodes -= val.total_nodes();
    ...
    let (mut val, visit_modified) = turbo_tasks::read!(visitor(val))?;
    if visit_modified {
        val.normalize_shallow(arena.get_or_default());
...
/// Sync twin of the async `visit` above (hand-written pair; see `link`).
#[cfg(feature = "sync")]
fn visit<'a, 'l, F, RF>(
    arena: &'a ThreadLocal<Bump>,
    total_nodes: &mut u32,
