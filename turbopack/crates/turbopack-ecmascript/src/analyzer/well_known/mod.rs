...
pub async fn replace_well_known<'a>(
    ...
    Ok(match value {
        ...
        JsValue::Call(total, call) => {
            ...
            {
                return Ok((call.args()[0].clone_in(arena), true));
            }
        ...
        JsValue::Member(_, mut obj, mut prop) if matches!(&*obj, JsValue::WellKnownFunction(_)) => {
            ...
            well_known_function_member(arena, kind, take(&mut *prop))
        }
...
fn object_assign<'a>(arena: &'a Arena, args: BumpVec<'a, JsValue<'a>>) -> JsValue<'a> {
...
fn path_join<'a>(arena: &'a Arena, args: BumpVec<'a, JsValue<'a>>) -> JsValue<'a> {
    if args.is_empty() {
        return rcstr!(".").into();
    }
    ...
    for part in iter {
        ...
        if last_is_str && is_str {
            ...
            results.push(JsValue::alternatives(
                arena.vec_from_iter([rcstr!("/").into(), rcstr!("").into()]),
            ));
        }
        results.push(part);
        last_is_str = is_str;
...
fn path_resolve<'a>(
    ...
    for (idx, arg) in args.iter().enumerate().rev() {
        if idx != 0
            ...
            && str.starts_with('/')
        {
            return path_resolve(arena, cwd, arena.vec_from_iter(args.drain(idx..)));
        }
    }
...
pub fn import<'a>(arena: &'a Arena, args: BumpVec<'a, JsValue<'a>>) -> JsValue<'a> {
    match &args[..] {
...
fn require_context_require_keys<'a>(
    ...
    Ok(if args.is_empty() {
        JsValue::array(arena.vec_from_iter(val.0.keys().cloned().map(|k| k.into())))
    } else {
        JsValue::unknown(
            JsValue::call_from_parts(
                arena,
...
async fn well_known_object_member<'a>(
    ...
    let new_value = match kind {
        WellKnownObjectKind::GlobalObject => global_object(arena, prop),
        ...
        WellKnownObjectKind::ImportMeta => match prop.as_str() {
            ...
            _ => {
                return Ok((
                    JsValue::member(arena, JsValue::WellKnownObject(kind), prop),
                    false,
                ));
            }
        },
...
fn global_object<'a>(arena: &'a Arena, prop: JsValue<'a>) -> JsValue<'a> {
    match prop.as_str() {
...
fn node_pre_gyp<'a>(arena: &'a Arena, prop: JsValue<'a>) -> JsValue<'a> {
    match prop.as_str() {
...
fn express<'a>(arena: &'a Arena, prop: JsValue<'a>) -> JsValue<'a> {
    match prop.as_str() {
...
fn protobuf_loader<'a>(arena: &'a Arena, prop: JsValue<'a>) -> JsValue<'a> {
    match prop.as_str() {
