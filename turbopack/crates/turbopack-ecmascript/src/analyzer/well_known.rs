...
pub async fn well_known_function_call(
    ...
    Ok(match kind {
        ...
        WellKnownFunctionKind::PathResolve(cwd) => path_resolve(Arc::unwrap_or_clone(cwd), args),
        ...
        WellKnownFunctionKind::ProcessCwd => {
                ...
                && let Some(cwd) = &*compile_time_info.environment().cwd().await?
            {
                ...
                JsValue::unknown(
                    JsValue::call(Arc::new(JsValue::WellKnownFunction(kind)), args),
                    true,
...
pub fn import(args: Vec<JsValue>) -> JsValue {
    match &args[..] {
        ...
        _ => JsValue::unknown(
            JsValue::call(
                Arc::new(JsValue::WellKnownFunction(WellKnownFunctionKind::Import)),
                args,
            ),
            true,
...
fn require(args: Vec<JsValue>) -> JsValue {
    if args.len() == 1 {
        if let Some(s) = args[0].as_str() {
            ...
            JsValue::unknown(
                JsValue::call(
                    Arc::new(JsValue::WellKnownFunction(WellKnownFunctionKind::Require)),
                    args,
                ),
                true,
...
fn well_known_function_member(kind: WellKnownFunctionKind, prop: JsValue) -> (JsValue, bool) {
    let new_value = match (kind, prop.as_str()) {
        ...
        (kind, _) => {
            return (
                JsValue::member(Arc::new(JsValue::WellKnownFunction(kind)), Arc::new(prop)),
                false,
            );
        }
    };
...
async fn well_known_object_member(
    ...
    let new_value = match kind {
        ...
        _ => {
            return Ok((
                JsValue::member(Arc::new(JsValue::WellKnownObject(kind)), Arc::new(prop)),
                false,
            ));
        }
    };
...
fn path_module_member(kind: WellKnownObjectKind, prop: JsValue) -> JsValue {
    match (kind, prop.as_str()) {
        ...
        (.., Some("resolve")) => {
            ...
            JsValue::WellKnownFunction(WellKnownFunctionKind::PathResolve(Arc::new(JsValue::from(
                "",
            ))))
        }
