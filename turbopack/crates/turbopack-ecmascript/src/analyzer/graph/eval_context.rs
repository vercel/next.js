...
///
/// This is stored inside the (cached, `'static`) [`ParseResult`][crate::parse::ParseResult], so it
/// must NOT borrow a per-analysis arena. Methods that build [`JsValue`]s instead take an
/// `arena: &'a Arena` parameter and return `JsValue<'a>` tied to that arena's lifetime.
#[derive(Debug)]
...
impl EvalContext {
    ...
    pub(super) fn eval_prop_name<'a>(&self, arena: &'a Arena, prop: &PropName) -> JsValue<'a> {
        match prop {
    ...
    fn eval_tpl<'a>(&self, arena: &'a Arena, e: &Tpl, raw: bool) -> JsValue<'a> {
        ...
        match values.len() {
            ...
            _ => JsValue::concat(arena.vec_from_iter(values)),
        }
    }

    pub(super) fn eval_ident<'a>(&self, arena: &'a Arena, i: &Ident) -> JsValue<'a> {
        let id = i.to_id();
    ...
    pub fn eval<'a>(&self, arena: &'a Arena, e: &Expr) -> JsValue<'a> {
        debug_assert!(
            GLOBALS.is_set(),
        ...
        match e {
            ...
            Expr::Bin(BinExpr {
                ...
                arena.vec_from_iter([self.eval(arena, left), self.eval(arena, right)]),
            ),
            ...
            Expr::Call(CallExpr {
                ...
                let args = arena.vec_from_iter(args.iter().map(|arg| self.eval(arena, &arg.expr)));
            ...
            Expr::Array(arr) => {
                ...
                let arr = arena.vec_from_iter(arr.elems.iter().map(|e| match e {
            ...
            Expr::Object(obj) => {
                JsValue::object(arena.vec_from_iter(obj.props.iter().map(|prop| match prop {
    ...
    pub fn eval_single_expr_lit<'a>(arena: &'a Arena, expr_lit: &RcStr) -> Result<JsValue<'a>> {
