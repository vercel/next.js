        ...
        pub(super) fn enter_fn(
            ...
            JsValue::function(
                ...
                match return_values.len() {
                    ...
                    _ => JsValue::alternatives(arena.vec_from_iter(return_values)),
                },
            )
        }
...
impl<'a, 'e> Analyzer<'a, 'e> {
        ...
        match callee {
            ...
            Callee::Expr(box expr) => {
                if let Expr::Member(MemberExpr { obj, prop, .. }) = unparen(expr) {
                    let obj_value = self.arena.alloc(self.eval_context.eval(self.arena, obj));
                    let prop_value = match prop {
                        // TODO avoid clone
                        MemberProp::Ident(i) => self.arena.alloc(i.sym.clone().into()),
                        MemberProp::PrivateName(_) => self.arena.alloc(JsValue::unknown_empty(
                            false,
                            rcstr!("private names in member expressions are not supported"),
                        )),
                        MemberProp::Computed(ComputedPropName { expr, .. }) => {
                            self.arena.alloc(self.eval_context.eval(self.arena, expr))
                        }
                    };
                    ...
                    let fn_value = self.arena.alloc(self.eval_context.eval(self.arena, expr));
    ...
    fn check_member_expr_for_effects<'ast: 'r, 'r>(
        ...
        let obj_value = self
            .arena
            .alloc(self.eval_context.eval(self.arena, &member_expr.obj));
...
impl<'a, 'e> VisitAstPath for Analyzer<'a, 'e> {
        ...
        // LHS
        {
            ...
            let pat_value = match (n.op, n.left.as_ident()) {
                ...
                (AssignOp::AddAssign, Some(key)) => {
                    ...
                    JsValue::add(self.arena.vec_from_iter([left, right]))
                }
    ...
    fn visit_var_declarator<'ast: 'r, 'r>(
        ...
        // LHS
        {
                ...
                && let Some(init) = &n.init
            {
                ...
                let pat_value =
                    Some(if should_include_undefined {
                        JsValue::alternatives(self.arena.vec_from_iter([
                            init_value,
    ...
    fn visit_unary_expr<'ast: 'r, 'r>(
        ...
        if n.op == UnaryOp::TypeOf && self.analyze_mode.is_code_gen() {
            let arg_value = self.arena.alloc(self.eval_context.eval(self.arena, &n.arg));
    ...
    fn visit_labeled_stmt<'ast: 'r, 'r>(
        ...
        prev_effects.push(Effect::Conditional {
            condition: self
                .arena
                .alloc(JsValue::unknown_empty(true, rcstr!("labeled statement"))),
...
impl<'a, 'e> Analyzer<'a, 'e> {
    fn add_conditional_if_effect_with_early_return(
        ...
        let condition = self.arena.alloc(self.eval_context.eval(self.arena, test));
