...
//! [`AstParentKind`]s leading to it from the program root. Stored flat, those paths can be
...
//! nests one level deeper. Pathological, often generated, code can hit this.  This module removes
//! the redundancy by storing
//!
...
//! ```
//!
//! Silly but the binding usage location for `a` and `b` share a common prefix.  That prefix can get
//! arbitrarily long and this structure fixes.
//!
...
impl AstPathTrieBuilder {
    ...
    fn push(&mut self, parent: AstPathId, kind: AstParentKind) -> AstPathId {
...
mod tests {
    ...
    fn to_vec(trie: &AstPathTrie, id: AstPathId) -> Vec<AstParentKind> {
        let mut vec: Vec<_> = trie.iter_rev(id).collect();
        vec.reverse();
        vec
    }
    ...
    fn root_is_empty() {
        ...
        assert_eq!(to_vec(&trie, AstPathId::ROOT), vec![]);
    ...
    fn walks_up_from_a_leaf() {
        ...
        assert_eq!(to_vec(&trie, parent), vec![module_body(0), module_item()]);
        assert_eq!(
    ...
    fn finds_the_innermost_match() {
        ...
        let found = trie
            ...
            .unwrap();
        assert_eq!(to_vec(&trie, found), vec![module_body(0), expr_bin()]);
    ...
    fn encodes_and_decodes() {
        ...
        assert_eq!(to_vec(&decoded, id), to_vec(&trie, id));
