...
mod tests {
    ...
    use super::get_disjoint;

    // The old writer-guard stress test is still useful as a check for concurrent insertions and
    // same-shard pairs, but the shared-reference API no longer mutates the values it returns.
    #[cfg(not(miri))]
    #[test]
    #[cfg_attr(
        target_family = "wasm",
    ...
    fn stress_deadlock() {
        ...
        scope(|scope| {
            for indices in indices {
                ...
                scope.spawn(move || {
                    for i in indices {
                        let (a, b) = get_disjoint(map, i, i + 1, || 0);
                        assert_eq!(*a, 0);
                        assert_eq!(*b, 0);
                    }
                });
