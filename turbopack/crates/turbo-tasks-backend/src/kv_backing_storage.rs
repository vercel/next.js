...
//
// This flag lets us avoid the cache invalidation / error suppression within Vercel so that we feel
// these pain points and fix the root causes of bugs.
fn should_invalidate_on_panic() -> bool {
    static VALUE: LazyLock<bool> = LazyLock::new(|| {
        env::var_os("TURBO_ENGINE_SKIP_INVALIDATE_ON_PANIC").is_none_or(OsStr::is_empty)
    });
    *VALUE
}
    ...
    for KeyValueDatabaseBackingStorage<T>
{
    ...
    fn next_free_task_id(&self) -> Result<TaskId> {
        Ok(TaskId::try_from(
            get_infra_u32(&self.inner.database, META_KEY_NEXT_FREE_TASK_ID)
    ...
    fn next_session_id(&self) -> Result<SessionId> {
        Ok(SessionId::try_from(
            get_infra_u32(&self.inner.database, META_KEY_SESSION_ID)
