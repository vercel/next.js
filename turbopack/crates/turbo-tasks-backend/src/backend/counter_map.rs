...
impl<K: Hash + Eq, V: CounterValue, const I: usize> CounterMap<K, V, I> {
    ...
    ///
    /// Only referenced by the `update_{field}` wrappers generated by the
    /// `#[task_storage]` macro for transient `CounterMap` fields. Those wrappers
    /// are themselves currently unused, so the only references are dead — hence
    /// `#[allow(dead_code)]` rather than a `cfg` gate (the generated code
    /// references this method unconditionally, so gating would break the build).
    #[allow(dead_code)]
