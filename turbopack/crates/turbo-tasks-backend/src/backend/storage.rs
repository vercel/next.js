...
impl Storage {
    ...
    /// Retains a locked task with the ability to detach its exact Papaya entry.
    ...
    /// Scan a batch of resident task IDs under one Papaya protection guard.
    ...
    pub fn evict_after_snapshot(&self, parent_span: Option<Id>) -> EvictionCounts {
        ...
        let totals = turbo_tasks::scope_unbounded::scope_unbounded_with(
            ...
            |_, task_id, evicted| {
                ...
                // Transient tasks are retained unless GC has deleted them.
                if task_id.is_transient() && !task.flags.deleted() {
                    evicted.unevictable_reasons[UnevictableReason::Transient.index()] += 1;
                    return std::ops::ControlFlow::Continue(());
                }
                // All GC'd tasks were tombstoned during the snapshot (or are not persisted), so
                // they can be removed. A transient deleted task has no persistent task type.
                let mut remove_task = false;
                let mut task_type_to_remove = None;
                if task.flags.deleted() {
                    task_type_to_remove = task.get_persistent_task_type().cloned();
                    evicted.full += 1;
                    remove_task = true;
                } else {
...
/// A locked resident task that may be detached before returning its ordinary write guard.
/// Papaya protection keeps the old node allocated until this guard drops after removal.
pub struct TaskEntryGuard<'a> {
    storage: &'a Storage,
    inner: StorageWriteGuard<'a>,
}

impl<'a> TaskEntryGuard<'a> {
    pub fn discard(self) {
        self.storage.map.remove_locked(&self.inner.inner);
    }
