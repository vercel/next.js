...
pub(super) fn resurrect_deleted<'e, C: ExecuteContext<'e>>(
    ...
    if task.deleted() {
        ...
        // acquired its guard. When its type is still resident, treat the resurrected task as new so
        // the next snapshot restores the task-type index deleted by that tombstone, and persist
        // Data so the type used to verify the index entry is restored too. This also keeps
        // the task unevictable until that snapshot. If eviction already dropped the type,
        // the task cannot be re-indexed here; re-execution below still gives the normal
        // resurrection path a chance to recover it.
        if task.get_persistent_task_type().is_some() {
