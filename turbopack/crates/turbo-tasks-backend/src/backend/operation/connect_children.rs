...
/// Mutates the parent while its task guard is live, then returns owned work that can be applied
/// after the guard — and therefore its shared execute-context borrow — has ended.
