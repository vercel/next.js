...
use anyhow::Result;
use turbo_tasks::{TaskInput, TransientValue, Vc, read};
...
async fn test_transient_operation(transient_arg: TransientValue<i32>) -> Result<()> {
    ...
    let called_with_transient_resolved = read!(called_with_transient.to_resolved())?;
    let called_with_persistent_resolved = read!(called_with_persistent.to_resolved())?;
