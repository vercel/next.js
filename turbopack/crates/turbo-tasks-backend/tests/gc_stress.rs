...
async fn gc_re_rooting_stays_flat() {
    ...
    let no_gc_growth = baseline + generation_task_count(WIDTH) * (ROUNDS as usize);
    println!(
    ...
    // Without GC the persistent resident set would climb by a whole generation per round (each
    // generation's subtree nodes + leaves persisted forever), so a bound within a small constant
    // of the baseline is only satisfiable if each generation's garbage is actually collected.
    assert!(
        max_resident <= baseline + generation_task_count(WIDTH),
    ...
    let expected_min_collected = generation_task_count(WIDTH) * (ROUNDS as usize) / 2;
    assert!(
