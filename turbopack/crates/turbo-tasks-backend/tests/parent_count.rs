...
/// `parent_count` must be **persisted**, not just tracked in memory: it is a `category = "meta"`
/// field, so it is written into the task's meta blob and restored on the next session. Getting that
/// category wrong would leave every restored task looking parentless (and therefore collectible)
/// after a restart, which only a real snapshot + DB reopen catches.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
