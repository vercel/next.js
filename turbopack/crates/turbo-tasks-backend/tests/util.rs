...
//! [`TurboTasksBackend`] test hooks (`snapshot_and_evict_for_testing`, …).
//!
...
///
/// The directory is rooted under `CARGO_TARGET_TMPDIR/.cache/`, with the test `name` as a prefix
/// so failed runs are easy to find on disk. The returned [`tempfile::TempDir`] cleans up on drop,
/// so callers keep it alive until the backend has finished shutting down.
