...
function applyEcmascriptMergedUpdate(update: EcmascriptMergedUpdate) {
  ...
  applyEcmascriptMergedUpdateShared({
    ...
    instantiateModuleFn: instantiateModule, // now wraps shared logic
    applyModuleFactoryNameFn: applyModuleFactoryName,
    moduleFactories,
    devModuleCache,
    autoAcceptRootModules: false, // Browser HMR requires explicit accept calls
  })
}

function handleApply(chunkListPath: ChunkListPath, update: HMRPayload) {
  switch (update.type) {
    case 'partial': {
