...
pub async fn get_nodejs_runtime_code(
    ...
    code.push_code(&*turbo_tasks::read!(shared_base_external_utils_code)?);
    code.push_code(&*turbo_tasks::read!(shared_node_external_utils_code)?);
    code.push_code(&*turbo_tasks::read!(runtime_base_code)?);
