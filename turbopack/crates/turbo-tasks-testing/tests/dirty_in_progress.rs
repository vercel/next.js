...
use turbo_tasks::{emit, CollectiblesSource, OperationVc, RcStr, State, ValueToString, Vc};
...
async fn compute(input: Vc<ChangingInput>) -> Result<Vc<Output>> {
    ...
    let collectibles = OperationVc::new(operation).peek_collectibles::<Box<dyn ValueToString>>();
