...
//!
//! Registered hooks are called in their order of registration.
//!
...
// We use a `BTreeMap` for predictable iteration order. We could use a `DashMap` or the `slab`
// crate, but we anticipate that setting up and tearing down hooks is rare.
static PANIC_HOOKS: RwLock<BTreeMap<NonZeroU64, ArcPanicHook>> = RwLock::new(BTreeMap::new());
