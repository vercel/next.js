...
use super::{
    content::EcmascriptBuildNodeEntryChunkContent, version::EcmascriptBuildNodeEntryChunkVersion,
};
use crate::ecmascript::node::{
    update::{EcmascriptNodeChunkUpdate, update_ecmascript_node_chunk},
...
/// Computes an update for an entry chunk by comparing the versions of its child chunks.
#[turbo_tasks::function]
pub(crate) async fn update_ecmascript_node_entry_chunk(
    content: Vc<EcmascriptBuildNodeEntryChunkContent>,
    ...
    let from_version = if let Some(from) = ResolvedVc::try_downcast_type::<
        EcmascriptBuildNodeEntryChunkVersion,
    ...
    // Build a map of chunk paths to their old versions
    ...
    let merged_module_map = MergedModuleMap::new(from.chunk_versions.to_vec());

    // Process each child chunk content
    let to_contents = content_ref
        .chunk_contents
        .iter()
        .map(|chunk_content| async move {
            let entries = chunk_content.entries().await?;
            let chunk_content_ref = chunk_content.await?;
            let output_root = chunk_content_ref.chunking_context.output_root().await?;
            let path = chunk_content_ref.chunk.path().await?;
            Ok((*chunk_content, entries, output_root, path))
        })
        .try_join()
        .await?;
    ...
    for (chunk_content, entries, output_root, path) in &to_contents {
            ...
            from_versions_by_chunk_path.swap_remove(chunk_path)
        {
            ...
            let update = update_ecmascript_node_chunk(**chunk_content, from_version).await?;
            ...
            match update {
                ...
                EcmascriptNodeChunkUpdate::Partial(chunk_partial) => {
