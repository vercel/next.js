...
pub mod html;
// The live HTTP-serving path (hyper request/response, tokio sockets, websocket HMR
// updates) is genuinely async-runtime-bound and only meaningful with a running server.
// It is gated on `tokio_runtime` so the no-tokio `sync` build links no tokio; the pure
// content-source computation graph (`source`, `html`, `introspect`) stays dual.
...
mod serving {
    ...
    use turbo_tasks::{
        Completion, Effects, NonLocalValue, OperationVc, PrettyPrintError, ResolvedVc,
        TurboTasksApi, Vc, read_strongly_consistent_and_apply_effects, run_once_with_reason,
    ...
    use crate::{
        ...
        source::{ContentSource, ContentSourceSideEffect},
        update::UpdateServer,
    };
    ...
    /// *inside* this task (where eventually-consistent reads are legal); the caller reads the
    /// result strongly consistently so the work is finished before the response is observed.
    #[turbo_tasks::function(operation, root)]
                ...
                async move {
                    let handler = move |request: Request<hyper::Body>| {
                        ...
                        let future = async move {
                            ...
                            // before this request. Later added side effects are not relevant for
                            // this.
                                ...
                                if !side_effects.is_empty() {
                                    ...
                                    let join_handle = tokio::spawn(run_once_with_reason(
                                        ...
                                        async move {
                                            ...
                                            // task and read its result strongly consistently, so
                                            // this
                                            // top-level task performs no eventually-consistent
                                            // read.
                        ...
                        async move {
                            match future.await {
                                ...
                                Err(e) => {
                                    ...
                                    Ok(Response::builder().status(500).body(hyper::Body::from(
...
#[cfg(feature = "tokio_runtime")]
pub use serving::{DevServer, DevServerBuilder};
