...
use napi::{
    Env, JsValue, UnknownRef,
    bindgen_prelude::{Function, Object, Unknown as JsUnknown},
};
...
use crate::{
    ...
    utils::{as_function, as_object, get_named_function, get_named_object, get_named_unknown},
};
...
impl VisitorsRef {
    /// Materialize the stored references into live JS functions for the
    /// duration of a visit call.
    fn functions<'env>(&self, env: &'env Env) -> Visitors<Function<'env>> {
        Visitors {
            enter: self
                .enter
                .as_ref()
    ...
    /// Materialize the stored references into live JS objects for the
    /// duration of a visit call.
    fn objects<'env>(&self, env: &'env Env) -> Visitors<Object<'env>> {
        Visitors {
            enter: self
                .enter
                .as_ref()
...
impl<'env> Visitors<Object<'env>> {
    fn named(&self, stage: VisitStage, name: &str) -> Option<Function<'env>> {
        self.for_stage(stage)
            .and_then(|m| get_named_function(m, name))
    }

    fn custom(&self, stage: VisitStage, obj: &str, name: &str) -> Option<Function<'env>> {
        self.for_stage(stage)
            .and_then(|m| get_named_unknown(m, obj).ok())
            .and_then(|v| match v.get_type() {
                Ok(napi::ValueType::Function) => as_function(v),
                Ok(napi::ValueType::Object) => {
                    let o = as_object(v)?;
                    get_named_function(&o, name)
                }
                _ => None,
            })
    }
...
impl JsVisitor {
    pub fn new(env: Env, visitor: Object<'_>) -> Self {
        let mut types = VisitTypes::empty();
        macro_rules! get {
      ($name: literal, $( $t: ident )|+) => {{
        let res: Option<Function<'_>> = get_named_function(&visitor, $name);
        ...
        res.and_then(|res| res.to_unknown().create_ref().ok())
      }};
    }

        macro_rules! map {
      ($name: literal, $( $t: ident )|+) => {{
        let obj: Option<Object<'_>> = get_named_object(&visitor, $name);

        if obj.is_some() {
          types |= $( VisitTypes::$t )|+;
        }

        obj.and_then(|obj| obj.to_unknown().create_ref().ok())
      }};
    }
...
impl<'i> Visitor<'i, AtRule<'i>> for JsVisitor {
    ...
    fn visit_stylesheet<'o>(
        ...
        if self.types.contains(VisitTypes::RULES) {
            let env = self.env;
            let visit_stylesheet = self.visit_stylesheet.functions(&env);
    ...
    fn visit_rule_list(
        ...
        if self.types.contains(VisitTypes::RULES) {
            let env = self.env;
            let rule_map = self.rule_map.objects(&env);
            let visit_rule = self.visit_rule.functions(&env);
                    ...
                    let name = match value {
                        ...
                        CssRule::Unknown(v) => {
                            ...
                            if let Some(visit) = rule_map.custom(stage, "unknown", name) {
                                ...
                                let res = call_visit_function(&visit, &js_value)?;
    ...
    fn visit_declaration_block(
        ...
        if self.types.contains(VisitTypes::PROPERTIES) {
            let env = self.env;
            let property_map = self.property_map.objects(&env);
            let visit_declaration = self.visit_declaration.functions(&env);
    ...
    fn visit_media_list(
        ...
        if self.types.contains(VisitTypes::MEDIA_QUERIES) {
            let env = self.env;
            let visit_media_query = self.visit_media_query.functions(&env);
            visit_list(
    ...
    fn visit_media_feature_value(
        ...
        {
            let env = self.env;
            let env_map = self.env_map.objects(&env);
            let visit_env = self.visit_env.functions(&env);
    ...
    fn visit_token_list(
        ...
        if self.types.contains(VisitTypes::TOKENS) {
            let env = self.env;
            let visit_token = self.visit_token.functions(&env);
            let token_map = self.token_map.objects(&env);
            let visit_function = self.visit_function.functions(&env);
            let function_map = self.function_map.objects(&env);
            let visit_variable = self.visit_variable.functions(&env);
            let visit_env = self.visit_env.functions(&env);
            let env_map = self.env_map.objects(&env);
...
fn call_visit_function<'env>(
    visit: &Function<'env>,
    arg: &JsUnknown<'env>,
) -> napi::Result<JsUnknown<'env>> {
    // `this` in the JS visitor is left as `undefined`.
    visit.call(*arg)
}
...
fn visit_declaration_list<'i, 'env, C: FnMut(&mut Property<'i>) -> napi::Result<()>>(
    env: &Env,
    list: &mut Vec<Property<'i>>,
    visit_declaration: &Visitors<Function<'env>>,
    property_map: &Visitors<Object<'env>>,
    visit_children: C,
) -> napi::Result<()> {
    visit_list(
        list,
