...
impl<T> FrozenSet<T> {
    ...
    /// Creates a [`FrozenSet`] from a pre-sorted iterator with unique elements.
    ///
    /// # Correctness
    ///
    /// The caller must ensure that:
    /// - The iterator yields elements sorted in ascending order according to [`T: Ord`][Ord]
    /// - There are no duplicate elements
    ///
    /// If these invariants are not upheld, the set will behave incorrectly (e.g.,
    /// [`FrozenSet::contains`] may fail to find elements that are present), but no memory
    /// unsafety will occur.
    pub fn from_uniq_sorted_iter(iter: impl IntoIterator<Item = T>) -> Self {
        FrozenSet {
            map: FrozenMap::from_uniq_sorted_iter(iter.into_iter().map(|t| (t, ()))),
        }
    }

    /// Creates a [`FrozenSet`] from an unsorted iterator, sorting by element. This is more
    /// efficient than [`FromIterator`] if you know that the iterator does not contain duplicate
    /// entries.
    ///
    /// # Correctness
    ///
    /// The caller must ensure that there are no duplicate elements.
    ///
    ...
    /// will occur.
    pub fn from_uniq_iter(iter: impl IntoIterator<Item = T>) -> Self
    where
        T: Ord,
    {
        FrozenSet {
            map: FrozenMap::from_uniq_iter(iter.into_iter().map(|t| (t, ()))),
        }
    }
}

impl<T: Ord> FromIterator<T> for FrozenSet<T> {
    /// Creates a [`FrozenSet`] from an iterator of elements.
    ///
    /// If there are duplicate elements, only one copy is kept (the last one encountered).
    fn from_iter<I: IntoIterator<Item = T>>(iter: I) -> Self {
        FrozenSet {
            map: FrozenMap::from_iter(iter.into_iter().map(|t| (t, ()))),
        }
    }
}
...
impl<T> From<BTreeSet<T>> for FrozenSet<T> {
    ...
    fn from(set: BTreeSet<T>) -> Self {
        FrozenSet::from_uniq_sorted_iter(set)
    }
}
    ...
    S: BuildHasher,
{
    ...
    fn from(set: HashSet<T, S>) -> Self {
        FrozenSet {
            map: FrozenMap::from_uniq_iter(set.into_iter().map(|t| (t, ()))),
        }
    }
}
...
impl<T: Ord> From<Vec<T>> for FrozenSet<T> {
    /// Creates a [`FrozenSet`] from a vector of elements.
    ///
    /// If there are duplicate elements, the last copy is kept.
    fn from(elements: Vec<T>) -> Self {
        Self::from_iter(elements)
    }
}

impl<T: Ord> From<Box<[T]>> for FrozenSet<T> {
    /// Creates a [`FrozenSet`] from a boxed slice of elements.
    ///
    /// If there are duplicate elements, the last copy is kept.
...
impl<T> From<&[T]> for FrozenSet<T>
where
    T: Ord + Clone,
{
    /// Creates a [`FrozenSet`] from a slice of elements.
    ///
    /// If there are duplicate elements, the last copy is kept.
    fn from(elements: &[T]) -> Self {
        FrozenSet::from(Vec::from(elements))
    }
}
...
// Manual implementation because the derive would add an unnecessary `T: Clone` type bounds.
