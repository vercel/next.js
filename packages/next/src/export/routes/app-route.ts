...
export async function exportAppRoute(
  ...
  experimental: Required<
    Pick<ExperimentalConfig, 'dynamicIO' | 'cacheComponents' | 'authInterrupts'>
  >,
  buildId: string
  ...
  try {
    ...
    if (
      ...
      !isPageMetadataRoute &&
      // We don't disable static gen when dynamicIO or cacheComponents is
      // enabled because we expect that anything dynamic in the GET handler
      // will make it dynamic and thus avoid the cache surprises that led to
      // us removing static gen unless specifically opted into
