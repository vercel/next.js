    ...
    switch (prop) {
      ...
      case 'formData':
        throw new DynamicServerError(
          `Route couldn't be rendered statically because it accessed accessed \`request.${prop}\`. See more info here: https://nextjs.org/docs/messages/dynamic-server-error`
        )
      case 'clone':
        return (
          target[requestCloneSymbol] ||
...
const staticGenerationNextUrlHandlers = {
  get(
    ...
    switch (prop) {
      ...
      case 'origin':
        throw new DynamicServerError(
          `Route couldn't be rendered statically because it accessed accessed \`nextUrl.${prop}\`. See more info here: https://nextjs.org/docs/messages/dynamic-server-error`
        )
      case 'clone':
        return (
          target[urlCloneSymbol] ||
    ...
    switch (prop) {
      ...
      case 'formData':
        throw new StaticGenBailoutError(
          `Route with \`dynamic = "error"\` couldn't be rendered statically because it accessed accessed \`request.${prop}\`.`
        )
      case 'clone':
        return (
          target[requestCloneSymbol] ||
...
const requireStaticNextUrlHandlers = {
  get(
    ...
    switch (prop) {
      ...
      case 'origin':
        throw new StaticGenBailoutError(
          `Route with \`dynamic = "error"\` couldn't be rendered statically because it accessed accessed \`nextUrl.${prop}\`.`
        )
      case 'clone':
        return (
          target[urlCloneSymbol] ||
