...
function installProcessErrorHandlers(
  ...
  process.on('unhandledRejection', (reason: unknown) => {
    ...
    // detect all of these, but when dynamicIO (or cacheComponents) is enabled,
    // we could suppress at least some of them by waiting to log the error until
    // after all in-progress renders have completed. Then, only log errors for
    // which there was not a corresponding "rejectionHandled" event.
