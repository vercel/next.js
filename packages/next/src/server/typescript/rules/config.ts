...
> = {
  ...
  unstable_requireStatic: {
    ...
    options: {
      ...
      false:
        'Indicate that this segment must use the default behavior and should not have `requireStatic = "shell" | "prefetch" | "navigation"` applied to it by another segment. Errors if another segment on the route uses `requireStatic` with those values.',
    } satisfies DocsOptionsObject<
