...
> = {
  ...
  runtime: {
    ...
    } satisfies DocsOptionsObject<FullAppSegmentConfig['runtime'] | 'experimental-edge'>,
...
function createAutoCompletionOptionValue(sort: number, name: string, apiName: string) {
  const ts = getTs()
          ...
          if (!ALLOWED_EXPORTS.includes(name.text) && !API_DOCS[name.text]) {
            ...
            if (value && options) {
              ...
              if (ts.isStringLiteral(value) || ts.isNoSubstitutionTemplateLiteral(value)) {
                ...
                if (!allowedValues.includes(val) && !API_DOCS[name.text].isValid?.(val)) {
                  isInvalid = true
                  displayedValue = val
                }
              } else if (
