...
describe('RouteHandlers', () => {
  ...
  it('will return false when there is no matching handler', async () => {
    ...
    handlers.set(RouteType.APP_PAGE, handler)
  ...
  it('will throw when multiple handlers are added for the same type', () => {
    const handlers = new RouteHandlers()
    const handler = { handle: jest.fn() }
    expect(() => handlers.set(RouteType.APP_PAGE, handler)).not.toThrow()
    expect(() => handlers.set(RouteType.APP_ROUTE, handler)).not.toThrow()
    expect(() => handlers.set(RouteType.APP_PAGE, handler)).toThrow()
    expect(() => handlers.set(RouteType.APP_ROUTE, handler)).toThrow()
  })
  ...
  it('will call the correct handler', async () => {
    ...
    handlers.set(RouteType.APP_PAGE, goodHandler)
    handlers.set(RouteType.APP_ROUTE, badHandler)
