...
> {
  ...
  public async handle(
    ...
    // In Turbopack dev mode, _getUserland re-fetches the live module from
    // devModuleCache on every request so that server HMR updates are picked up
    // without restarting the process. In all other modes, fall back to the
    // static snapshot captured at construction time.
    const liveUserland: AppRouteUserlandModule = this._getUserland
      ? await this._getUserland()
      : this.userland

    // Get the handler function for the given method using the live userland.
    // Guard against invalid HTTP methods to prevent RCE.
    const handler = isHTTPMethod(req.method)
