  ...
  const cachedFn = {
    [name]: async function (...args: any[]) {
      ...
      if (stream === undefined) {
        ...
        if (entry) {
          debug(`found entry ${serializedCacheKey}`, {
            timestamp: entry.timestamp,
            revalidate: entry.revalidate,
            expire: entry.expire,
          })

          if (shouldDiscardCacheEntry(entry, workStore, implicitTags)) {
            debug(`discarding stale entry ${serializedCacheKey}`)
            entry = undefined
          }
        } else {
          debug(`entry not found ${serializedCacheKey}`)
        }
...
function shouldDiscardCacheEntry(
  ...
  if (implicitTags) {
    ...
    if (entry.timestamp <= implicitTags.expiration) {
      debug(
        `entry was created at ${entry.timestamp}, before implicit tags were revalidated at ${implicitTags.expiration}`
      )
