...
// ---------------------------------------------------------------------------
// Shared types (imported by debug-channel-server.web.ts via `import type`)
// ---------------------------------------------------------------------------
...
export type DebugChannelPair = {
  ...
  serverSide: DebugChannelServer | Writable
...
// Runtime invariant: this module must never be loaded in edge runtime.
// The .js switcher should prevent this, but this guard catches
// direct imports or switcher bugs.
if (process.env.NEXT_RUNTIME === 'edge') {
  throw new Error(
    'debug-channel-server.node.ts must not be loaded in edge runtime. ' +
      'Import from debug-channel-server instead.'
  )
}
...
export function toNodeDebugChannel(
  ...
  const writerStream = webDebugChannel.writable.getWriter()
  return new Writable({
    write(
      chunk: Buffer | Uint8Array,
      _encoding: string,
      ...
      writerStream.write(chunk).then(() => callback(), callback)
    },
    final(callback: (error?: Error | null) => void) {
      writerStream.close().then(() => callback(), callback)
    },
    destroy(_err, callback) {
      writerStream.abort(_err).then(
        () => callback(_err),
        () => callback(_err)
      )
    },
