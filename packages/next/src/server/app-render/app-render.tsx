...
import { isDynamicReason, staticOnly } from './dynamic-rendering'
...
async function renderToHTMLOrFlightImpl(
  ...
  // We track postpones in SSR because we want to know if the HTML it produces
  // has any postponed holes caused by dyanmic API usage. We don't care about postpoend
  ...
  const htmlRendererPostponeHandler = (reason: string) => {
    if (
      ...
      isDynamicReason(staticGenerationStore.prerenderState, reason)
    ) {
      // This postpone originated from dynamic rendering APIs in Next.js
      // This could be from using dynamic APIs in SSR but more commonly this
      // is becuase you using something dynamic in RSC and it was rendered in SSR
      htmlRendererEncounteredDynamic = true
    }
  }
    ...
    async ({
      ...
      const renderer = createStaticRenderer({
        ...
        streamOptions: {
          ...
          onPostpone: htmlRendererPostponeHandler,
      ...
      try {
        ...
        if (prerenderState) {
          ...
          if (htmlRendererEncounteredDynamic) {
            // This is the Dynamic HTML case. we serialize the postponed state and return the stream as is
            if (!postponed) {
              throw new Error(
          ...
          // Next we check if we have an entirely static render. This may require waiting
          // for the entire flighr render to finish which can happen even after the SSR render
          // is complete
          if (staticOnly(prerenderState)) {
            // We eagerly read the inlined data stream. When this resolves we know the flight render
            // has completed and all data has been flushed.
            ...
            // We recheck that we are still entirely static
            if (staticOnly(prerenderState)) {
              // This is the Static case. We can serialize the entire stream and return it
              return {
          ...
          // Finally we must be in the Dynamic Data case where the HTML is complete but we need to
