...
describe('isCsrfOriginAllowed', () => {
  ...
  it('should return true when allowedOrigins contains originDomain with matching pattern', () => {
    ...
    expect(isCsrfOriginAllowed('asdf.vercel.com', ['**.vercel.com'])).toBe(true)
    expect(isCsrfOriginAllowed('asdf.jkl.vercel.com', ['**.vercel.com'])).toBe(true)
  })

  it("should correctly handle origins that don't have a TLD (eg for localhost)", () => {
    // Single level wildcard
    expect(isCsrfOriginAllowed('subdomain.localhost', ['*.localhost'])).toBe(true)
    ...
    expect(isCsrfOriginAllowed('subdomain.localhost', ['localhost'])).toBe(false)

    // Multiple patterns
    expect(isCsrfOriginAllowed('subdomain.localhost', ['localhost', '*.localhost'])).toBe(true)
    expect(isCsrfOriginAllowed('a.b.localhost', ['localhost', '*.localhost', '**.localhost'])).toBe(
      true
    )
  })
  ...
  it('should return false when allowedOrigins contains originDomain with non-matching pattern', () => {
    ...
    expect(isCsrfOriginAllowed('asdf.jkl.vercel.com', ['*.vercel.com'])).toBe(false)
    expect(isCsrfOriginAllowed('asdf.jkl.vercel.app', ['**.vercel.com'])).toBe(false)
  })
