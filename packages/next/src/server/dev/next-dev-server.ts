...
export default class DevServer extends Server {
  ...
  >
  /**
   * Serializes updates to the on-disk prerender manifest. `getStaticPaths` can
   * resolve for several pages at the same time, and each resolution performs a
   * read-modify-write cycle on that one file.
   */
  private prerenderManifestUpdateQueue: Promise<void> = Promise.resolve()
    ...
    const nextInvoke = withCoalescedInvoke(__getStaticPaths)(
      ...
      .then(async (res) => {
        ...
        if (
          ...
          //
          // The read-modify-write cycle below is serialized against other
          // pages resolving their static paths concurrently. Without it the
          // cycles interleave and corrupt the manifest: both `writeFile` calls
          // write from offset 0, so a shorter payload finishing last only
          // overwrites a prefix of a longer one and leaves its tail in place.
          // `getPrerenderManifest()` runs on every request in dev, so a corrupt
          // manifest takes down the entire dev server, surfacing as a
          // `SyntaxError` from `JSON.parse` on unrelated routes.
          const fallbackMode = res.value.fallbackMode
          const update = this.prerenderManifestUpdateQueue.then(async () => {
            ...
            if (updatedManifest === rawExistingManifest) return

            // Write through a temporary file so a reader can never observe a
            // partially written manifest.
            const tmpPath = `${manifestPath}.${process.pid}.tmp`
            try {
              await fs.promises.writeFile(tmpPath, updatedManifest)
          ...
          // Keep the queue usable if this update fails, while still surfacing
          // the failure to the caller.
          this.prerenderManifestUpdateQueue = update.catch(() => {})
          await update
        }
