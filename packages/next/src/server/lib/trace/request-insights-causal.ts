...
const MAX_PARENT_REQUEST_ID_LENGTH = 128
const MAX_FETCH_INDEX = 1_000_000
const CAUSAL_TOKEN_PATTERN = /^[A-Za-z0-9_-]{16,64}$/
const PARENT_REQUEST_ID_PATTERN = /^[A-Za-z0-9_-]{1,128}$/
...
export class RequestInsightsCausalRegistry {
  ...
  mintCausalToken({
    ...
    const parent = normalizeParent({ parentRequestId, parentFetchIndex })
...
function normalizeParent(
  parent: RequestInsightsCausalParent
): RequestInsightsCausalParent | undefined {
  if (
    parent.parentRequestId.length > MAX_PARENT_REQUEST_ID_LENGTH ||
    !PARENT_REQUEST_ID_PATTERN.test(parent.parentRequestId) ||
    !Number.isInteger(parent.parentFetchIndex) ||
