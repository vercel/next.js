...
import { I18NProvider, LocaleInfo } from '../../future/helpers/i18n-provider'
...
export function getResolveRoutes(
  ...
  const i18n =
    config.i18n?.locales && config.i18n.locales.length > 0
      ? new I18NProvider(config.i18n)
      : undefined
  const basePath =
    config.basePath && config.basePath.length > 1
      ? new BasePathNormalizer(config.basePath)
      : undefined
  const trailingSlash =
    config.trailingSlash && !config.skipMiddlewareUrlNormalize
      ? new TrailingSlashNormalizer(true)
      : undefined
  ...
  async function resolveRoutes({
    ...
    let initialLocaleResult: LocaleInfo | undefined
    ...
    async function checkTrue() {
      ...
      if (basePath) {
        ...
        if (!info.hadBasePath) return null

        curPathname = info.pathname
      }

      // Analyze the pathname to detect the locale if it's enabled.
      ...
      for (const route of dynamicRoutes) {
        ...
        if (invokedOutputs?.has(route.page)) continue

        // Try to match this pathname against the route.
        const params = route.match(
          // If the route supports locales, we should use the pathname where
          // the locale has been stripped. Otherwise, we should use the
          // original pathname.
          route.supportsLocales && localeResult
        ...
        output = await fsChecker.getItem(
          // If the route should have had a base path, it would have been
          // validate above. Therefore, we can safely add the base path back
          // to the pathname in order to get the correct output.
          basePath ? basePath.denormalize(route.page) : route.page
        )
    ...
    async function handleRoute(
      ...
      if (route.name === 'check_fs' && parsedUrl.pathname) {
        ...
        const matchedOutput = await fsChecker.getItem(curPathname)
        ...
        if (
          ...
          if (
            ...
            if (matchedOutput.locale) {
              parsedUrl.query.__nextLocale = matchedOutput.locale
            }
            return {
              parsedUrl,
              resHeaders,
      ...
      if (!opts.minimalMode && route.name === 'middleware') {
        ...
        if (
          ...
          if (middlewareHeaders['x-middleware-rewrite']) {
            ...
            if (i18n) {
              const { detectedLocale } = i18n.analyze(curPathname)
              if (detectedLocale) {
