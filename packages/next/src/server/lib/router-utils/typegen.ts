...
function generateLayoutSlotMap(routesManifest: RouteTypesManifest): string {
  ...
  // Sort routes deterministically for consistent output
  const sortedLayoutRoutes = Object.entries(routesManifest.layoutRoutes).sort(([a], [b]) =>
    a.localeCompare(b)
  )
...
export function generateLinkTypesFile(routesManifest: RouteTypesManifest): string {
...
export function generateValidatorFile(routesManifest: RouteTypesManifest): string {
...
export function generateValidatorFileStrict(routesManifest: RouteTypesManifest): string {
  const generateValidations = (
    paths: string[],
    type:
      | 'AppPageConfig'
...
export function generateRouteTypesFile(routesManifest: RouteTypesManifest): string {
...
export function generateRouteTypesFileStrict(routesManifest: RouteTypesManifest): string {
  const routeTypes = generateRouteTypes(routesManifest)
  const paramTypes = generateParamTypes(routesManifest)
  const layoutSlotMap = generateLayoutSlotMap(routesManifest)
