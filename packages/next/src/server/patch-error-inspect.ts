...
function frameToString(
  ...
  if (
    sourceURL !== null &&
    // Sourcemapped positions can also produce `file:` URLs with a single
    // slash, e.g. when a source map consumer normalized them.
...
function getSourcemappedFrameIfPossible(
  ...
  if (sourceMapCacheEntry === undefined) {
    ...
    if (path.isAbsolute(sourceURL)) {
      ...
      // Node.js keys its source map cache by `pathToFileURL` output, which
