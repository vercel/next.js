...
export function getSocialImageFallbackMetadataBase(
  ...
  if (isMetadataBaseMissing) {
    ...
    console.log()
    Log.warnOnce(
      `metadata.metadataBase is not set for resolving social open graph or twitter images, fallbacks to "${fallbackMetadata.origin}". See https://nextjs.org/docs/api-reference/metadata#metadatabase`
    )
  }
