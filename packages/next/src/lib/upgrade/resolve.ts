...
async function resolveReact(
  app: UpgradeApp,
  target: PackageRelease
): Promise<string> {
  const compatible = (version: string) =>
    semver.valid(version) &&
    !semver.prerelease(version) &&
    target.peerDependencies?.react &&
    target.peerDependencies?.['react-dom'] &&
    semver.satisfies(version, target.peerDependencies.react) &&
    semver.satisfies(version, target.peerDependencies['react-dom']) &&
    !(
      app.routers.includes('app') &&
      semver.gte(target.version, '14.3.0-canary.45') &&
      semver.major(version) < 19
    )
  if (app.reactVersion === app.reactDomVersion && compatible(app.reactVersion))
    return app.reactVersion
  const { value } = await fetchJSON(`${NPM_REGISTRY}react`)
  const versions = Object.keys((value as { versions?: object })?.versions ?? {})
    .filter(compatible)
    .sort(semver.rcompare)
  for (const version of versions) {
    const dom = await readPackageMetadata('react-dom', version)
    if (
      dom.version === version &&
      dom.peerDependencies?.react &&
      semver.satisfies(version, dom.peerDependencies.react)
    )
      return version
  }
  throw new Error('No published compatible React/React DOM pair was found.')
}

export function getCodemodExclusions(config: UpgradeConfig): string[] {
  return [
    'cache-components-instant-false',
    ...(config.partialPrefetching === true ? [] : ['remove-partial-prefetch']),
  ]
}
...
export interface UpgradeDependencies {
  ...
  resolveReact: (app: UpgradeApp, target: PackageRelease) => Promise<string>
...
export async function resolveUpgrade(
  ...
  try {
    ...
    const reactVersion = await deps.resolveReact(app, selected)
    ...
    const args = [
      ...
      reactVersion,
      '--no-turbopack',
      ...getCodemodExclusions(config).flatMap((name) => [
        '--skip-codemod',
        name,
      ]),
    ]
