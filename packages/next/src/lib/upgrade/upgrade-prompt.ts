...
export async function runDevWithUpgradePrompt(
  ...
  // The PTY child has its own process group. Suspend it with the supervisor
  // even while upgrade assessment is pending and stdin is not in raw mode.
  const onSuspend = () => {
    if (exitCode === null) {
      try {
  ...
  if (exitCode !== null && !humanPromptStarted) {
    ...
    cleanup()
    process.off('exit', onExit)
    process.exit(childExitCode(exitCode))
  }
  ...
  const onInput = (data: Buffer) => {
    ...
    // Raw mode turns Ctrl+Z into a byte, so use the same suspension path as
    // the shell's SIGTSTP during assessment.
    onSuspend()
