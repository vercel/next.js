...
export async function prepareUpgrade(
  ...
  if (
    !selected.nodeRange ||
    !semver.satisfies(process.versions.node, selected.nodeRange)
  ) {
    throw new Error(
      `Next.js ${selected.version} requires Node.js ${selected.nodeRange ?? '(version unavailable)'}. Update Node.js before continuing.`
    )
  }
