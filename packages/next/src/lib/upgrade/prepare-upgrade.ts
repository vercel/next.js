...
export async function prepareUpgrade(
  ...
  if (
    ...
    throw new Error(
      `Next.js ${target.version} requires Node.js ${target.nodeRange ?? '(metadata unavailable)'}. Install a supported runtime before continuing.`
    )
  }
...
async function readSecuritySnapshot(): Promise<SecuritySnapshot> {
