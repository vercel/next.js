...
Preserve its intended behavior, React pair where compatible, bundler choices and
optional feature configuration. Scope shared workspace changes to required
compatibility work and verify affected callers. Keep unrelated edits out of commits.
...
Next.js target, compatible React pair, `--yes`, and optional-adoption exclusions.
...
Keep adoption/optimization work outside this upgrade. In particular, the selected
command excludes Cache Components `instant = false` adoption and excludes
partial-prefetch cleanup unless the app already enables that feature.
...
- Verify the bundler used by each supported dev/build script. The codemod's
  `--no-turbopack` option preserves directly recognized scripts and flags custom
  commands for review. When preserving Webpack across Next.js 16, use the target guide's
  `--webpack` script changes and run checks through those scripts. A successful
  Turbopack build does not verify an app that must continue using Webpack.
