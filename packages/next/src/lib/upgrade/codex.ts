...
export async function launchCodex(
  ...
  try {
    while (true) {
      ...
      for (const line of lines) {
        ...
        if (
          ...
          printUpgradeReceipt({
            ...
            // codex exec owns the active writer; resume after it exits or stops.
            open: null,
            logs: eventsPath,
            errors: stderrPath,
            stop,
      ...
      if (exited) {
        // Exit may arrive during the read, leaving us with an older snapshot.
        // Read once after exit before deciding the session ID is missing.
        if (!exitedBeforeRead) {
          continue
        }
