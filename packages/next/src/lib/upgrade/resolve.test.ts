...
function dependencies(): UpgradeDependencies {
  return {
    ...
    resolveReact: jest.fn(async () => '18.3.1'),
...
describe('codemod compatibility diagnostics', () => {
  ...
  it('distinguishes a successful probe with unsupported options', async () => {
    ...
    ).rejects.toThrow('does not support --skip-codemod')
  })
})
    ...
    expect(
      await resolveUpgrade(
        { directory: '/app' },
        {
          ...dependencies(),
          checkCodemod: async () => {
            throw new Error('Missing --skip-codemod')
          },
        }
      )
    ).toMatchObject({ status: 'blocked', reason: 'Missing --skip-codemod' })
  })
  it('never selects Cache Components adoption and only removes enabled partial prefetch', () => {
    expect(
      getCodemodExclusions({ cacheComponents: true, partialPrefetching: true })
    ).toEqual(['cache-components-instant-false'])
    expect(getCodemodExclusions({})).toContain('remove-partial-prefetch')
  })
})
