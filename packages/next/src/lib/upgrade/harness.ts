...
export function selectHarness(
  ...
  if (installed.length === 1) return { kind: 'launch', harness: installed[0] }
    ...
    for (const directory of directories) {
      for (const extension of extensions) {
        ...
        try {
          ...
          if ((await stat(file)).isFile()) found = true
        } catch {}
        if (found) break
      }
      if (found) break
    }
    if (found) installed.push(name)
  }
  return installed
}
...
export function launchHarness(
  ...
  if (harness === 'codex') return launchCodex(prompt, directory)
  return new Promise((resolve, reject) => {
    ...
    output?.on('line', (line) => {
      ...
      if (/^\s*Starting background service(?:…|\.\.\.)\s*$/.test(plain)) return
