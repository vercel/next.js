...
import type { UpgradeContext, UpgradeResolution } from './types'
...
export async function prepareUpgradeResources(
  ...
  const deps: ResourceDependencies = {
    tempRoot: tmpdir(),
    docsRoot: resolve(__dirname, '../../docs'),
    workflowPath: join(__dirname, 'docs/security-upgrade.md'),
    migrationWorkflowPath: join(__dirname, 'docs/security-migration.md'),
