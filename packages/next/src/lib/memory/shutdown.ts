...
export function disableMemoryDebuggingMode(): void {
  ...
  Log.info(bold('Memory usage report:'))
  ...
  Log.info(` - Total time spent in GC: ${totalTimeInGcMs.toFixed(2)}ms`)
  ...
  Log.info(` - Peak heap usage: ${(peakHeapUsage / 1024 / 1024).toFixed(2)} MB`)
  Log.info(` - Peak RSS usage: ${(peakRssUsage / 1024 / 1024).toFixed(2)} MB`)
}
