...
export default async function build(
  ...
  try {
    ...
    await nextBuildSpan.traceAsyncFn(async () => {
      ...
      // Generate deploymentId - auto-generate if experimental flag is enabled, otherwise use user-provided string
      const deploymentId = nextBuildSpan
        .traceChild('generate-deploymentid')
        .traceSyncFn(() =>
          generateDeploymentId(
            config.deploymentId,
            config.experimental?.autoGenerateDeploymentId ?? false,
            nanoid
          )
        )
