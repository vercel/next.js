...
export async function getPageStaticInfo(params: {
  ...
  if (
    ...
    if (pageType === 'app') {
      if (config) {
        const message = `\`export const config\` in ${pageFilePath} is deprecated. Please change \`runtime\` property to segment export config. See https://nextjs.org/docs/api-reference/segment-config`
        if (isDev) {
