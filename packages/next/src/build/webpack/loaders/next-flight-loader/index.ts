...
type NextFlightLoaderOptions = { appDir: string }

const segmentConfigNames = new Set([
  'config',
...
const nextFlightLoader: webpack.LoaderDefinitionFunction<NextFlightLoaderOptions> =
  function (source, sourceMap) {
    ...
    const isPageOrLayout =
      this.resourcePath.startsWith(appDir) &&
      /(page|layout)\./.test(path.basename(this.resourcePath))
    ...
    if (buildInfo.rsc?.type === RSC_MODULE_TYPES.client) {
      // @ts-expect-error: sourceType is on parser
...
`
            ...
            const isSegmentConfigInPageOrLayout =
              isPageOrLayout && segmentConfigNames.has(ref)
            if (!isSegmentConfigInPageOrLayout) {
              esmSource += `
...
export default nextFlightLoader
