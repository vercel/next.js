...
const NextServerCSSLoader: webpack.LoaderDefinitionFunction<NextServerCSSLoaderOptions> = function (
  content
) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (isCSSModule === undefined) {
      this.emitWarning(
        new Error("No 'cssModules' option was found for the next-flight-css-loader plugin.")
      )
