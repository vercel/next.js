  ...
  if (NEXT_RSC_ERR_REACT_API.test(message)) {
    ...
    if (isPagesDir) {
      formattedMessage = message.replace(
        ...
        `\n\nYou're importing a component that needs $1. That only works in a Server Component which is not supported in the pages/ directory. Read more: https://nextjs.org/docs/rendering/server-and-client-components\n\n`
      )
    ...
    formattedMessage = message.replace(
      NEXT_RSC_ERR_CLIENT_METADATA_EXPORT,
      `\n\nYou are attempting to export "$1" from a component marked with "use client", which is disallowed. Either remove the export, or the "use client" directive. Read more: https://nextjs.org/docs/api-reference/metadata\n\n`
    )
    ...
    formattedMessage = message.replace(
      ...
      `\n\n"metadata" and "generateMetadata" cannot be exported at the same time, please keep one of them. Read more: https://nextjs.org/docs/api-reference/metadata\n\n`
    )
