  ...
  apply(resolver: Resolver) {
    ...
    target.tapAsync('OptionalPeerDependencyResolverPlugin', (request, resolveContext, callback) => {
      ...
      resolver.doResolve(
        ...
        resolveContext,
        (err, result) => {
          if (!result && request?.descriptionFileData?.peerDependenciesMeta && request.request) {
            const peerDependenciesMeta = request.descriptionFileData.peerDependenciesMeta as Record<
              string,
              { optional?: boolean }
            >
