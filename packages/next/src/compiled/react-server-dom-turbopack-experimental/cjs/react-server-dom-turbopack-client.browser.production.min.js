...
*/
'use strict';var q=require("next/dist/compiled/react-dom-experimental"),t={stream:!0};function u(a,b){if(a){var c=a[b[0]];if(a=c[b[2]])c=a.name;else{a=c["*"];if(!a)throw Error('Could not find the module "'+b[0]+'" in the React SSR Manifest. This is probably a bug in the React Server Components bundler.');c=b[2]}return 4===b.length?[a.id,a.chunks,c,1]:[a.id,a.chunks,c]}return b}var v=new Map;
...
null!==p&&"function"===typeof p.then){l++;var R=e++;n=function(){try{var x=JSON.stringify(h,g),F=k;F.append(b+R,x);l--;0===l&&d(F)}catch(ka){f(ka)}};p.then(n,n);return"$"+R.toString(16)}f(p);return null}finally{l--}}if("function"===typeof h.then){null===k&&(k=new FormData);l++;var S=e++;h.then(function(p){try{var x=JSON.stringify(p,g);p=k;p.append(b+S,x);l--;0===l&&d(p)}catch(F){f(F)}},function(p){f(p)});return"$@"+S.toString(16)}if(fa(h))return h;if(h instanceof FormData){null===k&&(k=new FormData);
...
return"$T"+D(c,h).toString(16)}if("symbol"===typeof h){if(void 0===c)throw Error("Symbols cannot be passed to a Server Function without a temporary reference set. Pass a TemporaryReferenceSet to the options.");return"$T"+D(c,h).toString(16)}if("bigint"===typeof h)return"$n"+h.toString(10);throw Error("Type "+typeof h+" is not supported as an argument to a Server Function.");}var e=1,l=0,k=null;a=JSON.stringify(a,g);null===k?d(a):(k.set(b+"0",a),0===l&&d(k))}function na(a,b){G.set(a,b)}
...
48);continue;case 3:m=e.indexOf(10,l);break;case 4:m=l+h,m>e.length&&(m=-1)}var E=e.byteOffset+l;if(-1<m)h=new Uint8Array(e.buffer,E,m-l),va(a,g,r,w,h),l=m,3===k&&l++,h=g=r=k=0,w.length=0;else{e=new Uint8Array(e.buffer,E,e.byteLength-l);w.push(e);h-=e.byteLength;break}}a._rowState=k;a._rowID=g;a._rowTag=r;a._rowLength=h;return f.read().then(c).catch(d)}}function d(g){Q(a,g)}var f=b.getReader();f.read().then(c).catch(d)}
