...
function y(a){for(var b=a[1],c=[],d=0;d<b.length;d++){var f=b[d],h=u.get(f);if(void 0===h){h=__turbopack_load__(f);c.push(h);var e=u.set.bind(u,f,null);h.then(e,x);u.set(f,h)}else null!==h&&c.push(h)}return 4===a.length?0===c.length?w(a[0]):Promise.all(c).then(function(){return w(a[0])}):0<c.length?Promise.all(c):null}var z=p.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactDOMCurrentDispatcher,A=Symbol.for("react.element"),B=Symbol.for("react.lazy"),C=Symbol.for("react.postpone"),D=Symbol.iterator;
