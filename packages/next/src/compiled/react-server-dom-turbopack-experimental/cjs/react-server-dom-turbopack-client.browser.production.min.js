...
function z(a){for(var b=a[1],c=[],d=0;d<b.length;d++){var f=b[d],k=w.get(f);if(void 0===k){k=__turbopack_load__(f);c.push(k);var e=w.set.bind(w,f,null);k.then(e,y);w.set(f,k)}else null!==k&&c.push(k)}return 4===a.length?0===c.length?x(a[0]):Promise.all(c).then(function(){return x(a[0])}):0<c.length?Promise.all(c):null}
