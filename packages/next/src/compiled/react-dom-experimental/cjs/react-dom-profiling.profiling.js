  ...
  if (null === finishedGesture) ensureRootIsScheduled(root);
    ...
    try {
      ...
      if ("none" !== rotate && "" !== rotate) {
        var parts$284 = rotate.split(" ");
        transform =
          1 === parts$284.length
            ? "rotate(" + parts$284[0] + ") " + transform
            : 2 === parts$284.length
              ? "rotate" +
                parts$284[0].toUpperCase() +
                "(" +
                parts$284[1] +
                ") " +
                transform
              : "rotate3d(" + parts$284.join(", ") + ") " + transform;
      }
      var translate = computedStyle.translate;
      if ("none" !== translate && "" !== translate) {
        var parts$285 = translate.split(" ");
        transform =
          (3 === parts$285.length ? "translate3d" : "translate") +
          "(" +
          parts$285.join(", ") +
          ") " +
          transform;
    ...
    var readyCallback = function () {
        ...
        for (i = 0; i < animations.length; i++) {
          ...
          if (
            ...
            var timing$289 = pseudoElement.getTiming();
            anim =
              rangeEnd -
              (("number" === typeof timing$289.duration
                ? timing$289.duration
                : 0) +
                timing$289.delay) *
                longestDuration;
            groupName = rangeEnd - timing$289.delay * longestDuration;
            if (
              "reverse" === timing$289.direction ||
              "alternate-reverse" === timing$289.direction
            )
              (timing$289 = anim), (anim = groupName), (groupName = timing$289);
            animateGesture(
...
var isomorphicReactPackageVersion$jscomp$inline_2183 = React.version;
if (
  "19.2.0-experimental-6a7650c7-20250405" !==
  isomorphicReactPackageVersion$jscomp$inline_2183
)
  throw Error(
    formatProdErrorMessage(
      527,
...
var internals$jscomp$inline_2807 = {
  bundleType: 0,
...
if ("undefined" !== typeof __REACT_DEVTOOLS_GLOBAL_HOOK__) {
  var hook$jscomp$inline_2808 = __REACT_DEVTOOLS_GLOBAL_HOOK__;
  if (
    !hook$jscomp$inline_2808.isDisabled &&
    hook$jscomp$inline_2808.supportsFiber
  )
    try {
      (rendererID = hook$jscomp$inline_2808.inject(
        internals$jscomp$inline_2807
      )),
        (injectedHook = hook$jscomp$inline_2808);
    } catch (err) {}
}
function noop() {}
