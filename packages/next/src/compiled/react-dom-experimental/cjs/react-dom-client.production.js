...
function scheduleGesture(
  ...
  null === prev ? (root.gestures = gesture) : (prev.next = gesture);
  ensureRootIsScheduled(root);
  return gesture;
}
...
function updateSwipeTransition(previous, current, next) {
  ...
  if (null !== queue.pending) {
    if (64 === rootRenderLanes) {
      ...
      rootRenderLanes = rootRenderLanes.gestures;
...
if ("undefined" !== typeof __REACT_DEVTOOLS_GLOBAL_HOOK__) {
  var hook$jscomp$inline_2383 = __REACT_DEVTOOLS_GLOBAL_HOOK__;
  if (
    !hook$jscomp$inline_2383.isDisabled &&
    hook$jscomp$inline_2383.supportsFiber
  )
    try {
      (rendererID = hook$jscomp$inline_2383.inject(
        internals$jscomp$inline_2382
      )),
        (injectedHook = hook$jscomp$inline_2383);
    } catch (err) {}
}
