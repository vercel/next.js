...
null!==q&&"function"===typeof q.then){m++;var N=l++;k=function(){try{var w=JSON.stringify(h,g),B=e;B.append(b+N,w);m--;0===m&&c(B)}catch(ja){f(ja)}};q.then(k,k);return"$"+N.toString(16)}f(q);return null}finally{m--}}if("function"===typeof h.then){null===e&&(e=new FormData);m++;var O=l++;h.then(function(q){try{var w=JSON.stringify(q,g);q=e;q.append(b+O,w);m--;0===m&&c(q)}catch(B){f(B)}},function(q){f(q)});return"$@"+O.toString(16)}if(ea(h))return h;if(h instanceof FormData){null===e&&(e=new FormData);
...
return"$T"+E(d,h).toString(16)}if("symbol"===typeof h){k=h.description;if(Symbol.for(k)!==h)throw Error("Only global symbols received from Symbol.for(...) can be passed to Server Functions. The symbol Symbol.for("+(h.description+") cannot be found among global symbols."));return"$S"+k}if("bigint"===typeof h)return"$n"+h.toString(10);throw Error("Type "+typeof h+" is not supported as an argument to a Server Function.");}var l=1,m=0,e=null;a=JSON.stringify(a,g);null===e?c(a):(e.set(b+"0",a),0===m&&
c(e))}var G=new WeakMap;function ma(a){var b,d,c=new Promise(function(f,g){b=f;d=g});ia(a,"",void 0,function(f){if("string"===typeof f){var g=new FormData;g.append("0",f);f=g}c.status="fulfilled";c.value=f;b(f)},function(f){c.status="rejected";c.reason=f;d(f)});return c}
