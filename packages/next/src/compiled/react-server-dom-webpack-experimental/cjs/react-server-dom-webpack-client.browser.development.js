      ...
      switch (tag) {
        ...
        case 75:
          a: {
            ...
            getResponseLedgers(response).types.set(id, { kind: streamState });
          }
          break;
        case 89:
          row = parseInt(row, 16);
          streamState = getResponseLedgers(response);
          row = streamState.types.get(row);
          void 0 !== row &&
            ((response = createPendingChunk(response)),
            (row = {
              chunk: response,
              type: row,
              pendingCaptureUnits: [],
            ...
            streamState.totals.set(id, row),
            (response.$$typeof = REACT_LEDGER_TOTAL_TYPE),
            (response.total = row));
          break;
        case 81:
          streamState = parseModel(response, row);
          response = getResponseLedgers(response);
          row = response.units;
          id = getOrCreateUnit(response, id);
          tag = streamState[0];
          null !== tag &&
            ((row = row.get(parseInt(tag, 16))),
            void 0 !== row &&
              ((tag = row.children),
              null === tag && (row.children = tag = []),
              tag.push(id),
              markUnitDirty(response, row)));
          streamState = streamState[1];
          if (0 !== streamState.length) {
            row = response.totals;
            tag = [];
            for (chunk = 0; chunk < streamState.length; chunk++)
              (error = row.get(parseInt(streamState[chunk], 16))),
                void 0 !== error &&
                  (tag.push(error.type), error.pendingCaptureUnits.push(id));
            id.capturedLedgers = tag;
          }
        ...
        case 90:
          streamState = parseModel(response, row);
          response = getResponseLedgers(response);
          row = response.units.get(id);
          if (
            void 0 !== row &&
            ((id = response.types.get(parseInt(streamState[0], 16))),
            void 0 !== id)
          )
            switch (
              (markUnitDirty(response, row),
              (tag = row.cells),
              null === tag && (row.cells = tag = new Map()),
              (response = tag.get(id)),
              void 0 === response &&
                tag.set(id, (response = createLedgerCell(id))),
              response.kind)
            ) {
              case 0:
                response.state = !0;
                break;
              case 1:
                response.state = (response.state | streamState[1]) >>> 0;
                break;
              case 2:
                id = streamState[1];
                streamState = response.state;
                if (null === streamState || id < streamState)
                  response.state = id;
                break;
              case 3:
                id = streamState[1];
              ...
              default:
                for (
                  id = streamState[1], streamState = 0;
                  streamState < id.length;
                  streamState++
                )
                  response.state.add(id[streamState]);
            }
          break;
        case 70:
          streamState = parseModel(response, row);
          ...
          if (void 0 !== row)
            for (
              markUnitDirty(response, row),
                id = row.references,
                null === id && (row.references = id = []),
                row = 0;
              row < streamState.length;
              row++
            )
              (tag = getOrCreateUnit(response, parseInt(streamState[row], 16))),
                id.push(tag);
          break;
        default:
          "" === row
