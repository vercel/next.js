...
function y(a){for(var b=a.chunks,c=[],d=0;d<b.length;d++){var e=b[d],f=v.get(e);if(void 0===f){f=globalThis.__next_chunk_load__(e);c.push(f);var k=v.set.bind(v,e,null);f.then(k,x);v.set(e,f)}else null!==f&&c.push(f)}if(a.async){if(b=w.get(a.id))return"fulfilled"===b.status?null:b;var h=Promise.all(c).then(function(){return globalThis.__next_require__(a.id)});h.then(function(l){h.status="fulfilled";h.value=l},function(l){h.status="rejected";h.reason=l});w.set(a.id,h);return h}return 0<c.length?Promise.all(c):null}
