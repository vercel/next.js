 ...
 */
'use strict';var m=require("util"),p=require("react-dom"),q=require("react"),r={stream:!0};
function t(a,b){if(a){var c=a.ssrManifest[b.id],d=c[b.name];if(d)c=d.name;else{d=c["*"];if(!d)throw Error('Could not find the module "'+b.id+'" in the React SSR Manifest. This is probably a bug in the React Server Components bundler.');c=b.name}var e=a.chunkLoading.prefix;a=a.chunkLoading.crossOrigin;for(var f=b.chunks,k=0;k<f.length;k++){var h=e+f[k],l=v.current;l&&(null===a?l.preinit(h,{as:"script"}):l.preinit(h,{as:"script",crossOrigin:a}))}return{id:d.id,chunks:d.chunks,name:c,async:!!b.async}}return b}
var w=new Map,x=new Map;function y(){}
function z(a){for(var b=a.chunks,c=[],d=0;d<b.length;d++){var e=b[d],f=w.get(e);if(void 0===f){f=Promise.resolve();c.push(f);var k=w.set.bind(w,e,null);f.then(k,y);w.set(e,f)}else null!==f&&c.push(f)}if(a.async){if(b=x.get(a.id))return"fulfilled"===b.status?null:b;var h=Promise.all(c).then(function(){return globalThis.__next_require__(a.id)});h.then(function(l){h.status="fulfilled";h.value=l},function(l){h.status="rejected";h.reason=l});x.set(a.id,h);return h}return 0<c.length?Promise.all(c):null}
var v=p.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.Dispatcher,A=Symbol.for("react.element"),B=Symbol.for("react.lazy"),C=Symbol.for("react.default_value"),D=Symbol.iterator;function E(a){if(null===a||"object"!==typeof a)return null;a=D&&a[D]||a["@@iterator"];return"function"===typeof a?a:null}var F=Array.isArray,H=new WeakMap;function I(a){return Number.isFinite(a)?0===a&&-Infinity===1/a?"$-0":a:Infinity===a?"$Infinity":-Infinity===a?"$-Infinity":"$NaN"}
function J(a,b,c,d){function e(l,g){if(null===g)return null;if("object"===typeof g){if("function"===typeof g.then){null===h&&(h=new FormData);k++;var G=f++;g.then(function(n){n=JSON.stringify(n,e);var u=h;u.append(b+G,n);k--;0===k&&c(u)},function(n){d(n)});return"$@"+G.toString(16)}if(g instanceof FormData){null===h&&(h=new FormData);var Z=h;l=f++;var aa=b+l+"_";g.forEach(function(n,u){Z.append(aa+u,n)});return"$K"+l.toString(16)}return!F(g)&&E(g)?Array.from(g):g}if("string"===typeof g){if("Z"===
