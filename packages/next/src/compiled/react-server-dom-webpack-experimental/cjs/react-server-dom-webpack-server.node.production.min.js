...
function tb(a){for(var b=a.chunks,d=[],c=0;c<b.length;c++){var e=b[c],f=S.get(e);if(void 0===f){f=Promise.resolve();d.push(f);var g=S.set.bind(S,e,null);f.then(g,sb);S.set(e,f)}else null!==f&&d.push(f)}if(a.async){if(b=rb.get(a.id))return"fulfilled"===b.status?null:b;var h=Promise.all(d).then(function(){return globalThis.__next_require__(a.id)});h.then(function(k){h.status="fulfilled";h.value=k},function(k){h.status="rejected";h.reason=k});rb.set(a.id,h);return h}return 0<d.length?Promise.all(d):null}
function T(a){if(a.async){var b=rb.get(a.id);if("fulfilled"===b.status)b=b.value;else throw b.reason;}else b=globalThis.__next_require__(a.id);return"*"===a.name?b:""===a.name?b.__esModule?b.default:b:b[a.name]}function U(a,b,d,c){this.status=a;this.value=b;this.reason=d;this._response=c}U.prototype=Object.create(Promise.prototype);
