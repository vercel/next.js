...
function ma(a){return function(b,c){return"string"===typeof c?ja(a,this,b,c):"object"===typeof c&&null!==c?(b=c[0]===z?{$$typeof:z,type:c[1],key:c[2],ref:null,props:c[3],_owner:null}:c,b):c}}function X(a){U(a,Error("Connection closed."))}function Y(a){a=a&&a.callServer?a.callServer:void 0;var b=new Map;a={_bundlerConfig:null,_callServer:void 0!==a?a:ka,_chunks:b,_partialRow:"",_stringDecoder:null,_fromJSON:null};a._stringDecoder=new TextDecoder;a._fromJSON=ma(a);return a}
function Z(a,b){function c(f){var k=f.value;if(f.done)X(a);else{f=k;k=a._stringDecoder;for(var h=f.indexOf(10);-1<h;){var l=a._partialRow;var g=f.subarray(0,h);g=k.decode(g);W(a,l+g);a._partialRow="";f=f.subarray(h+1);h=f.indexOf(10)}a._partialRow+=k.decode(f,q);return e.read().then(c).catch(d)}}function d(f){U(a,f)}var e=b.getReader();e.read().then(c).catch(d)}exports.createFromFetch=function(a,b){var c=Y(b);a.then(function(d){Z(c,d.body)},function(d){U(c,d)});return V(c,0)};
exports.createFromReadableStream=function(a,b){b=Y(b);Z(b,a);return V(b,0)};
exports.createFromXHR=function(a,b){function c(){for(var k=a.responseText,h=f,l=k.indexOf("\n",h);-1<l;)h=e._partialRow+k.slice(h,l),W(e,h),e._partialRow="",h=l+1,l=k.indexOf("\n",h);e._partialRow+=k.slice(h);f=k.length}function d(){U(e,new TypeError("Network error"))}var e=Y(b),f=0;a.addEventListener("progress",c);a.addEventListener("load",function(){c();X(e)});a.addEventListener("error",d);a.addEventListener("abort",d);a.addEventListener("timeout",d);return V(e,0)};
