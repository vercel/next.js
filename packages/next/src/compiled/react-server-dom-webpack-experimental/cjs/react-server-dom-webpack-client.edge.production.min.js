...
function z(a){for(var b=a.chunks,c=[],d=0;d<b.length;d++){var g=b[d],h=w.get(g);if(void 0===h){h=globalThis.__next_chunk_load__(g);c.push(h);var n=w.set.bind(w,g,null);h.then(n,y);w.set(g,h)}else null!==h&&c.push(h)}if(a.async){if(b=x.get(a.id))return"fulfilled"===b.status?null:b;var e=Promise.all(c).then(function(){return globalThis.__next_require__(a.id)});e.then(function(m){e.status="fulfilled";e.value=m},function(m){e.status="rejected";e.reason=m});x.set(a.id,e);return e}return 0<c.length?Promise.all(c):null}
