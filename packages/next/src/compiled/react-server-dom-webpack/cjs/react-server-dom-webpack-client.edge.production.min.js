...
function A(a){for(var b=a.chunks,c=[],d=0;d<b.length;d++){var f=b[d],g=w.get(f);if(void 0===g){g=globalThis.__next_chunk_load__(f);c.push(g);var n=w.set.bind(w,f,null);g.then(n,z);w.set(f,g)}else null!==g&&c.push(g)}if(a.async){if(b=y.get(a.id))return"fulfilled"===b.status?null:b;var l=Promise.all(c).then(function(){return globalThis.__next_require__(a.id)});l.then(function(h){l.status="fulfilled";l.value=h},function(h){l.status="rejected";l.reason=h});y.set(a.id,l);return l}return 0<c.length?Promise.all(c):null}
