...
$a(k);h===b?(d=e.length,c=k.length,e+=k):e=10>k.length&&40>e.length+k.length?e+k:e+"..."}e+="}"}return void 0===b?e:-1<d&&0<c?(a=" ".repeat(d)+"^".repeat(c),"\n  "+e+"\n  "+a):"\n  "+e}var ab=aa.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ContextRegistry,bb=aa.__SECRET_SERVER_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
if(!bb)throw Error('The "react" package in this environment is not configured correctly. The "react-server" condition must be enabled in any environment that runs React Server Components.');var O=JSON.stringify,cb=bb.ReactCurrentDispatcher,db=bb.ReactCurrentCache;function eb(a){console.error(a)}function fb(){}
...
function ib(a){if(a){var b=D;Ja(null);for(var d=0;d<a.length;d++){var c=a[d],e=c[0];c=c[1];if(!ab[e]){var f={$$typeof:ya,_currentValue:C,_currentValue2:C,_defaultValue:C,_threadCount:0,Provider:null,Consumer:null,_globalName:e};f.Provider={$$typeof:xa,_context:f};ab[e]=f}Ka(ab[e],c)}a=D;Ja(b);return a}return null}
