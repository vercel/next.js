...
function v(a){for(var b=a.chunks,c=[],d=0;d<b.length;d++){var e=b[d],g=q.get(e);if(void 0===g){g=globalThis.__next_chunk_load__(e);c.push(g);var h=q.set.bind(q,e,null);g.then(h,u);q.set(e,g)}else null!==g&&c.push(g)}if(a.async){if(b=t.get(a.id))return"fulfilled"===b.status?null:b;var k=Promise.all(c).then(function(){return globalThis.__next_require__(a.id)});k.then(function(f){k.status="fulfilled";k.value=f},function(f){k.status="rejected";k.reason=f});t.set(a.id,k);return k}return 0<c.length?Promise.all(c):null}
