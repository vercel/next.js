...
function kb(a,b,d,c){var e=c.$$async?c.$$id+"#async":c.$$id,f=a.writtenClientReferences,g=f.get(e);if(void 0!==g)return b[0]===D&&"1"===d?"$L"+g.toString(16):"$"+g.toString(16);try{var h=a.bundlerConfig,k=c.$$id;g="";var q=h[k];if(q)g=q.name;else{var u=k.lastIndexOf("#");-1!==u&&(g=k.slice(u+1),q=h[k.slice(0,u)]);if(!q)throw Error('Could not find the module "'+k+'" in the React Client Manifest. This is probably a bug in the React Server Components bundler.');}var n={id:q.id,chunks:q.chunks,name:g,
async:!!c.$$async};a.pendingChunks++;var z=a.nextChunkId++,na=w(n);var A=z.toString(16)+":I"+na+"\n";a.completedImportChunks.push(A);f.set(e,z);return b[0]===D&&"1"===d?"$L"+z.toString(16):"$"+z.toString(16)}catch(wb){return a.pendingChunks++,b=a.nextChunkId++,d=P(a,wb),Q(a,b,d),"$"+b.toString(16)}}
...
function jb(a){var b=Ya.current;Ya.current=Pa;var d=O;I=O=a;try{var c=a.pingedTasks;a.pingedTasks=[];for(var e=0;e<c.length;e++){var f=c[e];var g=a;if(0===f.status){Ea(f.context);try{var h=f.model;if("object"===typeof h&&null!==h&&h.$$typeof===D){var k=h,q=f.thenableState;f.model=h;h=S(g,k.type,k.key,k.ref,k.props,q);for(f.thenableState=null;"object"===typeof h&&null!==h&&h.$$typeof===D;)k=h,f.model=h,h=S(g,k.type,k.key,k.ref,k.props,null)}var u=ea(g,f.id,h);g.completedJSONChunks.push(u);g.abortableTasks.delete(f);
...
function cb(a){if(a){var b=F;Ea(null);for(var d=0;d<a.length;d++){var c=a[d],e=c[0];c=c[1];Xa[e]||(Xa[e]=ca.createServerContext(e,xa));Fa(Xa[e],c)}a=F;Ea(b);return a}return null}function pb(a,b){var d="",c=a[b];if(c)d=c.name;else{var e=b.lastIndexOf("#");-1!==e&&(d=b.slice(e+1),c=a[b.slice(0,e)]);if(!c)throw Error('Could not find the module "'+b+'" in the React Server Manifest. This is probably a bug in the React Server Components bundler.');}return{id:c.id,chunks:c.chunks,name:d,async:!1}}
var T=new Map,qb=new Map;function rb(){}
function sb(a){for(var b=a.chunks,d=[],c=0;c<b.length;c++){var e=b[c],f=T.get(e);if(void 0===f){f=globalThis.__next_chunk_load__(e);d.push(f);var g=T.set.bind(T,e,null);f.then(g,rb);T.set(e,f)}else null!==f&&d.push(f)}if(a.async){if(b=qb.get(a.id))return"fulfilled"===b.status?null:b;var h=Promise.all(d).then(function(){return globalThis.__next_require__(a.id)});h.then(function(k){h.status="fulfilled";h.value=k},function(k){h.status="rejected";h.reason=k});qb.set(a.id,h);return h}return 0<d.length?Promise.all(d):
null}function tb(a){if(a.async){var b=qb.get(a.id);if("fulfilled"===b.status)b=b.value;else throw b.reason;}else b=globalThis.__next_require__(a.id);return"*"===a.name?b:""===a.name?b.__esModule?b.default:b:b[a.name]}function U(a,b,d,c){this.status=a;this.value=b;this.reason=d;this._response=c}U.prototype=Object.create(Promise.prototype);
...
function ub(a,b){if("pending"===a.status||"blocked"===a.status){var d=a.reason;a.status="rejected";a.reason=b;null!==d&&W(d,b)}}function vb(a,b,d,c,e,f){var g=pb(a._bundlerConfig,b);a=sb(g);if(d)d=Promise.all([d,a]).then(function(h){h=h[0];var k=tb(g);return k.bind.apply(k,[null].concat(h))});else if(a)d=Promise.resolve(a).then(function(){return tb(g)});else return tb(g);d.then(xb(c,e,f),yb(c));return null}var X=null,Y=null;
...
function Z(a,b){var d=a._chunks,c=d.get(b);c||(c=a._formData.get(a._prefix+b),c=null!=c?new U("resolved_model",c,null,a):new U("pending",null,null,a),d.set(b,c));return c}function xb(a,b,d){if(Y){var c=Y;c.deps++}else c=Y={deps:1,value:null};return function(e){b[d]=e;c.deps--;0===c.deps&&"blocked"===a.status&&(e=a.value,a.status="fulfilled",a.value=c.value,null!==e&&W(e,c.value))}}function yb(a){return function(b){return ub(a,b)}}
...
c?-0:-Infinity;case "N":return NaN;case "u":return;case "D":return new Date(Date.parse(c.slice(2)));case "n":return BigInt(c.slice(2));default:c=parseInt(c.slice(1),16);a=Z(a,c);switch(a.status){case "resolved_model":V(a)}switch(a.status){case "fulfilled":return a.value;case "pending":case "blocked":return c=X,a.then(xb(c,b,d),yb(c)),null;default:throw a.reason;}}return c}
...
function Cb(a,b,d){a._formData.append(b,d);var c=a._prefix;if(b.startsWith(c)&&(a=a._chunks,b=+b.slice(c.length),(b=a.get(b))&&"pending"===b.status&&(c=b.value,a=b.reason,b.status="resolved_model",b.value=d,null!==c)))switch(V(b),b.status){case "fulfilled":W(c,b.value);break;case "pending":case "blocked":b.value=c;b.reason=a;break;case "rejected":a&&W(a,b.reason)}}function Db(a){zb(a,Error("Connection closed."))}function Eb(a,b){return function(){return nb(b,a)}}
...
new Blob(u,{type:q});d._formData.append(f,n,k);c--;if(0===c){for(n=0;n<e.length;n+=2)Cb(d,e[n],e[n+1]);e.length=0}})});a.on("finish",function(){Db(d)});a.on("error",function(f){zb(d,f)});return Z(d,0)};
exports.renderToPipeableStream=function(a,b,d){var c=ab(a,b,d?d.onError:void 0,d?d.context:void 0,d?d.identifierPrefix:void 0),e=!1;mb(c);return{pipe:function(f){if(e)throw Error("React currently only supports piping to one writable stream.");e=!0;nb(c,f);f.on("drain",Eb(f,c));return f},abort:function(f){ob(c,f)}}};
