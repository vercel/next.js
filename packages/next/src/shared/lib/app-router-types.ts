...
export const enum PrefetchHint {
   ...
   * Propagates upward so the root reflects the entire subtree
   *  Used only to suppress the dev-time `<Link prefetch={true}>` warning —
   * unlike PrefetchDisabled, it has no effect on the actual prefetch behavior.
   */
  SubtreeHasInstantFalse = 0b100000000000,

  // TODO(prerender-data-tracking): audit and re-use the parts of this comment that
  // are still relevant
  ...
  /** The shell should use a static request. */
  ShouldAttemptStaticShell = 0b1000000000000,
  /** The prefetch should use a static request */
  ShouldAttemptStaticPrefetch = 0b10000000000000,
}
