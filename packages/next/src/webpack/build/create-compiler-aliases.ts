...
import { NEXT_PROJECT_ROOT } from 'next/dist/build/next-dir-paths'
 ...
 * `webpack-config.ts`.
 */
const INSTRUMENTATION_CLIENT_STUB_PATH = require.resolve(
  './webpack/loaders/instrumentation-client-stub'
)
...
export function createWebpackAliases({
  ...
  return {
    ...
    [RSC_ACTION_VALIDATE_ALIAS]:
      'next/dist/webpack/build/webpack/loaders/next-flight-loader/action-validate',

    [RSC_ACTION_CLIENT_WRAPPER_ALIAS]:
      'next/dist/webpack/build/webpack/loaders/next-flight-loader/action-client-wrapper',

    [RSC_ACTION_PROXY_ALIAS]:
      'next/dist/webpack/build/webpack/loaders/next-flight-loader/server-reference',

    [RSC_ACTION_ENCRYPTION_ALIAS]: 'next/dist/server/app-render/encryption',

    [RSC_CACHE_WRAPPER_ALIAS]:
      'next/dist/webpack/build/webpack/loaders/next-flight-loader/cache-wrapper',
    [RSC_DYNAMIC_IMPORT_WRAPPER_ALIAS]:
...
export function createVendoredReactAliases(
  ...
  if (environmentCondition === 'browser' && reactCondition === 'client') {
    ...
    reactAlias = {
      ...
      'react-dom/server.edge$':                /* ❌ */ `next/dist/webpack/build/webpack/alias/react-dom-server${bundledReactChannel}.js`,
    ...
    reactAlias = {
      ...
      'react-dom$':                            /* ✅ */ `next/dist/compiled/react-dom${bundledReactChannel}`,
      'react-dom/client$':                     /* ✅ */ `next/dist/compiled/react-dom${bundledReactChannel}/client`,
      'react-dom/server$':                     /* ✅ */ `next/dist/webpack/build/webpack/alias/react-dom-server${bundledReactChannel}.js`,
      'react-dom/server.browser$':             /* ✅ */ `next/dist/compiled/react-dom${bundledReactChannel}/server.browser`,
      // optimizations to ignore the legacy build of react-dom/server in `server.edge` build
      'react-dom/server.edge$':                /* ✅ */ `next/dist/webpack/build/webpack/alias/react-dom-server${bundledReactChannel}.js`,
