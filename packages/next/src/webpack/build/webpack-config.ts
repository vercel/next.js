...
import { webpack } from 'next/dist/webpack/compiled/webpack/webpack'
...
} from 'next/dist/build/load-jsconfig'
...
} from 'next/dist/build/next-dir-paths'
...
const DEFAULT_TRANSPILED_PACKAGES =
  require('next/dist/lib/default-transpiled-packages.json') as string[]
...
export default async function getBaseWebpackConfig(
  ...
  if (useSWCLoader) {
    ...
    const binaryTarget = (
      require('next/dist/build/swc') as typeof import('next/dist/build/swc')
    ...
    plugins: [
      ...
      isClient &&
        new CopyFilePlugin({
          ...
          cacheKey: process.env.__NEXT_VERSION ?? nextVersion,
  ...
  const cache: webpack.Configuration['cache'] = {
    ...
    version: `${__dirname}|${process.env.__NEXT_VERSION ?? nextVersion}|${configVars}`,
