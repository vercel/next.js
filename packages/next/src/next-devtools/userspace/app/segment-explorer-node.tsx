...
function SegmentTrieNode({ type, pagePath }: { type: string; pagePath: string }): React.ReactNode {
...
export function SegmentViewNode({
  ...
  const segmentNode = <SegmentTrieNode key={type} type={type} pagePath={pagePath} />
...
export function SegmentStateProvider({ children }: { children: ReactNode }) {
  const [boundaryType, setBoundaryType] = useState<SegmentBoundaryType | null>(null)
