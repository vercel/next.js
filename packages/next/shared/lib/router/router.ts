...
export default class Router implements BaseRouter {
  ...
  async getRouteInfo({
    ...
    try {
      /**
       * If there is already existing information about the route and `shallow`
       * is being enabled, we should not run any fetching method and instead
       * return the information we already have. This should happen only when
       * the current route is the same we are loading.
       */
      ...
      /**
       * If there is existing and *complete* info about the existing route and
       * we are *not* in development we will use that info instead of fetching
       * it again. For production this is disabled due to HMR being able to
      ...
      /**
       * Load the route information in case we don't have it already. This
       * goes to the server and gets all information about the component
       * that corresponds to the route.
       */
      const routeInfo =
        cachedRouteInfo ||
      ...
      /**
       * In development we check if the routeInfo Component is a valid React
       * component and if it is not we throw an error.
       */
      if (process.env.NODE_ENV !== 'production') {
      ...
      /**
       * For now we will fetch data when a route is for an SSG page,
       * SSR page or a RSC page.
       */
      ...
      const { props } = await this._getData(async () => {
        if (shouldFetchData && !useStreamedFlightData) {
          const { response, json } = await fetchNextData({
            dataHref: dataHref!,
