...
export default class BaseServer {
  ...
  protected require<T = any>(moduleId: string): T {
    this.notImplemented()
  }

  protected async requireAsync<T = any>(moduleId: string): Promise<T> {
    this.notImplemented()
  }
  ...
  protected async getMiddlewareInfo(params: {
    dev?: boolean
    distDir: string
    page: string
    serverless: boolean
  ...
  protected async executeMiddleware(params: {
    name: string
  ...
  protected async renderComponentToHTML(
    req: IncomingMessage,
    res: ServerResponse,
    pathname: string,
    query: NextParsedUrlQuery,
  ...
  protected async serveStaticFile(
    req: IncomingMessage,
    res: ServerResponse,
    path: string
  ): Promise<void> {
  ...
  public constructor({
    ...
    if (!dev) {
      this.pagesManifest = this.require(pagesManifestPath)
      if (!this.minimalMode) {
        this.middlewareManifest = this.require(middlewareManifestPath)
      }
    }
  ...
  protected getCustomRoutes(): CustomRoutes {
    const customRoutes = this.require(join(this.distDir, ROUTES_MANIFEST))
  ...
  protected getPrerenderManifest(): PrerenderManifest {
    ...
    const manifest = this.require(join(this.distDir, PRERENDER_MANIFEST))
  ...
  protected async runMiddleware(params: {
    ...
    for (const middleware of this.middleware || []) {
      if (middleware.match(params.parsedUrl.pathname)) {
        ...
        const middlewareInfo = await this.getMiddlewareInfo({
          dev: this.renderOpts.dev,
          distDir: this.distDir,
        ...
        result = await this.executeMiddleware({
  ...
  private async handleApiRequest(
    ...
    const pageModule = await this.requireAsync(builtPagePath)
    query = { ...query, ...params }
