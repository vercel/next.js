...
export type RenderResult = StaticRenderResult | DynamicRenderResult

export class StaticRenderResult {
  #chunks: string[]
  ...
  static resultOrEmpty(result: RenderResult | null): StaticRenderResult {
    if (result) {
      if (!(result instanceof StaticRenderResult)) {
        throw new Error(
          'invariant: result was not static. This is a bug in Next.js'
        )
      }
    }
    return StaticRenderResult.empty
  }

  static empty = new StaticRenderResult([])
}

export class DynamicRenderResult {
