 ...
 */
export async function recursiveReadDir(dir: string, filter: RegExp, ignore?: RegExp, includeDirs?: boolean, arr: string[] = [], rootDir: string = dir): Promise<string[]> {
  ...
  await Promise.all(result.map(async (part: string) => {
    ...
    if (pathStat.isDirectory()) {
      ...
      await recursiveReadDir(absolutePath, filter, ignore, includeDirs, arr, rootDir)
      return
    }
