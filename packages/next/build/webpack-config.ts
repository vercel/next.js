...
export default function getBaseWebpackConfig (dir: string, {dev = false, debug = false, isServer = false, buildId, config, target = 'server', entrypoints, selectivePageBuilding = false, pkgsString = '', envString = ''}: {dev?: boolean, debug?: boolean, isServer?: boolean, buildId: string, config: any, target?: string, entrypoints: WebpackEntrypoints, selectivePageBuilding?: boolean, envString?: string, pkgsString?: string}): webpack.Configuration {
  ...
  let webpackConfig: webpack.Configuration = {
    ...
    plugins: [
      ...
      !isServer && selectivePageBuilding && new ChunkGraphPlugin(buildId, path.resolve(dir), { filename: CHUNK_GRAPH_MANIFEST, envString, pkgsString }),
      ...(dev ? (() => {
