...
import {
  ...
  type DirectoryWithPageExtensions,
} from '../utils/url'
...
export default defineRule({
  ...
  create(context) {
    ...
    const rootDirsWithPageExtensions = rootDirs.map((rootDir) => ({
      rootDir,
      pageExtensions: getPageExtensions(rootDir),
    }))

    const getDirectoryPageExtensions = (directory: string) => {
      const matchingRootDir = rootDirsWithPageExtensions
        .filter(
          ({ rootDir }) =>
            directory === rootDir ||
        ...
        .sort(
          (rootDirA, rootDirB) =>
            rootDirB.rootDir.length - rootDirA.rootDir.length
        )[0]

      return matchingRootDir?.pageExtensions ?? defaultPageExtensions
    }
    ...
    const foundPagesDirs: DirectoryWithPageExtensions[] = pagesDirs
      .filter((dir) => {
      ...
      .map((directory) => ({
        directory,
        pageExtensions: getDirectoryPageExtensions(directory),
      }))
    ...
    const foundAppDirs: DirectoryWithPageExtensions[] = appDirs
      .filter((dir) => {
        if (fsExistsSyncCache[dir] === undefined) {
          fsExistsSyncCache[dir] = fs.existsSync(dir)
        }
