 ...
 */
function createPageExtensionRegex(pageExtensions: string[]): RegExp {
  // Escape special regex characters in extensions and join with |
  const escapedExtensions = pageExtensions
    .map((ext) => ext.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
    .join('|')
  return new RegExp(`\\.(${escapedExtensions})$`)
}
...
function parseUrlForPages(
  ...
  const extensionRegex = createPageExtensionRegex(pageExtensions)
  ...
  fsReadDirSyncCache[directory].forEach((dirent) => {
    if (extensionRegex.test(dirent.name)) {
      if (indexRegex.test(dirent.name)) {
        // Remove the index.extension part
        const withoutIndex = dirent.name.replace(indexRegex, '')
        res.push(`${urlprefix}${withoutIndex}`)
      }
      // Remove the extension part
      const withoutExtension = dirent.name.replace(extensionRegex, '')
      res.push(`${urlprefix}${withoutExtension}`)
    } else {
  ...
  fsReadDirSyncCache[directory].forEach((dirent) => {
    if (extensionRegex.test(dirent.name)) {
      if (pageRegex.test(dirent.name)) {
        // Remove the page.extension part
        const withoutPage = dirent.name.replace(pageRegex, '')
        res.push(`${urlprefix}${withoutPage}`)
      } else if (!layoutRegex.test(dirent.name)) {
        // Remove the extension part for route files
        const withoutExtension = dirent.name.replace(extensionRegex, '')
        res.push(`${urlprefix}${withoutExtension}`)
      }
    } else {
