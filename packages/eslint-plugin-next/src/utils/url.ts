...
/**
 * Build a regex pattern from page extensions array
 */
function buildPageExtensionRegex(pageExtensions: string[]): RegExp {
  const extensionPattern = pageExtensions.map(ext => ext.replace('.', '\\.')).join('|')
...
function parseUrlForPages(
  ...
  const res = []
  const extensionRegex = buildPageExtensionRegex(pageExtensions)
  const indexRegex = new RegExp(`^index\\.(${pageExtensions.map(ext => ext.replace('.', '\\.')).join('|')})$`)
...
function parseUrlForAppDir(
  ...
  const res = []
  const extensionRegex = buildPageExtensionRegex(pageExtensions)
  const pageRegex = new RegExp(`^page\\.(${pageExtensions.map(ext => ext.replace('.', '\\.')).join('|')})$`)
  const layoutRegex = new RegExp(`^layout\\.(${pageExtensions.map(ext => ext.replace('.', '\\.')).join('|')})$`)
