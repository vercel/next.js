...
function buildExtensionPattern(pageExtensions: string[]) {
  const exts = pageExtensions.length ? pageExtensions : DEFAULT_PAGE_EXTENSIONS
  return `\\.(?:${exts.map(escapeRegExp).join('|')})$`
}
...
function parseUrlForPages(
  ...
  const res = []
  const extensionPattern = buildExtensionPattern(pageExtensions)
  const extensionRegex = new RegExp(extensionPattern)
  const indexRegex = new RegExp(`^index${extensionPattern}`)
...
function parseUrlForAppDir(
  ...
  const extensionPattern = buildExtensionPattern(pageExtensions)
  const extensionRegex = new RegExp(extensionPattern)
  const pageRegex = new RegExp(`^page${extensionPattern}`)
  const layoutRegex = new RegExp(`^layout${extensionPattern}`)
  fsReadDirSyncCache[directory].forEach((dirent) => {
    if (extensionRegex.test(dirent.name)) {
