...
/**
 * Build a RegExp that matches any of the given page extensions.
 */
function buildExtensionRegex(pageExtensions: string[]): RegExp {
...
function parseUrlForPages(
  ...
  fsReadDirSyncCache[directory].forEach((dirent) => {
    if (extRegex.test(dirent.name)) {
      const nameWithoutExt = dirent.name.replace(extRegex, '')
  ...
  fsReadDirSyncCache[directory].forEach((dirent) => {
    if (extRegex.test(dirent.name)) {
      ...
      if (nameWithoutExt === 'page') {
        // `page.tsx` is the route leaf file; the URL is the parent prefix
        res.push(`${urlprefix}`)
