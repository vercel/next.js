...
function parseUrlForPages(
  ...
  const res = []
  const extRegex = new RegExp(`\\.(${pageExtensions.join('|')})$`)
  fsReadDirSyncCache[directory].forEach((dirent) => {
    if (extRegex.test(dirent.name)) {
      if (new RegExp(`^index\\.(${pageExtensions.join('|')})$`).test(dirent.name)) {
        res.push(
          `${urlprefix}${dirent.name.replace(new RegExp(`^index\\.(${pageExtensions.join('|')})$`), '')}`
        )
      }
...
function parseUrlForAppDir(
  ...
  fsReadDirSyncCache[directory].forEach((dirent) => {
    if (extRegex.test(dirent.name)) {
      if (new RegExp(`^page\\.(${pageExtensions.join('|')})$`).test(dirent.name)) {
        res.push(`${urlprefix}${dirent.name.replace(new RegExp(`^page\\.(${pageExtensions.join('|')})$`), '')}`)
      } else if (!new RegExp(`^layout\\.(${pageExtensions.join('|')})$`).test(dirent.name)) {
        res.push(`${urlprefix}${dirent.name.replace(extRegex, '')}`)
      }
    } else {
      const dirPath = path.join(directory, dirent.name)
...
function ensureLeadingSlash(targetPath: string) {
