...
/**
 * Builds a regex pattern that matches files with the given page extensions.
 */
function buildPageExtensionsRegex(extensions: string[]): RegExp {
...
function parseUrlForPages(
  ...
  const extensions = pageExtensions || ['tsx', 'ts', 'jsx', 'js']
  const extRegex = buildPageExtensionsRegex(extensions)
  const indexRegex = new RegExp(`^index\\.(${extensions.map((e) => e.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})$`)
...
function parseUrlForAppDir(
  ...
  const extensions = pageExtensions || ['tsx', 'ts', 'jsx', 'js']
  const extRegex = buildPageExtensionsRegex(extensions)
  const pageRegex = new RegExp(`^page\\.(${extensions.map((e) => e.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})$`)
  const layoutRegex = new RegExp(`^layout\\.(${extensions.map((e) => e.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})$`)
