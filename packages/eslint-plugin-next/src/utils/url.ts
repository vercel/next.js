...
function extensionRegExp(pageExtensions: string[]) {
  return pageExtensions.map(escapeRegExp).join('|')
}
...
function parseUrlForPages(
  ...
  const ext = extensionRegExp(pageExtensions)
  const filePattern = new RegExp(`\\.(${ext})$`)
  const indexPattern = new RegExp(`^index\\.(${ext})$`)
  ...
  fsReadDirSyncCache[directory].forEach((dirent) => {
    if (filePattern.test(dirent.name)) {
      ...
      res.push(`${urlprefix}${dirent.name.replace(filePattern, '')}`)
    } else {
...
function parseUrlForAppDir(
  ...
  const ext = extensionRegExp(pageExtensions)
  const filePattern = new RegExp(`\\.(${ext})$`)
  const pagePattern = new RegExp(`^page\\.(${ext})$`)
  const layoutPattern = new RegExp(`^layout\\.(${ext})$`)
  fsReadDirSyncCache[directory] ??= fs.readdirSync(directory, {
    withFileTypes: true,
  })
  const res = []
