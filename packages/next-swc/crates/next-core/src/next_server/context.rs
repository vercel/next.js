...
use crate::{
    ...
    next_shared::{
        ...
        transforms::{
            emotion::get_emotion_transform_plugin, module_rule_ecmascript_transform_plugins,
            next_react_server_components::get_next_react_server_components_transform_plugin,
            relay::get_relay_transform_plugin,
...
pub async fn get_server_module_options_context(
    ...
    // This matches to the behavior of existing webpack config, if the issuer layer
    // is ssr or pages-browser (client bundle for the browser) applies
    // client-specific swc transforms.
    //
    // This enables correct emotion transform and other hydration between server and
    // client bundles.
    // Ref: https://github.com/vercel/next.js/blob/4bbf9b6c70d2aa4237defe2bebfa790cdb7e334e/packages/next/src/build/webpack-config.ts#L1421-L1426
    ...
    let base_source_transforms: Vec<Vc<TransformPlugin>> = [
    ...
    let ecma_transform_rule = |transforms: &[Vc<TransformPlugin>]| {
        module_rule_ecmascript_transform_plugins(enable_mdx_rs.is_some(), transforms, &[])
    };
    ...
    let module_options_context = match ty.into_value() {
        ...
        | ServerContextType::PagesData { .. }
        | ServerContextType::PagesApi { .. } => {
            let mut custom_source_transforms: Vec<Vc<TransformPlugin>> = [
            ...
            internal_custom_rules.push(ecma_transform_rule(&custom_source_transforms));

            if let ServerContextType::Pages { .. } = ty.into_value() {
                custom_source_transforms.push(get_next_react_server_components_transform_plugin(
                    false, None,
                ));
            }
            custom_source_transforms.extend(base_source_transforms);

            base_next_server_rules.push(ecma_transform_rule(&custom_source_transforms));
        ...
        ServerContextType::AppSSR { app_dir, .. } => {
            let mut custom_source_transforms: Vec<Vc<TransformPlugin>> = vec![
                styled_components_transform_plugin,
                styled_jsx_transform_plugin,
            ]
            .into_iter()
        ...
        ServerContextType::AppRSC {
            ...
            if let Some(ecmascript_client_reference_transition_name) =
                ecmascript_client_reference_transition_name
            {
                custom_source_transforms.push(Vc::cell(Box::new(ClientDirectiveTransformer::new(
                    ecmascript_client_reference_transition_name,
                )) as _));
            }

            let base_ecma_transform_rule = module_rule_ecmascript_transform_plugins(
                enable_mdx_rs.is_some(),
                &custom_source_transforms,
                &[],
            );
            internal_custom_rules.push(base_ecma_transform_rule);
            ...
            custom_source_transforms.extend(base_source_transforms);
            base_next_server_rules.push(module_rule_ecmascript_transform_plugins(
                enable_mdx_rs.is_some(),
                &custom_source_transforms,
                &[],
            ));
        ...
        ServerContextType::AppRoute { .. } => {
            base_next_server_rules.push(ecma_transform_rule(&base_source_transforms));
