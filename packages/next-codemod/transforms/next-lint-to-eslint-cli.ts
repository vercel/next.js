...
export default function transformer(
  ...
  if (existingConfig.exists === false) {
    ...
    if (existingConfig.isLegacy && existingConfig.path) {
      ...
      // Run @eslint/migrate-config with the project's own package manager, so
      // that e.g. a pnpm project doesn't get an `npx` invocation.
      const npxCommand = getNpxCommand(getPkgManager(projectRoot), projectRoot)
      const command = `${npxCommand} @eslint/migrate-config ${existingConfig.path}`
