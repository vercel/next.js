...
describe('app-document - Rendering via HTTP', () => {
  ...
  function render(path: string) {
    return next.render(path, undefined, {
  ...
  describe('_document', () => {
    ...
    it('renders ctx.renderPage with enhancer correctly', async () => {
      const $ = await render$('/?withEnhancer=true')
    ...
    it('renders ctx.renderPage with enhanceComponent correctly', async () => {
      const $ = await render$('/?withEnhanceComponent=true')
      const nonce = 'RENDERED'
      expect($('#render-page-enhance-component').text().includes(nonce)).toBe(
        true
      )
    ...
    it('renders ctx.renderPage with enhanceApp correctly', async () => {
      const $ = await render$('/?withEnhanceApp=true')
      const nonce = 'RENDERED'
    ...
    it('renders ctx.renderPage with enhanceApp and enhanceComponent correctly', async () => {
      const $ = await render$('/?withEnhanceComponent=true&withEnhanceApp=true')
      const nonce = 'RENDERED'
      expect($('#render-page-enhance-app').text().includes(nonce)).toBe(true)
      expect($('#render-page-enhance-component').text().includes(nonce)).toBe(
        true
    ...
    it('adds a timestamp to link tags with preload attribute to invalidate the cache (DEV only)', async () => {
      const $ = await render$('/')
  ...
  describe('_app', () => {
    ...
    if (isNextDev) {
      it('should show valid error when thrown in _app getInitialProps', async () => {
        ...
        expect(await render('/')).toContain('page-index')
        ...
        let foundErr = false
        try {
          await retry(async () => expect(await render('/')).toContain(errMsg))
          foundErr = true
        } finally {
