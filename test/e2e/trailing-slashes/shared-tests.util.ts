...
export function testShouldRedirect(next: NextInstance, expectations: [string, string][]) {
  it.each(expectations)('%s should redirect to %s', async (route, expectedLocation) => {
...
export function testShouldResolve(next: NextInstance, expectations: [string, string, string][]) {
  ...
  it.each(expectations)(
    ...
    async (route, expectedPage, expectedRouterPath) => {
      ...
      try {
        ...
        const routerPathname = await browser.elementByCss('#router-pathname').text()
...
export function testExternalLinkShouldRewriteTo(
  ...
  it.each(expectations)('%s should have href %s', async (linkPage, expectedHref) => {
    const $ = await next.render$(linkPage)
    expect($('#link').attr('href')).toBe(expectedHref)
  })
}

export function testLinkShouldRewriteTo(next: NextInstance, expectations: [string, string][]) {
  it.each(expectations)('%s should have href %s', async (linkPage, expectedHref) => {
    const $ = await next.render$(linkPage)
    expect($('#link').attr('href')).toBe(expectedHref)
  })

  it.each(expectations)('%s should navigate to %s', async (linkPage, expectedHref) => {
    let browser: Playwright | undefined
    try {
  ...
  it.each(expectations)('%s should push route to %s', async (linkPage, expectedHref) => {
    let browser: Playwright | undefined
    try {
      browser = await next.browser(linkPage)
