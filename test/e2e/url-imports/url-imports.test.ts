import { getBrowserBodyText, startStaticServer, stopApp, retry } from 'next-test-utils'
import { FileRef, nextTestSetup, isNextDev } from 'e2e-utils'
import { join } from 'path'

// experimental.urlImports is not implemented in Turbopack
;(process.env.IS_TURBOPACK_TEST ? describe.skip : describe)(`Handle url imports`, () => {
  let staticServer
  let staticServerPort
  beforeAll(async () => {
    staticServerPort = 12345
    staticServer = await startStaticServer(join(__dirname, 'source'), undefined, staticServerPort)
  })
  afterAll(async () => {
  ...
  const expectedClient = new RegExp(expectedServer.source.replace(/<!-- -->/g, ''))
  ...
  for (const page of ['/static', '/ssr', '/ssg']) {
    ...
    it(`should client-render the ${page} page`, async () => {
      ...
      await retry(async () => expect(await getBrowserBodyText(browser)).toMatch(expectedClient))
    })
  }
  ...
  it(`should render a static url image import`, async () => {
    ...
    await retry(async () =>
      expect(await browser.elementByCss('#static-image').getAttribute('src')).toMatch(
  ...
  it(`should allow url import in css`, async () => {
    ...
    await retry(async () =>
      expect(await browser.elementByCss('#static-css').getComputedCss('background-image')).toMatch(
  ...
  it('should respond on value api', async () => {
    const data = await next.fetch('/api/value').then((res) => res.ok && res.json())
