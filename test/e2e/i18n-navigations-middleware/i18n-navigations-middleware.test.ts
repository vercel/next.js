...
describe('i18n-navigations-middleware', () => {
  ...
  it('should respect selected locale when navigating to a dynamic route', async () => {
    ...
    const dynamicLink = await browser.waitForElementByCss("[href='/de/dynamic/1']")
    expect(await browser.elementById('current-locale').text()).toBe('Current locale: de')

    // navigate to dynamic route
    await dynamicLink.click()

    // the locale should still be "de"
    expect(await browser.elementById('dynamic-locale').text()).toBe('Locale: de')
  })
