...
describe('basePath query/hash handling', () => {
  ...
  it.each([
    ...
    { search: '?hello=', hash: '#a', query: { hello: '' } },
  ])('is correct during query updating $hash $search', async ({ hash, search, query }) => {
    const browser = await webdriver(next.url, `${basePath}${search || ''}${hash || ''}`)

    await check(() => browser.eval('window.next.router.isReady ? "ready" : "not ready"'), 'ready')
    ...
    expect(JSON.parse(await browser.eval('JSON.stringify(next.router.query)'))).toEqual(query || {})
  })
