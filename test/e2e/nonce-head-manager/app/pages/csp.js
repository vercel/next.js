...
const Page = () => {
  ...
  const [scriptURL, toggleScriptSource] = React.useReducer((currentScriptURL, id) => {
    const scriptSource =
      typeof currentScriptURL === 'string' ? currentScriptURL : currentScriptURL.pathname
  ...
  const scriptSource = typeof scriptURL === 'string' ? scriptURL : scriptURL.href
