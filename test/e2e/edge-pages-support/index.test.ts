...
import { normalizeRegEx, withQuery } from 'next-test-utils'
...
createNextDescribe(
  ...
  ({ next, isNextStart }) => {
    ...
    it('should have correct query for pages/api', async () => {
      const res = await next.fetch(withQuery('/api/hello', { a: 'b' }))
    ...
    it('should have correct query for pages/api dynamic', async () => {
      const res = await next.fetch(withQuery('/api/id-1', { a: 'b' }))
      expect(res.status).toBe(200)
      expect(await res.json()).toEqual({
        hello: 'again',
        query: {
    ...
    it('should have correct query/params on /[id]', async () => {
      const $ = await next.render$('/123', { hello: 'world' })
    ...
    it('should have correct query/params on rewrite', async () => {
      const $ = await next.render$('/rewrite-me', { hello: 'world' })
      expect($('#page').text()).toBe('/index')
      const props = JSON.parse($('#props').text())
    ...
    it('should have correct query/params on dynamic rewrite', async () => {
      const $ = await next.render$('/rewrite-me-dynamic', { hello: 'world' })
      expect($('#page').text()).toBe('/[id]')
      const props = JSON.parse($('#props').text())
    ...
    it('should respond to _next/data for [id] correctly', async () => {
      const res = await next.fetch(
        withQuery(`/_next/data/${next.buildId}/321.json`, { hello: 'world' }),
        {
          headers: {
            'x-nextjs-data': '1',
          },
      ...
      const { pageProps: props } = JSON.parse(text)
