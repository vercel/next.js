...
This module doesn't have an associated type`)
        expect(next.cliOutput.match(/Unknown module type/g).length).toBeLessThanOrEqual(1)
        expect(next.cliOutput.match(/Missing module type/g).length).toBeLessThanOrEqual(1)
      } else {
        expect(next.cliOutput).toContain(`pkg/index.ts
