...
describe('opentelemetry use cache and after', () => {
  ...
  it('does not trace a second background refresh for a deduplicated invocation', async () => {
    const key = `deduplicated-${Date.now()}`
    const pathname = `/api/cache?background=1&key=${key}`
    const beforePrimeSpanIds = currentSpanIds()

    await retry(async () => {
      expect((await next.fetch(pathname, { method: 'POST' })).status).toBe(200)
      expect(
        getCollector()
          .getSpans()
          .some(
    ...
    const refreshTraceId = 'abcdef0123456789abcdef0123456789'
    const joinedTraceId = 'fedcba9876543210fedcba9876543210'
    let schedulingSpan: SavedSpan | undefined
    try {
      await retry(async () => {
        expect(
          (
            await next.fetch(`${pathname}&hold-background=1`, {
              method: 'POST',
              headers: {
                traceparent: `00-${refreshTraceId}-123456789abcdef0-01`,
              },
            })
          ).status
        ).toBe(200)
        schedulingSpan = getCollector()
          .getSpans()
          .find(
            (span) =>
              span.traceId === refreshTraceId &&
              span.attributes?.['next.span_type'] === 'UseCache.execute' &&
              span.attributes?.['next.cache.background_refresh'] === true
          )
        expect(schedulingSpan).toBeDefined()
      })

      const beforeJoinedSpanIds = currentSpanIds()
      expect(
        (
          await next.fetch(pathname, {
            method: 'POST',
            headers: {
              traceparent: `00-${joinedTraceId}-123456789abcdef0-01`,
            },
          })
        ).status
      ).toBe(200)
      const joinedSpans = (
        await getNewUseCacheSpans(beforeJoinedSpanIds, (spans) =>
          spans.some((span) => span.traceId === joinedTraceId)
        )
      ).filter((span) => span.traceId === joinedTraceId)
      expect(joinedSpans).toHaveLength(1)
      expect(joinedSpans[0].attributes?.['next.cache.background_refresh']).toBe(
        undefined
      )
    } finally {
      await next.fetch(`/api/cache?release-background=1&key=${key}`, {
        method: 'POST',
      })
    }

    await retry(() => {
      const revalidationSpans = getCollector()
        .getSpans()
        .filter(
          (span) =>
            (span.traceId === refreshTraceId ||
              span.traceId === joinedTraceId) &&
            span.attributes?.['next.span_type'] === 'UseCache.revalidate'
        )
      expect(revalidationSpans).toHaveLength(1)
      expect(revalidationSpans[0].parentId).toBe(schedulingSpan?.id)
    })
  })
