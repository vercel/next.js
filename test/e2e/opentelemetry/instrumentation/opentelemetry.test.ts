...
describe('opentelemetry', () => {
  ...
  for (const env of [
    ...
    ;(process.env.__NEXT_CACHE_COMPONENTS ? describe.skip : describe)(env.name, () => {
      describe('app router', () => {
              ...
              spans: [
                {
                  ...
                  attributes: {
                    'next.route': '/api/app/[param]/data',
                    'next.span_name': 'executing api route (app) /api/app/[param]/data',
            ...
            [
              {
                ...
                attributes: {
                  ...
                  'next.span_name': 'executing api route (app) /api/app/[param]/data/edge',
                  'next.span_type': 'AppRouteRouteHandlers.runHandler',
                },
                kind: 0,
                status: { code: 0 },
        ...
        it('should handle error in RSC', async () => {
          await next.fetch('/app/param/rsc-fetch/error?status=error', env.fetchInit)
      ...
      describe('pages', () => {
        ...
        it('should handle getServerSideProps on edge', async () => {
          await next.fetch('/pages/param/edge/getServerSideProps', env.fetchInit)
        ...
        it('should handle getServerSideProps exceptions', async () => {
          await next.fetch('/pages/param/getServerSidePropsError', env.fetchInit)
              ...
              spans: [
                {
                  ...
                  attributes: {
                    'next.route': '/pages/[param]/getServerSidePropsError',
                    'next.span_name': 'getServerSideProps /pages/[param]/getServerSidePropsError',
                    'next.span_type': 'Render.getServerSideProps',
                    'error.type': 'Error',
                  },
                  kind: 0,
        ...
        it('should handle getServerSideProps returning notFound', async () => {
          await next.fetch('/pages/param/getServerSidePropsNotFound', env.fetchInit)
            ...
            [
              {
                ...
                attributes: {
                  'next.span_name': 'executing api route (pages) /api/pages/[param]/edge',
                  'next.span_type': 'Node.runHandler',
                },
                kind: 0,
                status: { code: 0 },
...
describe('opentelemetry with disabled fetch tracing', () => {
  ...
  ;(process.env.__NEXT_CACHE_COMPONENTS ? describe.skip : describe)('root context', () => {
...
async function expectTrace(collector: Collector, match: SpanMatch[], edgeOnly?: boolean) {
  ...
  const expectedTargets = new Set(
    match.map((m) => m.attributes?.['http.target'] as string | undefined).filter(Boolean)
  )
  ...
  await check(async () => {
    ...
    for (const span of spansForTree) {
      ...
      span.traceId = span.traceId === EXTERNAL.traceId ? span.traceId : '[trace-id]'
      ...
      span.spans.sort((a, b) => {
        ...
        return (
          (a.attributes?.['next.route'] ?? '').localeCompare(b.attributes?.['next.route'] ?? '') ??
          0
        )
      })
    }
