...
describe('parallel-routes-catchall-slotted-non-catchalls', () => {
  ...
  it('should match default and dynamic segment paths before catch-all', async () => {
    ...
    expect(await browser.elementById('nested-children').text()).toBe('/[locale]/nested/page.tsx')

    // since there is a default defined in @slot0, we expect that to match
    expect(await browser.elementById('slot0').text()).toBe('/[locale]/nested/@slot0/default.tsx')

    // since there is a default defined in @slot1, we expect that to match
    expect(await browser.elementById('slot1').text()).toBe('/[locale]/nested/@slot1/default.tsx')
    ...
    expect(await browser.elementById('nested-children').text()).toBe('/[locale]/nested/default.tsx')

    // since there is a page defined in @slot0/foo, we expect that to match
    expect(await browser.elementById('slot0').text()).toBe('/[locale]/nested/@slot0/foo/page.tsx')
    ...
    expect(await browser.elementById('slot0').text()).toBe('/[locale]/nested/@slot0/bar/page.tsx')
    ...
    expect(await browser.elementById('slot1').text()).toBe('/[locale]/nested/@slot1/baz/page.tsx')
