...
test('JavaScript mounts a registered server fixture and hydrates its client', async () => {
  const fixture = await browser()
  const root = await fixture.mount('greeting', { label: 'JavaScript' })
  expect(new URL(fixture.page.url()).pathname).toMatch(/^\/docs\//)
  expect(await root.getByRole('heading').textContent()).toBe('Hello JavaScript')
