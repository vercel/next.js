...
export default async function Page() {
  ...
  const cacheInner = unstable_cache(
    async () => {
      ...
      const data = await fetch('https://next-data-api-endpoint.vercel.app/api/random?something', {
  ...
  return (
    <>
      ...
      <p id="page-data2">revalidate 360 (tags: thankyounext, justputit): {data2}</p>
      <p id="page-data3">revalidate 10 (tags: thankyounext): {JSON.stringify(cachedData)}</p>
      <p id="now">{Date.now()}</p>
