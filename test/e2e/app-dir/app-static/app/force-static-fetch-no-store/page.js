...
export default async function Page() {
  ...
  const data2 = await fetch('https://next-data-api-endpoint.vercel.app/api/random?page', {
    next: {
      revalidate: 0,
    },
  }).then((res) => res.text())

  const data3 = await fetch('https://next-data-api-endpoint.vercel.app/api/random?page', {
    cache: 'no-cache',
  }).then((res) => res.text())
