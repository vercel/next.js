...
describe.each(['default', 'custom'] as const)(
  'metadata-streaming-edge-bots (%s bot configuration)',
  (configuration) => {
    const { next } = nextTestSetup({
      files: {
        app: new FileRef(`${__dirname}/app`),
        'next.config.ts': new FileRef(
          `${__dirname}/${configuration}.config.ts`
        ),
      },
    })
    ...
    describe.each([
      ...
      it.each([
        ...
        async (userAgent, defaultBlocking) => {
