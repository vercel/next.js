...
describe('WebSocket Route Handlers', () => {
  ...
  it('does not proxy an absolute-form upgrade target', async () => {
    let upstreamConnections = 0
    const upstream = http.createServer((_request, response) => {
      response.end('private upstream response')
    })
    upstream.on('connection', () => upstreamConnections++)
  ...
  it('does not render an afterFiles rewrite to a page during an upgrade', async () => {
    const direct = await requestWebSocketUpgrade(next, '/dashboard')
    expect(direct).toMatchObject({ status: 401, body: 'Unauthorized' })

    const rewritten = await requestWebSocketUpgrade(next, '/dash')
    expect(rewritten).toMatchObject({ status: 404, body: 'Not Found' })
    expect(rewritten.body).not.toContain('dashboard page content')
  })
