...
describe('app dir - forbidden with customized boundary', () => {
  ...
  it('should match dynamic route forbidden boundary correctly', async () => {
    ...
    expect(await browserError.elementByCss('#forbidden').text()).toBe('dynamic/[id] forbidden')
  ...
  it('should escalate forbidden to parent layout if no forbidden boundary present in current layer', async () => {
    const browserDynamic = await next.browser('/dynamic-layout-without-forbidden')
    ...
    const browserDynamicId = await next.browser('/dynamic-layout-without-forbidden/403')
    expect(await browserDynamicId.elementByCss('h1').text()).toBe('Root Forbidden')
  })
})
