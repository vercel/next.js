...
describe('unstable_after - error stacks', () => {
  ...
  it('nested', async () => {
    ...
    expect(loggedError).toEqual(
      outdent`
      An error occurred in a function passed to \`unstable_after()\`: Error: kaboom
          at throws (_.js:0:0)
  ...
  describe('mixing callbacks and promises', () => {
    ...
    it.failing('nested-promise-above-callback-2', async () => {
      ...
      // FIXME: why aren't we catching `unstable_after(bar())` as a promise and bailing out?
      expect(loggedError).toEqual(
        outdent`
        An error occurred in a function passed to \`unstable_after()\`: Error: kaboom
            at throws (_.js:0:0)
