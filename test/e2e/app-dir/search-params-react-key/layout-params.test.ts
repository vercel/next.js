...
describe('app dir - search params keys', () => {
  ...
  it('should keep the React router instance the same when changing the search params', async () => {
    ...
    const searchParams = await browser.waitForElementByCss('#search-params').text()
    ...
    await check(async () => {
      const newSearchParams = await browser.waitForElementByCss('#search-params').text()

      const count = await browser.waitForElementByCss('#count').text()

      return newSearchParams !== searchParams && count === '2' ? 'success' : 'retry'
    }, 'success')
    ...
    await check(async () => {
      ...
      return newSearchParams !== searchParams && count === '4' ? 'success' : 'retry'
    }, 'success')
  })
})
