...
;(process.env.IS_TURBOPACK_TEST ? describe.skip : describe)('app-dir create root layout', () => {
  ...
  if (isDev) {
    ...
    describe('page.tsx', () => {
      beforeAll(async () => {
        next = await createNext({
          files: {
            'app/page.tsx': new FileRef(path.join(__dirname, 'app/route/page.js')),
            'next.config.js': new FileRef(path.join(__dirname, 'next.config.js')),
          },
        })
      })
    ...
    describe('build', () => {
      it('should break the build if a page is missing root layout', async () => {
        const next = await createNext({
          skipStart: true,
          files: {
            'app/page.js': new FileRef(path.join(__dirname, 'app/route/page.js')),
            'next.config.js': new FileRef(path.join(__dirname, 'next.config.js')),
          },
        })
