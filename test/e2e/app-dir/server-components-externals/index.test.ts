...
describe('app-dir - server components externals', () => {
  ...
  it('should have externals for those in config.serverExternalPackages', async () => {
    ...
    expect(text).toBe(path.join(next.testDir, 'node_modules', 'external-package'))
    expect(subpath).toBe(path.join(next.testDir, 'node_modules', 'external-package', 'subpath'))
  })
  ...
  if (!isTurbopack) {
    it('should externalize serversExternalPackages for server rendering layer', async () => {
      ...
      const ssrBundle = await next.readFile(`${next.distDir}/server/app/client/page.js`)
      expect(ssrBundle).not.toContain('external-package-mark:index')
      expect(ssrBundle).not.toContain('external-package-mark:subpath')

      await next.fetch('/')
      const rscBundle = await next.readFile(`${next.distDir}/server/app/page.js`)
