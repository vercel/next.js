...
describe('parameter matching', () => {
  ...
  if (isNextStart) {
    ...
    it('emits one route matcher for each effective prefix behavior', async () => {
      ...
      // Matcher-only prefixes have no source shell to validate. The explicit
      // fallback below them is the required shell, even with a concrete child.
      expect(matchers[sourceRoute].throwOnEmptyStaticShell).toBe(false)
      expect(
        matchers['/en/catalog/[top]/items/[bottom]'].throwOnEmptyStaticShell
      ).toBe(false)
      expect(
        matchers['/en/catalog/t1/items/[bottom]'].throwOnEmptyStaticShell
      ).toBe(true)
    ...
    it('uses the existing fallback-false adapter routing contract', async () => {
      const { routing, outputs, distDir, repoRoot } = JSON.parse(
      ...
      for (const [pattern, pathname] of [
        [sourceRoute, '/fr/catalog/t2/items/b2'],
        [blockingRoute, '/en/catalog/t2/items/b2'],
        [fallbackRoute, '/en/catalog/t1/items/b2'],
      ]) {
        const emittedRoute = routesBySource.get(pattern)!
        expect(emittedRoute).toBeDefined()
        expect(emittedRoute.has).toEqual(
          pattern === sourceRoute ? fallbackFalseHas : undefined
        )
      ...
      // Specialized matchers use the original page module and manifest. They
      // do not need synthetic page entrypoints in the deployed function.
      const parentOutput = outputs.appPages.find(
        (output) => output.pathname === sourceRoute
      )!
      expect(parentOutput).toBeDefined()
      const manifestPath = path.join(distDir, 'app-path-routes-manifest.json')
      expect(parentOutput.assets[path.relative(repoRoot, manifestPath)]).toBe(
        manifestPath
      )
      for (const pattern of [blockingRoute, fallbackRoute]) {
        const aliasPath = path.join(distDir, 'server/app', `${pattern}/page.js`)
        expect(
          parentOutput.assets[path.relative(repoRoot, aliasPath)]
        ).toBeUndefined()
      }

      // A closed prefix does not make the whole page a build-time allowlist.
      // Fully closed pages still retain that restriction.
      const seededOutput = outputs.prerenders.find(
        (output) => output.pathname === '/en/catalog/t1/items/b1'
      )!
      expect(seededOutput).toBeDefined()
      expect(seededOutput.parentOutputId).toBe(parentOutput.id)
      expect(seededOutput.parentFallbackMode).toBeUndefined()
      expect(
        outputs.prerenders.find(
          (output) => output.pathname === '/not-found-only/t1/items/b1'
        )?.parentFallbackMode
      ).toBe(false)
    })
  }
...
describe('parameter matching development shell validation', () => {
  ...
  it.each(['/fallback/t1/b1', '/fallback/t1/b2', '/fallback/t2/b2'])(
    'treats an explicitly fallback param as unknown for %s',
    async (pathname) => {
      const outputIndex = next.cliOutput.length
      await renderAndWaitForValidation(pathname)
      // Webpack's formatted stderr can arrive after the stdout validation
      // markers. Wait for the error from this request, not their pipe ordering.
      await retry(() => {
        expect(next.cliOutput.slice(outputIndex)).toContain(
          'Error: Route "/fallback/[top]/[bottom]"'
        )
      })
    }
  )
