...
;(isPPREnabled ? describe.skip : describe)('app-dir - metadata-static-generation', () => {
  ...
  it('should contain async generated metadata in head for simple static page', async () => {
    ...
    expect($(`${rootSelector} meta[name="description"]`).attr('content')).toBe(
  ...
  it('should contain async generated metadata in head static page with suspenseful content', async () => {
    ...
    expect($(`${rootSelector} title`).text()).toBe('suspenseful page - static')
  })
