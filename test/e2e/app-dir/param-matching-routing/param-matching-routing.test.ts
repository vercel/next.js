...
describe('param-matching-routing', () => {
  ...
  it.each([
    ...
    // Server params currently expose URI-encoded values, as also asserted by
    // prerender-encoding. Matching configuration must not change that contract.
    ['/es/docs/space%20here/install', 'es/space%20here/install'],
    ['/es/docs/space%20here/100%25', 'es/space%20here/100%25'],
    ['/open/docs/space%20here/install', 'space%20here/install'],
    ['/open/docs/space%20here/100%25', 'space%20here/100%25'],
  ])(
  ...
  describe('revalidation', () => {
    it.each([
      ...
      ['/es/catalog/t1/items/b1', 'es/t1/b1'],
      [
        '/en/catalog/revalidation-top/items/revalidation-bottom',
        'en/revalidation-top/revalidation-bottom',
      ],
      [
        '/es/on-demand/revalidation-top/items/revalidation-bottom',
        'es/revalidation-top/revalidation-bottom',
      ],
    ])(
      'revalidates a seeded or runtime-generated output without changing route admission: %s',
      async (pathname, params) => {
    ...
    it('regenerates a shared dynamic-tail shell without capturing a request value', async () => {
      ...
      const response = await next.fetch('/revalidate', {
        ...
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          pathname: '/[lang]/dynamic/[top]/[bottom]',
          type: 'page',
        }),
      })
    ...
    it.each([
      '/en/closed/unlisted',
      '/fr/on-demand/revalidation-top/items/one',
    ])('does not admit an invalidated, unlisted URL: %s', async (pathname) => {
