...
| Parameter encoding survives routing and cache lookup | This suite: multi-part catch-all with spaces and a literal percent sign; preserve existing URI-encoded server params with and without matching configuration |
...
| Cache invalidation does not change route admission | This suite: `revalidatePath` for fully closed and partially closed routes, including runtime-generated outputs and invalid URLs; route-pattern invalidation regenerates a shared dynamic-tail shell without capturing either request's dynamic value |
...
- [Unseeded prefixes](../../../production/app-dir/param-matching-unseeded-prefix/param-matching-unseeded-prefix.test.ts): explicit policies extend the prerenderable prefix; empty/nonempty shell inference; unconfigured tails stay dynamic.
...
with the unchanged adapter; captured routing regexes alone cannot prove this.
