...
;(process.env.IS_TURBOPACK_TEST ? describe.skip : describe)('app dir - rsc webpack loader', () => {
  ...
  it('should support webpack loader rules', async () => {
    ...
    expect(
      await browser.eval(`window.getComputedStyle(document.querySelector('#red')).color`)
    ).toBe('rgb(255, 0, 0)')
  })
})
