  ...
  if (request.nextUrl.pathname.startsWith('/search-params/target-page')) {
    ...
    if (
      ...
      return NextResponse.rewrite(
        new URL('/search-params/target-page?searchParam=rewrittenSearchParam', request.url)
      )
    }
  }

  if (request.nextUrl.pathname.startsWith('/search-params-with-greeting')) {
    return NextResponse.rewrite(new URL('/search-params?greeting=hello', request.url))
  }
