...
describe('segment cache (incremental opt in)', () => {
  ...
  it('skips prefetching a dynamic route when PPR is disabled if everything up ' +
    'to its loading.tsx boundary is already cached', async () => {
    let act
    const browser = await next.browser('/', {
      beforePageLoad(p) {
        act = createRouterAct(p)
  ...
  it('prefetches a shared layout on a PPR-enabled route that was previously ' +
    'omitted from a non-PPR-enabled route', async () => {
    let act
  ...
  it('when a link is prefetched with <Link prefetch=true>, no dynamic request ' +
    'is made on navigation', async () => {
    let act
    const browser = await next.browser('/mixed-fetch-strategies', {
      beforePageLoad(p) {
        act = createRouterAct(p)
    ...
    await act(
      async () => {
        const checkbox = await browser.elementById('ppr-enabled-prefetch-true')
        await checkbox.click()
      },
      {
  ...
  it('when prefetching with prefetch=true, refetches cache entries that only ' +
    'contain partial data', async () => {
    let act
    const browser = await next.browser('/mixed-fetch-strategies', {
      beforePageLoad(p) {
        act = createRouterAct(p)
  ...
  it('when prefetching with prefetch=true, refetches partial cache entries ' +
    "even if there's already a pending PPR request", async () => {
