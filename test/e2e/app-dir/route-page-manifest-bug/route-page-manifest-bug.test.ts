...
describe('route-page-manifest-bug', () => {
  ...
  it('should work when requesting route handler after page', async () => {
    ...
    expect(await browser.elementByCss('#page-title').text()).toBe('Page that would break')
    ...
    await browser.back()
    expect(await browser.waitForElementByCss('#page-title').text()).toBe('Page that would break')
    await browser.refresh()
    expect(await browser.waitForElementByCss('#page-title').text()).toBe('Page that would break')
    await browser.refresh()
    expect(await browser.waitForElementByCss('#page-title').text()).toBe('Page that would break')
