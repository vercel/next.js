...
describe('use-cache', () => {
  ...
  it('should support lazy elements as input for "use cache" functions', async () => {
    const outputIndex = next.cliOutput.length
    const browser = await next.browser('/input-with-lazy')
    ...
    expect(cliOutput).not.toContain(
      'Error: Route "/input-with-lazy": A component accessed data, headers, params, searchParams, or a short-lived cache'
    )
  })
})
