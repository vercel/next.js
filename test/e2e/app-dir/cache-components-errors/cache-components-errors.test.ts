      ...
      describe('with `connection()`', () => {
        if (isNextDev) {
          ...
          it.skip('should show a redbox error', async () => {
            ...
            await expect(browser).toDisplayCollapsedRedbox(`
               {
                 "description": "\`connection()\` cannot be called inside "use cache: private". A private cache entry may be produced before a navigation request, so it cannot depend on the request lifecycle. Learn more: https://nextjs.org/docs/messages/next-request-in-use-cache",
                 "environmentLabel": null,
                 "label": "Runtime Error",
