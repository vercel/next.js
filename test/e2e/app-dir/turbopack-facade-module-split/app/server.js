...
;(async () => {
  ...
  startApp('host', hostPort, dev)
  startApp('remote', remotePort, dev)

  await Promise.all([waitForServer(hostPort), waitForServer(remotePort)])

  const server = http.createServer((req, res) => {
    const isRemote =
      req.url.startsWith('/remote-component') ||
      req.url.startsWith('/remote-assets')
    proxy(req, res, isRemote ? remotePort : hostPort)
  })
