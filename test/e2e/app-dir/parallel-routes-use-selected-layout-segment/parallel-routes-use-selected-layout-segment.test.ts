...
describe('parallel-routes-use-selected-layout-segment', () => {
  ...
  it('hard nav to router page and soft nav around other router pages', async () => {
    ...
    await check(() => browser.elementById('navSegment').text(), 'navSegment (parallel route):')
    await check(() => browser.elementById('authSegment').text(), 'authSegment (parallel route):')
    await check(() => browser.elementById('routeSegment').text(), 'routeSegment (app route):')

    await browser.elementByCss('[href="/foo"]').click()
    await check(() => browser.elementById('navSegment').text(), 'navSegment (parallel route):')
    await check(() => browser.elementById('authSegment').text(), 'authSegment (parallel route):')
    await check(() => browser.elementById('routeSegment').text(), 'routeSegment (app route): foo')
  })
