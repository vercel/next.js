...
describe.each(['default', 'babelrc'] as const)('react-compiler %s', (variant) => {
    ...
    files:
      variant === 'babelrc'
        ...
        : {
            ...
            'reference-library': new FileRef(join(__dirname, 'reference-library')),
          },
  ...
  it('should memoize Components', async () => {
    ...
    expect(await browser.elementByCss('[data-testid="parent-commits"]').text()).toEqual(
      'Parent commits: 1'
    )
  ...
  it('should name functions in dev', async () => {
    ...
    await browser.waitForElementByCss('[data-testid="call-frame"][aria-busy="false"]', 5000)

    const callFrame = await browser.elementByCss('[data-testid="call-frame"]').text()
    ...
    if (isNextDev) {
      ...
      expect(normalizeCodeLocInfo(callFrame)).toEqual(`    at ${devFunctionName} (**)`)
    } else {
      expect(normalizeCodeLocInfo(callFrame)).not.toEqual(`    at ${devFunctionName} (**)`)
    }
  })
  ...
  it('throws if the React Compiler is used in a React Server environment', async () => {
    ...
    const cliOutput = normalizeCodeLocInfo(stripAnsi(next.cliOutput.slice(outputIndex)))
    if (isNextDev) {
