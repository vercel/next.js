...
describe('typeof window replace', () => {
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    it('Replaces `typeof window` with object for client code', async () => {
      let allContent = ''
      const files = buildManifest.pages['/'].filter((item) => item.endsWith('.js'))
      for (const file of files) {
        const content = await fs.readFile(path.join(appDir, '.next', file), 'utf8')
        allContent += content
      }
    ...
    it('Replaces `typeof window` with undefined for server code', async () => {
      ...
      for (const file of allFilesInDotNextServerChunks) {
        const content = await fs.readFile(path.join(chunksFilesDir, file), 'utf8')
        allContent += content
      }
      ...
      for (const file of allFilesInDotNextServerPages) {
        const content = await fs.readFile(path.join(pagesFilesDir, file), 'utf8')
        allContent += content
      }
