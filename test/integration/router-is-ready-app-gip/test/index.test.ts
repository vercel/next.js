...
import { findPort, launchApp, killApp, nextStart, nextBuild, File, check } from 'next-test-utils'
...
const checkIsReadyValues = (browser, expected = []) => {
  return check(async () => {
    const values = JSON.stringify((await browser.eval('window.isReadyValues')).sort())
