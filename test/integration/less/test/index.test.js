...
const fixturesDir = join(__dirname, '../..', 'less-fixtures')
...
describe('LESS Support', () => {
  describe('Friendly Webpack Error', () => {
    ...
    it('should be a friendly error successfully', async () => {
      ...
      expect(stderr).toContain(
        'Run `npm i less` or `yarn add less` inside your workspace.'
      )
      expect(stderr).toContain(
  ...
  describe('Basic Module Modify Vars Support', () => {
    const appDir = join(fixturesDir, 'basic-module-modify-vars')
  ...
  describe('CSS Compilation and Prefixing', () => {
    ...
    it(`should've emitted a source map`, async () => {
      ...
      expect({ version, mappings, sourcesContent }).toMatchInlineSnapshot(`
        Object {
          "mappings": "AACA,4BAEI,SADJ,CADA,gCAEI,SADJ,CADA,uBAEI,SADJ,CAKA,cACE,2CAHF",
  ...
  describe('Invalid CSS in _document', () => {
    ...
    it('should fail to build', async () => {
      ...
      expect(stderr).toContain('Failed to compile')
      expect(stderr).toContain('styles.module.less')
  ...
  describe('Invalid Global CSS', () => {
    ...
    it('should fail to build', async () => {
      ...
      expect(code).not.toBe(0)
      expect(stderr).toContain('Failed to compile')
      expect(stderr).toContain('styles/global.less')
      expect(stderr).toMatch(
  ...
  describe('Can hot reload CSS without losing state', () => {
    ...
    it('should update CSS color without remounting <input>', async () => {
      ...
      try {
        ...
        const cssFile = new File(join(appDir, 'styles/global1.less'))
        try {
