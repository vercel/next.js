...
describe('next.config.js validation', () => {
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    it.each([
      ...
      `,
        outputs: ['Array must contain at least 1 element(s) at "pageExtensions"'],
      },
    ])('it should validate correctly for $name', async ({ outputs, configContent }) => {
    ...
    it('should allow undefined environment variables', async () => {
      ...
      expect(result.stdout + result.stderr).not.toContain('"env.QUX" is missing')
    })
  })
})
