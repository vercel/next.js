...
describe('config telemetry', () => {
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    it('detects i18n and image configs for session start', async () => {
      ...
      try {
        const event1 = /NEXT_CLI_SESSION_STARTED[\s\S]+?{([\s\S]+?)}/.exec(stderr).pop()
      ...
      try {
        const event2 = /NEXT_CLI_SESSION_STARTED[\s\S]+?{([\s\S]+?)}/.exec(stderr2).pop()
    ...
    ;(process.env.IS_TURBOPACK_TEST ? it.skip : it)(
      ...
      async () => {
        ...
        const featureUsageEvents = findAllTelemetryEvents(stderr, 'NEXT_BUILD_FEATURE_USAGE')
    ...
    ;(process.env.IS_TURBOPACK_TEST ? it.skip : it)(
      ...
      async () => {
        ...
        await fs.rename(path.join(appDir, 'next.config.swc'), path.join(appDir, 'next.config.js'))
        await fs.rename(path.join(appDir, 'jsconfig.swc'), path.join(appDir, 'jsconfig.json'))
        const { stderr } = await nextBuild(appDir, [], {
          stderr: true,
          env: { NEXT_TELEMETRY_DEBUG: '1' },
        })
        await fs.rename(path.join(appDir, 'next.config.js'), path.join(appDir, 'next.config.swc'))
        await fs.rename(path.join(appDir, 'jsconfig.json'), path.join(appDir, 'jsconfig.swc'))
        const featureUsageEvents = findAllTelemetryEvents(stderr, 'NEXT_BUILD_FEATURE_USAGE')
        // eslint-disable-next-line jest/no-standalone-expect
        expect(featureUsageEvents).toEqual(
          expect.arrayContaining([
    ...
    it('emits telemetry for usage of middleware', async () => {
      await fs.writeFile(path.join(appDir, 'middleware.js'), `export function middleware () { }`)
      ...
      const buildOptimizedEvents = findAllTelemetryEvents(stderr, 'NEXT_BUILD_OPTIMIZED')
    ...
    it('emits telemetry for usage of swc plugins', async () => {
      ...
      await fs.rename(path.join(appDir, 'package.swc-plugins'), path.join(appDir, 'package.json'))
      ...
      await fs.rename(path.join(appDir, 'package.json'), path.join(appDir, 'package.swc-plugins'))

      const pluginDetectedEvents = findAllTelemetryEvents(stderr, 'NEXT_SWC_PLUGIN_DETECTED')
    ...
    it('emits telemetry for default React Compiler options', async () => {
      ...
      try {
        const event = /NEXT_CLI_SESSION_STARTED[\s\S]+?{([\s\S]+?)}/.exec(stderr).pop()
    ...
    it('emits telemetry for configured React Compiler options', async () => {
      ...
      try {
        ...
        expect(event).toMatch(/"reactCompilerPanicThreshold": "critical_errors"/)
      } catch (err) {
        require('console').error('failing stderr', stderr, err)
        throw err
      } finally {
