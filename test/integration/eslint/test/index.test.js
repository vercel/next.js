...
describe('ESLint', () => {
  describe('Next Build', () => {
    ...
    test('base directories are linted by default during builds', async () => {
      ...
      expect(output).toContain(
        'Warning: The `<img>` element does not benefit from Image Optimization. Use `<Image />` from `next/image` instead'
      )
      ...
      if (fileOutput && fileOutput.length) {
        ...
        expect(fileOutput[0].messages).toEqual(
          expect.arrayContaining([
            ...
            expect.objectContaining({
              message:
                'The `<img>` element does not benefit from Image Optimization. Use `<Image />` from `next/image` instead. See: https://nextjs.org/docs/messages/no-img-element',
            }),
          ])
        )
