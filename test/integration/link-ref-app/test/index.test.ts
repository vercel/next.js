...
import { retry, findPort, launchApp, killApp, nextStart, nextBuild, waitFor } from 'next-test-utils'
...
const didPrefetch = async (pathname) => {
  ...
  await retry(async () => {
    expect(requests).toEqual(expect.arrayContaining([expect.stringContaining('/')]))
  })
