...
describe.each(['app', 'pages'] as const)('CNA options matrix - %s', (pagesOrApp) => {
  ...
  const allFlagValues = {
    ...
    ts: ['--js', '--ts'],
    importAlias: ['--import-alias=@acme/*', '--import-alias=@/*', '--no-import-alias'],
    ...
    srcDirAndCompiler: ['--src-dir --react-compiler', '--no-src-dir --no-react-compiler'],
  ...
  const getCombinations = (items: string[][]): string[][] => {
    ...
    return first.flatMap((value) => children.map((child) => [...value.split(' '), ...child]))
  }
  ...
  it.each(testCases)('$name', async ({ flags }) => {
    await useTempDir(async (cwd) => {
      ...
      const { exitCode } = await run(
        [projectName, ...flags, ...(process.env.NEXT_RSPACK ? ['--rspack'] : [])],
        nextTgzFilename,
        {
          cwd,
        }
