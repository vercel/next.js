...
import { renderViaHTTP, findPort, launchApp, nextBuild, killApp } from 'next-test-utils'
...
describe('TypeScript Image Component', () => {
  ...
  it('should remove global image types when disabled (dev)', async () => {
    const content = await fs.readFile(nextConfig, 'utf8')
    await fs.writeFile(nextConfig, content.replace('// disableStaticImages', 'disableStaticImages'))
