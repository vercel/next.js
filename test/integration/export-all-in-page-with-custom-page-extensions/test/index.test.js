...
describe('Re-export all exports from page is disallowed for custom pageExtensions', () => {
  it('shows error when a page re-export all exports', async () => {
    ...
    expect(stderr.split('\n\n')[1]).toMatchInlineSnapshot(`
      "./pages/contact.page.js:3:1
      Syntax error: Using \`export * from '...'\` in a page is disallowed. Please use \`export { default } from '...'\` instead.
      Read more: https://nextjs.org/docs/messages/export-all-in-page"
    `)
  })

  it('SWC: shows error when a page re-export all exports', async () => {
    const f = new File(join(appDir, '.babelrc'))
    f.delete()
    try {
      const { code, stderr } = await nextBuild(appDir, undefined, {
        stderr: true,
  ...
  it('builds without error when no `export * from "..."` is used in pages', async () => {
    const f = new File(join(appDir, 'pages', 'contact.page.js'))
  ...
  it('SWC: builds without error when no `export * from "..."` is used in pages', async () => {
    const f = new File(join(appDir, 'pages', 'contact.page.js'))
    f.replace(/^export \*/gm, '// export *')
    const babelFile = new File(join(appDir, '.babelrc'))
    babelFile.delete()
    try {
      const { code, stderr } = await nextBuild(appDir, undefined, {
        stderr: true,
