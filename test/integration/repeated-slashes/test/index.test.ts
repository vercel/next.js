...
function runTests({ isDev = false, isExport = false, isPages404 = false }) {
  ...
  const didNotReload = async (browser) => {
    for (let i = 0; i < 4; i++) {
      ...
      if (result !== true) {
        throw new Error(`did not find window.errorLoad, current url: ${await browser.url()}`)
      }
  ...
  if (!isExport) {
    it('should normalize repeated slashes in redirects correctly', async () => {
      const res = await fetchViaHTTP(appPort, '/redirect-forward-slashes', undefined, {
        redirect: 'manual',
      })
      ...
      const res2 = await fetchViaHTTP(appPort, '/redirect-back-slashes', undefined, {
        redirect: 'manual',
      })
  ...
  it('should handle double slashes correctly', async () => {
    ...
    expect(await browser.eval('document.documentElement.innerHTML')).toContain(notFoundContent)
  })
  ...
  it('should handle double slashes correctly with encoded', async () => {
    ...
    await didNotReload(browser)
    expect(await browser.eval('window.location.pathname')).toBe('/%2Fgoogle.com')
    expect(await browser.eval('document.documentElement.innerHTML')).toContain(notFoundContent)
  })
  ...
  it('should handle slashes in next/link correctly', async () => {
    const browser = await webdriver(appPort, `/invalid${isExport ? '.html' : ''}`)
    const invalidHrefs = [
      '//google.com',
      '//google.com?hello=1',
      '//google.com#hello',
  ...
  it('should handle slashes in router push correctly', async () => {
    for (const item of [
      ...
      await browser.eval(
        `window.next.router.push("${item.href}"${item.as ? `, "${item.as}"` : ''})`
      )
      await check(() => browser.eval('document.readyState'), /interactive|complete/)
  ...
  it('should have no error from encoded slashes in router push', async () => {
    for (const item of [
      ...
      expect(await browser.eval('window.location.hash')).toBe(item.hash || '')
      expect(await browser.eval('window.next.router.pathname')).toBe(item.page)
      expect(await browser.eval('window.next.router.asPath')).toBe(item.as || item.href)
