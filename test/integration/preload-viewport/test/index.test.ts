...
describe('Prefetching Links in viewport', () => {
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    it('should de-dupe inflight SSG requests', async () => {
      ...
      expect(nextDataRequests.filter((reqUrl) => reqUrl.includes('/ssg/slow.json')).length).toBe(2)
    })
    ...
    it('should handle timed out prefetch correctly', async () => {
      try {
        ...
        await browser.elementByCss('#link-another').click().waitForElementByCss('#another')
    ...
    it('should prefetch with link in viewport onload', async () => {
      ...
      try {
        ...
        await retry(async () => {
          ...
          expect(hrefs).toEqual(expect.arrayContaining([expect.stringContaining(chunk)]))
        })
      } finally {
    ...
    it('should prefetch rewritten href with link in viewport onload', async () => {
      ...
      try {
        ...
        await retry(async () => {
          ...
          let chunk = getClientBuildManifestLoaderChunkUrlPath(appDir, '/ssg/dynamic/[slug]')
          expect(hrefs).toEqual(expect.arrayContaining([expect.stringContaining(chunk)]))
        })
    ...
    it('should prefetch with link in viewport when href changes', async () => {
      ...
      try {
        ...
        let chunkFirst = getClientBuildManifestLoaderChunkUrlPath(appDir, '/first')
        let chunkAnother = getClientBuildManifestLoaderChunkUrlPath(appDir, '/another')
    ...
    it('should prefetch with link in viewport on scroll', async () => {
      ...
      try {
        ...
        await retry(async () => {
          ...
          const hrefs = await Promise.all(links.map((link) => link.getAttribute('href')))
          let chunk = getClientBuildManifestLoaderChunkUrlPath(appDir, '/another')
          expect(hrefs).toEqual(expect.arrayContaining([expect.stringContaining(chunk)]))
        })
      } finally {
        if (browser) await browser.close()
    ...
    it('should prefetch with link in viewport and inject script on hover', async () => {
      ...
      try {
        ...
        await retry(async () => {
          ...
          const srcProps = await Promise.all(scripts.map((script) => script.getAttribute('src')))
          let chunk = getClientBuildManifestLoaderChunkUrlPath(appDir, '/another')
          expect(srcProps).toEqual(expect.arrayContaining([expect.stringContaining(chunk)]))
        })
      } finally {
        if (browser) await browser.close()
      }
    ...
    it('should inject script on hover with prefetching disabled', async () => {
      ...
      try {
        ...
        await retry(async () => {
          ...
          const hrefs = await Promise.all(links.map((link) => link.getAttribute('href')))
          expect(hrefs).toEqual(expect.not.arrayContaining([expect.stringContaining(chunkAnother)]))
        })
    ...
    it('should inject script on hover with prefetching disabled and fetch data', async () => {
      ...
      try {
        ...
        let chunkBasic = getClientBuildManifestLoaderChunkUrlPath(appDir, '/ssg/basic')
    ...
    it('should not prefetch when prefetch is explicitly set to false', async () => {
      ...
      await retry(async () => {
        ...
        const hrefs = await Promise.all(links.map((link) => link.getAttribute('href')))
        let chunk = getClientBuildManifestLoaderChunkUrlPath(appDir, '/another')
        expect(hrefs).toEqual(expect.not.arrayContaining([expect.stringContaining(chunk)]))
      })
    })
    ...
    it('should not duplicate prefetches', async () => {
      ...
      let chunk = getClientBuildManifestLoaderChunkUrlPath(appDir, '/dynamic/[hello]')
      // Verify encoding
    ...
    it('should not re-prefetch for an already prefetched page', async () => {
      ...
      await check(() => browser.eval('document.documentElement.innerHTML'), /to \/first/)
