  ...
  () => {
    ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
      ...
      it('Should fail to build when no next.config.js', async () => {
        ...
        expect(stderr).toMatch(/Images.*cannot.*be imported within.*pages[\\/]_document\.js/)
    ...
    `)
        ...
        expect(code).not.toBe(0)
        if (process.env.NEXT_RSPACK) {
          expect(stderr).toContain('You may need an appropriate loader to handle this file type')
        } else {
        ...
        // Skip: Rspack loaders cannot access module issuer info for location details
        if (!process.env.NEXT_RSPACK) {
          expect(stderr).not.toMatch(/Images.*cannot.*be imported within.*pages[\\/]_document\.js/)
        }
      })
    })
  }
