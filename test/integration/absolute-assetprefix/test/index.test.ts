...
describe('absolute assetPrefix with path prefix', () => {
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    beforeAll(async () => {
      ...
      cdn = http.createServer((clientReq, clientRes) => {
        ...
        const proxyReq = http.request(
          ...
          (proxyRes) => {
            ...
            proxyRes.headers['Access-Control-Allow-Origin'] = `http://localhost:${appPort}`
      ...
      buildId = await fs.readFile(path.resolve(__dirname, '../.next/BUILD_ID'), 'utf8')
    ...
    it('should fetch from cache correctly', async () => {
      ...
      expect(
        cdnAccessLog.filter((path) => path === `/_next/data/${buildId}/about.json`)
      ).toHaveLength(0)
    })
    ...
    it('should work with getStaticPaths prerendered', async () => {
      ...
      expect(cdnAccessLog).not.toContain(`/_next/data/${buildId}/gsp-fallback/prerendered.json`)
    })
    ...
    it('should work with getStaticPaths fallback', async () => {
      ...
      expect(cdnAccessLog).not.toContain(`/_next/data/${buildId}/gsp-fallback/fallback.json`)
    })
    ...
    it('should work with getServerSideProps', async () => {
      ...
      expect(cdnAccessLog).not.toContain(`/_next/data/${buildId}/gssp.json?prop=foo`)
    })
  })
})
