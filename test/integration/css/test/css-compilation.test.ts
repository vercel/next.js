  ...
  experimental: {
    useLightningcss: ${useLightningcss}
        ...
        it(`should've compiled and prefixed`, async () => {
          ...
          const cssContent = await fetchViaHTTP(appPort, stylesheetUrl).then((res) => res.text())

          const cssContentWithoutSourceMap = cssContent.replace(/\/\*.*?\*\/\n?/g, '').trim()
          ...
          const sourceMapContent = await fetchViaHTTP(appPort, actualSourceMapUrl).then((res) =>
            res.text()
          )
          ...
          const inlineCssContent = inlineStyle
            .html()
            .replace(/media-query-test.jsx-[a-f0-9]{16}/g, 'media-query-test.jsx-HASH')
          if (process.env.IS_TURBOPACK_TEST && useLightningcss) {
  ...
  experimental: {
    useLightningcss: ${useLightningcss}
        ...
        it(`should've preloaded the CSS file and injected it in <head>`, async () => {
          ...
          const styleIndexes = allPreloads.flatMap((p, i) => (p.attribs.as === 'style' ? i : []))
  ...
  experimental: {
    useLightningcss: ${useLightningcss}
        ...
        it(`should've emitted a single CSS file`, async () => {
          ...
          const cssContent = (await fetchViaHTTP(appPort, stylesheet).then((res) => res.text()))
            .replace(/\/\*.*?\*\/\n?/g, '')
            .trim()

          expect(cssContent.replace(/\/\*.*?\*\/\n?/g, '').trim()).toMatch(/nprogress/)
        })
      })
    })
    ...
    useLightningcss: ${useLightningcss}
        ...
        it(`should've emitted a single CSS file`, async () => {
          ...
          if (process.env.IS_TURBOPACK_TEST && useLightningcss) {
            ...
            expect(cssContent.replace(/\/\*.*?\*\/\n?/g, '').trim()).toMatchInlineSnapshot(
              `".other{color:#00f}.test{color:red}"`
            )
          } else {
            expect(cssContent.replace(/\/\*.*?\*\/\n?/g, '').trim()).toMatchInlineSnapshot(
