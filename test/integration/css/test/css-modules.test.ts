...
useLightningcss: ${useLightningcss}
    ...
    ;(process.env.IS_TURBOPACK_TEST ? describe.skip : describe)('Development Mode', () => {
    ...
    it('should have the correct color (css ordering) during hot reloads', async () => {
      ...
      try {
        ...
        try {
          ...
          await check(
            () =>
              browser.eval(`window.getComputedStyle(document.querySelector('#yellowText')).color`),
            'rgb(1, 1, 1)'
          )
          await check(
            () =>
              browser.eval(`window.getComputedStyle(document.querySelector('#blueText')).color`),
            'rgb(0, 0, 255)'
          )
        } finally {
          cssFile.restore()
...
;(process.env.IS_TURBOPACK_TEST ? describe.skip : describe)('CSS Modules Composes Ordering', () => {
