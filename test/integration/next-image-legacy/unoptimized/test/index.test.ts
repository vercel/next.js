...
function runTests() {
  it('should not optimize any image', async () => {
    ...
    expect(await browser.elementById('internal-image').getAttribute('src')).toMatch('data:')
    expect(await browser.elementById('static-image').getAttribute('src')).toMatch('data:')
    expect(await browser.elementById('external-image').getAttribute('src')).toMatch('data:')
    ...
    await check(async () => {
      ...
      return browser.eval(`document.getElementById("external-image").currentSrc`)
