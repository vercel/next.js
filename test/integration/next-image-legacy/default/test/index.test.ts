...
function runTests(mode) {
  ...
  it('should callback native onLoad in most cases', async () => {
    const browser = await webdriver(appPort, '/on-load')
    await browser.eval(
      `document.getElementById("footer").scrollIntoView({behavior: "smooth"})`
    )
  ...
  it('should callback native onError when error occurred while loading image', async () => {
    const browser = await webdriver(appPort, '/on-error')
    await check(
      () => browser.eval(`document.getElementById("img1").currentSrc`),
      /test\.png/
    )
  ...
  if (mode === 'dev') {
    ...
    it('should warn when priority prop is missing on LCP image', async () => {
      const browser = await webdriver(appPort, '/priority-missing-warning')
  ...
  it('should emit image for next/dynamic with non ssr case', async () => {
    const browser = await webdriver(appPort, '/dynamic-static-img')
