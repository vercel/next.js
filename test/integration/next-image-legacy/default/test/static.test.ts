...
const runTests = () => {
  ...
  it('should load direct imported image', async () => {
    ...
    expect(src).toMatch(
      /_next\/image\?url=%2F_next%2F(static|immutable)%2Fmedia%2Ftest-rect(.+)\.jpg&w=828&q=75/
    )
  ...
  it('should load staticprops imported image', async () => {
    const src = await browser
      ...
      .getAttribute('src')
    expect(src).toMatch(
      /_next\/image\?url=%2F_next%2F(static|immutable)%2Fmedia%2Fexif-rotation(.+)\.jpg&w=256&q=75/
    )
    const fullSrc = new URL(src, `http://localhost:${appPort}`)
    const res = await fetch(fullSrc)
    expect(res.status).toBe(200)
