...
import { check, findPort, killApp, nextBuild, nextStart, waitFor } from 'next-test-utils'
...
function runTests() {
  ...
  it('should support passing through arbitrary attributes', async () => {
    expect(await browser.hasElementByCssSelector('img#attribute-test')).toBeTruthy()
    expect(await browser.elementByCss('img#attribute-test').getAttribute('data-demo')).toBe(
      'demo-value'
    )
  })
  ...
  it('should correctly generate src even if preceding slash is included in prop', async () => {
    expect(await browser.elementById('preceding-slash-image').getAttribute('src')).toBe(
  ...
  it('should add a srcset even with preceding slash in prop', async () => {
    expect(await browser.elementById('preceding-slash-image').getAttribute('srcset')).toBe(
  ...
  it('should use imageSizes when width matches, not deviceSizes from next.config.js', async () => {
    ...
    expect(await browser.elementById('icon-image-16').getAttribute('srcset')).toBe(
    ...
    expect(await browser.elementById('icon-image-32').getAttribute('srcset')).toBe(
  ...
  it('should not add a srcset if unoptimized attribute present', async () => {
    expect(await browser.elementById('unoptimized-image').getAttribute('srcset')).toBeFalsy()
  })
  it('should keep auto parameter if already set', async () => {
    expect(await browser.elementById('image-with-param-auto').getAttribute('src')).toBe(
      'https://example.com/myaccount/foo.png?auto=compress&fit=max&w=1024'
    )
  })
  it('should keep width parameter if already set', async () => {
    expect(await browser.elementById('image-with-param-width').getAttribute('src')).toBe(
      'https://example.com/myaccount/foo.png?auto=format&w=500&fit=max'
    )
  })
  it('should keep fit parameter if already set', async () => {
    expect(await browser.elementById('image-with-param-fit').getAttribute('src')).toBe(
...
function lazyLoadingTests() {
  ...
  it('should not have loaded the second image immediately', async () => {
    expect(await browser.elementById('lazy-mid').getAttribute('src')).toBe(emptyImage)
    expect(await browser.elementById('lazy-mid').getAttribute('srcset')).toBeFalsy()
  })
  it('should pass through classes on a lazy loaded image', async () => {
    expect(await browser.elementById('lazy-mid').getAttribute('class')).toBe('exampleclass')
  })
  it('should load the second image after scrolling down', async () => {
    ...
    let buffer = 150
    await browser.eval(`window.scrollTo(0, ${topOfMidImage - (viewportHeight + buffer)})`)
  ...
  it('should not have loaded the third image after scrolling down', async () => {
    expect(await browser.elementById('lazy-bottom').getAttribute('src')).toBe(emptyImage)
    expect(await browser.elementById('lazy-bottom').getAttribute('srcset')).toBeFalsy()
  })
  it('should load the third image, which is unoptimized, after scrolling further down', async () => {
    ...
    let buffer = 150
    await browser.eval(`window.scrollTo(0, ${topOfBottomImage - (viewportHeight + buffer)})`)
    await waitFor(200)
  ...
  it('should load the fourth image lazily after scrolling down', async () => {
    expect(await browser.elementById('lazy-without-attribute').getAttribute('src')).toBe(emptyImage)
    expect(await browser.elementById('lazy-without-attribute').getAttribute('srcset')).toBeFalsy()
    let viewportHeight = await browser.eval(`window.innerHeight`)
    let topOfBottomImage = await browser.eval(
    ...
    await browser.eval(`window.scrollTo(0, ${topOfBottomImage - (viewportHeight + buffer)})`)
    await waitFor(200)
    expect(await browser.elementById('lazy-without-attribute').getAttribute('src')).toBe(
      'https://example.com/myaccount/lazy4.jpg?auto=format&fit=max&w=1600'
    )
    expect(await browser.elementById('lazy-without-attribute').getAttribute('srcset')).toBe(
  ...
  it('should load the fifth image eagerly, without scrolling', async () => {
    ...
    expect(await browser.elementById('eager-loading').getAttribute('srcset')).toBeTruthy()
  })

  it('should load the sixth image, which has lazyBoundary property after scrolling down', async () => {
    expect(await browser.elementById('lazy-boundary').getAttribute('src')).toBe(emptyImage)
    expect(await browser.elementById('lazy-boundary').getAttribute('srcset')).toBeFalsy()
    let viewportHeight = await browser.eval(`window.innerHeight`)
    let topOfBottomImage = await browser.eval(
    ...
    await check(() => {
      return browser.eval('document.querySelector("#lazy-boundary").getAttribute("src")')
    }, 'https://example.com/myaccount/lazy6.jpg?auto=format&fit=max&w=1600')

    await check(() => {
      return browser.eval('document.querySelector("#lazy-boundary").getAttribute("srcset")')
  ...
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    describe('SSR Image Component Tests', () => {
      ...
      it('should add a preload tag for a priority image, with arbitrary host', async () => {
        expect(await hasPreloadLinkMatchingUrl('https://arbitraryurl.com/withpriority3.png')).toBe(
          true
        )
      })
      ...
      it('should add data-nimg data attribute based on layout', async () => {
        expect(await browser.elementById('image-with-sizes').getAttribute('data-nimg')).toBe(
          'responsive'
        )
        expect(await browser.elementById('basic-image').getAttribute('data-nimg')).toBe('intrinsic')
      })
      it('should not pass config to custom loader prop', async () => {
        browser = await webdriver(appPort, '/loader-prop')
        expect(await browser.elementById('loader-prop-img').getAttribute('src')).toBe(
          'https://example.vercel.sh/success/foo.jpg?width=1024'
        )
        expect(await browser.elementById('loader-prop-img').getAttribute('srcset')).toBe(
