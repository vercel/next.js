...
export default renderViaHTTP => {
  test('should load the page properly', async t => {
    ...
    try {
      ...
      await t.expect(text).eql('This is the contact page.')
  ...
  test('should detect the changes and display it', async t => {
    let browser
    try {
      browser = await webdriver(t.fixtureCtx.appPort, '/hmr/about')
      const text = await browser.elementByCss('p').text()
  ...
  test('should not reload unrelated pages', async t => {
    let browser
    try {
      browser = await webdriver(t.fixtureCtx.appPort, '/hmr/counter')
      ...
      await t.expect(text).eql('COUNT: 2')
      ...
      await t.expect(newText).eql('COUNT: 2')
  ...
  test('should update styles correctly', async t => {
    let browser
    try {
      browser = await webdriver(t.fixtureCtx.appPort, '/hmr/style')
  ...
  // Added because of a regression in react-hot-loader, see issues: #4246 #4273
  // Also: https://github.com/zeit/styled-jsx/issues/425
  test('should update styles in a stateful component correctly', async t => {
    let browser
    const pagePath = join(
      __dirname,
      '../',
  ...
  // Added because of a regression in react-hot-loader, see issues: #4246 #4273
  // Also: https://github.com/zeit/styled-jsx/issues/425
  test('should update styles in a dynamic component correctly', async t => {
    let browser = null
    ...
    try {
      ...
      await t.expect(initialHtml.includes('100px')).ok()

      const $initialHtml = cheerio.load(initialHtml)
      const initialServerClassName = $initialHtml('#dynamic-component').attr(
        'class'
      )

      await t.expect(initialClientClassName).eql(initialServerClassName)
      ...
      await t.expect(browserHtml.includes('font-size:200px;')).eql(true)
      await t.expect(browserHtml.includes('font-size:100px;')).eql(false)

      const editedHtml = await renderViaHTTP('/hmr/style-dynamic-component')
      await t.expect(editedHtml.includes('200px')).ok()
      const $editedHtml = cheerio.load(editedHtml)
      const editedServerClassName = $editedHtml('#dynamic-component').attr(
        'class'
      )

      await t.expect(editedClientClassName === editedServerClassName).eql(true)
    } finally {
      // Finally is used so that we revert the content back to the original regardless of the test outcome
      // restore the about page content.
      writeFileSync(pagePath, originalContent, 'utf8')
