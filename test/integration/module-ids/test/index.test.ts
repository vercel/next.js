...
import { killApp, findPort, launchApp, nextBuild, renderViaHTTP, getDistDir } from 'next-test-utils'
...
describe('minified module ids', () => {
  ;(!process.env.IS_TURBOPACK_TEST || process.env.TURBOPACK_DEV ? describe.skip : describe)(
    'production mode',
    () => {
      let ssrBundles = ''
      let staticBundles = ''
      ...
      beforeAll(async () => {
        ...
        const ssrBundleBasenames = (await fs.readdir(ssrPath)).filter((p) => p.match(/\.js$/))
        ...
        const staticBundleBasenames = (await fs.readdir(staticPath)).filter((p) => p.match(/\.js$/))
      ...
      it('should have no long module ids for external modules', async () => {
        ...
        expect(ssrBundles).toContain('the content of an external module with a long name')
      })
