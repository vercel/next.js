  ...
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    it(`should've emitted a single CSS file`, async () => {
      ...
      if (process.env.IS_TURBOPACK_TEST) {
        expect(cssContent.replace(/\/\*.*?\*\//g, '').trim()).toMatchInlineSnapshot(
          `".index-module__VJHdSq__redText{color:red}"`
        )
      } else {
        expect(cssContent.replace(/\/\*.*?\*\//g, '').trim()).toMatchInlineSnapshot(
    ...
    it(`should've injected the CSS on server render`, async () => {
      ...
      if (process.env.IS_TURBOPACK_TEST) {
        ...
        expect($('#verify-red').attr('class')).toMatchInlineSnapshot(`"index_redText__honUV"`)
      }
    })
  })
})
...
describe('3rd Party CSS Module Support', () => {
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    it(`should've injected the CSS on server render`, async () => {
      ...
      if (process.env.IS_TURBOPACK_TEST) {
        expect($('#verify-div').attr('class')).toMatchInlineSnapshot(`"index-module__jAE1EW__foo"`)
      } else {
        expect($('#verify-div').attr('class')).toMatchInlineSnapshot(`"index_foo__6TgnK"`)
      }
    })
  })
})
...
describe.skip('Invalid Global CSS Module Usage in node_modules', () => {
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    it('should fail to build', async () => {
      ...
      expect(stderr).toMatch(/Global CSS.*cannot.*be imported from within.*node_modules/)
      // Skip: Rspack loaders cannot access module issuer info for location details
      if (!process.env.NEXT_RSPACK) {
        expect(stderr).toMatch(/Location:.*node_modules[\\/]example[\\/]index\.mjs/)
      }
...
describe('Valid CSS Module Usage from within node_modules', () => {
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    it(`should've emitted a single CSS file`, async () => {
      ...
      if (process.env.IS_TURBOPACK_TEST) {
        expect(cssContent.replace(/\/\*.*?\*\//g, '').trim()).toMatchInlineSnapshot(
          `".index-module__kwuKnq__redText{color:red}"`
        )
      } else {
        expect(cssContent.replace(/\/\*.*?\*\//g, '').trim()).toMatchInlineSnapshot(
  ...
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    it(`should've emitted a single CSS file`, async () => {
      ...
      if (process.env.IS_TURBOPACK_TEST) {
        ...
        expect(cssContent.replace(/\/\*.*?\*\//g, '').trim()).toMatchInlineSnapshot(
          `"._post__home__yRmHz{background:#f00}"`
        )
      }
    })
  })
...
describe('Catch-all Route CSS Module Usage', () => {
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    it('should apply styles correctly', async () => {
      ...
      const bg = await browser.elementByCss('#my-div').getComputedCss('background-color')
