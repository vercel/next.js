...
describe('Build Output', () => {
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    for (const experimental of configs) {
      describe(`Basic Application Output (experimental: ${JSON.stringify(experimental)})`, () => {
        let stdout
        ...
        beforeAll(async () => {
          ...
          if (hasExperimentalConfig) {
            nextConfig.write(`module.exports = { experimental: ${JSON.stringify(experimental)} };`)
          }
        ...
        it.skip('should not deviate from snapshot', async () => {
          ...
          const parsePageSize = (page) =>
            stdout.match(new RegExp(` ${page} .*?((?:\\d|\\.){1,} (?:\\w{1,})) `))[1]

          const parsePageFirstLoad = (page) =>
            stdout.match(
              new RegExp(` ${page} .*?(?:(?:\\d|\\.){1,}) .*? ((?:\\d|\\.){1,} (?:\\w{1,}))`)
            )[1]
          ...
          expect(webpackSize.endsWith('kB') || webpackSize.endsWith(' B')).toBe(true)
        ...
        it('should print duration when rendering or get static props takes long', () => {
          ...
          for (const check of [
            // summary
            expect.stringMatching(/\/\[propsDuration\]\/\[renderDuration\] \(\d+ ms\)/),
