...
const runTests = () => {
  it('should show error for invalid mulit-match', async () => {
    ...
    expect(stderr).toContain('To use a multi-match in the destination you must add')
    expect(stderr).toContain('https://nextjs.org/docs/messages/invalid-multi-match')
  })
}
