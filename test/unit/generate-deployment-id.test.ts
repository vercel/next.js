...
describe('resolveAndSetDeploymentId', () => {
  ...
  describe('Character validation', () => {
    ...
    // Edge cases for environment variable handling (no validation)
    describe('environment variable edge cases', () => {
      ...
      it('should accept Vercel-style deployment IDs without validation', () => {
        const vercelId = 'dpl_abc123-DEF456.vercel.app'
        process.env.NEXT_DEPLOYMENT_ID = vercelId
        const result = resolveAndSetDeploymentId(undefined, 'env-var')
        expect(result).toBe(vercelId)
      })

      it('should accept platform deployment IDs with dots and hyphens', () => {
        const platformId = 'deploy_123-ABC.netlify.app'
        process.env.NEXT_DEPLOYMENT_ID = platformId
        const result = resolveAndSetDeploymentId(undefined, 'env-var')
        expect(result).toBe(platformId)
      })

      it('should handle very long platform deployment IDs', () => {
        const longPlatformId =
          'vercel-deployment-1234567890abcdef1234567890abcdef1234567890abcdef'
        process.env.NEXT_DEPLOYMENT_ID = longPlatformId
        const result = resolveAndSetDeploymentId(undefined, 'env-var')
        expect(result).toBe(longPlatformId)
      })
    })

    // Edge cases for config vs env precedence
    describe('precedence edge cases', () => {
      ...
      it('should use env var even when config is empty string', () => {
        const envId = 'env-should-win'
        process.env.NEXT_DEPLOYMENT_ID = envId
        const result = resolveAndSetDeploymentId('', 'user-config')
        expect(result).toBe(envId)
      })

      it('should use config when both are set', () => {
        const configId = 'config-takes-precedence'
        const envId = 'env-should-be-ignored'
        process.env.NEXT_DEPLOYMENT_ID = envId
        const result = resolveAndSetDeploymentId(configId, 'user-config')
