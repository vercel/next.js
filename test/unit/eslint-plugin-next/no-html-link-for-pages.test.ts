...
describe('no-html-link-for-pages', function () {
  ...
  it('detects pages with custom pageExtensions from settings.next', function () {
...
`
    ...
    assert.notEqual(
      report,
      undefined,
      'Expected the rule to flag /about as a known page when pageExtensions is configured.'
    )
    assert.equal(
      report.message,
