...
import { PHASE_DEVELOPMENT_SERVER, CONFIG_FILE } from 'next/constants'
...
describe('config', () => {
  it('Should get the configuration', () => {
    ...
    expect(userConfig.customConfig).toBe(true)
    expect(userConfig.configOrigin).toBe(CONFIG_FILE)
  })
  ...
  it('Should not pass the userConfig if next.config.js is missing', () => {
    const { userConfig } = loadConfig(PHASE_DEVELOPMENT_SERVER, null)
    expect(userConfig).toBe(undefined)
  })
