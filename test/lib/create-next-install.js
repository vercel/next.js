...
async function createNextInstall({
  ...
  return await parentSpan.traceChild('createNextInstall').traceAsyncFn(async (rootSpan) => {
    const origRepoDir = path.join(__dirname, '../../')
    const installDir = path.join(tmpDir, `next-install-${randomBytes(32).toString('hex')}`, subDir)
    let tmpRepoDir
    ...
    if (pkgPathsEnv) {
      ...
      tmpRepoDir = path.join(tmpDir, `next-repo-${randomBytes(32).toString('hex')}`, subDir)
      ...
      for (const item of [
        ...
        await rootSpan.traceChild(`copy ${item} to temp dir`).traceAsyncFn(() =>
          fs.copy(path.join(origRepoDir, item), path.join(tmpRepoDir, item), {
            filter: (item) => {
              return (
      ...
      if (process.env.NEXT_TEST_WASM) {
        ...
        const hasWasmBinary = fs.existsSync(path.join(wasmPath, 'package.json'))
        if (hasWasmBinary) {
      ...
      pkgPaths = await rootSpan.traceChild('linkPackages').traceAsyncFn((span) =>
        linkPackages({
          repoDir: tmpRepoDir,
          parentSpan: span,
        })
    ...
    if (beforeInstall !== undefined) {
      await rootSpan.traceChild('beforeInstall').traceAsyncFn(async (span) => {
