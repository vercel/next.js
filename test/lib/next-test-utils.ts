...
/**
 * Blur placeholders for statically imported PNGs are generated by sharp, and
 * sharp 0.35 bundles a newer libvips that encodes them differently. Node 18
 * cannot install 0.35 (it requires Node 20.9) and resolves 0.34 instead, so the
 * expected bytes depend on whichever version this run installed.
 */
export function getStaticPngBlurBase64(): string {
  // sharp is an optional dependency of `next`, so it is not hoisted to the
  // root, and 0.35's `exports` map hides `./package.json` from `require`.
  const { version } = JSON.parse(
    readFileSync(
      path.join(
        __dirname,
        '../../packages/next/node_modules/sharp/package.json'
      ),
      'utf8'
    )
  ) as { version: string }
  return version.startsWith('0.34.')
    ? 'iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAMAAADz0U65AAAAElBMVEUAAAA6OjolJSWwsLAfHx/9/f2oxsg2AAAACXBIWXMAAAsTAAALEwEAmpwYAAAAH0lEQVR4nGNgwAaYmKAMZmYIzcjKyghmsDAysmDTAgAEXAAhXbseDQAAAABJRU5ErkJggg=='
    : 'iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAMAAADz0U65AAAAElBMVEUAAAA6OjolJSWwsLAfHx/9/f2oxsg2AAAACXBIWXMAAAsSAAALEgHS3X78AAAAH0lEQVQImWNgwAaYmKAMZmYIzcjKyghmsDAysmDTAQAEXAAh8CaqOAAAAABJRU5ErkJggg=='
}
