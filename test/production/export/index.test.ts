...
createNextDescribe(
  ...
  ({ next }) => {
    ...
    it("should only output 404.html without trailingSlash", async () => {
      ...
      expect(await fileExist(path.join(outNoTrailSlash, "404.html"))).toBe(
        true
      );
    });

    it("should not duplicate /index with trailingSlash", async () => {
      expect(await fileExist(path.join(outdir, "index/index.html"))).toBe(
        false
      );
    ...
    it("exportTrailingSlash is not ignored", async () => {
      ...
      await next.patchFile(nextConfigPath, nextConfig);
      expect(
        await fileExist(path.join(tmpOutdir, "404/index.html"))
      ).toBeTrue();
    });
  }
);
