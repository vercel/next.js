...
describe('Required Server Files', () => {
  ;(process.env.TURBOPACK_DEV ? describe.skip : describe)('production mode', () => {
    ...
    beforeAll(async () => {
      ...
      requiredFilesManifest = await next.readJSON(join('.next/required-server-files.json'))
    ...
    it('should render dynamic SSR page correctly with x-matched-path', async () => {
      ...
      const html2 = await renderViaHTTP(appPort, '/some-other-path?slug=second', undefined, {
        headers: {
          'x-matched-path': '/dynamic/[slug]',
        },
      })
    ...
    it('should return data correctly with x-matched-path for optional catch-all route', async () => {
      const res = await fetchViaHTTP(appPort, `/_next/data/${buildId}/catch-all.json`, undefined, {
        headers: {
          'x-matched-path': '/catch-all/[[...rest]]',
        },
      })
    ...
    it('should handle 404s properly', async () => {
      for (const pathname of ['/static/some-file.js', '/non-existent', '/404']) {
