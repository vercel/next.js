...
;(process.env.IS_TURBOPACK_TEST ? describe.skip : describe)('Subresource Integrity', () => {
  describe.each(['node', 'edge', 'pages'] as const)('with %s runtime', (runtime) => {
    ...
    async function renderWithPolicy(policy: string | null, reportOnly?: boolean) {
    ...
    it('includes an integrity attribute on scripts', async () => {
      ...
      const polyfillsScript = $('head script[src^="/_next/static/chunks/polyfills"]')
