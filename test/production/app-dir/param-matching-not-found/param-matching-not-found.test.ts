...
describe('coherent not-found parameter configurations', () => {
  ...
  it('accepts inheritance, explicit agreement, and unrelated parameter definitions', async () => {
    ...
    // Check that accepting the policies still produces each route's outputs.
    // If the manifest changes, preserve that scenario rather than exposing
    // internals solely to keep these assertions working.
