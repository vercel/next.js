...
;(process.env.IS_TURBOPACK_TEST ? describe.skip : describe)('Disable fallback polyfills', () => {
  ...
  it('Fallback polyfills added by default', async () => {
    ...
    console.log(`Index page size (with polyfills): ${indexPageSizeKB.toFixed(2)} kB`)
  ...
  it('Reduced bundle size when polyfills are disabled', async () => {
    ...
    const indexPageSizeKB = await getIndexPageSize()
    console.log(`Index page size (without polyfills): ${indexPageSizeKB.toFixed(2)} kB`)
