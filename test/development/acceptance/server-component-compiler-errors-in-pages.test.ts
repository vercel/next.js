  ...
  ({ next }) => {
    test("importing 'next/headers' in pages", async () => {
      ...
      expect(await session.getRedboxSource()).toMatchInlineSnapshot(`
        ...
        You're importing a component that needs next/headers. That only works in a Server Component which is not supported in the pages/ directory. Read more: https://nextjs.org/docs/rendering/server-and-client-components
    ...
    test("importing 'server-only' in pages", async () => {
      ...
      expect(await session.getRedboxSource()).toMatchInlineSnapshot(`
        ...
        You're importing a component that needs server-only. That only works in a Server Component which is not supported in the pages/ directory. Read more: https://nextjs.org/docs/rendering/server-and-client-components
