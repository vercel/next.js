...
describe('fetch-get-set-cookie', () => {
  ...
  it('should fetch getSetCookie and cache properly on server component', async () => {
    const $ = await next.render$('/')
    expect($('p').text()).toBe('["foo=foo","bar=bar"]')

    // re-fetch to ensure the fetch cache is set properly
    const $2 = await next.render$('/')
    expect($2('p').text()).toBe('["foo=foo","bar=bar"]')
  })
