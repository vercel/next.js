...
describe('hmr-app-and-pages', () => {
  ...
  it('should do HMR when app router and pages router have shared CSS', async () => {
    ...
    expect(await browser.elementByCss('body').getComputedCss('background-color')).toEqual(
      'rgb(255, 255, 255)'
    )

    await next.patchFile('app/styles.css', (content) =>
      content.replace('background-color: rgb(255, 255, 255);', 'background-color: rgb(255, 0, 0);')
    )
    ...
    await next.patchFile('app/styles.css', (content) =>
      content.replace('background-color: rgb(255, 0, 0);', 'background-color: rgb(255, 255, 255);')
    )
