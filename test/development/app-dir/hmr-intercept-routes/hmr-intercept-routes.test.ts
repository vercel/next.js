...
;(process.env.IS_TURBOPACK_TEST ? describe : describe.skip)('hmr-intercept-routes', () => {
  ...
  it('should update intercept routes via HMR', async () => {
    ...
    const parallelDefaultContent = await next.readFile('fixtures/@intercept/default.js')

    const parallelInterceptContent = await next.readFile('fixtures/@intercept/(.)intercept/page.js')
    ...
    await next.patchFile('app/@intercept/(.)intercept/page.js', parallelInterceptContent)
    ...
    expect(await browser.elementById('default-intercept').text()).toBe("I'm the default intercept")
    ...
    expect(await browser.elementById('intercept').text()).toBe("I'm the intercept")
