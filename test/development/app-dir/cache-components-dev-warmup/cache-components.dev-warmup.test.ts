...
describe.each([
...
])('cache-components-dev-warmup - $description', ({ fixturePath, hasRuntimePrefetch }) => {
  ...
  async function testInitialLoad(path: string, assertLogs: (browser: Playwright) => Promise<void>) {
    ...
    expect(next.cliOutput).not.toContain('AbortError: This operation was aborted')
  ...
  async function testNavigation(path: string, assertLogs: (browser: Playwright) => Promise<void>) {
  ...
  async function revalidatePath(path: string) {
    const response = await next.fetch(`/revalidate?path=${encodeURIComponent(path)}`)
    if (!response.ok) {
      throw new Error(
