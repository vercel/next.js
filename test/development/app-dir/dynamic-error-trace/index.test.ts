...
describe('app dir - dynamic error trace', () => {
  ...
  it('should show the error trace', async () => {
    ...
    await expect(browser).toDisplayRedbox(`
     {
       "description": "Route / with \`dynamic = "error"\` couldn't be rendered statically because it used \`headers()\`. \`headers()\` returns a Promise and must be unwrapped with \`await\` or \`React.use()\` before accessing its properties. See more info here: https://nextjs.org/docs/app/building-your-application/rendering/static-and-dynamic#dynamic-rendering",
