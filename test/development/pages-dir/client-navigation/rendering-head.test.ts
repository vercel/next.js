...
describe('Client Navigation rendering <Head />', () => {
  ...
  test('header renders default viewport', async () => {
    const html = await render('/default-head')
    expect(html).toContain('<meta name="viewport" content="width=device-width" data-next-head=""/>')
  })
  ...
  test('header helper dedupes tags', async () => {
    ...
    const dedupeLink = '<link rel="stylesheet" href="dedupe-style.css" data-next-head=""/>'
    expect(html).toContain(dedupeLink)
    expect(html.substring(html.indexOf(dedupeLink) + dedupeLink.length)).not.toContain(
      '<link rel="stylesheet" href="dedupe-style.css"'
    )
    expect(html).toContain(
      '<link rel="alternate" hrefLang="en" href="/last/en" data-next-head=""/>'
    )
    expect(html).not.toContain('<link rel="alternate" hrefLang="en" href="/first/en"')
  })
  ...
  test('header helper dedupes tags with the same key as the default', async () => {
    ...
    expect(html).toContain('<meta name="viewport" content="width=500" data-next-head=""/>')
  })

  test('header helper avoids dedupe of specific tags', async () => {
    const html = await render('/head')
    console.log(html)
    expect(html).toContain('<meta property="article:tag" content="tag1" data-next-head=""/>')
    expect(html).toContain('<meta property="article:tag" content="tag2" data-next-head=""/>')
    expect(html).not.toContain('<meta property="dedupe:tag" content="tag3"')
    expect(html).toContain('<meta property="dedupe:tag" content="tag4" data-next-head=""/>')
    expect(html).toContain('<meta property="og:image" content="ogImageTag1" data-next-head=""/>')
    expect(html).toContain('<meta property="og:image" content="ogImageTag2" data-next-head=""/>')
    expect(html).toContain(
    ...
    expect(html).toContain('<meta property="fb:pages" content="fbpages1" data-next-head=""/>')
    expect(html).toContain('<meta property="fb:pages" content="fbpages2" data-next-head=""/>')
  })

  test('header helper avoids dedupe of meta tags with the same name if they use unique keys', async () => {
    const html = await render('/head')
    expect(html).toContain('<meta name="citation_author" content="authorName1" data-next-head=""/>')
    expect(html).toContain('<meta name="citation_author" content="authorName2" data-next-head=""/>')
  })
  ...
  test('header helper renders boolean attributes correctly children', async () => {
    const html = await render('/head')
    expect(html).toContain('<script src="/test-async-false.js" data-next-head="">')
    expect(html).toContain('<script src="/test-async-true.js" async="" data-next-head="">')
    expect(html).toContain('<script src="/test-defer.js" defer="" data-next-head="">')
  })
  ...
  it('should place charset element at the top of <head>', async () => {
    ...
    const documentHeadElement = '<meta name="keywords" content="document head test"/>'
  ...
  test('custom meta properties are rendered only once', async () => {
    ...
    const ogTitleElements = await browser.elementsByCss('meta[property="og:title"]')
    ...
    const descriptionElements = await browser.elementsByCss('meta[name="description"]')
