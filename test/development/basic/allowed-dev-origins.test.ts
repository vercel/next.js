...
describe.each([['', '/docs']])('allowed-dev-origins, basePath: %p', (basePath: string) => {
  ...
  describe('warn mode', () => {
    beforeAll(async () => {
      ...
      await retry(async () => {
        ...
        const res = await fetchViaHTTP(next.appPort, '/_next/static/chunks/pages/_app.js')
    ...
    it('should warn about WebSocket from cross-site', async () => {
      ...
      try {
        ...
        await retry(async () => {
          expect(await browser.elementByCss('#status').text()).toBe('connected')
        })
  ...
  describe('block mode', () => {
    ...
    it('should not allow loading internal middleware from cross-site', async () => {
      ...
      try {
        ...
        await retry(async () => {
          expect(await browser.elementByCss('#status').text()).toBe('Unauthorized')
        })
      } finally {
        server.close()
      }
