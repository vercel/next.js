...
describe.each([[''], ['/docs']])('misc basic dev tests, basePath: %p', (basePath: string) => {
  ...
  it('should allow access to public files', async () => {
    ...
    const legacy = await renderViaHTTP(next.url, basePath + '/static/legacy.txt')
  ...
  describe('With Security Related Issues', () => {
    ...
    it('should handle encoded / value for trailing slash correctly', async () => {
      const res = await fetchViaHTTP(next.url, basePath + '/%2fexample.com/', undefined, {
        redirect: 'manual',
      })
