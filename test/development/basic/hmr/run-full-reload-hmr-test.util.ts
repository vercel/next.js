...
export function runFullReloadHmrTest(nextConfig: { basePath: string; assetPrefix: string }) {
  ...
  it('should warn about full reload in cli output - anonymous page function', async () => {
    ...
    const browser = await next.browser(basePath + '/hmr/anonymous-page-function')
    const cliWarning =
    ...
    const currentFileContent = await next.readFile('./pages/hmr/anonymous-page-function.js')
    ...
    expect(await browser.waitForElementByCss('#updated').text()).toBe('hello world!!!')
  ...
  it('should warn about full reload in cli output - runtime-error', async () => {
    ...
    const cliWarning = 'Fast Refresh had to perform a full reload due to a runtime error.'
    ...
    const currentFileContent = await next.readFile('./pages/hmr/runtime-error.js')
    const newFileContent = currentFileContent.replace('whoops', '<p id="updated">whoops</p>')
