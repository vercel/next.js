...
use napi::{
    Env, JsFunction, JsObject, Status, Unknown as JsUnknown,
    bindgen_prelude::{External, ExternalRef, within_runtime_if_available},
...
use crate::{
    next_api::{
        ...
        utils::{
            DetachedVc, NapiDiagnostic, NapiIssue, RootTask, SendableExternalRef, TurbopackResult,
            func_ref_to_js_function, get_diagnostics, get_issues,
...
pub fn project_entrypoints_subscribe(
    ...
    #[napi(ts_arg_type = "{ __napiType: \"Project\" }")] project: SendableExternalRef<
        ProjectInstance,
    >,
    func: napi::bindgen_prelude::FunctionRef<JsUnknown<'static>, JsUnknown<'static>>,
) -> napi::Result<External<RootTask>> {
    let turbopack_ctx = project.turbopack_ctx.clone();
    let container = project.container;
    let func: JsFunction = func_ref_to_js_function(&env, &func)?;
    subscribe(
        turbopack_ctx.clone(),
        func,
        move || {
...
#[tracing::instrument(level = "info", name = "get HMR events", skip(env, project, func), fields(target = %target))]
#[napi(ts_return_type = "{ __napiType: \"RootTask\" }")]
...
pub fn project_update_info_subscribe(
    ...
    let func: ThreadsafeFunction<UpdateMessage, (), NapiUpdateMessage, Status, true> = func
        .create_threadsafe_function(|ctx| {
...
pub fn project_compilation_events_subscribe(
    ...
    let func: JsFunction = func_ref_to_js_function(&env, &func)?;
    let tsfn: ThreadsafeFunction<Arc<dyn CompilationEvent>, (), JsObject, Status, true> = func
        .create_threadsafe_function(|ctx| {
