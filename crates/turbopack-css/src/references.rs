...
use swc_css::ast::{ImportPrelude, ImportPreludeHref, Url, UrlValue};
use swc_css::visit::{self, Visit, VisitWith};
...
use turbopack_core::reference::{AssetReferencesVc, AssetReference, AssetReferenceVc};
use turbopack_core::chunk::{ChunkableAssetReference, ChunkableAssetReferenceVc};
	...
	fn visit_import_prelude(&mut self, i: &ImportPrelude) {
		...
		if let Some(src) = src {
			self.references.push(
				UrlAssetReferenceVc::new(self.context, RequestVc::parse(Value::new(src.into()))).into(),
			);
		}
