...
pub async fn main_inner(
    ...
    if matches!(strate, Strategy::Development { .. }) {
        options.dev = true;
    ...
    if strate.randomized() {
        routes = Box::new(shuffle(routes))
    }

    let start = Instant::now();
    let count = render_routes(tt, routes, strate, factor, limit).await?;
...
pub async fn render_routes(
    ...
    tracing::info!(
        "rendering routes with {} parallel and strate {}",
        factor,
        strategy
    );
