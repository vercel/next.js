...
/// Main function to run HMR benchmarks directly
fn main() -> Result<()> {
    println!("Running HMR Benchmarks...");

    // Small scale benchmark
    println!("\n=== Small Scale Benchmark (50 modules) ===");
    let small_benchmark = HmrBenchmark::new(50)?;
    println!(
        "Created test app with {} modules",
        small_benchmark.module_count()
    );

    let initial_duration = small_benchmark.benchmark_initial_compilation()?;
    println!("Initial compilation: {:?}", initial_duration);

    let hmr_duration = small_benchmark.benchmark_hmr_update(5)?;
    println!("5 HMR updates: {:?}", hmr_duration);

    let subscription_duration = small_benchmark.benchmark_hmr_subscription()?;
    println!("HMR subscription test: {:?}", subscription_duration);

    // Medium scale benchmark
    println!("\n=== Medium Scale Benchmark (200 modules) ===");
    let medium_benchmark = HmrBenchmark::new(200)?;
    println!(
        "Created test app with {} modules",
        medium_benchmark.module_count()
    );

    let initial_duration = medium_benchmark.benchmark_initial_compilation()?;
    println!("Initial compilation: {:?}", initial_duration);

    let hmr_duration = medium_benchmark.benchmark_hmr_update(10)?;
    println!("10 HMR updates: {:?}", hmr_duration);

    // Large scale benchmark
    println!("\n=== Large Scale Benchmark (500 modules) ===");
    let large_benchmark = HmrBenchmark::new(500)?;
    println!(
        "Created test app with {} modules",
        large_benchmark.module_count()
    );

    let initial_duration = large_benchmark.benchmark_initial_compilation()?;
    println!("Initial compilation: {:?}", initial_duration);

    let hmr_duration = large_benchmark.benchmark_hmr_update(20)?;
    println!("20 HMR updates: {:?}", hmr_duration);
