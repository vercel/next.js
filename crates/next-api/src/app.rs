...
use turbopack_core::{
    asset::AssetContent,
    chunk::{
        ChunkGroupResult, ChunkingContext, ChunkingContextExt, EvaluatableAssets,
        ...
        if *self.project.per_page_module_graph().await? {
            ...
            async move {
                let rsc_entry_chunk_group = ChunkGroupEntry::Entry {
                    modules: server_action_loader_modules
                        .await?
                        .iter()
                        .copied()
...
impl AppEndpoint {
    ...
    async fn output(self: Vc<Self>) -> Result<Vc<AppEndpointOutput>> {
        ...
        client_assets.extend(
            service_worker_output_assets(project, module_graph)
                .await?
                .iter()
                .copied(),
        );
