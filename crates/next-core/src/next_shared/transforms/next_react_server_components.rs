...
pub fn get_next_react_server_components_transform_rule(
    ...
    let cache_components_enabled = *turbo_tasks::read!(next_config.enable_cache_components())?;
    let use_cache_enabled = *turbo_tasks::read!(next_config.enable_use_cache())?;
    let taint_enabled = *turbo_tasks::read!(next_config.enable_taint())?;
    let page_extensions = turbo_tasks::read!(next_config
        .page_extensions())
        ?
        .iter()
        .map(|s| s.to_string())
    ...
    Ok(get_ecma_transform_rule(
        turbo_tasks::read!(next_react_server_components_transform_plugin(
