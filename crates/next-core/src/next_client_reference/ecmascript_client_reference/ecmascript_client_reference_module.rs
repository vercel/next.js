...
impl EcmascriptClientReferenceModule {
    ...
    async fn proxy_module(&self) -> Result<Vc<Box<dyn EcmascriptChunkPlaceable>>> {
        // Strip parts from the ident to normalize across part-specific CR modules.
        // This ensures that CR modules created for the same source but different
        // import parts (e.g., full module vs Export("default")) share the same
        // proxy module, preventing duplicate module ident issues when the proxy
        // gets split into facade+locals during name mangling.
        //
        // We always go through AssetIdent::new() (even when parts is already
        // empty) to ensure both paths produce the same Vc identity via the
        // turbo_tasks cache in AssetIdent::new_inner().
        let base_ident = {
            let ident = self.server_ident.await?;
            let mut base = (*ident).clone();
            base.parts.clear();
            AssetIdent::new(base)
        };
        Ok(create_proxy_module(
            base_ident,
...
/// Creates a proxy module for client references. This is a standalone function
/// (not a method on EcmascriptClientReferenceModule) so that turbo_tasks caching
/// deduplicates across different CR module instances that share the same base
/// identity (same source, same context, same client module) but differ only in
/// their import part.
#[turbo_tasks::function]
async fn create_proxy_module(
    ...
    let server_module_path = &*server_ident.to_string().await?;
