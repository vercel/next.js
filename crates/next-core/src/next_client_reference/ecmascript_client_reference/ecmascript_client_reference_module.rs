...
impl EcmascriptClientReferenceModule {
    ...
    async fn proxy_module(&self) -> Result<Vc<Box<dyn EcmascriptChunkPlaceable>>> {
        ...
        // underlying client component, with a different `self.server_ident` (and hence a
        // different `self`) each time. Stripping `.parts` here, before delegating to the
        // standalone `proxy_module_for`, is what lets that function's own memoization (keyed on
        // its arguments, not on which `EcmascriptClientReferenceModule` called it) treat both
        // forms as the same call — see `proxy_module_for` for why delegating to a method on
        // `self` isn't enough by itself.
        let mut canonical_server_ident = self.server_ident.owned().await?;
        canonical_server_ident.parts.clear();
        Ok(proxy_module_for(
            *self.server_asset_context,
            canonical_server_ident.into_vc(),
            *self.client_module,
        ))
    }
}

/// Builds (and analyzes) the generated proxy module for a client reference — the file that
/// re-exports the client component's names as `registerClientReference(...)` calls, or as a
/// single `createClientModuleProxy(...)` call for a non-ESM client module.
///
/// This is a standalone, explicitly-keyed function rather than a method on
/// `EcmascriptClientReferenceModule` (`&self`) *specifically* so that turbo-tasks' task
/// memoization — which is keyed on a function's arguments, not on "which caller reached it" —
/// properly interns one result across every `EcmascriptClientReferenceModule` that wraps the same
/// underlying client component. A method on `&self` does not get this for free: two different
/// `EcmascriptClientReferenceModule` cells (one for the plain named-export form of an import, one
/// for its always-present module-evaluation form) both delegate here, and without this being a
/// standalone function keyed on their *shared* inputs, each would independently build its own
/// `VirtualSource`+resolved module — which, if the two forms' generated file content happened not
/// to be identical (see `proxy_module`'s stripping of `.parts` from the ident it passes in here),
/// would produce two distinct modules at the same file path and crash the module graph's
/// "Duplicate module idents" invariant once each independently split into a facade and a locals
/// submodule (this crash predates that split existing, but was invisible before it, since neither
/// raw module was ever inserted into the module graph as its own node).
#[turbo_tasks::function]
async fn proxy_module_for(
    server_asset_context: Vc<Box<dyn AssetContext>>,
    canonical_server_ident: Vc<AssetIdent>,
    client_module: Vc<Box<dyn EcmascriptChunkPlaceable>>,
) -> Result<Vc<Box<dyn EcmascriptChunkPlaceable>>> {
    let mut code = CodeBuilder::default();
    let is_esm: bool;

    let server_module_path = &*canonical_server_ident.to_string().await?;
    ...
    if let EcmascriptExports::EsmExports(exports) = &*client_module.get_exports().await? {
        is_esm = true;
            ...
            if export_name == "default" {
                writedoc!(
                    ...
                    call_err = StringifyJs(&format!(
                        "Attempted to call the default export of {server_module_path} from the \
                         server, but it's on the client. It's not possible to invoke a client \
                         function from the server, it can only be rendered as a Component or \
                ...
                writedoc!(
                    ...
                    call_err = StringifyJs(&format!(
                        "Attempted to call {export_name}() from the server but {export_name} is \
    ...
    let proxy_source = VirtualSource::new(
        canonical_server_ident.await?.path.clone().join(
    ...
    let proxy_module = server_asset_context
