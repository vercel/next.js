...
use turbopack_ecmascript::{
    ...
    analyzer::{Arena, ConstantNumber, ConstantValue, JsValue, ObjectPart, graph::EvalContext},
...
pub async fn parse_segment_config_from_source(
    ...
    // Arena for the `JsValue`s produced while evaluating config expressions; freed when this
    // function returns.
    let arena = Arena::new();
...
async fn parse_config_value<'a>(
    ...
    let get_value = || {
        ...
        init.map(|init| eval_context.eval(arena, init)).map(|v| {
