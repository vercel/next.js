...
impl AppPageLoaderTreeBuilder {
    ...
    async fn write_modules_entry(
        ...
        if let Some(path) = path {
            let tuple_code =
                turbo_tasks::read!(self.base.create_module_tuple_code(module_type, path, depth))?;
    ...
    async fn write_metadata(
        ...
        turbo_tasks::read!(self.write_metadata_items(app_page, "icon", icon.iter(), depth))?;
        turbo_tasks::read!(self.write_metadata_items(app_page, "apple", apple.iter(), depth))?;
        turbo_tasks::read!(self.write_metadata_items(app_page, "twitter", twitter.iter(), depth))?;
        turbo_tasks::read!(self.write_metadata_items(
            app_page,
            "openGraph",
            open_graph.iter(),
            depth
        ...
        if let Some(global_metadata) = global_metadata {
            turbo_tasks::read!(self.write_metadata_manifest(global_metadata.manifest.clone()))?;
        }
        self.loader_tree_code += "  },";
        Ok(())
    }
    ...
    async fn write_metadata_manifest(&mut self, manifest: Option<MetadataItem>) -> Result<()> {
        ...
        let metadata_manifest_route = turbo_tasks::read!(get_metadata_route_name(manifest))?;
    ...
    async fn write_metadata_items<'a>(
        ...
        for item in it {
            turbo_tasks::read!(self.write_metadata_item(app_page, name, item, depth))?;
        }
    ...
    async fn write_metadata_item(
        ...
        match item {
            MetadataWithAltItem::Static { path, alt_path } => {
                turbo_tasks::read!(self.write_static_metadata_item(
                    app_page,
                    name,
                    item,
                    path.clone(),
            ...
            MetadataWithAltItem::Dynamic { path, .. } => {
                ...
                let module = turbo_tasks::read!(self.base.process_source(source).to_resolved())?;
                self.base
                    .inner_assets
    ...
    async fn write_static_metadata_item(
        ...
        let module = turbo_tasks::read!(self.base.process_module(module).to_resolved())?;
        self.base
            .inner_assets
            .insert(inner_module_id.into(), module);
        ...
        let metadata_route = fill_static_metadata_segment(
            &pathname_prefix,
            &turbo_tasks::read!(get_metadata_route_name(item.clone().into()))?,
        );
        writeln!(
            self.loader_tree_code,
        ...
        let content_type = turbo_tasks::read!(get_content_type(path))?;
    ...
    async fn walk_tree(
        ...
        let global_metadata = &*turbo_tasks::read!(global_metadata)?;
        turbo_tasks::read!(self.write_metadata(
            app_page,
            metadata,
        ...
        turbo_tasks::read!(self.write_modules_entry(
            AppDirModuleType::Layout,
            layout.clone(),
            depth
        ))?;
        turbo_tasks::read!(self.write_modules_entry(
            AppDirModuleType::Error,
            error.clone(),
            depth
        ))?;
        turbo_tasks::read!(self.write_modules_entry(
            AppDirModuleType::Loading,
            loading.clone(),
            depth
        ))?;
        turbo_tasks::read!(self.write_modules_entry(
            AppDirModuleType::Template,
            template.clone(),
            depth
        ))?;
        turbo_tasks::read!(self.write_modules_entry(
            AppDirModuleType::NotFound,
            not_found.clone(),
            depth
        ))?;
        turbo_tasks::read!(self.write_modules_entry(
            AppDirModuleType::Forbidden,
            forbidden.clone(),
            depth
        ))?;
        turbo_tasks::read!(self.write_modules_entry(
            AppDirModuleType::Unauthorized,
            unauthorized.clone(),
            depth
        ))?;
        turbo_tasks::read!(self.write_modules_entry(AppDirModuleType::Page, page.clone(), depth))?;
        turbo_tasks::read!(self.write_modules_entry(
            AppDirModuleType::DefaultPage,
            default.clone(),
            depth
        ))?;
        turbo_tasks::read!(self.write_modules_entry(
        ...
        for (key, parallel_route) in parallel_routes.iter() {
            ...
            turbo_tasks::read!(Box::pin(self.walk_tree(parallel_route, false, next_depth)))?;
            writeln!(self.loader_tree_code, ",")?;
        }
        writeln!(self.loader_tree_code, "}}, {{")?;
    ...
    async fn build(
        ...
        turbo_tasks::read!(self.walk_tree(loader_tree, true, 0))?;
...
impl AppPageLoaderTreeBuilder {
    ...
    fn walk_tree(&mut self, loader_tree: &AppPageLoaderTree, root: bool, depth: u32) -> Result<()> {
        use std::fmt::Write;
        ...
        for (key, parallel_route) in parallel_routes.iter() {
            ...
            self.walk_tree(parallel_route, false, next_depth)?;
            writeln!(self.loader_tree_code, ",")?;
        }
        writeln!(self.loader_tree_code, "}}, {{")?;
    ...
    fn build(mut self, loader_tree: Vc<AppPageLoaderTree>) -> Result<AppPageLoaderTreeModule> {
        let loader_tree = &*turbo_tasks::read!(loader_tree)?;
