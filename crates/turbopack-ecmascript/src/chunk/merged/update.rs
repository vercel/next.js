...
use crate::chunk::{
    ...
    version::EcmascriptChunkVersionReadRef,
};
...
pub(super) async fn update_ecmascript_merged_chunk(
    ...
    let mut from_versions_by_chunk_path: IndexMap<_, _> = from
        .versions
        .iter()
        .map(|version| (&*version.chunk_server_path, version))
        .collect();

    let merged_module_map = MergedModuleMap::new(from.versions.iter().cloned().collect());

    let content = content.await?;
    let to_contents = content
        .contents
        .iter()
        .map(|content| async move { Ok((*content, content.await?)) })
        .try_join()
        .await?;

    let mut merged_update = EcmascriptMergedUpdate::default();

    for (content, content_ref) in &to_contents {
        ...
        let chunk_update = if let Some(from_version) =
            from_versions_by_chunk_path.remove(chunk_server_path)
        {
            ...
            match update {
                ...
                EcmascriptChunkUpdate::Partial(chunk_partial) => {
                    ...
                    for (module_id, module_code) in chunk_partial.modified {
                        let entry =
                            EcmascriptModuleEntry::new(&*module_id, module_code, chunk_server_path);
                        merged_update.entries.insert(module_id, entry);
                    }
    ...
    for (chunk_server_path, chunk_version) in from_versions_by_chunk_path {
        let hashes = &chunk_version.module_factories_hashes;
