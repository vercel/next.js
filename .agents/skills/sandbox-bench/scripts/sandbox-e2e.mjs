...
const REACT_GH_REPO =
  CONFIG.reactRepoUrl.match(/github\.com[:/]+([^/]+\/[^/.]+)/)?.[1] ?? 'react/react';
const NEXT_GH_REPO =
  CONFIG.nextRepoUrl.match(/github\.com[:/]+([^/]+\/.+?)(?:\.git)?$/)?.[1] ?? 'vercel/next.js';
...
function parseArgs() {
  ...
  const parseArmSpec = (spec, flag) =>
    spec.split(',').filter(Boolean).map(s => {
      ...
      if (!name || !src) throw new Error(`bad arm "${s}" in ${flag}, want name=<ref>`);
      return {name, ref: src};
    });
  ...
  if (arms.length > 2) throw new Error('--arms takes one arm (vs synced react) or two (base first)');
  if (nextArms.length && nextArms.length !== 2) throw new Error('--next-arms needs exactly two arms (base first)');
  return {
    arms,
    nextArms,
    pr,
...
async function resolvePrArms(pr, repo, repoUrl, defaultBranch) {
  ...
  for (let attempt = 1; ; attempt++) {
    try {
      await execFileP('git', ['-C', repo, 'fetch', '-q', repoUrl,
        `+refs/pull/${num}/head:${ns}/pr-${num}`,
        `+refs/heads/${defaultBranch}:${ns}/upstream-${defaultBranch}`]);
      break;
    } catch (e) {
      if (attempt >= 3) throw e;
      console.error(`fetch attempt ${attempt} failed (${e.message.split('\n')[0].slice(0, 80)}); retrying...`);
      await new Promise(r => setTimeout(r, 5000 * attempt + Math.random() * 5000));
    }
  }
  const cand = (await execFileP('git', ['-C', repo, 'rev-parse', `${ns}/pr-${num}`])).stdout.trim();
  const base = (await execFileP('git', ['-C', repo, 'merge-base', `${ns}/upstream-${defaultBranch}`, cand])).stdout.trim();
  await execFileP('git', ['-C', repo, 'update-ref', '-d', `${ns}/pr-${num}`]).catch(() => {});
  await execFileP('git', ['-C', repo, 'update-ref', '-d', `${ns}/upstream-${defaultBranch}`]).catch(() => {});
  console.error(`PR #${num}: cand=${cand.slice(0, 12)} base=${base.slice(0, 12)} (merge-base with ${defaultBranch})`);
  return [{name: 'base', ref: base}, {name: `pr${num}`, ref: cand}];
}
...
async function assertCiGreen(sha, armName, allowUngated) {
  if (!ciVerdicts.has(sha)) {
    let verdict;
    try {
      const out = (await execFileP('gh', ['api', '--paginate',
        `repos/${REACT_GH_REPO}/commits/${sha}/check-runs?per_page=100`,
        '--jq', '[.check_runs[] | {name, conclusion}]'], {maxBuffer: 1 << 24})).stdout;
      const checks = out.trim().split('\n').filter(Boolean)
      ...
      if (checks.length === 0) {
        ...
        const IGNORED = /devtools|^cleanup$|^stale$|_artifacts$|^sizebot|^dependabot/i;
        ...
        verdict = bad.length === 0 ? 'green'
          : `CI not green (${bad.length}/${relevant.length} relevant checks): ` +
            [...new Set(bad.map(c => `${c.name} (${c.conclusion})`))].slice(0, 8).join('; ');
      }
    } catch (e) {
  ...
  if (allowUngated) {
    console.error(`arm ${armName}: react ${sha.slice(0, 12)} UNGATED (${verdict}) — proceeding per --allow-ungated`);
    return;
  }
  throw new Error(
    ...
    'gate the ref yourself (node sandbox-gate.mjs --arms ' + armName + '=<ref>), and then ' +
...
async function syncedReactSha(nextRef) {
  const pkg = (await execFileP('git', ['-C', NEXT_REPO_LAZY(), 'show', `${nextRef}:package.json`],
    {maxBuffer: 1 << 24})).stdout;
  const m = JSON.parse(pkg).devDependencies?.['react-builtin']
    ?.match(/-([0-9a-f]{8,40})-\d{8}$/);
  if (!m) throw new Error(`cannot parse synced react sha from ${nextRef}:package.json react-builtin`);
  return m[1];
}
...
async function fetchNextRef(repo, spec, dst) {
  for (let attempt = 1; ; attempt++) {
    try {
      await execFileP('git', ['-C', repo, 'fetch', '-q', CONFIG.nextRepoUrl, `+${spec}:${dst}`]);
      return;
    } catch (e) {
      if (attempt >= 3) throw e;
...
async function nextShaFor(ref) {
  ...
  if (/^[0-9a-f]{7,40}$/i.test(ref) || ref.startsWith('refs/')) {
    try {
      sha = (await execFileP('git', ['-C', repo, 'rev-parse', '--verify', `${ref}^{commit}`]))
        .stdout.trim();
    } catch {}
  }
  if (!sha) {
    ...
    sha = (await execFileP('git', ['-C', repo, 'rev-parse', `${dst}^{commit}`])).stdout.trim();
    console.error(`next ${ref}: ${release ? `${release} = ` : ''}${sha.slice(0, 12)}`);
  }
...
async function resolveArms(cfg) {
  let arms;
  if (cfg.pr) {
    arms = (await resolvePrArms(cfg.pr, REACT_REPO_LAZY(), CONFIG.reactRepoUrl, 'main'))
    ...
    const base = (await execFileP('git', ['-C', repo, 'merge-base', synced, cfg.arms[0].ref])).stdout.trim();
    console.error(`react base = merge-base(${cfg.arms[0].ref}, synced ${synced}) = ${base.slice(0, 12)}`);
    arms = [
    ...
    arms = (await resolvePrArms(cfg.nextPr, NEXT_REPO_LAZY(), CONFIG.nextRepoUrl, 'canary'))
      .map(a => ({name: a.name, ref: cfg.reactRef || null, nextRef: a.ref}));
  } else {
    arms = cfg.nextArms.map(a => ({name: a.name, ref: cfg.reactRef || null, nextRef: a.ref}));
  }
...
async function commitTitle(repo, ref) {
  try {
    return (await execFileP('git', ['-C', repo, 'log', '-1', '--format=%s', ref])).stdout.trim();
  } catch {
    return undefined;
  }
}
async function describeRun(cfg) {
  ...
  if (num) {
    ...
    try {
      pr.title = (await execFileP('gh',
        ['api', `repos/${ghRepo}/pulls/${num}`, '--jq', '.title'])).stdout.trim();
    } catch {}
  }
  const arms = [];
  for (const a of cfg.arms) {
    arms.push({name: a.name, title: reactVaries
...
const T95 = [12.71, 4.30, 3.18, 2.78, 2.57, 2.45, 2.36, 2.31, 2.26, 2.23, 2.20, 2.18, 2.16, 2.14, 2.13];
function makeLive(baseName, metrics, keyOf) {
  const rows = [];
  return (vm, row) => {
    rows.push({vmIdx: vm, ...row});
    const key = keyOf(row);
    const other = rows.find(r => r.vmIdx === vm && keyOf(r) === key &&
      r.block === row.block && r.run === row.run && r.arm !== row.arm);
    if (!other) return;
    ...
    for (const metric of metrics) {
      ...
      for (const c of cands) {
        const b = rows.find(r => r.vmIdx === c.vmIdx && keyOf(r) === key &&
          r.block === c.block && r.run === c.run && r.arm === baseName);
        if (b && b[metric] > 0) deltas.push((c[metric] - b[metric]) / b[metric]);
      }
      n = deltas.length;
      if (n < 2) return;
      const mean = deltas.reduce((a, b2) => a + b2, 0) / n;
      const sd = Math.sqrt(deltas.reduce((a, b2) => a + (b2 - mean) ** 2, 0) / (n - 1));
      const ci = (T95[n - 2] ?? 2.0) * sd / Math.sqrt(n);
      const p = pairedP(deltas);
      parts.push(`${metric} ${(mean * 100).toFixed(1)}%±${(ci * 100).toFixed(1)} p=${p.toFixed(3)}`);
    }
...
function sbExec(vm, timeout, script, tag, onRow) {
  return new Promise((resolve, reject) => {
    const child = spawn(
      VERCEL,
      ['sandbox', 'exec', vm, ...SCOPE, '--timeout', timeout, '--', 'bash', '-c', script],
    ...
    child.stderr.on('data', c => {
      out += c;
      process.stderr.write(String(c).split('\n').filter(Boolean).map(l => `[${tag}!] ${l}\n`).join(''));
    });
    child.on('exit', code =>
...
async function runDetached(vm, tag, script, onLine, deadlineMin) {
  ...
  await sb(['exec', vm, '--timeout', '2m', '--', 'bash', '-c',
  ...
  while (true) {
    ...
    if (Date.now() > deadline) throw new Error(`${tag}: detached loop deadline exceeded`);
    let out;
    try {
      ...
      if (++failures >= 6) throw new Error(`${tag}: ${failures} consecutive poll failures: ${e.message.slice(0, 200)}`);
      continue;
    }
    ...
    if (m) {
      ...
      if (m[2] === 'LOOPFAIL') throw new Error(`${tag}: remote loop failed; tail:\n${body.slice(-1500)}`);
    }
  }
}
...
async function takeSnapshot(vm, cacheDir, key) {
  ...
  if (!id) throw new Error(`could not parse snapshot id from: ${out.slice(-500)}`);
...
async function ensureReactBuildSnapshot(refSha) {
  const lock = await execFileP('git', ['-C', REACT_REPO_LAZY(), 'show', `${refSha}:yarn.lock`], {maxBuffer: 1 << 28});
  ...
  console.error(`creating react build-env snapshot (one-time for this yarn.lock)...`);
  await sb(['create', '--name', vm, '--runtime', 'node24', '--vcpus', '8',
    '--timeout', '45m', '--non-persistent', '--network-policy', 'allow-all',
    '--tag', 'purpose=sandbox-bench', '--silent']);
  try {
    const src = path.join(os.tmpdir(), `react-snap-src-${key}.tgz`);
    await execFileP('bash', ['-c', `git -C ${REACT_REPO_LAZY()} archive ${refSha} | gzip -1 > ${src}`]);
    await sb(['cp', src, `${vm}:/vercel/sandbox/src.tgz`]);
    fs.rmSync(src, {force: true});
    await sb(['exec', vm, '--timeout', '10m', '--sudo', '--',
...
async function ensureExperimentSnapshot(cfg) {
  ...
  const key = await sha256(SETUP_VERSION + 'exp3' +
    cfg.arms.map(a => `${a.name}=${armId(a)}`).join());
  ...
  console.error(`creating experiment snapshot (one-time for arms=${cfg.arms.map(armId).join(',')}; ` +
    `trees cached: ${cfg.arms.filter(a => a.treeCached).map(a => a.name).join(',') || 'none'})...`);
  try {
    await sb(['create', '--name', vm, '--runtime', 'node24', '--vcpus', '16',
      '--timeout', '1h', '--non-persistent', '--network-policy', 'allow-all',
      '--tag', 'purpose=sandbox-bench', '--silent']);
  } catch (e) {
  ...
  try {
    for (const tgz of new Set(cfg.arms.filter(a => !a.treeCached).map(a => a.nextTgz))) {
    ...
    const extractCached = cfg.arms.filter(a => a.treeCached).map(a => `
echo "PHASE extract-${a.name} $(date +%s)"
mkdir -p /vercel/sandbox/next-${a.name} && tar -xzf /vercel/sandbox/tree-${a.name}.tgz -C /vercel/sandbox/next-${a.name}`).join('\n');
    const installs = cfg.arms.filter(a => !a.treeCached).map(a => `
...
pnpm install --frozen-lockfile >/tmp/i-${a.name}.log 2>&1 || (tail -10 /tmp/i-${a.name}.log; exit 1)`).join('\n');
    const builds = cfg.arms.filter(a => !a.treeCached).map((a, i) => `
(
  set -e
...
BUILD_${i}=$!`).join('\n');
    const waits = cfg.arms.filter(a => !a.treeCached).map((a, i) =>
      `wait $BUILD_${i} || (tail -10 /tmp/b-${a.name}.log /tmp/s-${a.name}.log /tmp/n-${a.name}.log; exit 1)`).join('\n');
    // Warm + verify runs per tree, sequential (they bind the same port).
    const verifies = cfg.arms.map(a => `
cd /vercel/sandbox/next-${a.name}
...
echo "tree ${a.name} ready"`).join('\n');
    ...
    for (const arm of cfg.arms) {
      if (!arm.treeCached) {
        ...
        await sb(['cp', `${vm}:/vercel/sandbox/tree-${arm.name}-out.tgz`, treeTmp]);
        if (fs.existsSync(treeTmp) && fs.statSync(treeTmp).size >= 50_000_000) {
          ...
          console.error(`tree cache download for ${arm.name} too small; skipping cache (snapshot unaffected)`);
        }
      }
    }
    await sbExec(vm, '5m', `rm -f /vercel/sandbox/tree-*.tgz /vercel/sandbox/tree-*-out.tgz; echo cleaned`, 'expsnap');
...
async function tryCiArtifactArm(sha, cached, pack) {
  try {
    const runs = JSON.parse((await execFileP('gh', ['api',
    ...
    if (!id) return false;
    const arts = JSON.parse((await execFileP('gh', ['api',
      `repos/${REACT_GH_REPO}/actions/runs/${id}/artifacts?name=artifacts_combined`],
      {maxBuffer: 1 << 24})).stdout);
    const art = arts.artifacts?.find(a => a.name === 'artifacts_combined' && !a.expired);
    if (!art) return false;
    console.error(`downloading CI artifacts for ${sha.slice(0, 12)} (run ${id})...`);
    ...
    try {
      await execFileP('bash', ['-c',
        ...
        `unzip -q a.zip && tar -xzf build.tgz && ${pack}`], {maxBuffer: 1 << 24});
    } finally {
      fs.rmSync(work, {recursive: true, force: true});
    }
    return fs.existsSync(cached) && fs.statSync(cached).size > (1 << 20);
  } catch (e) {
    console.error(`CI artifact lookup failed (${e.message.slice(0, 100)}); building remotely`);
    return false;
  }
}
...
async function ensureRefArm(arm) {
  const sha = (await execFileP('git', ['-C', REACT_REPO_LAZY(), 'rev-parse', arm.ref])).stdout.trim();
  arm.sha = sha;
  // Key on the build recipe too: a changed target list must not serve
  // stale artifacts from the shared cache.
  const recipe = crypto.createHash('sha256').update(E2E_BUILD_TARGETS).digest('hex').slice(0, 6);
  ...
  if (await tryCiArtifactArm(sha, cached,
  ...
  await sb(['create', '--name', vm, '--snapshot', snap, '--vcpus', '8',
    '--timeout', '45m', '--non-persistent', '--network-policy', 'allow-all',
    '--tag', 'purpose=sandbox-bench', '--silent']);
  try {
    ...
    try {
      ...
      await execFileP('git', ['-C', work, 'fetch', '-q', '--depth', '1', REACT_REPO_LAZY(), tmpRef]);
      await execFileP('git', ['-C', work, 'checkout', '-q', 'FETCH_HEAD']);
      await execFileP('bash', ['-c', `cd ${work} && COPYFILE_DISABLE=1 tar --no-xattrs -czf ${src} .`]);
    } finally {
      await execFileP('git', ['-C', REACT_REPO_LAZY(), 'update-ref', '-d', tmpRef]);
      fs.rmSync(work, {recursive: true, force: true});
    }
    await sb(['cp', src, `${vm}:/vercel/sandbox/src.tgz`]);
    fs.rmSync(src, {force: true});
...
echo arm built`,
    ...
    if (!fs.existsSync(cached) || fs.statSync(cached).size < 1 << 20) {
      ...
      throw new Error(`arm ${arm.name}: downloaded artifact missing or too small`);
    }
...
async function stage(cfg, tmp) {
  ...
  for (const arm of cfg.arms) {
    ...
    if (arm.ref) {
      arm.sha = (await execFileP('git', ['-C', REACT_REPO_LAZY(), 'rev-parse', arm.ref])).stdout.trim();
...
async function runVm(index, cfg, expSnap, outDir) {
  ...
  writeStatus({vms: {...statusState.vms, [vm]: {state: 'booting', rows: 0}}});
  await sb(['create', '--name', vm, '--snapshot', expSnap, '--vcpus', '8',
    '--timeout', '2h', '--non-persistent', '--network-policy', 'allow-all',
    '--tag', 'purpose=sandbox-bench', '--silent']);
  try {
...
wc -l /vercel/sandbox/results.jsonl`;
    let vmRows = 0;
    const out = await runDetached(vm, tag, loop, row => {
      vmRows++;
      interimRows.push({vm: index, ...row});
      writeStatus({vms: {...statusState.vms, [vm]: {state: 'measuring', rows: vmRows}}});
      cfg.live(index, row);
    }, 110);
    writeStatus({vms: {...statusState.vms, [vm]: {...statusState.vms[vm], state: 'collecting'}}});
    const local = path.join(outDir, `results-vm${index}.jsonl`);
    await sb(['cp', `${vm}:/vercel/sandbox/results.jsonl`, local]);
    const remoteCount = Number(out.match(/(\d+) \/vercel\/sandbox\/results\.jsonl/)?.[1] ?? NaN);
    const localCount = fs.readFileSync(local, 'utf8').trim().split('\n').filter(Boolean).length;
    if (!Number.isFinite(remoteCount) || localCount !== remoteCount) {
      throw new Error(`${tag}: downloaded ${localCount} rows, remote reported ${remoteCount} — truncated transfer`);
    }
...
async function plan(cfg) {
  const lines = [];
  lines.push(`mode: ${cfg.pr ? `react PR ${cfg.pr}` : cfg.arms.length ? 'react A/B' : cfg.nextPr ? `next PR ${cfg.nextPr}` : 'next A/B'}`);
  lines.push(`scope: team=${CONFIG.team ?? '<UNSET — ask user, then: node config.mjs set team=... project=...>'} project=${CONFIG.project ?? '<UNSET>'}`);
  lines.push(`next repo: ${NEXT_REPO_LAZY()}`);
  if (cfg.pr || cfg.arms.length || cfg.reactRef) {
    const rr = REACT_REPO_LAZY();
    lines.push(`react repo: ${rr}${fs.existsSync(path.join(rr, '.git')) ? '' : ' (would clone on real run)'}`);
  } else {
  ...
  try {
    ...
    for (const arm of arms) {
      if (!arm.ref) {
        lines.push(`arm ${arm.name}: react=vendored next=${arm.nextSha.slice(0, 12)}`);
        continue;
      }
      let reactSha = '?';
      try {
        reactSha = (await execFileP('git', ['-C', REACT_REPO_LAZY(), 'rev-parse', arm.ref])).stdout.trim().slice(0, 12);
      } catch {}
      const cached = reactSha !== '?' && fs.existsSync(path.join(CACHE, `arm-${reactSha}.tgz`));
      lines.push(`arm ${arm.name}: react=${arm.ref} (${reactSha}${cached ? ', build cached' : ', would build remotely ~15m'}) next=${arm.nextSha.slice(0, 12)}`);
      lines.push(`  CI gate: react commit must be CI-green (or --allow-ungated after sandbox-gate.mjs)`);
    }
  } catch (e) {
    lines.push(`arms: unresolved in dry-run (${e.message.split('\n')[0].slice(0, 120)})`);
  }
  lines.push(`then: experiment snapshot (cached by content key; ~45m if cold) -> ` +
  ...
  lines.push(`then: boot-level analysis (n=${cfg.vms} boots) -> claims at p<0.01 on A/A-validated infra`);
  ...
  for (const route of cfg.routes.split(',')) {
    for (const phase of ['single-client', 'under-load']) {
      ...
      for (const vmIdx of new Set(interimRows.map(r => r.vm))) {
        const deltas = [];
        for (const r of interimRows.filter(x => x.vm === vmIdx && x.route === route &&
            x.phase === phase && x.arm === cand)) {
          const b = interimRows.find(x => x.vm === vmIdx && x.route === route &&
            x.phase === phase && x.arm === base && x.block === r.block && x.run === r.run);
          if (b && b.rps > 0 && r.rps > 0) deltas.push((r.rps - b.rps) / b.rps);
        }
        if (deltas.length) perBoot.push(deltas.reduce((a, b) => a + b, 0) / deltas.length);
      }
      ...
      cells.push(`    ${label.padEnd(19)} ${mean > 0 ? '+' : ''}${(mean * 100).toFixed(1)}% rps (${confStr} confidence)`);
    }
  }
...
const digest = setInterval(() => {
  ...
  const vmSummary = Object.entries(states).map(([k, n]) => `${n} ${k}`).join(', ');
  console.log(`progress: ${statusState.phase}` +
    (statusState.rowsExpected ? ` — rows ${rows}/${statusState.rowsExpected}` : '') +
    (vmSummary ? ` (${vmSummary})` : '') + interimSummary());
}, 120_000);
digest.unref?.();
writeStatus({label: cfg.label, phase: 'resolving arms', pid: process.pid,
  startedAt: new Date().toISOString(), vms: {}, rowsExpected: null});
try {
  ...
  cfg.live = makeLive(cfg.arms[0].name, ['rps', 'median', 'p95'], r => `${r.route} ${r.phase}`);
  await stage(cfg, tmp);
  console.error(`arms: ${cfg.arms.map(a => `${a.name}=${armId(a)}`).join(' ')}`);
  writeStatus({phase: 'building arms + experiment snapshot',
    arms: cfg.arms.map(a => `${a.name}=${armId(a)}`)});
  const expSnap = await ensureExperimentSnapshot(cfg);
  writeStatus({phase: 'measuring', expSnap,
    rowsExpected: cfg.vms * cfg.blocks * cfg.runs * 2 * cfg.routes.split(',').length * 2});
  if (cfg.prepare) {
