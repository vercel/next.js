...
export function formatStat(name, candName, baseName, s) {
  ...
  const meta = m ? ` [${m.unit}; ${(s.mean > 0) === (m.better === 'higher') ? 'IMPROVEMENT' : 'regression'} if real]` : '';
  return (
...
export function analyzeE2eRows(rows, baseName, candName, metrics) {
  const fps = {};
  for (const r of rows) (fps[r.arm] ||= new Set()).add(r.ver ? `${r.fp}/${r.ver}` : `${r.fp}`);
  console.log(
  ...
  if ((fps[baseName]?.size ?? 0) !== 1 || (fps[candName]?.size ?? 0) !== 1) {
    ...
    console.log('!! inconsistent fingerprints within an arm — RESULTS INVALID, no stats');
    process.exitCode = 1;
    return false;
  }
  ...
  for (const phase of [...new Set(rows.map(r => `${r.route ?? ''} ${r.phase}`))].sort()) {
    ...
    for (const metric of metrics) {
      ...
      for (const vm of [...new Set(rows.map(r => r.vm))]) {
        const deltas = [];
        for (const r of rows.filter(x => x.vm === vm && inPhase(x) && x.arm === candName)) {
          const b = rows.find(x => x.vm === vm && inPhase(x) && x.arm === baseName &&
            x.block === r.block && x.run === r.run);
          if (b && b[metric] > 0 && r[metric] > 0) deltas.push((r[metric] - b[metric]) / b[metric]);
        }
        perBoot.push(deltas);
      }
