...
// importer change.
const IDENT = new Set(['vm', 'arm', 'block', 'run', 'route', 'phase',
  'fp', 'ver', 'cpu', 'errors', 'payload', 'round']);
...
function harnessSha() {
  try {
    return execFileSync('git', ['-C', path.dirname(new URL(import.meta.url).pathname),
...
export function importRun(db, dir) {
  ...
  try {
    ...
    db.prepare(`INSERT INTO runs
      ...
      VALUES (?,?,?,?,?,?,?)`).run(
      runId, meta?.label ?? runId.replace(/^run-/, '').replace(/-[a-z0-9]+$/, ''),
      kind, status?.startedAt ?? null, new Date().toISOString(),
      harnessSha(), meta ? JSON.stringify(meta) : null);
    for (const {boot, row, micro} of pendingRows) {
      const {lastInsertRowid: sid} = insSample.run(
        ...
        row.fp ?? null, row.ver ?? null, row.cpu ?? null, row.errors ?? 0);
      samples++;
    ...
    for (const e of fs.readdirSync(dir)) {
      ...
      for (const f of walk(path.join(dir, e))) {
        ...
        insArt.run(runId, boot, arm, rel, artifactKind(rel),
          crypto.createHash('sha256').update(raw).digest('hex'),
          raw.length, zlib.gzipSync(raw));
        artifacts++;
      }
    }
    db.exec('COMMIT');
...
export function loadRows(db, runId) {
  const samples = db.prepare('SELECT * FROM samples WHERE run_id = ?').all(runId);
  if (samples.length === 0) throw new Error(`no samples for run ${runId}`);
  const meas = db.prepare(`SELECT m.sample_id, m.metric, m.value FROM measurements m
    JOIN samples s ON s.sample_id = m.sample_id WHERE s.run_id = ?`).all(runId);
  const byId = new Map();
  for (const s of samples) {
    byId.set(s.sample_id, {vm: Number(s.boot), arm: s.arm, block: Number(s.block),
      run: Number(s.run), route: s.route, phase: s.phase,
      fp: s.fingerprint, ver: s.version, errors: Number(s.errors)});
  }
...
export function verify(db, runId) {
  ...
  for (const {run_id} of runs) {
    const orphanSamples = db.prepare(`SELECT COUNT(*) c FROM samples s
      WHERE s.run_id = ? AND NOT EXISTS
      (SELECT 1 FROM measurements m WHERE m.sample_id = s.sample_id)`).get(run_id).c;
    if (orphanSamples > 0) {
      problems.push(`${run_id}: ${orphanSamples} samples with no measurements`);
    }
    const arms = db.prepare(`SELECT arm, COUNT(DISTINCT COALESCE(fingerprint,'') ||
      '/' || COALESCE(version,'')) fps, COUNT(*) n
      FROM samples WHERE run_id = ? GROUP BY arm`).all(run_id);
    if (arms.length !== 2) {
      problems.push(`${run_id}: expected 2 arms, found ${arms.length}`);
    }
    for (const a of arms) {
      if (a.fps > 1) {
        problems.push(`${run_id}: arm ${a.arm} has ${a.fps} distinct fingerprints`);
      }
    }
    if (arms.length === 2 && arms[0].n !== arms[1].n) {
      problems.push(`${run_id}: unpaired sample counts ` +
        `(${arms[0].arm}=${arms[0].n} vs ${arms[1].arm}=${arms[1].n})`);
    }
    for (const art of db.prepare(
      'SELECT artifact_id, name, sha256, bytes, data FROM artifacts WHERE run_id = ?')
      .all(run_id)) {
      const raw = zlib.gunzipSync(art.data);
      if (raw.length !== Number(art.bytes) ||
...
export function exportRuns(db, outFile, runIds) {
  ...
  const copyRun = copy('runs',
    ['run_id', 'label', 'kind', 'started_at', 'imported_at', 'harness_sha', 'meta']);
  const copySample = copy('samples', ['sample_id', 'run_id', 'boot', 'arm', 'block',
    'run', 'route', 'phase', 'fingerprint', 'version', 'cpu', 'errors']);
  const copyMeas = copy('measurements', ['sample_id', 'metric', 'value']);
  const copyArt = copy('artifacts', ['run_id', 'boot', 'arm', 'name', 'kind',
    'sha256', 'bytes', 'data']);
  for (const runId of runIds) {
    const run = db.prepare('SELECT * FROM runs WHERE run_id = ?').get(runId);
    if (!run) throw new Error(`run ${runId} not in db`);
    copyRun(run);
    for (const s of db.prepare('SELECT * FROM samples WHERE run_id = ?').all(runId)) {
      copySample(s);
      for (const m of db.prepare(
        'SELECT * FROM measurements WHERE sample_id = ?').all(s.sample_id)) {
        copyMeas(m);
      }
    }
    for (const a of db.prepare('SELECT * FROM artifacts WHERE run_id = ?').all(runId)) {
      copyArt(a);
    }
  }
  out.exec('COMMIT');
...
if (isMain()) {
  ...
  if (cmd === 'import') {
    ...
    for (const dir of rest) {
      ...
      console.log(`${r.runId}: ${r.boots} boots, ${r.samples} samples, ${r.artifacts} artifacts`);
    }
    ...
    for (const r of db.prepare(`SELECT r.run_id, r.kind, r.started_at,
      ...
      FROM runs r ORDER BY r.started_at`).all()) {
      console.log(`${r.run_id}  ${r.kind}  boots=${r.boots} samples=${r.samples} artifacts=${r.artifacts}  ${r.started_at ?? ''}`);
    }
  } else {
