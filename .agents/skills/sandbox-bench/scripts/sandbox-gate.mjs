...
function parseArgs() {
  ...
  if (args.pattern !== '' && !/^[A-Za-z0-9|_.-]+$/.test(args.pattern)) {
    ...
    throw new Error(`--pattern must match [A-Za-z0-9|_.-]+, got: ${args.pattern}`);
  }
  return args;
}
...
async function runDetached(vm, tag, script, deadlineMin) {
  ...
  while (true) {
    ...
    if (m) {
      ...
      if (m[2] === 'LOOPFAIL') throw new Error(`${tag}: remote gate failed; tail:\n${body.slice(-3000)}`);
    }
  }
}
...
async function ensureReactBuildSnapshot(refSha) {
  ...
  const lock = (await execFileP('git', ['-C', REACT, 'show', `${refSha}:yarn.lock`],
    {maxBuffer: 256 * 1024 * 1024})).stdout;
...
async function ensureReactBuildSnapshotForLock(refSha, lock) {
  const key = crypto.createHash('sha256').update(SETUP_VERSION + lock).digest('hex').slice(0, 16);
  ...
  try {
    ...
    await execFileP('bash', ['-c', `git -C ${REACT} archive ${refSha} | gzip -1 > ${src}`]);
    await sb(['cp', src, `${vm}:/vercel/sandbox/src.tgz`]);
    fs.rmSync(src, {force: true});
    await sb(['exec', vm, '--timeout', '10m', '--sudo', '--',
      'dnf', 'install', '-y', '-q', 'java-21-amazon-corretto-headless']);
    await runDetached(vm, 'react-snap',
    ...
    const out = await sb(['snapshot', vm, '--stop', '--expiration', '30d'], {withStderr: true});
    const snapId = out.match(/snap_[A-Za-z0-9]+/)?.[0];
    if (!snapId) throw new Error(`could not parse snapshot id from: ${out.slice(-500)}`);
...
async function main() {
  ...
  if (DRY_RUN) {
    console.log(`[dry-run] gate plan: ${args.arms.map(a => `${a.name}=${a.ref}`).join(', ')}`);
    console.log(`[dry-run] per arm: 1 VM from react build-env snapshot (lazy-created), ` +
  ...
  try {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (r.status === 'fulfilled') {
        ...
        console.log(`  bench this exact sha: --arms cand=${r.value.sha} (a ref can move between gate and bench)`);
      } else {
        failed++;
        console.log(`GATE ${args.arms[i].name}: FAIL — ${r.reason.message.slice(0, 2500)}`);
      }
    }
