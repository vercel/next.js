...
if (vms.length === 0) {
  console.error('status.json lists no VMs — the run died before measurement; nothing to collect. ' +
...
async function sb(args) {
  ...
  const {stdout} = await execFileP(CONFIG.vercelBin, scoped, {maxBuffer: 1 << 26});
  return stdout;
}
...
while (pending.size > 0 && Date.now() < deadline) {
  for (const [vm, idx] of [...pending]) {
    ...
    if (state === 'LOOPOK') {
      ...
      const tail = await sb(['exec', vm, '--timeout', '2m', '--', 'bash', '-c',
    ...
    try {
      ...
      console.error(`${vm}: rm failed: ${e.message.split('\n')[0].slice(0, 100)}`);
    }
    pending.delete(vm);
  }
  if (pending.size > 0) await new Promise(r => setTimeout(r, 60000));
}
for (const [vm] of pending) console.error(`${vm}: deadline exceeded, left running`);

if (collected.length === 0) {
  console.error('nothing collected');
  process.exit(1);
}
writeStatus({phase: `done (recovered ${collected.length}/${vms.length} boots)`});
console.log(`\ncollected ${collected.length}/${vms.length} boots; analysis:`);
const {stdout} = await execFileP('node',
  [path.join(path.dirname(new URL(import.meta.url).pathname), 'bench-analyze.mjs'), dir],
  {maxBuffer: 1 << 24});
console.log(stdout);
