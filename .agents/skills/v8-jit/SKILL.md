...
Every JavaScript object has an internal "hidden class" (V8 calls it a *Map*,
the spec calls it a *Shape*). Objects that share the same property names, added
...
class StreamProcessor {
  ...
  handleChunk(chunk: Uint8Array) { processChunk(chunk, this.id) }
}
```
...
function render(node) {
  switch (node.type) {
    case 'div': return { tag: 'div', children: node.children }
    case 'span': return { tag: 'span', text: node.text }
    case 'img': return { src: node.src, alt: node.alt }
...
```js
function hotFunction(x) { return x + 1 }
