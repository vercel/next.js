...
export function _rewriteUrlForNextExport (url) {
  ...
  // process.env.NEXT_EXPORT_TRAILING_SLASHES is used by webpack.DefinePlugin,
  // and takes value of exportTrailingSlashes from next.config.js at runtime

  // webpack.DefinePlugin() doesn't run during testing, so don't break things
  let exportTrailingSlashes = (typeof process.env.NEXT_EXPORT_TRAILING_SLASHES === 'undefined') ? true : process.env.NEXT_EXPORT_TRAILING_SLASHES
