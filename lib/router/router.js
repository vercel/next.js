...
import { reloadIfPrefetched, getPrefetchedData } from '../prefetch'
...
function loadJSON (url, fn) {
  // Try to get the JSON from the prefetched URLs.
  const prefetchedDataThen = getPrefetchedData(url)
  if (prefetchedDataThen) {
    prefetchedDataThen
      .then((d) => fn(null, d))
      .catch(() => fn(new Error('Failed to load JSON for ' + url)))

    // Send a fake xhr object
    return {
      abort () {},
      status: 'prefetched'
    }
  ...
  // Otherwise, download directly and parse it.
