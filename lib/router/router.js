...
export default class Router {
  ...
  async fetchComponent (route, as) {
    ...
    const pageRes = await this.fetchRoute(route)
    let jsData
    ...
    if (pageRes.data) {
      ...
      jsData = pageRes.data = await pageRes.text()
    }
  ...
  doFetchRoute (route) {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => {
        reject(new Error(`Error loading ${route}`))
      }

      const onLoaded = (name) => {
        window.__NEXT_DATA__.emitter.off('page-loaded', onLoaded)
        resolve({Component: window.__NEXT_DATA__[name]})
      }
      window.__NEXT_DATA__.emitter.on('page-loaded', onLoaded)
