...
export default class Router extends EventEmitter {
  ...
  push (url, as = url, options = {}) {
    const formattedUrl = url && typeof url === 'object' ? format(url) : url
    const formattedAs = as && typeof as === 'object' ? format(as) : as
    return this.change('pushState', formattedUrl, formattedAs, options)
  }

  replace (url, as = url, options = {}) {
    const formattedUrl = url && typeof url === 'object' ? format(url) : url
    const formattedAs = as && typeof as === 'object' ? format(as) : as
    return this.change('replaceState', formattedUrl, formattedAs, options)
  }
