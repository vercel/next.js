...
import { loadGetInitialProps, LockManager } from '../utils'
...
export default class Router extends EventEmitter {
  ...
  async prefetch (url) {
    ...
    const route = toRoute(pathname)
    // We don't need to fetch the route, if it's already fetched
    if (this.prefetchedRoutes[route]) return
    ...
    if (this.prefetchingRoutes[route]) {
      done()
      return
    }
  ...
  async fetchComponent (route) {
    ...
    let res = this.prefetchedRoutes[route]
    if (!res) {
      res = await fetchUrl(route)
    }
