...
export default class Router {
  constructor (pathname, query, as, { pageLoader, Component, ErrorComponent, err } = {}) {
    ...
    // Next.js supports multiple apps to work as a single app.
    // Each such app is known as a zone.
    // This field keep the host (hostname:port) of the zone
    // TODO: In the future, proxy infrastructure will tell us the zone used in
    // the SSR process.
    // We can't get the zone by parsing the URL. Proxy is masking it.
    this.zone = getZone(pathname)
  }
  ...
  async change (method, _url, _as, options) {
    ...
    // NOTE: If the zone is empty, that'll use the current zone.
    // That might lead to undefined behaviors in multi-zone apps.
    // So, it's a good idea to mention the zone always.
    const nextZone = getZone(url)
    // If we are going into a different zone, we need to do a hard reload.
