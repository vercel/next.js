...
export default class Router {
  constructor (pathname, query, as, { pageLoader, zone, Component, ErrorComponent, err } = {}) {
    ...
    // This field has a format like this:
    //  { "name": "zone-name", "url": "http://zone-url.com" }
    this.zone = zone
  }
  ...
  async change (method, _url, _as, options) {
    ...
    const { protocol, hostname, pathname, query } = parse(url, true)

    // If we are going into a different zone, we need to do a hard reload.
    // If this is the not a zone url, we assume the page is in the current zone.
    // (Or zone support is not used inside the app)
    if (this.zone && protocol === 'zone:' && hostname !== this.zone.name) {
