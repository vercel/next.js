...
export default class Router {
  constructor (pathname, query, as, { err, pageLoader } = {}) {
    // TODO handle err again
    ...
    if (typeof window !== 'undefined') {
      this.ErrorComponent = this.pageLoader.getPageForRoute('/_error')
  ...
  async getRouteInfo (route, pathname, query, as) {
    ...
    try {
      routeInfo = this.components[route]
      if (!routeInfo) {
        Page = await this.fetchComponent(route, as)
        routeInfo.Component = Page
      }

      const { Component, err } = routeInfo
  ...
  async fetchComponent (route, as) {
    ...
    const Page = await this.fetchRoute(route)
  ...
  doFetchRoute (route) {
    return new Promise((resolve, reject) => {
      // does the pageLoader already have that route?
      if (this.pageLoader.has(route)) {
        // return it
        return resolve(this.pageLoader.getPageForRoute(route))
      }
      // else listen for page load
      const componentLoaded = this.pageLoader.onPageLoaded(route, (Component) => {
        resolve(Component)
        componentLoaded()
      })
      // and mount that page bundle
      this.pageLoader.mountPageBundle(route)
    })
  }
...
// TODO use this again to do HMR with new JS pages
...
//   // TODO handle err again
//   return { Component: component, err: null }
// }
