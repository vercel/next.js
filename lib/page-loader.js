...
export default class PageLoader {
  ...
  // This method is called by the route code.
  registerPage (route, regFn, chunks) {
    const webpackIsHot = Boolean(webpackModule && webpackModule.hot)
    if (chunks && webpackIsHot) {
      chunks.forEach(c => {
        window.webpackManifest[c.id] = c.src
      })
    }
    const register = () => {
      try {
    ...
    if (webpackIsHot && webpackModule.hot.status() !== 'idle') {
