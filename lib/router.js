...
export default class Router {
  constructor (url, initialData) {
    ...
    // setup dynamic routes for caching
    this.dynamicRoutes = initialData ? initialData.ctx.dynamicRoutes : []

    // represents the current component key
    this.route = toRoute(parsed.pathname, this.dynamicRoutes)
  ...
  onPopState (e) {
    ...
    .then(async () => {
      ...
      this.route = toRoute(route, this.dynamicRoutes)
  ...
  update (_route, Component) {
    const route = toRoute(_route, this.dynamicRoutes)
...
function compareRoute (route, path) {
  const pathSegments = path.replace(/^\//, '').split('/')
  const routeSegments = route.replace(/@/g, '@@').split(/_@|\/@?/)
  if (routeSegments.length === pathSegments.length && routeSegments[0] === pathSegments[0]) {
    return true
  }
  return route === path
}
...
function toRoute (path, dynamicRoutes = []) {
  const route = path.replace(/\/$/, '') || '/'
  for (const r of dynamicRoutes) {
    if (compareRoute(r, route)) {
      return '/' + r
    }
  }
  return route
