...
export function getNextData () {
  const element = document && document.getElementById('server-app-state')

  if (element) {
    return JSON.parse(
      element.textContent.replace(/%3C\/script%3E/g, '</script>')
    )
  } else {
    console.error('no data element: #server-app-state')
    return {}
  }
}
