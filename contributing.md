...
1. Search [GitHub](https://github.com/vercel/next.js/pulls) for an open or closed PR that relates to your submission. You don't want to duplicate effort.
...
6. Run `yarn dev` to build and watch for code changes
7. In a new terminal, run `yarn types` to compile declaration files from TypeScript
8. The development branch is `canary` (this is the branch pull requests should be made against). On a release, the relevant parts of the changes in the `canary` branch are rebased into `master`.
