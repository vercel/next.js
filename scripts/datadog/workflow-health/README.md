...
- `failure-monitor.json`: a multi-alert `ci-pipelines alert` that triggers
  ...
  Empty evaluation windows are treated as zero (Datadog's default for
  count queries), so healthy quiet windows read as OK rather than NO DATA.
  If reporting stops entirely, the monitor goes NO DATA after 30 minutes
  (twice the evaluation window) and notifies, renotifying once a week.
- `skip-rate-monitor.json`: a formula-based `ci-pipelines alert` computing
  `skipped runs / all runs` per workflow over a trailing 3 day window. It
  alerts at 100%, where every completed run was skipped; the 3 day window
  smooths out weekends, when low run volume pushes the daily ratio up. A
  stretch of nothing but skips is often benign (`retry-tests`,
  `retry-deploy-tests`, and `Allow issue participants to reopen issues`
  skip by design when there is nothing to do), but it can also mean a
  condition broke and the workflow silently stopped doing work. To exclude
  a by-design skipper, add `-@ci.pipeline.name:"retry-tests"` (etc.) to
  both variable searches. The monitor renotifies once a week while it
  stays in alert, and it also notifies and renotifies on no data: its
  queries match every run, so multiple days without any pipeline events
  means the GitHub integration itself stopped reporting. The monitor only
  lists workflows that had at least one skipped run in the window: the
  formula
  joins its two variables on the group, so a group must exist in both. A
  workflow that is absent sits at 0% by construction, and any workflow able
  to alert necessarily has skipped runs and therefore appears.
...
Excluded on purpose even though they match the trigger criteria: the
Rspack workflows
...
are maintained by the Rspack team, not the Next.js team.
