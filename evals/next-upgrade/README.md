...
scenarios once with Codex and once with Claude in Vercel Sandbox, sequentially,
with a 20-minute budget per run. Both experiments require `sandbox: 'vercel'`;
missing credentials block execution without falling back to Docker. The experiments use the shared model mapping in `packages/next/src/lib/upgrade/models.ts`:
Codex uses `openai/gpt-5.6-luna` and Claude Code uses `anthropic/claude-haiku-4.5`.
Override them with `NEXT_UPGRADE_CODEX_MODEL` and `NEXT_UPGRADE_CLAUDE_MODEL`;
the native runner records the observed model when it is available. Newly launched
product sessions select the corresponding native CLI model explicitly. Existing
...
Additional models, repetitions and provider combinations are deferred.

The background-session driver now follows the session returned by the terminal
launcher. This revision has not been evaluated yet; earlier results do not establish
its background lifecycle or `--dry-run` behavior. Live desktop attachment is outside
this suite.

| Scenario | Entry | Required result |
| --- | --- | --- |
| same-major | Ordinary terminal → actual Next → native harness | Pages 15.5.23 → 15.5.24, React/bundler preserved, dev checks, complete local commit |
...
```sh
pnpm eval:upgrade
# A targeted rerun, still once per harness:
...
`.github/workflows/next_upgrade_evals.yml` runs this suite independently of core CI
and the public benchmark. Add `ci:upgrade-evals` to a same-repository PR to opt in.
Relevant later pushes rerun it and cancel a superseded run. Fork PRs are excluded.
Manual dispatch can select one scenario or all four once the workflow is available
on the default branch. Each scenario runs with both configured harnesses.

Configure the GitHub environment `next-upgrade-evals` before enabling a run:

- A valid Vercel-issued `VERCEL_OIDC_TOKEN` authenticates both Sandbox and Gateway,
  matching the existing eval runner. It is short-lived and needs renewal; a GitHub
  OIDC token is not interchangeable.
- Alternatively, secrets `VERCEL_TOKEN`, `AI_GATEWAY_API_KEY` and variables
  `VERCEL_TEAM_ID`, `VERCEL_PROJECT_ID` for the intended Vercel project.
- Optional environment reviewers to approve paid executions.

CI builds and packs the exact PR head, then runs the eight attempts sequentially
in Vercel Sandbox. Each attempt has a 20-minute agent budget, with a 240-minute CI
job limit covering setup/build overhead. Results, transcripts and grading evidence
are uploaded even after failures and retained for 14 days. Missing credentials
fail explicitly. This is an opt-in check; do not make it a required merge check
until the workflow is proven. The temporary core-CI skip does not control this job.
...
repository context: repository authority, Webpack, app behavior and
...
and reject ordinary Git pushes and GitHub/GitLab CLI publication attempts; any
