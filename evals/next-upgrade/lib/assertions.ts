    ...
    if (scenario.major) {
      ...
      expect
        .soft(
          ...
          'preserve the development bundler across majors'
        )
        .toContain('--webpack')
      expect
        .soft(
          manifest.scripts.build,
          'preserve the production bundler across majors'
        )
        .toContain('--webpack')
      ...
      expect(json(requireFromApp.resolve('react/package.json')).version).toBe(
        '18.3.1'
      )
      expect(
        json(requireFromApp.resolve('react-dom/package.json')).version
      ).toBe('18.3.1')
