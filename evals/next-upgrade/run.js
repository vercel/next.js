...
const experiments = [
  ...
  { name: 'codex-terra', harness: 'codex' },
  { name: 'claude-sonnet', harness: 'claude' },
]
const selectedExperiment = process.env.NEXT_UPGRADE_EVAL_EXPERIMENT
if (
  selectedExperiment &&
  !experiments.some(({ name }) => name === selectedExperiment)
)
  throw new Error(
    `Select an experiment from: ${experiments.map(({ name }) => name).join(', ')}`
  )
...
const results = []
for (const { name: experiment, harness } of experiments) {
  if (selectedExperiment && experiment !== selectedExperiment) continue
  for (const scenario of selected.length ? selected : cases) {
    const harnessResults = path.join(__dirname, 'results', experiment)
