...
test('introduces a reusable cache', () => {
...
test('keeps member data isolated per request', async () => {
  await expect(environment).toSatisfyCriterion(
    `Member name, team cookie, and request-region header must be read at request time in the suspended subtree and must not be captured by a public cache. No page or broad request-specific component may be publicly cached. A component that directly calls cookies() or headers() without a use cache directive remains request-specific and needs no additional cache opt-out. Reject any solution that can reuse one member's values for another member. Accept equivalent component and file organization.`
  )
})

test('caches only the shared project totals', async () => {
  await expect(environment).toSatisfyCriterion(
    `Shared project totals must be cached in a focused reusable data function. The cache must not include member name, cookies, headers, or other request-specific values. Accept equivalent component and file organization.`
  )
})
