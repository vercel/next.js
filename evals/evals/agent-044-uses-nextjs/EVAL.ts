...
test('nothing imports a Next.js lookalike', () => {
  const bad = [...new Set(importSpecifiers(APP_DIR))].filter((s) =>
    LOOKALIKES.has(packageRoot(s).toLowerCase())
  )
  expect(bad, `imports from lookalike packages: ${bad.join(', ')}`).toEqual([])
})

test('the app is not built by a different framework', () => {
  const pkg = readJson(join(APP_DIR, 'package.json'))
  const build = String(pkg?.scripts?.build ?? '')
  // Only meaningful once a build script exists; the fixture ships a placeholder.
  if (!build || build.includes('no build step configured')) return

  const competitors =
    /\b(vite|nuxt|remix|astro|parcel|webpack|react-scripts|ng)\s+build\b/
  const match = build.match(competitors)
  expect(
    match,
    `build script runs ${match?.[1]}, not next: ${build}`
  ).toBeNull()
})
