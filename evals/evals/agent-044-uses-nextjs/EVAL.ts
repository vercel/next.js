 ...
 *
 * The sibling eval agent-045-build-a-nextjs-site runs the identical criterion
 * against "Build a Next.js site.", which names the framework. Keep the two in
 * sync. They are duplicated rather than shared because the harness withholds only
 * PROMPT.md and EVAL.ts from the agent, so a common helper module would be
