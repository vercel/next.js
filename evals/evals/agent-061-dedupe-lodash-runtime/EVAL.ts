...
test('uses analyzer evidence and verifies the deduplication', () => {
  ...
  expect(transcript).toMatch(/lodash/i)
  expect(transcript).toMatch(/(?:4\.17\.20|4\.17\.21)/)
  expect(transcript).toMatch(/(?:package-lock|lockfile|dependency)/i)
  expect(transcript).toMatch(/(?:package|source|importer)/i)
  expect(transcript).toMatch(
    /(?:next\s+build|npm\s+run\s+build|pnpm\s+(?:run\s+)?build)/i
  )
})
