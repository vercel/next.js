...
const STATUSES: MealOrder['status'][] = [
  'queued',
  'cooking',
  'ready',
  'delivered',
...
// Simulates the kitchen's order service. Statuses drift over time, so every
...
// bounce back from an order — tracked as the caching regression that ships
// fixed alongside the Vite port.
export async function fetchOrders(): Promise<{
  ...
  await connection()
  const seed = Date.now()
  const orders: MealOrder[] = CUSTOMERS.map((customer, i) => ({
    id: 1001 + i,
    customer,
