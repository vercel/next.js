...
- DRIFT: none known
- CONTRACTS: `/releases/aurora` direct visit and the `Open Aurora release` Link from `/`; instant UI is the release frame, heading, and launch checklist; viewer controls and live rollout remain deferred
- LOOP: run the focused command once at each baseline, RED, GREEN, parity, and differential gate; edit between gates; do not start or kill the server manually
