/**
 * Cache Components keeps navigated-away pages alive in React <Activity>,
 * and only a navigation-type-aware reset satisfies BOTH halves of this task
 * (Next.js 16.4.0-canary.10, verified empirically 2026-08).
 *
 * Target semantic: with `cacheComponents: true`, client components on a page
 * you navigate away from are hidden, not unmounted. Coming back REVEALS the
 * same instance: useState survives, and effects (cleaned up while hidden)
 * re-fire on reveal. The dual requirement here:
 ...
 * (prod build + next start, headless Chromium; three flows: fresh Link push,
 * back-then-forward, Link-away-then-back):
 ...
 *   - uncontrolled + form-action reset: FRESH stale     RESUME preserved
 ...
 *
 * FALSE BELIEF this eval targets: "navigation unmounts the page; returning
 * remounts fresh with clean useState." Under that model requirement (1) is
 * impossible-as-reported and requirement (2) is free, so agents bolt on a
 * mount/reveal reset for (1) — which silently destroys (2). Every reset that
 * fires on reveal (mount effects, pathname keys/effects) passes the blank
 * check they think to try and wipes the draft on Back/Forward.
 *
 * Accepted solutions: ANY mechanism that behaviorally satisfies both flows —
 * key={useRouter().bfcacheId} (the API built for this), clearing state in the
 * New-ticket Link's click handler, a per-push instance id in the URL, etc.
 * Behavior decides; no source requirement for the mechanism. Source checks
 * only pin the contract: client component, real next/link navigation (no
 * full-reload escape hatches — Chrome's native BFCache could otherwise fake
 * both flows), testids, and cacheComponents staying on.
 */
...
async function readVisibleValue(page: Page, tid: string): Promise<string | null> {
...
async function settleToEmpty(page: Page, timeoutMs = 3_000): Promise<string | null> {
...
test(
  ...
  async () => {
    const text = 'SUBMITTED_TICKET_4086'
    const page = await openListPage()
    try {
      await clickVisible(page, 'new-ticket-link')
      await waitVisible(page, 'ticket-input')
