  ...
  return {
    ...
    async linkPackages({ repoDir, nextSwcVersion: nextSwcVersionSpecified, parentSpan }) {
      ...
      const nextSwcVersion =
        nextSwcVersionSpecified ?? pkgDatas.get('@next/swc')?.packedPackageTarPath ?? null
        ...
        for (const [
          ...
          if (packageName === '@next/swc') {
            ...
            try {
              ...
              require('console').log('using swc binaries: ', swcBinariesDirContents.join(', '))
            } catch (err) {
              if (err.code === 'ENOENT') {
                ...
                .traceAsyncFn(async (handlePackageSpan) => {
                  ...
                  if (packageName === '@next/swc') {
                    ...
                    await handlePackageSpan
                      ...
                      .traceAsyncFn(() =>
                        fs.promises.rename(nativeGitignorePath, renamedGitignorePath)
                      )
                    cleanup = async () => {
                  ...
                  await handlePackageSpan
                    ...
                    .traceAsyncFn(() =>
                      Promise.all([
                        fs.promises.rename(path.join(pkgPath, packedFileName), packedPkgPath),
                        cleanup?.(),
                      ])
                    )
                })
