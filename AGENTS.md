...
- **Diagnosing flaky `act`-based tests**: When a test using `createRouterAct` hangs (whether in CI or locally) it would normally surface as the generic "Exceeded timeout of 60000 ms for a test." `act` ships with an **opt-in** watchdog that, when enabled, periodically writes a diagnostic to stderr containing the stuck phase, in-flight RSC fetch URLs, and the test call site. Enable with `ROUTER_ACT_WATCHDOG_MS=<ms>` (off by default to avoid noise on slow CI); `ROUTER_ACT_WATCHDOG_INTERVAL_MS` controls re-emit cadence (default 5s). To reproduce a flake locally, use `node scripts/repro-flake.mjs --test <path> [--grep <pattern>] [--runs N]` which captures per-iteration logs and turns the watchdog on automatically. See `$router-act` (Diagnosing Hangs section).
