...
| `blocking-route` fires even though a parent layout has `<Suspense>` around `{children}` | Validation is segment-scoped. The boundary must live in the suspending segment itself or its `loading.tsx`; a parent's Suspense around `{children}` does not protect the child segment's data access, regardless of fallback content. The runtime message "outside of `<Suspense>`" reads as "outside of a Suspense within this segment." |
