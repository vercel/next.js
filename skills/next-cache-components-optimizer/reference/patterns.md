...
- **Don't blanket the full prefetch.** It fetches _all_ the target's dynamic data; enabling it for every visible link is wasteful. Scope `prefetch={true}` to the per-link-prefetch targets only, using the guide for [choosing which links to prefetch](https://nextjs.org/docs/app/guides/optimizing-prefetching#choose-which-links-to-prefetch) and [hover-triggered prefetch](https://nextjs.org/docs/app/guides/prefetching#hover-triggered-prefetch) when many links are visible.
