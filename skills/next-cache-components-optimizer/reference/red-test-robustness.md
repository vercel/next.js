...
2. **Make the test self-validating** — for any route with deferred content, also assert that the
   ...
   cookie on the document request and suspends dynamic data. The only genuinely vacuous case is a
   build produced WITHOUT the testing API, which defense #1 above covers.
