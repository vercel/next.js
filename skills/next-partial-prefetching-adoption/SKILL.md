...
When restoring the target changes caching or invalidation, follow the project's existing verification approach and [Revalidating after mutations](https://nextjs.org/docs/app/getting-started/caching#revalidating-after-mutations) in the Caching guide. Reuse or extend an applicable suite for the affected lifecycle. If the project doesn't test this type of behavior, do not introduce new test infrastructure during adoption; verify it manually in production and record the expected and observed results. A green `instant()` test proves readiness, not cache correctness. Ask the user only when the intended behavior is unclear.
