...
- **A browser you can drive.** Test-backed preservation uses a production-mode Playwright suite, either existing or explicitly set up with the user's approval. The development insight path and the post-flag URL-data sweep use [`next-dev-loop`](https://github.com/vercel/next.js/tree/canary/skills/next-dev-loop); install it before either development pass unless it is already available (`npx skills add https://github.com/vercel/next.js/tree/canary/skills/next-dev-loop`). If the app is webpack-pinned, drive a browser directly (`agent-browser`, Playwright) — you lose the framework cross-checks, not the insights; they're still in the overlay and the dev log.
...
- **The production-mode `instant()` suite — the primary record for test-backed preservation.** Reuse the app's production build, auth, and Playwright setup. Read an existing `instant-nav.rig.md` first. If the project needs this setup and the user chooses the test-backed path, follow the shared [rig template](https://github.com/vercel/next.js/blob/canary/skills/next-cache-components-optimizer/rig-template.md) instead of duplicating its discovery instructions here. The same tests define the legacy target before adoption and become the work queue after each destination opts into Partial Prefetching. Development can help investigate a failure, but only this suite decides whether the prefetched UI was preserved.
...
Keep the global flag **off** while building the baseline or recording the manual target, then adopt each destination with `export const prefetch = 'partial'`. Enabling the flag before locking the target would remove the legacy baseline and silence the [`instant-link-prefetch-partial`](https://nextjs.org/docs/messages/instant-link-prefetch-partial) insight used by the development path. The route export is enough for the unchanged tests to exercise Partial Prefetching on that destination. If the flag is already on in unshipped work, use the pre-flag commit for the baseline. Ask the user how to ship it, in the language of PRs:
...
If it does not, explain the additional setup and ask once:

> I recommend preserving this UI with `instant()` tests. This project does not yet have the production-mode end-to-end setup they require, so this option also adds the shared build, authentication, and Playwright setup. I can set that up, or document the target and verify it manually.

Use **test-backed preservation** when the project already has an applicable suite or the user explicitly chooses to set it up. This is the ideal path because the same assertions drive the migration and stay as regression coverage. Follow the shared rig template for new setup; do not reproduce it inside this skill. For manual preservation, keep the table as the before/target inventory, use it with the guide's preservation patterns, and record the result. Manual does not mean “ignore the target.”
