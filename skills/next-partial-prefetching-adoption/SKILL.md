...
- **Manual:** follow the [runtime prefetching guide](https://nextjs.org/docs/app/guides/runtime-prefetching), confirm the opted-in link against a production run, and delete the marker when the selected result is verified.
...
- [`next-partial-prefetching-optimizer`](https://github.com/vercel/next.js/tree/canary/skills/next-partial-prefetching-optimizer) — experimental loop that adds selected URL-specific UI to an exact link's prefetched result and locks it in with `instant()`.
