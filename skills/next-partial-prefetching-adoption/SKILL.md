...
When restoring the target adds or expands a cache boundary, first check whether any write path can change the cached result. If there are no writers, no on-demand invalidation is required. If there are writers, follow the Caching guide's [mutable data guidance](https://nextjs.org/docs/app/getting-started/caching#keep-mutable-data-fresh): connect every relevant writer to invalidation and verify a populated-cache → write → fresh-read lifecycle. Reuse or extend the project's applicable suite. If the project doesn't test this type of behavior, do not introduce new test infrastructure during adoption; verify it manually in production and record the expected and observed results. A green `instant()` test proves readiness, not cache correctness. `cacheLife` is not a substitute for invalidating after a write. Ask the user only when the intended behavior is unclear.
...
- Every new or expanded cache was checked for writers. When writers exist, each relevant write path invalidates the cached read and a populated-cache → write → fresh-read lifecycle is verified through an applicable existing test suite or a recorded manual check when the project has no such coverage.
