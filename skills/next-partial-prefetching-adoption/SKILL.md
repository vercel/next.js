...
2. **Adopt the destination.** Add `export const prefetch = 'partial'`. That clears the insight for every link pointing at it. If the route reads URL data (`params`, `searchParams`), the default link still warms only its skeleton (the guide's [URL data](https://nextjs.org/docs/app/guides/adopting-partial-prefetching#url-data) section). Classify the existing full-prefetch call before changing its data behavior:
   - **Preservation:** the link already delivered specific URL-dependent UI before migration and the user confirms that behavior is part of the existing contract. Keep `prefetch={true}`, apply the guide's audit-table row including any documented caching needed to preserve that UI, verify it under `next start`, and record it as **preserved / already optimized**. Do not add a marker or send it through the optimizer.
   - **Net-new or undecided:** the desired UI was not an established pre-migration contract, or preserving it requires a product/freshness decision that has not been made. Do not add caching in adoption. Keep `prefetch={true}` while the migration is in progress and mark the route for the separate selection step:
...
## step 5: select runtime-prefetch goals (optional handoff)

The adoption audit marks candidates instead of optimizing them. Grep for `TODO(runtime-prefetch)` and review the list with the user in one conversation. For each route, ask whether its URL-dependent content should join that link's prefetched result or whether streaming after the shared App Shell is the intended experience. Explain the [per-link prefetching trade-offs](https://nextjs.org/docs/app/guides/runtime-prefetching#per-link-prefetching-trade-offs) in app terms: a runtime prefetch can cost one server invocation per prefetchable link, so a bounded navigation can justify viewport prefetching while a large grid may need intent-triggered prefetching or no upgrade.

Do not fold the implementation into this adoption pass. Adoption is the user-facing, app-wide migration; net-new runtime-prefetch readiness and cost changes belong to a separate test-driven loop. Remove markers for links classified as preserved/already optimized and for declined candidates. For accepted net-new cases, record the source link, destination, content the user wants ready, and viewport/intent posture, then hand them to the experimental [`next-partial-prefetching-optimizer`](https://github.com/vercel/next.js/tree/canary/skills/next-partial-prefetching-optimizer). It strengthens the existing `instant()` guard where possible: the App Shell must stay visible under the lock, and the selected target must become visible for that exact link. The optimizer removes each accepted marker when its test is GREEN. Keep optimization as its own commit or PR.
...
- [`next-partial-prefetching-optimizer`](https://github.com/vercel/next.js/tree/canary/skills/next-partial-prefetching-optimizer) — experimental agentic loop that adds selected URL-specific UI to an exact link's prefetched result and locks it in with `instant()`.
