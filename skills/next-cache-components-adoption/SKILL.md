...
- Show, don't tell. When the harness can expose the existing `next-dev-loop` session, drive the route live so the user sees the static shell → fallback → final content sequence in real time. Otherwise, attach the before/after screenshots you captured; don't open a second browser only for presentation.
