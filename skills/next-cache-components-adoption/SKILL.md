...
Adoption has two milestones:

- **A. Cache Components enabled, build clean.** `cacheComponents: true` is on, the codemod has run (Incremental) or every blocking route is fixed in place (Direct), and `next build` completes with no blocking-route errors. Setup for B. (steps 1–2.)
- **B. Remove `instant = false`.** The inner loop where adoption actually happens. Walk the route tree top-down, one feature at a time. Per-route success bar: dev loop reports no errors _and_ `next build` passes. Check in with the user after every feature. Expect to spend most of the time here. (steps 2–3.)
...
2. **Sync-IO at module/render time** (`new Date()`, `Date.now()`, `Math.random()`, `crypto.randomUUID()`). These fail the build even with `instant = false` — the opt-out doesn't suppress them. If they're in a shared layout, they block every route under it. The codemod can't fix them; you have to translate each one (cache it with `"use cache"` if it's stable, or wrap it in `await connection()` + `<Suspense>` if it's per-request) before milestone A can go green. Grep the whole repo for these calls before running anything else.
...
- **`next dev`** — the working surface. Visit a route; its blocking errors surface in the dev overlay with full stack traces and fix cards linking the per-error docs. Work one route at a time — errors don't accumulate in one place. The route itself still returns HTTP 200, so read the overlay (or `.next-dev.log`), not status codes. A cleared overlay is one half of route-clean — the other half is browser verification (see [step 2](#step-2-the-inner-loop-remove-opt-outs-one-feature-at-a-time)) and a green build for that route.
- **`next build`** — detection only. Use it during milestone A (to confirm the codemod opted every route out and no shared layout still has a sync-IO blocker) and as a spot-check during milestone B (no opt-outs left, every route prerenders). The moment you start removing opt-outs in step 2, `next build` stops being enough on its own — even when the build surfaces a clean compile error, you still don't know what ended up in the static shell vs streamed. Don't fall back to `next build` because its errors are easier to read. By default the build stops at the first blocking route, so it's also poor for sizing the work. Two flags help when iterating: `--debug-build-paths` builds only the routes you name (comma-separated glob patterns of file paths relative to the project root, e.g. `--debug-build-paths="app/admin/**/page.tsx"` — not URL paths; `--debug-build-paths="app/(marketing)/about/page.tsx"` — not `/about`; `--debug-build-paths="app/admin"` matches nothing and silently builds zero routes), and `--debug-prerender` (dev-only) prints a fuller stack trace so the error names the originating file and line.
...
A green build or a cleared overlay isn't proof the route actually behaves — Cache Components is a runtime concern (a static shell with streamed data). Verify after every fix, not only at the end.
...
The strategy choice doesn't change the work — only how PRs are sliced. [Step 2's loop](#step-2-the-inner-loop-remove-opt-outs-one-feature-at-a-time) is identical for both.

Ask the user. Phrase it as a PR-shape question, not a sizing call: _"Do you want to land milestone A (codemod opts every route out, build goes green) as a separate PR first, and do the route fixes in follow-ups (Incremental)? Or do everything in one PR (Direct)?"_ Even on a tiny app, Incremental still has value (review-sized PR, revertible, the `// TODO: Cache Components adoption` markers double as the work queue for next session). Don't pick on their behalf.
...
- **Incremental** — codemod pre-step + the loop. Run the codemod to opt every page and layout out, get a clean build, stop and check in with the user (see [end of milestone A](#end-of-milestone-a-check-in)), then enter [step 2's loop](#step-2-the-inner-loop-remove-opt-outs-one-feature-at-a-time) and ship each feature as a follow-up PR.
- **Direct** — skip the pre-step. Enable the flag and enter the loop immediately; the build's blocking routes are the work queue.
...
### end of milestone A: check in

Incremental only. Stop here before starting step 2. Tell the user:

- What you did: ran the `cache-components-instant-false` codemod (or its manual equivalent), set `cacheComponents: true`, fixed any sync-IO blockers the codemod can't (list them), confirmed `next build` is green.
- What changed: every `app/**/{page,layout,default}` got `export const instant = false` with a `// TODO: Cache Components adoption` comment, except `"use client"`/`"use server"` files and any that already had an `instant` export.
- What to sanity-check: the diff is mostly mechanical (new exports + comments). The build is green. Routes still behave exactly as they did before — the opt-outs preserve current behavior; no rendering changes yet.
- The question: "Want to open this as its own PR before we start removing the opt-outs, or keep going on this branch?" Wait for the answer.

Moving to step 2 without checking in defeats the point of Incremental.

### direct

Set `cacheComponents: true`. The build will list the blocking routes — those are the work queue. Move to [step 2](#step-2-the-inner-loop-remove-opt-outs-one-feature-at-a-time); the loop body is the same as Incremental's, except there are no opt-outs to remove first.

## step 2: the inner loop, remove opt-outs one feature at a time

Pick one feature (a single product feature — `app/settings/profile/**`, `app/posts/[slug]/**` — not a whole top-level app like `app/dashboard/**`). Finish it end-to-end. Check in with the user after every feature before starting the next — each feature is a PR-sized checkpoint, not a substep.
...
Use the **with-a-browser** loop unless a browser is genuinely unreachable. Pick once at the start of milestone B and stick with it for the whole feature. The [`next-dev-loop`](#verifying-each-fix-at-runtime) skill is the source of truth for what counts as "browser available" and how to install it.
...
Used when there's no way to drive a browser — CI, sandbox, the user has no `next dev` running and you can't start one. Weaker signal: confirms the build is green and the route prerenders, but not what ended up in the static shell vs streamed.
...
- Flag the route as build-only-verified when you hand the feature off. Each `◐` route still needs a browser pass before milestone B is done.
...
- Each route visited in the browser: streaming has settled, every `<Suspense>` fallback resolves to its real content.
- You can show the user the rendered result (screenshot, or describe what you observed) per route.

Then check in with the user:

- What you did: which routes you touched, which fix you applied per route (cached with `"use cache"` / wrapped in `<Suspense>` / left as a documented Block).
- What changed: opt-outs removed, fallbacks added, caching boundaries introduced.
- What to sanity-check: show the rendered result per route.
- The question: "Open this feature as a PR and move to the next? Or stop here?" Wait for the answer.

Milestone B is done only when every feature is clean — every remaining `instant = false` sits under a reason comment, `grep -rln "TODO: Cache Components adoption" app` returns nothing. Point the user at [further reading](#further-reading) if they want to push the experience further, or stop and ship.
