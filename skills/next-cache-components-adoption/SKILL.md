...
**You need a browser for this step.** A green build means no route is _opted out_, not that navigations are instant. The build's route table is silent on this too: a route can print `◐` (Partial Prerender) because _some_ shell streams above it (e.g. only a root-layout `<Suspense>`) while still blocking the client navigation into it. Don't trust the build output for milestone C — drive the app in dev and read the Insights.

These warnings fire on **navigation**, not on hover or prefetch (dev doesn't prefetch), and they only surface in the dev overlay's **Insights** tab — there is no build-time or `.next-dev.log` equivalent. So navigate into each route to surface them, in one of three ways (see "How to surface the errors" above for details):

- The **`next-dev-loop`** skill if you have it.
- Any browser you can drive yourself.
- If you can't drive a browser, **ask the user** to navigate the routes and paste the Insights, or commit milestone B and hand off Step 4. Don't silently skip it.

Next.js surfaces instant-navigation validation warnings: dev-only signals (they don't block the build) that look like the blocking-prerender errors you cleared in Step 2, and you fix them the same way — read the warning and apply the fix it names. They appear in the **Insights** tab with fix cards and a **Copy as prompt** button, the same as the blocking-prerender errors in Step 2.
